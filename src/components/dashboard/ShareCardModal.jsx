...
export default function ShareCardModal({
  ...
  const handleCopy = () => {
    navigator.clipboard
      ?.writeText('Check out my Cadi Score! 🚀 cadi.cleaning')
      .then(() => {
        setCopied(true);
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative w-full max-w-sm">
        ...
        <div
          ...
          style={{
            background:
              'linear-gradient(145deg, #010a4f 0%, #05124a 40%, #091660 70%, #0d1e78 100%)',
            aspectRatio: '9/16',
            maxHeight: '75vh',
          }}
        >
          ...
          <div className="relative h-full flex flex-col px-7 py-8 justify-between">
            <div className="flex items-center justify-between">
              ...
              <span
                className={`text-xs font-bold px-3 py-1 rounded-full border ${sc.bg} ${sc.text} bg-opacity-20 border-current/30`}
              >
                {sc.label}
              </span>
            </div>
            ...
            <div className="flex flex-col items-center text-center gap-4">
              <div>
                <p className="text-brand-skyblue/70 text-xs font-bold uppercase tracking-widest mb-1">
                  Cadi Score
                </p>
              ...
              <div className="relative" style={{ width: 140, height: 140 }}>
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  ...
                  <span className="text-xs font-bold mt-1" style={{ color: tierColor }}>
                    {tier}
                  </span>
                </div>
              </div>
            ...
            {earnedBadges.length > 0 && (
              <div className="flex flex-col items-center gap-2">
                <p className="text-brand-skyblue/50 text-[10px] font-bold uppercase tracking-widest">
                  Achievements
                </p>
                <div className="flex gap-3 justify-center">
                  {earnedBadges.map((b) => (
                    <div key={b.id} className="flex flex-col items-center gap-1">
                      <div
                        className={`w-10 h-10 rounded-xl bg-gradient-to-br ${b.color} flex items-center justify-center text-lg shadow-lg`}
                      >
                        {b.emoji}
                      </div>
                      <span className="text-[9px] text-white/50 font-semibold max-w-[40px] text-center leading-tight">
                        {b.name}
                      </span>
                    </div>
                  ))}
            ...
            <div className="flex flex-col items-center gap-1">
              ...
              <p className="text-brand-skyblue/40 text-[10px]">
                cadi.cleaning · Your Cleaning Business OS
              </p>
            </div>
          </div>
        </div>
