...
import {
  TRUST_LEVELS, AGENT_MODES, AGENTS, TONE_OPTIONS,
  upsertAgentSettings, loadAgentSettings,
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <p className={`text-sm font-bold ${selected ? 'text-[#1f48ff]' : 'text-[#010a4f]'}`}>{cfg.label}</p>
  ...
  return (
    <div className="flex items-center justify-between px-5 py-4">
      ...
      <div className="relative">
        ...
        <svg className="absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none text-gray-400" width="10" height="10" viewBox="0 0 10 10">
...
function TonePreview({ tone, businessName, signOffName }) {
  ...
  if (businessName) example = example.replace("Just checking in", `Just checking in from ${businessName}`);
  ...
  return (
    <div className="mt-2 p-3 rounded-xl bg-[#f0f4ff] border border-[#1f48ff]/10">
      ...
      {signOffName && (
        <p className="text-xs text-gray-500 mt-1">— {signOffName}</p>
      )}
    </div>
  );
}
...
export default function AgentSettings() {
  ...
  const loadModes = useCallback(async () => {
    ...
    if (data?.length) {
      const map = {};
      data.forEach(row => { map[row.agent] = row.mode; });
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-[#99c5ff]/20 shadow-sm overflow-hidden">
        ...
        <div className="p-5 space-y-2.5">
          {Object.keys(TRUST_LEVELS).map(val => (
            <TrustCard key={val} value={val} selected={trustLevel === val} onSelect={setTrustLevel} />
          ))}
          <button
          ...
          >
            {trustSaved ? <><Check size={14} /> Saved</> : savingTrust ? 'Saving…' : 'Save trust level'}
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-[#99c5ff]/20 shadow-sm overflow-hidden">
        ...
        <div className="p-5 space-y-5">
          ...
          {/* Tone */}
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-2 uppercase tracking-wide">Tone</label>
            <div className="space-y-2">
              {TONE_OPTIONS.map(opt => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <p className={`text-sm font-semibold ${tone === opt.value ? 'text-[#1f48ff]' : 'text-[#010a4f]'}`}>
                      {opt.label}
                    </p>
          ...
          >
            {voiceSaved ? <><Check size={14} /> Saved</> : savingVoice ? 'Saving…' : 'Save brand voice'}
          </button>
        </div>
      </div>
    </div>
