...
const STAFF = {
  ...
  staff_junior: {
    id: 'staff_junior',
    label: 'Junior staff',
    sub: 'Part-time, under 1yr',
    hourlyRate: 12.5,
  },
  staff_senior: {
    id: 'staff_senior',
    label: 'Senior staff',
    sub: 'Experienced, quality lead',
    hourlyRate: 15.0,
  },
  subcontractor: {
    id: 'subcontractor',
    label: 'Subcontractor',
    sub: 'Self-employed, invoices you',
    hourlyRate: 18.0,
  },
};
...
const COMMERCIAL_JOB_TYPES = [
  { id: 'regular', label: 'Regular contract', multiplier: 1.0 },
  { id: 'deep', label: 'Deep clean', multiplier: 1.4 },
  { id: 'postconstruction', label: 'Post-construction', multiplier: 1.8 },
  { id: 'event', label: 'Event clean', multiplier: 1.2 },
  { id: 'periodic', label: 'Periodic', multiplier: 1.3 },
];
...
const SL = ({ children, className = '' }) => (
  <p className={`text-xs font-bold tracking-widest uppercase text-gray-400 ${className}`}>
    {children}
  </p>
);
...
function Chip({ children, color = 'blue' }) {
  ...
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded-sm text-xs font-bold border ${s[color]}`}
    >
      {children}
    </span>
  );
        ...
        <div className="border-2 border-dashed border-gray-200 rounded-sm">
          <div className="flex flex-col items-center py-6 px-4 text-center">
            ...
            <p className="text-xs text-gray-400 max-w-xs">
              Cadi will analyse a photo of the job and suggest a price range. Rolling out shortly —
              your quotes still build by hand below.
            </p>
          </div>
        </div>
      </div>
...
function ProfitWaterfall({
  ...
  const mc =
    p.margin >= 40 ? 'text-emerald-600' : p.margin >= 20 ? 'text-amber-600' : 'text-red-500';
  ...
  const rows = [
    ...
    {
      label: 'Client pays',
      val: fmt2(p.clientPays),
      color: 'bg-brand-blue',
      textC: 'text-brand-navy',
      indent: false,
    },
    ...
    ...(p.labourCost > 0
      ? [
          {
            label: `Labour (${p.staffLabel})`,
            val: `-${fmt2(p.labourCost)}`,
            color: 'bg-amber-300',
    ...
    {
      label: 'Gross profit',
      val: fmt2(p.grossProfit),
      color: p.grossProfit >= 0 ? 'bg-emerald-400' : 'bg-red-400',
      textC: p.grossProfit >= 0 ? 'text-emerald-700' : 'text-red-600',
    },
    {
      label: `Income tax (~${pct(accounts.taxRate * 100)})`,
      val: `-${fmt2(p.incomeTax)}`,
      color: 'bg-red-300',
      textC: 'text-red-500',
      indent: true,
    },
    ...
    <Card className="overflow-hidden">
      <div className="px-4 py-3 bg-brand-navy text-white flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <span
            className={`text-sm font-bold tabular-nums ${mc.replace('text-', 'text-')}`}
            style={{ color: p.margin >= 40 ? '#10b981' : p.margin >= 20 ? '#d97706' : '#ef4444' }}
          >
            {pct(p.margin)} margin
          </span>
          <Chip color={p.margin >= 40 ? 'green' : p.margin >= 20 ? 'warn' : 'red'}>
            {p.margin >= 40 ? 'Strong' : p.margin >= 20 ? 'OK' : 'Low'}
          </Chip>
        </div>
      </div>
      ...
      <div className="h-2 bg-gray-100">
        <div
          className={`h-full ${mb} transition-all duration-500`}
        ...
        <div className="px-4 py-3 border-b border-gray-100 bg-gray-50 space-y-1.5">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              onPriceChange?.(
                priceForMargin({ targetMarginPct: m, hrs, staffId, extraCosts, accounts })
              );
            }}
            className="w-full accent-brand-blue"
          />
          <p className="text-xs text-gray-400">
            Drag to set target → quote price adjusts automatically
          </p>
        </div>
      )}
      ...
      <div className="divide-y divide-gray-100">
        {rows.map(({ label, val, color, textC, indent, bold }) => (
          <div
            key={label}
            className={`flex items-center gap-3 px-4 py-2.5 ${indent ? 'pl-8' : ''} ${bold ? 'bg-gray-50' : ''}`}
          >
            <span
              className={`text-xs flex-1 ${bold ? 'font-semibold text-gray-800' : 'text-gray-500'}`}
            >
              {label}
            </span>
            <div className="flex-1 h-2.5 bg-gray-100 rounded-sm overflow-hidden hidden sm:block">
              <div
                ...
                style={{
                  width: `${Math.min(Math.abs(p.price > 0 ? (parseFloat(val.replace(/[^0-9.]/g, '')) / p.price) * 100 : 0), 100)}%`,
                }}
              />
            </div>
            <span
              className={`text-xs font-mono w-20 text-right shrink-0 ${bold ? 'text-sm font-bold' : ''} ${textC}`}
            >
              {val}
            </span>
          </div>
...
function AddOnSection({ addons, setAddons, presets, label = 'Add-ons' }) {
  ...
  const addPreset = (preset) => {
    ...
    setAddons((prev) => [
      ...prev,
      { id: uid(), label: preset.label, price: preset.price, hrs: preset.hrs, custom: false },
    ]);
  };
        ...
        <div>
          ...
          {addons.length > 0 && (
            <p className="text-xs text-gray-400 mt-0.5">
              {addons.length} added · {fmt(total)} · {totalHrs.toFixed(1)}hrs
            </p>
          )}
        </div>
        <button
          onClick={addCustom}
          className="flex items-center gap-1 px-3 py-1.5 text-xs font-bold text-brand-blue bg-blue-50 border border-blue-200 rounded-sm hover:bg-blue-100 transition-colors"
        >
          + Custom row
        </button>
      </div>
          ...
          {presets.map((p) => {
            ...
            return (
              <button
                key={p.label}
                onClick={() =>
                  active ? remove(addons.find((a) => a.label === p.label)?.id) : addPreset(p)
                }
          ...
          <div className="divide-y divide-gray-100">
            {addons.map((a) => (
              <div key={a.id} className="grid grid-cols-12 gap-2 px-4 py-2.5 items-center">
                ...
                <button
                  onClick={() => remove(a.id)}
                  className="col-span-1 flex justify-end text-gray-300 hover:text-red-400 transition-colors text-sm"
                >
                  ✕
                </button>
              </div>
    ...
    <Card className="overflow-hidden">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-px bg-gray-100">
        {Object.values(STAFF).map((s) => (
          ...
          >
            <p className={`text-sm font-bold ${staffId === s.id ? 'text-white' : 'text-gray-800'}`}>
              {s.label}
            </p>
            <p
              className={`text-xs mt-0.5 ${staffId === s.id ? 'text-brand-skyblue' : 'text-gray-400'}`}
            >
              {s.hourlyRate > 0 ? `£${s.hourlyRate.toFixed(2)}/hr` : 'No cost to you'}
            </p>
            <p
              className={`text-xs mt-0.5 ${staffId === s.id ? 'text-brand-skyblue/70' : 'text-gray-400'}`}
            >
              {s.sub}
            </p>
          </button>
    ...
    <GCard>
      <div className="px-4 py-3 border-b border-[rgba(153,197,255,0.08)]">
        ...
        <p className="text-[10px] text-[rgba(153,197,255,0.45)] mt-0.5">
          Drafts don't count as income. Mark one accepted to log it to your accounts.
        </p>
      </div>
        ...
        {quotes.slice(0, 8).map((q) => {
          ...
          return (
            <div key={rowKey} className="flex items-center gap-3 px-4 py-2.5">
              ...
              {isAccepted ? (
                <span className="shrink-0 text-[10px] font-black text-emerald-400 bg-emerald-500/10 border border-emerald-500/30 rounded-full px-2 py-1">
                  ACCEPTED
                </span>
              ) : onAccept ? (
                <button
                ...
                <span className="shrink-0 text-[10px] font-black text-[rgba(153,197,255,0.5)] bg-[rgba(153,197,255,0.05)] border border-[rgba(153,197,255,0.1)] rounded-full px-2 py-1">
                  DRAFT
                </span>
              )}
            </div>
      ...
      <div className="p-4 space-y-3">
        ...
        <div>
          ...
          <textarea
            ...
            rows={2}
            placeholder="Key safe code, parking, any special instructions…"
        ...
        {totalPrice > 0 && (
          <div className="bg-brand-navy/5 border border-brand-navy/10 rounded-sm p-3">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-bold text-brand-navy text-lg tabular-nums">
                {fmt2(totalPrice)}
              </span>
            </div>
        ...
        {status === 'error' && (
          <div className="flex items-center gap-2 p-2.5 bg-red-50 border border-red-200 rounded-sm">
            ...
            <p className="text-xs text-red-600 font-semibold">
              Quote couldn't be saved. Check your connection and try again.
            </p>
          </div>
        )}
            ...
            className={`flex-1 py-3 text-xs font-bold uppercase tracking-wide rounded-sm transition-colors ${
              status === 'saved'
                ...
                : status === 'error'
                  ...
                  : customer && totalPrice > 0 && status !== 'saving'
...
function ResidentialTab({
  ...
  const mktStatus =
    mktDiff > propSizeObj.mkMax * 0.15
      ? {
          label: 'Above market — great if clients love you',
          color: 'text-amber-400',
          bg: 'bg-amber-500/10 border-amber-500/20',
        }
      : mktDiff < -(propSizeObj.mkMax * 0.15)
        ? {
            label: 'Below market — consider raising your rate',
            color: 'text-red-400',
            bg: 'bg-red-500/10 border-red-500/20',
          }
        : {
            label: 'In line with UK market rates',
            color: 'text-emerald-400',
            bg: 'bg-emerald-500/10 border-emerald-500/20',
          };

  // Room builder
  const roomPrice = useCallback(
    (roomId, condId, qty = 1) => {
  ...
  const addRoom = (roomId) => {
    ...
    if (existing) {
      ...
      setRooms((prev) => [
        ...prev,
        { id: Date.now(), roomId, qty: 1, condition: 'normal', userPrice: null, customised: false },
      ]);
    }
  };
  ...
  const roomsTotal = rooms.reduce(
    (s, r) =>
      s +
      (r.customised && r.userPrice !== null
        ? r.userPrice * r.qty
        : roomPrice(r.roomId, r.condition, r.qty)),
    0
  );
    ...
    <div className="p-4 lg:p-6 bg-[#010a4f] min-h-full space-y-4">
      <div>
        ...
        <p className="text-xs text-[rgba(153,197,255,0.5)] mt-0.5">
          Quick quote or build room-by-room — prices update as you type
        </p>
      </div>
        ...
        <div className="space-y-4">
            ...
            <div className="px-4 py-3 border-b border-[rgba(153,197,255,0.08)] flex items-center justify-between">
              <div>
                ...
                <p className="text-[10px] text-[rgba(153,197,255,0.4)] mt-0.5">
                  Every price derives from this — change it and all quotes update instantly
                </p>
              </div>
              {rateFromSync && (
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (!isNaN(v) && v > 0) {
                      setHourlyRate(v);
                      setRooms((prev) =>
                        prev.map((r) => (r.customised ? r : { ...r, userPrice: null }))
                      );
                    }
                  }}
                  onBlur={(e) => {
          ...
          <GCard>
            <div className="px-4 py-3 border-b border-[rgba(153,197,255,0.08)]">
              ...
              <p className="text-[10px] text-[rgba(153,197,255,0.4)] mt-0.5">
                {cleanTypeObj.desc} · ×{cleanTypeObj.mult} time multiplier
              </p>
            </div>
            <div className="p-3 grid grid-cols-2 sm:grid-cols-4 gap-2">
              {CLEAN_TYPES.map((ct) => {
                ...
                return (
                  <button
                    key={ct.id}
                    onClick={() => handleCleanTypeChange(ct.id)}
                  ...
                  >
                    <p
                      className={`text-xs font-black ${on ? 'text-white' : 'text-[rgba(153,197,255,0.7)]'}`}
                    >
                      {ct.label}
                    </p>
                    <p
                      className={`text-[10px] font-mono mt-1 font-bold ${on ? 'text-[#99c5ff]' : 'text-[rgba(153,197,255,0.35)]'}`}
                    >
                      ×{ct.mult}
                    </p>
                  </button>
          ...
          {mode === 'quick' && (
            <GCard>
              <div className="px-4 py-3 border-b border-[rgba(153,197,255,0.08)]">
                ...
                <p className="text-[10px] text-[rgba(153,197,255,0.4)] mt-0.5">
                  Select bedrooms — price calculates instantly
                </p>
              </div>
              <div className="p-3 grid grid-cols-5 gap-2">
                {PROPERTY_SIZES.map((ps) => {
                  ...
                  return (
                    ...
                    >
                      <p
                        className={`text-xs font-black leading-tight text-center ${on ? 'text-white' : 'text-[rgba(153,197,255,0.7)]'}`}
                      >
                        {ps.label}
                      </p>
                      <p
                        className={`text-sm font-black tabular-nums mt-1 ${on ? 'text-[#99c5ff]' : 'text-emerald-400'}`}
                      >
                        {fmt(price)}
                      </p>
                      <p
                        className={`text-[10px] mt-0.5 ${on ? 'text-[rgba(255,255,255,0.5)]' : 'text-[rgba(153,197,255,0.35)]'}`}
                      >
                        {ps[hrsKey]}h
                      </p>
                    </button>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-[10px] text-[rgba(153,197,255,0.45)]">
                      UK market range · {propSizeObj.label} {cleanTypeObj.label.toLowerCase()}
                    </p>
                    <p className="text-xs font-bold text-white mt-0.5">
                      {fmt(propSizeObj.mkMin)} – {fmt(propSizeObj.mkMax)}
                    </p>
                  </div>
                  <p className={`text-xs font-bold text-right ${mktStatus.color}`}>
                    {mktStatus.label}
                  </p>
                </div>
              </div>
          ...
          {mode === 'rooms' && (
            <GCard>
              ...
              {/* Add room chips */}
              <div className="px-4 py-3 border-b border-[rgba(153,197,255,0.08)]">
                <p className="text-[10px] font-bold tracking-[0.12em] uppercase text-[rgba(153,197,255,0.4)] mb-2">
                  Tap to add a room
                </p>
                <div className="flex flex-wrap gap-1.5">
                  {ROOM_TYPES.map((rt) => (
              ...
              {rooms.length === 0 ? (
                <div className="px-4 py-10 text-center">
                  <p className="text-sm text-[rgba(153,197,255,0.3)] font-semibold">
                    Tap a room above to start building
                  </p>
                </div>
              ) : (
                <>
                  {/* Column headers */}
                  <div
                    className="grid gap-2 px-4 py-2 bg-[rgba(153,197,255,0.03)] border-b border-[rgba(153,197,255,0.06)]"
                    style={{ gridTemplateColumns: '2fr 60px 2fr 80px 28px' }}
                  >
                    {['Room', 'Qty', 'Condition', 'Price £', ''].map((h) => (
                      <span
                        key={h}
                        className="text-[10px] font-bold tracking-[0.12em] uppercase text-[rgba(153,197,255,0.35)]"
                      >
                        {h}
                      </span>
                    ))}
                  ...
                  <div className="divide-y divide-[rgba(153,197,255,0.06)]">
                    {rooms.map((r) => {
                      ...
                      const dispPrice =
                        r.customised && r.userPrice !== null ? r.userPrice : autoPrice;
                        ...
                        >
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-[rgba(153,197,255,0.4)]">
                              {roomHrs(r.roomId, r.condition, r.qty).toFixed(1)}h · {estMins}min
                            </p>
                          </div>
                          ...
                          <div className="flex items-center gap-0.5">
                            <button
                              onClick={() =>
                                r.qty > 1 ? updateRoom(r.id, 'qty', r.qty - 1) : removeRoom(r.id)
                              }
                              className="w-5 h-5 flex items-center justify-center rounded-lg bg-[rgba(153,197,255,0.08)] border border-[rgba(153,197,255,0.15)] text-[rgba(153,197,255,0.5)] hover:text-red-400 text-xs transition-colors"
                            >
                              −
                            </button>
                            ...
                            <button
                              onClick={() => updateRoom(r.id, 'qty', r.qty + 1)}
                              className="w-5 h-5 flex items-center justify-center rounded-lg bg-[rgba(153,197,255,0.08)] border border-[rgba(153,197,255,0.15)] text-[rgba(153,197,255,0.5)] hover:text-[#99c5ff] text-xs transition-colors"
                            >
                              +
                            </button>
                          </div>
                          ...
                          <div>
                            <div className="flex gap-1 flex-wrap">
                              {ROOM_CONDITIONS.map((c) => (
                                <button
                                  key={c.id}
                                  onClick={() => updateRoom(r.id, 'condition', c.id)}
                          ...
                          <div>
                            <input
                              ...
                              onFocus={() => {
                                if (!r.customised) updateRoom(r.id, 'userPrice', autoPrice);
                              }}
                            ...
                            />
                            {r.qty > 1 && (
                              <p className="text-[10px] text-[rgba(153,197,255,0.35)] text-right mt-0.5">
                                {fmt2(dispPrice * r.qty)} total
                              </p>
                            )}
                          </div>
                          ...
                          <button
                            ...
                            className="text-[rgba(153,197,255,0.25)] hover:text-red-400 text-sm transition-colors flex justify-end"
                          >
                            ✕
                          </button>
                        </div>
        ...
        <div className="space-y-4 xl:sticky xl:top-4 self-start">
          {totalPrice > 0 ? (
            ...
            <GCard className="p-8 text-center">
              ...
              <p className="text-sm font-bold text-[rgba(153,197,255,0.5)]">
                Select a size or add rooms
              </p>
              <p className="text-xs text-[rgba(153,197,255,0.3)] mt-1">
                Profit breakdown appears here
              </p>
            </GCard>
          )}
            ...
            <GCard>
              ...
              <div className="divide-y divide-[rgba(153,197,255,0.06)]">
                {[
                  ...
                  ...(addonTotal > 0
                    ? [{ label: 'Add-ons', val: fmt2(addonTotal), note: `${addonHrs}hrs extra` }]
                    : []),
                  {
                    label: 'Total quote',
                    val: fmt2(totalPrice),
                    note: 'what you charge',
                    bold: true,
                  },
                ].map(({ label, val, note, bold }) => (
                  <div
                    key={label}
                    className={`flex items-center justify-between px-4 py-2.5 ${bold ? 'bg-[rgba(153,197,255,0.04)]' : ''}`}
                  >
                    <div>
                      <p
                        className={`text-xs font-semibold ${bold ? 'text-white font-black' : 'text-[rgba(153,197,255,0.7)]'}`}
                      >
                        {label}
                      </p>
                    ...
                    <p
                      className={`font-mono tabular-nums ${bold ? 'text-base font-black text-emerald-400' : 'text-sm text-white'}`}
                    >
                      {val}
                    </p>
                  </div>
...
const GSection = ({ label, subtotal, addLabel, onAdd }) => (
  <div className="flex items-center justify-between px-4 py-2.5 border-b border-[rgba(153,197,255,0.08)] bg-[rgba(153,197,255,0.03)]">
    <div>
      ...
      {subtotal > 0 && (
        <p className="text-[10px] text-[rgba(153,197,255,0.45)]">{fmt2(subtotal)} subtotal</p>
      )}
    </div>
    <button
      onClick={onAdd}
...
const GColHeader = ({ cols }) => (
  ...
  >
    {cols.map((c) => (
      <span
        key={c.label}
        className={`text-[10px] font-bold tracking-[0.12em] uppercase text-[rgba(153,197,255,0.35)] ${c.right ? 'text-right' : ''}`}
      >
        {c.label}
      </span>
    ))}
...
function CommercialTab({
  ...
  const [manHours, setManHours] = useState([
    { id: uid(), role: 'Cleaner', rate: defaultManRate, hrs: 4 },
  ]);
  ...
  const freqObjRaw =
    COMMERCIAL_FREQUENCIES.find((f) => f.id === frequency) ?? COMMERCIAL_FREQUENCIES[0];
  // For custom frequency, use the user's custom weeks as the multiplier
  const freqObj =
    freqObjRaw.unit === 'custom'
  ...
  // Cost totals
  const manHoursCost = manHours.reduce(
    (s, r) => s + (parseFloat(r.rate) || 0) * (parseFloat(r.hrs) || 0),
    0
  );
  const productsCost = products.reduce(
    (s, r) => s + (parseFloat(r.unitCost) || 0) * (parseFloat(r.qty) || 0),
    0
  );
  const equipmentCost = equipment.reduce(
    (s, r) => s + (parseFloat(r.costPerUse) || 0) * (parseFloat(r.qty) || 0),
    0
  );
  ...
  const totalCost =
    manHoursCost + productsCost + equipmentCost + oneTimePerVisit + overheadsCost + addonsCost;
  const totalHrs =
    manHours.reduce((s, r) => s + (parseFloat(r.hrs) || 0), 0) +
  ...
  const handleAI = (result) => {
    ...
    if (manHours.length > 0) {
      setManHours((prev) =>
        prev.map((r, i) => (i === 0 ? { ...r, hrs: result.estimatedHours ?? 4 } : r))
      );
    }
  };
  ...
  return (
    <div className="p-4 lg:p-6 bg-[#010a4f] min-h-full space-y-4">
      <div>
        ...
        <p className="text-xs text-[rgba(153,197,255,0.5)] mt-0.5">
          Cost-build calculator — build up from costs, add margin on top
        </p>
      </div>
        ...
        <div className="space-y-4">
          ...
          <GCard>
            ...
            <div className="p-4 space-y-4">
              <div>
                <p className="text-[10px] font-bold tracking-[0.12em] uppercase text-[rgba(153,197,255,0.45)] mb-2">
                  Sector
                </p>
              ...
              <div
                className={`grid gap-3 ${frequency === 'custom' ? 'grid-cols-3' : 'grid-cols-2'}`}
              >
                <div>
                  <p className="text-[10px] font-bold tracking-[0.12em] uppercase text-[rgba(153,197,255,0.45)] mb-1.5">
                    Frequency
                  </p>
                  <select
                    value={frequency}
                  ...
                  >
                    {COMMERCIAL_FREQUENCIES.map((f) => (
                      <option key={f.id} value={f.id} className="bg-[#010a4f]">
                        {f.label}
                      </option>
                    ))}
                  </select>
                ...
                {frequency === 'custom' && (
                  <div>
                    ...
                    <GInput
                      ...
                      value={customWeeks}
                      onChange={(e) =>
                        setCustomWeeks(Math.max(1, Math.min(52, parseInt(e.target.value) || 1)))
                      }
                      placeholder="e.g. 39"
                      ...
                      max="52"
                    />
                    <p className="text-[9px] text-[rgba(153,197,255,0.35)] mt-1">
                      e.g. schools = 38–39 weeks
                    </p>
                  </div>
                )}
                <div>
                ...
                <div className="flex flex-col justify-end">
                  <div className="px-3 py-2 rounded-xl bg-[rgba(153,197,255,0.04)] border border-[rgba(153,197,255,0.08)]">
                    ...
                    <p className="text-xs font-bold text-white">
                      {contractWeeks} weeks · {Math.round(contractWeeks / 4.33)} months
                    </p>
                  </div>
                </div>
              </div>
          ...
          <GCard>
            ...
            />
            {rateFromSync && (
              <p className="px-4 pt-2 text-[10px] text-emerald-400 font-semibold">
                ✓ Rate synced from your Settings
              </p>
            )}
            {manHours.length > 0 && (
              <>
                ...
                {manHours.map((r) => (
                  <div
                    key={r.id}
                    className="grid gap-2 px-4 py-2 border-t border-[rgba(153,197,255,0.06)] items-center"
                    style={{ gridTemplateColumns: '2fr 1fr 1fr 1fr auto' }}
                  >
                    <GInput
                      value={r.role}
                      onChange={(e) => updateRow(setManHours, r.id, 'role', e.target.value)}
                      placeholder="Role"
                    ...
                    <GInput
                      type="number"
                      value={r.rate}
                      onChange={(e) => updateRow(setManHours, r.id, 'rate', e.target.value)}
                    />
                    <GInput
                      type="number"
                      value={r.hrs}
                      onChange={(e) => updateRow(setManHours, r.id, 'hrs', e.target.value)}
                      step="0.5"
                    />
                    ...
                    <button
                      onClick={() => removeRow(setManHours, r.id)}
                      className="text-[rgba(153,197,255,0.25)] hover:text-red-400 text-sm transition-colors"
                    >
                      ✕
                    </button>
                  </div>
              ...
              <>
                ...
                {products.map((r) => (
                  ...
                  >
                    <GInput
                      value={r.name}
                      onChange={(e) => updateRow(setProducts, r.id, 'name', e.target.value)}
                    ...
                    <GInput
                      type="number"
                      value={r.unitCost}
                      onChange={(e) => updateRow(setProducts, r.id, 'unitCost', e.target.value)}
                      placeholder="0.00"
                    />
                    <GInput
                      type="number"
                      value={r.qty}
                      onChange={(e) => updateRow(setProducts, r.id, 'qty', e.target.value)}
                      placeholder="1"
                    />
                    <span className="text-xs font-mono font-black text-emerald-400 text-right tabular-nums">
                    ...
                    <button
                      onClick={() => removeRow(setProducts, r.id)}
                      className="text-[rgba(153,197,255,0.25)] hover:text-red-400 text-sm transition-colors"
                    >
                      ✕
                    </button>
              ...
              <>
                ...
                {equipment.map((r) => (
                  ...
                  >
                    <GInput
                      value={r.name}
                      onChange={(e) => updateRow(setEquipment, r.id, 'name', e.target.value)}
                      placeholder="e.g. Scrubber dryer"
                      className="text-xs font-sans"
                    />
                    <GInput
                      type="number"
                      value={r.costPerUse}
                      onChange={(e) => updateRow(setEquipment, r.id, 'costPerUse', e.target.value)}
                      placeholder="0.00"
                    />
                    <GInput
                      type="number"
                      value={r.qty}
                      onChange={(e) => updateRow(setEquipment, r.id, 'qty', e.target.value)}
                      placeholder="1"
                    />
                    <span className="text-xs font-mono font-black text-emerald-400 text-right tabular-nums">
                    ...
                    <button
                      onClick={() => removeRow(setEquipment, r.id)}
                      className="text-[rgba(153,197,255,0.25)] hover:text-red-400 text-sm transition-colors"
                    >
                      ✕
                    </button>
            ...
            {oneTimeEquip.length > 0 && (
              <>
                ...
                <div className="flex items-center gap-3 px-4 py-2.5 border-t border-[rgba(153,197,255,0.06)] bg-[rgba(153,197,255,0.03)]">
                  <span className="text-[10px] font-bold text-[rgba(153,197,255,0.5)]">
                    Contract length
                  </span>
                  <GInput
                    type="number"
                ...
                {oneTimeEquip.map((r) => {
                  const perVisit =
                    contractWeeks > 0 ? (parseFloat(r.cost) || 0) / contractWeeks : 0;
                  return (
                    <div
                      key={r.id}
                      className="grid gap-2 px-4 py-2 border-t border-[rgba(153,197,255,0.06)] items-center"
                    ...
                    >
                      <GInput
                        value={r.name}
                        onChange={(e) => updateRow(setOneTimeEquip, r.id, 'name', e.target.value)}
                        placeholder="e.g. Henry hoover"
                        className="text-xs font-sans"
                      />
                      <GInput
                        type="number"
                        value={r.cost}
                        onChange={(e) => updateRow(setOneTimeEquip, r.id, 'cost', e.target.value)}
                        placeholder="0.00"
                      />
                      ...
                      <button
                        onClick={() => removeRow(setOneTimeEquip, r.id)}
                        className="text-[rgba(153,197,255,0.25)] hover:text-red-400 text-sm transition-colors"
                      >
                        ✕
                      </button>
              ...
              <>
                ...
                {overheads.map((r) => (
                  ...
                  >
                    <GInput
                      value={r.label}
                      onChange={(e) => updateRow(setOverheads, r.id, 'label', e.target.value)}
                      placeholder="Travel, PPE, waste disposal…"
                      className="text-xs font-sans"
                    />
                    <GInput
                      type="number"
                      value={r.cost}
                      onChange={(e) => updateRow(setOverheads, r.id, 'cost', e.target.value)}
                      placeholder="0.00"
                    />
                    <button
                      onClick={() => removeRow(setOverheads, r.id)}
                      className="text-[rgba(153,197,255,0.25)] hover:text-red-400 text-sm transition-colors"
                    >
                      ✕
                    </button>
          ...
          {totalCost > 0 ? (
            <GCard>
              <div className="px-4 py-3 border-b border-[rgba(153,197,255,0.08)]">
                ...
                <p className="text-[10px] text-[rgba(153,197,255,0.45)] mt-0.5">
                  Cost: {fmt2(totalCost)} · {totalHrs.toFixed(1)} hrs
                </p>
              </div>
              <div className="p-4 space-y-4">
                {/* Margin slider */}
                <div>
                  <div className="flex justify-between mb-2">
                    <p className="text-[10px] font-bold tracking-[0.12em] uppercase text-[rgba(153,197,255,0.45)]">
                      Target margin
                    </p>
                  ...
                  {[
                    {
                      label: 'Your cost',
                      val: fmt2(totalCost),
                      sub: 'Break-even — below this you lose money',
                    ...
                    },
                    {
                      label: 'Break-even +5%',
                      val: fmt2(breakEven),
                      sub: 'Barely covers costs — too low to quote',
                    ...
                    {
                      label: `At ${targetMargin}% margin`,
                      val: fmt2(atTarget),
                      sub: 'Your target — adjust slider above',
                      border: 'border-[rgba(153,197,255,0.2)]',
                      bg: 'bg-[rgba(153,197,255,0.05)]',
                      text: 'text-white',
                      pick: atTarget,
                    },
                    {
                      ...
                      val: fmt2(rounded),
                      sub: 'Rounded to nearest £5 — recommended',
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <p className={`text-base font-black tabular-nums shrink-0 ${text}`}>
                            {val}
                          </p>
                        </div>
                      </button>
                  ...
                  <div className="flex items-center gap-2 bg-[rgba(153,197,255,0.05)] border border-[rgba(153,197,255,0.15)] rounded-xl px-3 py-2.5">
                    ...
                    {manualPrice && (
                      <button
                        ...
                        className="text-[10px] text-[rgba(153,197,255,0.35)] hover:text-white shrink-0"
                      >
                        reset
                      </button>
                    )}
            ...
            <GCard className="p-8 text-center">
              ...
              <p className="text-xs text-[rgba(153,197,255,0.3)] mt-1">
                Build up man hours, products and overheads
              </p>
            </GCard>
          )}
...
const GCard = ({ children, className = '' }) => (
  <div
    className={`relative overflow-hidden rounded-xl border border-[rgba(153,197,255,0.12)] bg-gradient-to-br from-[#010a4f] via-[#05124a] to-[#0d1e78] ${className}`}
  >
    ...
    <div
      ...
      style={{
        backgroundImage:
          'linear-gradient(#99c5ff 1px, transparent 1px), linear-gradient(90deg, #99c5ff 1px, transparent 1px)',
...
function PriceInput({ label, value, onChange, avgPrice, hint }) {
  return (
    <div className="px-4 py-3 bg-[rgba(153,197,255,0.05)] rounded-xl border border-[rgba(153,197,255,0.12)]">
      <p className="text-[10px] font-bold tracking-[0.12em] uppercase text-[rgba(153,197,255,0.5)] mb-1.5">
        {label}
      </p>
      <div className="flex items-center gap-2">
      ...
      {avgPrice && (
        <p className="text-[10px] text-[rgba(153,197,255,0.35)] mt-1">
          {value
            ? `Market avg for this size: £${avgPrice}`
            : `Market avg: £${avgPrice} — enter your own above`}
        </p>
      )}
...
function ExteriorTab({ accounts: _accounts, onSaveQuote, onAcceptQuote, customers = [] }) {
  ...
  const pwExtraRate = pwExtras.reduce(
    (s, id) => s + (PW_EXTRAS.find((e) => e.id === id)?.addPerSqm ?? 0),
    0
  );
  ...
  return (
    <div className="p-4 lg:p-6 bg-[#010a4f] min-h-full space-y-4">
      ...
      <div>
        ...
        <p className="text-xs text-[rgba(153,197,255,0.5)] mt-0.5">
          Enter your own prices — market averages shown as a guide
        </p>
      </div>
        ...
        <div className="space-y-4">
          ...
          <GCard>
            <div className="px-4 py-3 border-b border-[rgba(153,197,255,0.08)]">
              ...
              <p className="text-sm font-black text-white mt-0.5">
                Which services are you quoting?
              </p>
              ...
              {EXT_SERVICES.map((svc) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p
                      className={`text-xs font-black leading-tight ${on ? 'text-white' : 'text-[rgba(153,197,255,0.7)]'}`}
                    >
                      {svc.label}
                    </p>
                    <p
                      className={`text-[10px] mt-0.5 ${on ? 'text-[#99c5ff]' : 'text-[rgba(153,197,255,0.35)]'}`}
                    >
                      {svc.desc}
                    </p>
                    {on && (
                      <div className="absolute top-2 right-2 w-4 h-4 rounded-full bg-[#1f48ff] flex items-center justify-center text-white text-[9px] font-black">
                        ✓
                      </div>
                    )}
                  </button>
            ...
            <GCard>
              <div className="px-4 py-3 border-b border-[rgba(153,197,255,0.08)]">
                ...
                <p className="text-xs text-[rgba(153,197,255,0.4)] mt-0.5">
                  Used to show market average reference prices
                </p>
              </div>
              <div className="p-3 grid grid-cols-2 sm:grid-cols-3 gap-2">
                {EXT_SIZES.map((sz) => (
                  ...
                  <div className="flex gap-2">
                    {WIN_SCHEDULES.map((sc) => (
                      ...
                      >
                        ...
                        {propSize && (
                          <span
                            className={`block text-[10px] mt-0.5 font-black ${winSchedule === sc.id ? 'text-[#99c5ff]' : 'text-[rgba(153,197,255,0.4)]'}`}
                          >
                ...
                <div>
                  ...
                  <div className="space-y-1.5">
                    {WIN_ACCESS.map((a) => (
                      ...
                      >
                        <span>{a.label}</span>
                        <span
                          className={`font-mono text-[10px] ${a.premium > 0 ? 'text-[rgba(153,197,255,0.4)]' : 'text-[rgba(153,197,255,0.25)]'}`}
                        >
                  ...
                  {winAccess !== 'none' && (
                    <div className="mt-2 flex items-center gap-2 px-3 py-2.5 rounded-xl bg-[rgba(153,197,255,0.05)] border border-[rgba(153,197,255,0.12)]">
                      <span className="text-xs text-[rgba(153,197,255,0.5)] font-bold shrink-0">
                        Your add-on £
                      </span>
                      <input
                        type="number"
                      ...
                      {winAccessCustom && parseFloat(winAccessCustom) !== winAccessSugg && (
                        <button
                          onClick={() => setWinAccessCustom(String(winAccessSugg))}
                          className="text-[10px] text-[rgba(153,197,255,0.35)] hover:text-white shrink-0"
                        >
                          reset
                        </button>
                ...
                <div className="pt-1 border-t border-[rgba(153,197,255,0.08)] space-y-3">
                  ...
                  {(EQUIP_OPTS.windows ?? []).map((g) => (
                    <EquipGroup
                      key={g.id}
                      group={g}
                      value={equip[g.id] ?? g.opts[0].id}
                      onChange={(v) => setEquipVal(g.id, v)}
                    />
                  ))}
                </div>
                {/* Annual value */}
                {winPrice > 0 && (
                  <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
                    <div className="flex-1">
                      ...
                      <p className="text-[10px] text-[rgba(153,197,255,0.4)]">
                        {winSched?.visitsPerYear} visits/yr · £{winPrice}/visit
                      </p>
                    </div>
            ...
            <GCard>
              <div className="px-4 py-3 border-b border-[rgba(153,197,255,0.08)] flex items-center gap-2">
                ...
                <div className="flex-1">
                  ...
                  {gutterPrice > 0 && (
                    <p className="text-xs text-emerald-400 font-bold">
                      £{gutterPrice}/visit · £{gutterAnnual}/yr
                    </p>
                  )}
                </div>
              </div>
              <div className="p-4 space-y-4">
                ...
                >
                  ...
                  <span
                    className={`font-mono font-bold ${gutterConserv ? 'text-amber-400' : 'text-[rgba(153,197,255,0.35)]'}`}
                  >
                ...
                {gutterPrice > 0 && (
                  <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
                    <div className="flex-1">
                      ...
                      <p className="text-[10px] text-[rgba(153,197,255,0.4)]">
                        {gutterFreqObj.visitsPerYear}× per year · £{gutterPrice}/visit
                      </p>
                    </div>
                    <p className="text-xl font-black text-emerald-400 tabular-nums">
                      £{gutterAnnual}
              ...
              <div className="p-4 space-y-4">
                {propSize && (
                  <div className="flex items-center gap-3 px-3 py-3 rounded-xl bg-[rgba(153,197,255,0.06)] border border-[rgba(153,197,255,0.12)]">
                    <div className="flex-1">
                      <p className="text-[10px] font-bold tracking-wide uppercase text-[rgba(153,197,255,0.45)] mb-0.5">
                        Market guide range
                      </p>
                      <p className="text-[10px] text-[rgba(153,197,255,0.4)]">
                        Includes gutter clean + biocide treatment
                      </p>
                    </div>
                    <p className="text-lg font-black text-white">
                      £{roofRange.min.toLocaleString()}–£{roofRange.max.toLocaleString()}
                    </p>
                  </div>
                )}
                <div className="flex items-start gap-2.5 px-3 py-2.5 rounded-xl bg-amber-500/10 border border-amber-500/20">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-[rgba(153,197,255,0.5)] leading-relaxed mt-0.5">
                      Prices vary significantly by pitch, condition, moss coverage and access.
                      Always visit before quoting.
                    </p>
                  </div>
                </div>
                {!propSize && (
                  <p className="text-xs text-amber-400 font-semibold">
                    Select property size above to see guide range.
                  </p>
                )}
                {/* Equipment */}
                <div className="pt-1 border-t border-[rgba(153,197,255,0.08)] space-y-3">
            ...
            <GCard>
              <div className="px-4 py-3 border-b border-[rgba(153,197,255,0.08)] flex items-center gap-2">
                ...
                <div className="flex-1">
                  ...
                  {fasciaPrice > 0 && (
                    <p className="text-xs text-emerald-400 font-bold">£{fasciaPrice}</p>
                  )}
                </div>
              </div>
              <div className="p-4 space-y-4">
                ...
                <PriceInput
                  ...
                  hint={
                    !propSize
                      ? 'Select property size to see market average'
                      : 'Includes fascias, soffits and bargeboards'
                  }
                />
                {/* Conservatory */}
                <button
                  onClick={() => setFasciaConserv((v) => !v)}
                  className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-semibold border transition-all ${
                    fasciaConserv
                      ? 'bg-[#1f48ff]/20 border-[#1f48ff]/50 text-white'
                      : 'bg-[rgba(153,197,255,0.03)] border-[rgba(153,197,255,0.08)] text-[rgba(153,197,255,0.55)] hover:border-[rgba(153,197,255,0.2)]'
                ...
                >
                  ...
                  <span
                    className={`font-mono font-bold ${fasciaConserv ? 'text-amber-400' : 'text-[rgba(153,197,255,0.35)]'}`}
                  >
            ...
            <GCard>
              ...
              <div className="p-4 space-y-3">
                ...
                <p className="text-xs text-[rgba(153,197,255,0.55)] leading-relaxed">
                  Render prices depend on area, render type, staining and access. Use the site notes
                  to capture details for your follow-up quote.
                </p>
              </div>
            </GCard>
          )}
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5">
                    {PW_SURFACES.map((s) => (
                      ...
                      >
                        ...
                        <span
                          className={`text-[10px] font-mono mt-0.5 block ${pwSurface === s.id ? 'text-[#99c5ff]' : 'text-[rgba(153,197,255,0.3)]'}`}
                        >
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      min="0"
                      value={pwArea}
                      onChange={(e) => setPwArea(e.target.value)}
                      placeholder="e.g. 45"
                    ...
                    {PW_EXTRAS.map((ex) => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() =>
                            setPwExtras((prev) =>
                              on ? prev.filter((x) => x !== ex.id) : [...prev, ex.id]
                            )
                          }
                          className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-semibold border transition-all ${
                            on
                        ...
                        >
                          <span>{ex.label}</span>
                          <span
                            className={`font-mono ${on ? 'text-amber-400' : 'text-[rgba(153,197,255,0.3)]'}`}
                          >
                            +£{ex.addPerSqm}/m²
                          </span>
                        </button>
                ...
                {sqm > 0 && (
                  <div className="flex items-center justify-between px-3 py-2.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
                    <div>
                      ...
                      <p className="text-[10px] text-[rgba(153,197,255,0.4)]">
                        {sqm}m² × £{pwRateMin.toFixed(1)}–£{pwRateMax.toFixed(1)}/m²
                      </p>
                    </div>
                    <p className="text-lg font-black text-emerald-400">
                      £{pwPriceMin}–£{pwPriceMax}
                    </p>
                  </div>
                )}
              </div>
          ...
          {services.length > 0 && (
            <GCard className="p-4 space-y-3">
              ...
              {/* Customer autocomplete */}
              <div>
                <label className="block text-[10px] font-bold tracking-[0.12em] uppercase text-[rgba(153,197,255,0.45)] mb-1.5">
                  Customer name
                </label>
                <div className="relative">
                  <input
                    ...
                    <div className="absolute top-full left-0 right-0 mt-1 z-50 bg-[#0d1e78] border border-[rgba(153,197,255,0.18)] rounded-xl overflow-hidden shadow-2xl">
                      {custMatches.map((c) => (
                        ...
                        >
                          ...
                          <span className="text-[rgba(153,197,255,0.4)] text-[10px]">
                            {c.postcode}
                          </span>
                        </button>
                      ))}
                ...
                {customer &&
                  customers.find((c) => c.name.toLowerCase() === customer.toLowerCase()) && (
                    <p className="text-[10px] text-emerald-400 mt-1 font-semibold">
                      ✓ Existing customer — quote will link to their profile
                    </p>
                  )}
                {customer &&
                  !customers.find((c) => c.name.toLowerCase() === customer.toLowerCase()) && (
                    <p className="text-[10px] text-[rgba(153,197,255,0.35)] mt-1">
                      New name — will be created when job is booked
                    </p>
                  )}
              </div>
              <div>
        ...
        <div className="xl:sticky xl:top-4 space-y-4 self-start">
          {services.length === 0 ? (
            <GCard className="p-8 text-center">
              ...
              <p className="text-xs text-[rgba(153,197,255,0.3)] mt-1">
                Enter your own prices for an instant quote
              </p>
            </GCard>
          ) : (
            <GCard>
              ...
              <div className="px-4 py-4 border-b border-[rgba(153,197,255,0.08)]">
                ...
                {totalMin > 0 ? (
                  <p className="text-4xl font-black text-white tabular-nums">
                    £{totalMin}
                    {totalMin !== totalMax && services.includes('pressure') && sqm > 0
                      ? `–${totalMax}`
                      : ''}
                  </p>
                ) : (
                  <p className="text-sm text-[rgba(153,197,255,0.4)]">
                    Enter your prices on the left
                  </p>
                )}
                {services.includes('roof') && (
                  <p className="text-xs text-amber-400 font-semibold mt-1">
                    + roof: guide £{roofRange.min.toLocaleString()}–£
                    {roofRange.max.toLocaleString()}
                  </p>
                )}
                {services.includes('render') && (
                  <p className="text-xs text-[rgba(153,197,255,0.5)] mt-0.5">
                    + render: price on request from £420
                  </p>
                )}
              </div>
              ...
              <div className="divide-y divide-[rgba(153,197,255,0.06)]">
                {services.includes('windows') && winPrice > 0 && (
                  <div className="flex items-center gap-3 px-4 py-2.5">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-[10px] text-[rgba(153,197,255,0.4)]">
                        {winSched?.shortLabel} · {winSched?.visitsPerYear} visits/yr
                      </p>
                    </div>
                    <div className="text-right">
                ...
                {services.includes('gutters') && gutterPrice > 0 && (
                  <div className="flex items-center gap-3 px-4 py-2.5">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-[10px] text-[rgba(153,197,255,0.4)]">
                        {gutterFreqObj.label}
                        {gutterConserv ? ' · incl. conservatory' : ''}
                      </p>
                ...
                {services.includes('fascia') && fasciaPrice > 0 && (
                  <div className="flex items-center gap-3 px-4 py-2.5">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-[10px] text-[rgba(153,197,255,0.4)]">
                        One-off{fasciaConserv ? ' · incl. conservatory' : ''}
                      </p>
                    </div>
                ...
                {services.includes('render') && (
                  <div className="flex items-center gap-3 px-4 py-2.5">
                    ...
                    <p className="text-sm font-black text-[rgba(153,197,255,0.5)] tabular-nums">
                      from £420
                    </p>
                  </div>
                )}
              ...
              {services.includes('windows') && winPrice > 0 && (
                <div className="mx-3 mb-3 mt-1 px-3 py-2.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
                  <p className="text-[10px] font-bold text-emerald-300 uppercase tracking-wide mb-0.5">
                    Window contract value
                  </p>
              ...
              {equipSummary && (
                <div className="mx-3 mb-3 px-3 py-2.5 rounded-xl bg-[rgba(153,197,255,0.05)] border border-[rgba(153,197,255,0.08)]">
                  <p className="text-[10px] font-bold text-[rgba(153,197,255,0.5)] uppercase tracking-wide mb-1">
                    🧰 Equipment plan
                  </p>
                  {equipSummary.split(' | ').map((item, i) => (
                    <p key={i} className="text-[10px] text-[rgba(153,197,255,0.6)] leading-relaxed">
                      • {item}
                    </p>
                  ))}
                </div>
              ...
              <div className="p-3 space-y-2">
                ...
                >
                  {saved
                    ? '✓ Saved & logged to accounts'
                    : customer && totalMin > 0
                      ? 'Save quote + log to accounts →'
                      : !customer
                        ? 'Add customer name to save'
                        : 'Enter your prices to save'}
                </button>
                {customer && totalMin > 0 && !lastQuote && (
                  <p className="text-[10px] text-center text-[rgba(153,197,255,0.35)]">
                    Logs income to Accounts · syncs to Customer history
                  </p>
                )}
                  ...
                  <div className="space-y-2 pt-1">
                    <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-[rgba(153,197,255,0.4)]">
                          Logged to accounts · linked to customer
                        </p>
                      </div>
                    </div>
                    <button
                      onClick={() =>
                        navigate('/scheduler', { state: { customerName: lastQuote.customer } })
                      }
...
const COND_STYLES = {
  ...
  standard: {
    ...
    inactive: 'bg-white text-brand-blue border-blue-200 hover:bg-blue-50',
  },
  dirty: {
...
function WalkthroughTab({ accounts, onSaveQuote, customers = [] }) {
  ...
  const startWalkthrough = () => {
    ...
    setRooms(
      stack.map((typeId) => ({ uid: uid(), typeId, condition: 'standard', addons: [], note: '' }))
    );
    setStep('rooms');
  };

  const updateRoom = (id, upd) =>
    setRooms((prev) => prev.map((r) => (r.uid === id ? { ...r, ...upd } : r)));
  const removeRoom = (id) => setRooms((prev) => prev.filter((r) => r.uid !== id));
  const addRoom = (typeId) => {
    setRooms((prev) => [
      ...prev,
      { uid: uid(), typeId, condition: 'standard', addons: [], note: '' },
    ]);
    setShowAddRoom(false);
  };
  const toggleAddon = (roomId, label) =>
    setRooms((prev) =>
      prev.map((r) => {
        ...
        return {
          ...r,
          addons: has
            ? r.addons.filter((a) => a.label !== label)
            : [...r.addons, { label, price: '' }],
        };
      })
    );
  const updateAddonPrice = (roomId, label, price) =>
    setRooms((prev) =>
      prev.map((r) => {
        if (r.uid !== roomId) return r;
  ...
  const updateCustomAddon = (roomId, addonUid, upd) =>
    setRooms((prev) =>
      prev.map((r) => {
        if (r.uid !== roomId) return r;
  ...
  const removeAddon = (roomId, addonUid, label) =>
    setRooms((prev) =>
      prev.map((r) => {
        if (r.uid !== roomId) return r;
  ...
  const saveQuote = async () => {
    ...
    const roomLines = rooms.map((r) => {
      ...
      return `${type?.label ?? r.typeId} — ${cond?.label ?? r.condition}${
        r.addons.length
          ? ' + ' +
            r.addons
              .filter((a) => a.label)
              .map((a) => `${a.label}${a.price ? ' £' + a.price : ''}`)
              .join(', ')
          : ''
      }${r.note ? ' (' + r.note + ')' : ''}`;
    });
    ...
    return (
      <div className="max-w-xl mx-auto px-4 py-6 space-y-5">
        <div className="text-center pb-1">
          <p className="text-xs font-bold tracking-widest uppercase text-brand-blue mb-1">
            Walkthrough mode
          </p>
          <h3 className="text-xl font-bold text-brand-navy">Walk the property, build the quote</h3>
          <p className="text-sm text-gray-500 mt-1">
            Choose property type to pre-load the room list, then scope each room as you walk around.
          </p>
        </div>
          ...
          <div className="p-4">
            ...
            {propSize && (
              <p className="mt-2 text-xs text-gray-400 text-center">
                {(PROP_ROOM_STACK[propSize] ?? []).length} rooms pre-loaded · you can add more
            ...
            <div className="grid grid-cols-2 gap-2">
              {CLEAN_TYPES.map((ct) => (
                ...
                >
                  ...
                  <span
                    className={`text-[10px] mt-0.5 ${cleanType === ct.id ? 'text-white/60' : 'text-gray-400'}`}
                  >
                    {ct.desc}
                  </span>
                </button>
      ...
      <div className="flex flex-col h-full">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-brand-navy truncate">
                  {PROPERTY_SIZES.find((p) => p.id === propSize)?.label} ·{' '}
                  {CLEAN_TYPES.find((c) => c.id === cleanType)?.label}
                </p>
                <p className="text-xs text-gray-400">
                  {rooms.length} rooms · scope each as you walk through
                </p>
              </div>
                  ...
                  <div className="px-4 py-3 flex items-center justify-between border-b border-gray-100">
                    <div>
                      <p className="text-sm font-bold text-brand-navy">
                        {type?.label ?? room.typeId}
                      </p>
                      <p className="text-xs text-gray-400">
                        {type?.mins ?? 30} min base · £{type?.basePrice ?? 0} base price
                      </p>
                    </div>
                    <div className="flex items-center gap-3">
                          ...
                          <div key={addon.uid ?? addon.label} className="flex items-center gap-2">
                            {addon.custom ? (
                              <input
                                ...
                                value={addon.label}
                                onChange={(e) =>
                                  updateCustomAddon(room.uid, addon.uid, { label: e.target.value })
                                }
                            ...
                            <div className="flex items-center gap-1 shrink-0">
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  addon.custom
                                    ? updateCustomAddon(room.uid, addon.uid, {
              ...
              <Card>
                <div className="p-3">
                  ...
                  <button
                    ...
                    className="mt-2 w-full py-1.5 text-xs text-gray-400 hover:text-gray-600"
                  >
                    Cancel
                  </button>
                </div>
        ...
        <div className="border-t border-gray-200 bg-white px-4 py-3 shrink-0">
          <div className="max-w-xl mx-auto flex items-center gap-4">
            <div className="flex-1">
              <p className="text-xs text-gray-400">
                {rooms.length} rooms · {Math.round(totalHrs * 10) / 10} hrs est.
              </p>
    ...
    <div className="max-w-xl mx-auto px-4 py-6 space-y-4">
      ...
      <div className="px-4 py-3 rounded-sm bg-brand-navy/5 border border-brand-navy/10">
        <p className="text-[10px] font-bold tracking-widest uppercase text-gray-400 mb-1">
          Your breakdown (not shown to customer)
        </p>
        <div className="flex items-baseline gap-2">
          <p className="text-3xl font-black text-brand-navy">£{totalPrice}</p>
          {accounts?.vatRegistered && (
            <p className="text-xs text-gray-500">
              + VAT → client pays £{Math.round(totalPrice * 1.2)}
            </p>
          )}
        </div>
        <p className="text-xs text-gray-400 mt-0.5">
          {rooms.length} rooms · {Math.round(totalHrs * 10) / 10} hrs ·{' '}
          {CLEAN_TYPES.find((c) => c.id === cleanType)?.label}
        </p>
          ...
          {rooms.map((room) => {
            ...
            return (
              <div key={room.uid} className="flex justify-between text-xs text-gray-600">
                <span>
                  {type?.label ?? room.typeId}{' '}
                  <span className="text-gray-400">({cond?.label})</span>
                </span>
          ...
          <div className="flex items-center gap-3 px-4 py-3 rounded-sm bg-emerald-50 border border-emerald-200">
            ...
            <div className="flex-1">
              <p className="text-sm font-bold text-emerald-700">
                Quote saved for {lastQuote.customer}
              </p>
              <p className="text-xs text-emerald-600">
                £{lastQuote.price} · logged to accounts · linked to customer
              </p>
            </div>
          </div>
          <button
...
function CommercialWalkthroughTab({ accounts: _accounts, onSaveQuote, customers = [] }) {
  ...
  const pricePerVisit =
    marginPct < 100 ? Math.round(totalCost / (1 - marginPct / 100)) : totalCost * 10;
  ...
  function addCostRow(type) {
    const row =
      type === 'equipment'
        ? { uid: cwId(), label: '', cost: '', period: 'monthly', uses: '' }
        : type === 'overheads'
          ? { uid: cwId(), label: '', cost: '', period: 'month' }
          : { uid: cwId(), label: '', cost: '' };
  ...
  function updateCostRow(type, uid, field, val) {
    const setter =
      type === 'products' ? setProducts : type === 'equipment' ? setEquipment : setOverheads;
      ...
      defaults
        ...
        .map((typeId) => {
          ...
          return {
            ...
            mins: String(def?.suggestMins ?? 20),
            qty: 1,
            custom: false,
          };
        })
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="max-w-xl mx-auto px-4 py-6 space-y-6">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {CW_FREQS.map((f) => (
                  ...
                  >
                    {f.label}
                    {f.sub && <span className="opacity-60 ml-1">{f.sub}</span>}
                  </button>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                <button
                  onClick={() => setCleanerCount((n) => Math.max(1, n - 1))}
                  className="w-8 h-8 rounded-sm border border-gray-200 text-gray-700 font-bold hover:bg-gray-100"
                >
                  −
                </button>
                <span className="text-base font-black text-brand-navy w-6 text-center">
                  {cleanerCount}
                </span>
                <button
                  onClick={() => setCleanerCount((n) => n + 1)}
                  className="w-8 h-8 rounded-sm border border-gray-200 text-gray-700 font-bold hover:bg-gray-100"
                >
                  +
                </button>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                {knownHrs !== '' && (
                  ...
                  >
                    Clear — build from areas instead
                  </button>
                )}
              </div>
              <div className="flex items-center gap-2">
  ...
  if (step === 'areas') {
      ...
      return (
        <div className="space-y-2">
          ...
          {areas.map((area) => (
            <div
              key={area.uid}
              className="flex items-center gap-2 bg-white border border-gray-200 rounded-sm px-3 py-2"
            >
              {area.custom ? (
                <input
                  value={area.label}
              ...
              <div className="flex items-center gap-1 shrink-0">
                <button
                  onClick={() =>
                    updateArea(side, area.uid, 'qty', Math.max(1, (Number(area.qty) || 1) - 1))
                  }
                  className="w-6 h-6 rounded border border-gray-200 text-gray-500 text-xs font-bold hover:bg-gray-100 flex items-center justify-center"
                >
                  −
                </button>
                <span className="text-sm font-black text-brand-navy w-5 text-center">
                  {area.qty ?? 1}
                </span>
                <button
                  onClick={() => updateArea(side, area.uid, 'qty', (Number(area.qty) || 1) + 1)}
                  className="w-6 h-6 rounded border border-gray-200 text-gray-500 text-xs font-bold hover:bg-gray-100 flex items-center justify-center"
                >
                  +
                </button>
              ...
              <button
                onClick={() => removeArea(side, area.uid)}
    ...
    const totalVisitMins = activeAreas.all.reduce(
      (s, a) => s + (Number(a.mins) || 0) * (Number(a.qty) || 1),
      0
    );
      ...
      <div className="flex flex-col h-full">
              ...
              <div className="bg-brand-navy/5 border border-brand-navy/10 rounded-sm px-4 py-3 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-lg font-black text-brand-navy">
                    {Math.round(totalVisitMins / 6) / 10} hrs
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-[10px] font-bold tracking-widest uppercase text-gray-400">
        ...
        <div className="border-t border-gray-200 bg-white px-4 py-3 shrink-0">
          <div className="max-w-xl mx-auto flex items-center gap-4">
            <div className="flex-1">
              <p className="text-xs text-gray-400">
                {activeAreas.all.length} areas · {Math.round(totalManHrs * 10) / 10} man-hrs
              </p>
              <p className="text-sm font-bold text-brand-navy">
                {Math.round(visitDurHrs * 10) / 10} hrs on site
              </p>
            </div>
            <button
  ...
  if (step === 'costs') {
    ...
    const TAB_HINTS = {
      products:
        'Weekly spend on products/chemicals — auto-divided by your clean frequency to get the per-visit cost.',
      equipment:
        'Enter cost and choose how often you pay it. One-off items are spread across the number of uses you enter.',
      overheads:
        'Regular business costs — toggle monthly or per-visit. Monthly amounts are auto-divided by your clean frequency.',
    };
    ...
    function perVisitPreview(type, row) {
      ...
      if (type === 'equipment') {
        if (row.period === 'weekly')
          return `= £${(raw / Math.max(visitsPerWeek, 0.01)).toFixed(2)} / visit`;
        if (row.period === 'monthly')
          return `= £${(raw / Math.max(visitsPerMonth, 0.01)).toFixed(2)} / visit`;
            ...
            return (
              <div key={row.uid} className="space-y-1">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  />
                  <button
                    onClick={() => removeCostRow(type, row.uid)}
                ...
                {preview && <p className="text-[10px] text-brand-blue pl-2">{preview}</p>}
              </div>
            );
          })}
          <button
    ...
    return (
      <div className="flex flex-col h-full">
        <div className="flex-1 overflow-y-auto">
          <div className="max-w-xl mx-auto px-4 py-6 space-y-6">
            <button
              onClick={() => setStep(knownHrs !== '' ? 'setup' : 'areas')}
              className="text-xs text-gray-400 hover:text-gray-700"
            >
              ← Back to {knownHrs !== '' ? 'setup' : 'areas'}
            </button>
            ...
            <div className="bg-brand-navy/5 border border-brand-navy/10 rounded-sm px-4 py-3 space-y-1">
              ...
              <div className="flex items-baseline justify-between">
                <p className="text-sm text-gray-600">
                  {Math.round(totalManHrs * 10) / 10} man-hrs × £{rate}/hr
                </p>
        ...
        <div className="border-t border-gray-200 bg-white px-4 py-3 shrink-0">
          <div className="max-w-xl mx-auto flex items-center gap-4">
            <div className="flex-1">
              <p className="text-xs text-gray-400">
                Cost £{Math.round(totalCost)} · {marginPct}% margin
              </p>
        ...
        <div className="flex items-baseline justify-between">
          ...
          <div className="text-right">
            ...
            <p className="text-[10px] opacity-50">
              {selectedFreq.label} · {selectedFreq.visitsPerYear} visits/yr
            </p>
          </div>
        </div>
        <div className="border-t border-white/20 pt-3 grid grid-cols-3 gap-3 text-center">
          ...
          <div>
            ...
            <p className="text-sm font-bold">
              £{Math.round(grossMargin)} ({marginPct}%)
            </p>
          </div>
          <div>
          ...
          {activeAreas.int.length > 0 && (
            <div>
              ...
              {activeAreas.int.map((a) => (
                <p key={a.uid} className="text-xs text-gray-600">
                  · {a.label || 'Area'}
                  {(a.qty ?? 1) > 1 ? ` ×${a.qty}` : ''}
                </p>
              ))}
...
export default function PricingCalculator({
  ...
  const ACCENT = {
    ...
    commercial: 'text-brand-blue',
    exterior: 'text-orange-600',
    walkthrough: 'text-violet-600',
    siteaudit: 'text-teal-600',
  };
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50/50">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
        <div>
          <p className="text-xs font-bold tracking-widest uppercase text-brand-blue mb-0.5">
            Pricing
          </p>
          <h2 className="text-2xl font-bold text-brand-navy">Quote builder</h2>
        </div>
        <div
          className={`flex items-center gap-2 px-3 py-2 rounded-sm hidden sm:flex ${
            userHourlyRate
        ...
        >
          <span
            className={`w-2 h-2 rounded-full shrink-0 ${userHourlyRate ? 'bg-emerald-400 animate-pulse' : 'bg-amber-400'}`}
          />
          <div>
            <p
              className={`text-xs font-bold ${userHourlyRate ? 'text-brand-navy' : 'text-amber-700'}`}
            >
              {userHourlyRate
                ? `£${userHourlyRate}/hr · synced from settings`
                : '⚠ No hourly rate set — using estimates'}
            </p>
            <p className="text-xs text-gray-400">
              {userHourlyRate
      ...
      <div className="flex-1 overflow-y-auto">
        {activeTab === 'residential' && (
          <ResidentialTab
            ...
            customers={customers}
            initialQuotes={dbQuotes.filter((q) => q.type === 'residential')}
          />
        )}
        {activeTab === 'commercial' && (
          <CommercialTab
            ...
            onAcceptQuote={handleAcceptQuote}
            customers={customers}
            initialQuotes={dbQuotes.filter((q) => q.type === 'commercial')}
          />
        )}
        {activeTab === 'exterior' && (
          <ExteriorTab
            ...
            onAcceptQuote={handleAcceptQuote}
            customers={customers}
            initialQuotes={dbQuotes.filter((q) => !['residential', 'commercial'].includes(q.type))}
          />
        )}
        {activeTab === 'walkthrough' && (
          <WalkthroughTab accounts={accounts} onSaveQuote={handleSaveQuote} customers={customers} />
        )}
        {activeTab === 'siteaudit' && (
          <CommercialWalkthroughTab
            accounts={accounts}
