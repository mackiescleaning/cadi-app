...
import {
  ...
  addDividend,
  deleteDividend,
  listDirectorLoanEntries,
  addDirectorLoanEntry,
  deleteDirectorLoanEntry,
  dlaBalance,
  DLA_BIK_THRESHOLD,
  listCtAccruals,
  upsertCtAccrual,
...
function calculateVAT({ turnover, businessType, goods, otherInput, firstYear }) {
  const gross = turnover * 1.2;
  const vatCharged = turnover * 0.2;
  const goodsVAT = goods * 0.2;
...
function calculatePension(monthly, profit) {
  ...
  const rate = profit > 50270 ? 0.4 : profit > 12570 ? 0.2 : 0;
...
function GChip({ children, color = 'blue' }) {
  ...
  return (
    <span
      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-black border ${styles[color]}`}
    >
      {children}
    </span>
  );
    ...
    <GCard className="p-4 space-y-3">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <SL>
            Business address{' '}
            {isLtd && <span className="ml-1 text-red-300 font-black">required</span>}
          </SL>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="sm:col-span-2">
          <label className="block text-[10px] font-bold tracking-widest uppercase text-[rgba(153,197,255,0.45)] mb-1">
            Address line 1
          </label>
          <GInput
            type="text"
        ...
        <div className="sm:col-span-2">
          <label className="block text-[10px] font-bold tracking-widest uppercase text-[rgba(153,197,255,0.45)] mb-1">
            Address line 2 <span className="text-[rgba(153,197,255,0.3)]">(optional)</span>
          </label>
          <GInput
            type="text"
            value={line2}
        ...
        <div>
          <label className="block text-[10px] font-bold tracking-widest uppercase text-[rgba(153,197,255,0.45)] mb-1">
            County <span className="text-[rgba(153,197,255,0.3)]">(optional)</span>
          </label>
          <GInput
            type="text"
            value={county}
        ...
        <p className="text-[10px] text-[rgba(153,197,255,0.45)]">
          {saved ? (
            <span className="text-emerald-300 font-black">
              Saved — your next invoice will use this address.
            </span>
          ) : (
            'Used on every invoice you send.'
          )}
        </p>
        <button
          onClick={save}
...
function OverviewTab({ setActiveTab, entityType = 'sole_trader', bizSettings = {}, saveSettings }) {
  ...
  const ytdPaid = invoices.filter(
    (i) => i.status === 'paid' && (i.paidAt ?? '').slice(0, 10) >= YTD_START
  );
  ...
  const INSIGHTS = isLtd
    ? [
        {
          emoji: '💰',
          title: 'Director salary vs dividends',
          body: 'Pay yourself up to £9,100 as salary (no NI), then extract remaining profits as dividends at 8.75%.',
          action: 'tax-tools',
        },
        {
          emoji: '📦',
          title: 'Claim AIA on company equipment',
          body: '£1m Annual Investment Allowance — WFP, carpet cleaner, vans. Reduces your CT bill.',
        },
        {
          emoji: '🏛️',
          title: 'CT600 — file within 12 months',
          body: 'Your CT600 must be filed within 12 months of your accounting year end. Cadi tracks the deadline.',
          action: 'year-end',
        },
        {
          emoji: '🏢',
          title: 'Companies House annual accounts',
          body: 'Private companies must file accounts 9 months after the year end. Missing the deadline is a £150 fine.',
          action: 'year-end',
        },
      ]
    : isDemo
      ? [
          {
            emoji: '🚐',
            title: 'Log 1,620 unlogged miles — save £729',
            body: "At HMRC's 45p/mile rate you have £729 of unclaimed relief sitting idle.",
            action: 'mileage',
          },
          {
            emoji: '🎯',
            title: 'Pension contributions save £820/yr',
            body: '£4,100 into a SIPP = £820 tax relief. Money for retirement AND off your bill.',
            action: 'tax-tools',
          },
          {
            emoji: '⚙️',
            title: 'Claim AIA on new equipment',
            body: 'WFP brush head, carpet cleaner — Annual Investment Allowance = 100% first-year relief.',
          },
          {
            emoji: '🏛️',
            title: 'Q1 MTD update due 5 Aug 2026',
            body: "You're 6 days into Q1 2026/27. Your submission data is ready to preview.",
            action: 'hmrc',
          },
        ]
      : [
          {
            emoji: '🚐',
            title: 'Log every mile — 45p/mile HMRC relief',
            body: 'Record business journeys in the Routes tab. Every mile cuts your tax bill.',
            action: 'mileage',
          },
          {
            emoji: '🎯',
            title: 'Pension contributions cut your tax',
            body: 'SIPP contributions get 20% tax relief on top. Save for retirement, pay less now.',
            action: 'tax-tools',
          },
          {
            emoji: '⚙️',
            title: 'Claim AIA on equipment',
            body: 'WFP brush head, carpet cleaner, van — Annual Investment Allowance = 100% first-year relief.',
          },
          {
            emoji: '🏛️',
            title: 'MTD ITSA quarterly updates',
            body: 'Cadi tracks your quarterly submission windows and maps spend to SA103 boxes.',
            action: 'hmrc',
          },
        ];
  ...
  const headerSub = isLtd
    ...
    : isDemo
      ? 'Live from your invoices · demo expenses'
      : 'Live from your invoices · connect Money tab for expenses';
  const periodLabel = isLtd
    ? `Accounting year · as of ${asOfLabel}`
    : `Tax year 2026/27 · As of ${asOfLabel}`;
  ...
  return (
    <div className="space-y-5">
      ...
      {isLtd && (
        <GAlert type="blue">
          <strong>Limited company</strong> — Corporation Tax applies. Director salary & dividends
          shown below. MTD ITSA does not apply to your company. CT600 is filed via HMRC online or
          your accountant.
        </GAlert>
      )}
      ...
      <div className="grid grid-cols-2 gap-3">
        <GStatCard
          label="YTD income received"
          value={fmt(ytdIncome)}
          valueColor="text-emerald-400"
          sub={`${ytdPaid.length} paid invoices`}
        />
        <GStatCard
          label="Outstanding"
          value={fmt(outstanding)}
          valueColor={outstanding > 0 ? 'text-amber-400' : 'text-emerald-400'}
          sub={overdue > 0 ? `${fmt(overdue)} overdue` : 'Nothing overdue'}
        />
        <GStatCard
          label="YTD expenses"
          value={fmt(ytdExpenses)}
      ...
      {isLtd && (
        <GCard className="p-4 space-y-3">
          ...
          <p className="text-[11px] text-[rgba(153,197,255,0.45)] leading-relaxed">
            Optimal structure: salary up to the secondary NI threshold (no NI for company or
            director), then pay remaining profits as dividends.
          </p>
          <div className="grid grid-cols-3 gap-2">
            {[
              {
                label: 'Director salary',
                val: fmt(directorSalary),
                color: 'text-white',
                sub: 'No NI either side',
              },
              {
                label: 'Remaining profit',
                val: fmt(Math.max(0, annualisedProfit)),
                color: 'text-emerald-400',
                sub: 'Available as dividends',
              },
              {
                label: 'Corporation Tax',
                val: fmt(annualisedProfit > 0 ? ctEst : 0),
                color: 'text-amber-400',
                sub: annualisedProfit <= 50000 ? '19% small profits' : 'Marginal relief',
              },
            ].map(({ label, val, color, sub }) => (
              <div
                key={label}
                className="p-3 rounded-xl bg-[rgba(153,197,255,0.04)] border border-[rgba(153,197,255,0.08)]"
              >
                <SL className="mb-1">{label}</SL>
          ...
          <GAlert type="gold">
            ...
            Your accountant can confirm the optimal salary/dividend split for your personal
            circumstances.
          </GAlert>
        </GCard>
      )}
        ...
        {annualTarget > 0 ? (
          <>
            <div className="h-2 rounded-full bg-[rgba(153,197,255,0.08)] overflow-hidden">
              <div
                className="h-full rounded-full bg-[#1f48ff] transition-all"
                style={{ width: `${Math.min(ytdPct, 100)}%` }}
              />
            </div>
            <div className="flex justify-between mt-1.5 text-[10px] text-[rgba(153,197,255,0.35)]">
              <span>
                {ytdPct}% of {fmt(annualTarget)} target
              </span>
              <span>{fmt(annualTarget - ytdAll)} to go</span>
            </div>
          </>
        ) : (
          <p className="text-xs text-[rgba(153,197,255,0.55)]">
            Set a revenue target in Settings to track yearly progress.
          </p>
        )}
      </GCard>
      ...
      <GCard className="p-4">
        <div className="flex items-center justify-between mb-1">
          <SL>Tax reserve</SL>
          <GChip color={taxReserve >= taxEst ? 'green' : 'amber'}>
            {taxReserve >= taxEst ? '✓ On track' : '⚠ Short'}
          </GChip>
        </div>
        <div className="flex items-baseline gap-2 mb-2">
          ...
          <span className="text-xs text-[rgba(153,197,255,0.4)]">
            saved of {fmt(Math.max(taxEst, isLtd ? 3000 : 5118))} needed
          </span>
        </div>
        <GAlert type="gold">
          {isLtd ? (
            <>
              Set aside <strong>19–25% of company profit</strong> every quarter. CT is due 9 months
              after your year end — don't let it creep up.
            </>
          ) : (
            <>
              Set aside <strong>25% of every invoice</strong> from day one. Every £100 banked now is
              one less surprise in January.
            </>
          )}
        </GAlert>
      </GCard>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {INSIGHTS.map(({ emoji, title, body, action }) => (
          <GCard
            key={title}
            className="p-4 cursor-pointer hover:border-[rgba(153,197,255,0.25)] transition-colors"
...
function NinoForm({ onSave, saving }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!valid(nino)) {
      setErr('Invalid format — expected e.g. QQ123456C');
      return;
    }
    setErr('');
    try {
...
function ComplianceAndAdjustments({
  businessId,
  taxYear,
  adjBusy,
  setAdjBusy,
  ...
  const run = async (label, fn) => {
    ...
    try {
      await fn();
      setAdjResult({ success: true, message: `${label} submitted to HMRC.` });
    } catch (e) {
      setAdjResult({ success: false, message: `${label} failed: ${e.message || e}` });
    } finally {
      setAdjBusy(false);
      setOpen(null);
    }
  ...
  const submitBfLoss = async () => {
    ...
    await submitBroughtForwardLoss({
      businessId: biz,
      taxYear,
      lossAmount: Number(bfLoss.amount),
  ...
  return (
    <GCard className="p-4">
      <SectionDivider
        label="Compliance & adjustments · June 2026"
        right={<GChip color="ghost">MTD ITSA</GChip>}
      />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {/* Accounting basis */}
        <button
          onClick={() => setOpen(open === 'accounting' ? null : 'accounting')}
        ...
        >
          ...
          <p className="text-[10px] text-[rgba(153,197,255,0.5)] mt-0.5">
            Switch cash ↔ accruals in-year
          </p>
        </button>
        {/* Brought-forward loss */}
        <button
          onClick={() => setOpen(open === 'brought' ? null : 'brought')}
          className="text-left px-3 py-3 rounded-xl bg-[rgba(153,197,255,0.04)] border border-[rgba(153,197,255,0.1)] hover:border-[#99c5ff]/30 transition-colors"
        >
          <p className="text-xs font-black text-white">📉 Brought-forward loss</p>
          <p className="text-[10px] text-[rgba(153,197,255,0.5)] mt-0.5">
            Carry a prior-year loss forward
          </p>
        </button>
        {/* Carry-back loss */}
        <button
        ...
        >
          ...
          <p className="text-[10px] text-[rgba(153,197,255,0.5)] mt-0.5">
            Set off against a prior tax year
          </p>
        </button>
      </div>
      ...
      {open === 'accounting' && (
        <div className="mt-3 p-3 rounded-xl bg-[rgba(153,197,255,0.04)] border border-[rgba(153,197,255,0.1)] space-y-3">
          ...
          <button
            onClick={() => run('Accounting basis update', submitAccountingType)}
            disabled={adjBusy}
      ...
      {open === 'brought' && (
        <div className="mt-3 p-3 rounded-xl bg-[rgba(153,197,255,0.04)] border border-[rgba(153,197,255,0.1)] space-y-3">
          <GInput
            ...
            label="Loss amount £"
            value={bfLoss.amount}
            onChange={(e) => setBfLoss({ amount: e.target.value })}
            min="0"
            step="0.01"
            placeholder="0.00"
          />
          <button
            onClick={() => run('Brought-forward loss', submitBfLoss)}
            disabled={adjBusy || !bfLoss.amount}
            className="w-full px-3 py-2 rounded-xl bg-[#1f48ff] text-white text-[11px] font-black disabled:opacity-50"
          >
            {adjBusy ? 'Submitting…' : 'Submit to HMRC'}
          </button>
        ...
        <div className="mt-3 p-3 rounded-xl bg-[rgba(153,197,255,0.04)] border border-[rgba(153,197,255,0.1)] space-y-3">
          <div className="grid grid-cols-2 gap-2">
            <GInput
              ...
              label="Loss amount £"
              value={cbLoss.amount}
              onChange={(e) => setCbLoss((p) => ({ ...p, amount: e.target.value }))}
              min="0"
              step="0.01"
              placeholder="0.00"
            />
            <GInput
              type="text"
              label="Relief tax year"
              value={cbLoss.yearOfRelief}
              onChange={(e) => setCbLoss((p) => ({ ...p, yearOfRelief: e.target.value }))}
              placeholder="2025-26"
            />
          </div>
          <p className="text-[10px] text-[rgba(153,197,255,0.5)]">
            Carries a self-employment loss from {cbLoss.yearLossArose} back into{' '}
            {cbLoss.yearOfRelief || 'a prior year'}. Subject to HMRC carry-back rules — the relief
            amount may differ from the loss declared.
          </p>
          <button
            onClick={() => run('Carry-back loss', submitCarryBack)}
            disabled={adjBusy || !cbLoss.amount || !cbLoss.yearOfRelief}
            className="w-full px-3 py-2 rounded-xl bg-[#1f48ff] text-white text-[11px] font-black disabled:opacity-50"
          >
            {adjBusy ? 'Submitting…' : 'Submit to HMRC'}
          </button>
...
function HmrcTab({ entityType = 'sole_trader', setActiveTab }) {
  ...
  const qData = useMemo(() => {
    return TAX_QUARTERS.map((q) => {
      ...
      const expenses = isDemo ? (demoExp[q.id] ?? 0) : (ytdExpData.byQuarter?.[q.id] ?? 0);
      ...
      const tax = Math.max(0, (net - 12570 / 4) * 0.2);
      return {
        ...q,
        start,
        end,
  ...
  }, [TAX_YEAR, invoices, isDemo, ytdExpData.byQuarter, ytdExpData.rows]);
        ...
        <GCard className="p-5 space-y-4">
          <div className="flex items-start gap-4">
            ...
            <div>
              <p className="text-sm font-black text-white mb-1">
                MTD for Corporation Tax — not yet mandated
              </p>
              <p className="text-[11px] text-[rgba(153,197,255,0.55)] leading-relaxed">
                Making Tax Digital for Corporation Tax is confirmed by HMRC but has not yet been
                mandated. Your CT600 continues to be filed via HMRC Online Services or through your
                accountant.
              </p>
            </div>
          </div>
          <div className="space-y-2 pt-2 border-t border-[rgba(153,197,255,0.08)]">
            {[
              {
                icon: '📅',
                title: 'CT600 deadline',
                body: 'File within 12 months of your accounting period end date.',
              },
              {
                icon: '💷',
                title: 'Corporation tax payment',
                body: 'Pay within 9 months and 1 day of your accounting period end.',
              },
              {
                icon: '🏢',
                title: 'Companies House accounts',
                body: 'File annual accounts within 9 months of your period end.',
              },
            ].map(({ icon, title, body }) => (
              <div
                key={title}
                className="flex gap-3 p-3 rounded-xl bg-[rgba(153,197,255,0.04)] border border-[rgba(153,197,255,0.06)]"
              >
          ...
          <GAlert type="blue">
            Cadi will support MTD for Corporation Tax when it becomes available. Until then, use the{' '}
            <strong>Year End</strong> tab to track your CT filing deadlines.
          </GAlert>
        </GCard>
      </div>
    );
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-xs text-[rgba(153,197,255,0.45)] mt-0.5">
          Quarterly submissions · SA103 mapping · Tax year 2026/27
        </p>
      </div>
      ...
      {launchMode && (
        <div className="px-4 py-3 rounded-xl bg-[#1f48ff]/10 border border-[#1f48ff]/25 flex items-start gap-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-bold text-[#99c5ff]">
              You're MTD-ready — Cadi is keeping your digital records now
            </p>
            <p className="text-[11px] text-[rgba(153,197,255,0.55)] mt-0.5 leading-relaxed">
              Your quarterly totals below build in real time from invoices, bank transactions and
              logged expenses — exactly the figures MTD asks for. Direct submission to HMRC arrives
              later this year. Until then, export your figures for your accountant from the Year End
              tab.
            </p>
          </div>
          {setActiveTab && (
        ...
        <div className="px-4 py-3 rounded-xl bg-amber-500/15 border border-amber-500/30 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-amber-300/70 mt-0.5">
              Your HMRC authorisation has expired. Click "Connect to HMRC" below to relink your
              account — your data is safe and no submissions will be lost.
            </p>
          </div>
        </div>
      )}
      ...
      <GCard className="p-4">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-2">
              {hmrcLoading ? (
                ...
                <div
                  className={`w-2 h-2 rounded-full ${connected ? 'bg-emerald-400 animate-pulse' : 'bg-[rgba(153,197,255,0.3)]'}`}
                />
              )}
              <SL>
                {hmrcLoading
            ...
            <p className="text-sm font-black text-white mb-1">
              {connected
                ? 'Cadi is linked to your HMRC account'
                : launchMode
                  ? 'HMRC connection — coming later this year'
                  : 'Connect Cadi to HMRC'}
            </p>
            ...
            {hmrcError && <p className="text-red-400 text-[10px] mt-1.5">{hmrcError}</p>}
            {/* NINO prompt when connected but no NINO saved */}
            {isLive && connected && !nino && <NinoForm onSave={saveNino} saving={connecting} />}
          </div>
          <div className="flex flex-col items-end gap-2 shrink-0">
            {!isLive && <GChip color="amber">Demo mode</GChip>}
            {isLive && !connected && launchMode && <GChip color="blue">COMING SOON</GChip>}
          ...
          <div className="mt-3 pt-3 border-t border-[rgba(153,197,255,0.08)] flex items-center justify-between">
            <p className="text-[10px] text-[rgba(153,197,255,0.4)]">
              {obligations.length > 0
                ? `${obligations.length} obligation periods loaded`
        ...
        <div className="mt-4 pt-4 border-t border-[rgba(153,197,255,0.08)] grid grid-cols-3 gap-3">
          {[
            {
              icon: '📥',
              label: 'Reads your obligations',
              desc: 'Which quarters are due and when',
            },
            {
              icon: '📤',
              label: 'Submits quarterly updates',
              desc: 'Income + expense summaries to HMRC',
            },
            {
              icon: '🧮',
              label: 'Pulls tax calculation',
              desc: "HMRC's estimated tax liability back",
            },
          ].map(({ icon, label, desc }) => (
          ...
          return (
            <GAlert type={hasPenalties ? 'red' : 'blue'}>
              ...
              {!penaltiesBusy && hasPenalties && (
                <>
                  <strong>
                    HMRC has {items.length} penalt{items.length === 1 ? 'y' : 'ies'} on your
                    account.
                  </strong>{' '}
                  These are factored into your tax calculation. Contact HMRC if you believe a
                  penalty is wrong.
                </>
              )}
            </GAlert>
      ...
      {isLive && connected && nino && (
        <ComplianceAndAdjustments
          ...
          taxYear="2026-27"
          adjBusy={adjBusy}
          setAdjBusy={setAdjBusy}
          adjResult={adjResult}
          setAdjResult={setAdjResult}
        />
      )}
      ...
      <GAlert type="blue">
        ...
        Early voluntary filing from April 2026. Soft landing in year one — no late submission
        penalties on Q1–Q4. Final Declaration replaces your Self Assessment return.
      </GAlert>

      {/* Quarterly obligations timeline */}
      <SectionDivider
        label="2026/27 quarterly obligations"
        right={<GChip color="ghost">Tax year 6 Apr 2026 – 5 Apr 2027</GChip>}
      />
                  ...
                  className={`w-2 h-2 rounded-full ${
                    isFulfilled
                      ...
                      : isActive
                        ...
                        : 'bg-[rgba(153,197,255,0.2)]'
                  }`}
                />
              </div>
              <p className="text-[10px] text-[rgba(153,197,255,0.4)] mb-1">
                {q.start.slice(5).replace('-', ' ')} – {q.end.slice(5).replace('-', ' ')}
              </p>
              <p className="text-[10px] font-bold text-[rgba(153,197,255,0.55)]">
                Due {q.due} {Number(q.end.slice(0, 4)) + (q.id === 'Q4' ? 1 : 0)}
              </p>
              {isFulfilled ? (
                ...
                <p className="text-xs font-black text-emerald-400 tabular-nums mt-1.5">
                  {fmt(q.income)}
                </p>
              ) : (
                <p className="text-[10px] text-[rgba(153,197,255,0.25)] mt-1.5">
                  {isActive ? 'In progress' : 'Not yet'}
                </p>
              )}
            </button>
          );
            ...
            <GCard className="overflow-hidden">
              ...
              <div className="px-4 py-3 grid grid-cols-3 gap-3 border-b border-[rgba(153,197,255,0.06)]">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-[rgba(153,197,255,0.35)]">
                    {isActive ? 'From money tab' : 'Demo'}
                  </p>
                </div>
                <div>
                  <SL className="mb-1">Net profit</SL>
                  <p
                    className={`text-lg font-black tabular-nums ${q.net >= 0 ? 'text-white' : 'text-red-400'}`}
                  >
                    {fmt(q.net)}
                  </p>
                  <p className="text-[10px] text-[rgba(153,197,255,0.35)]">
              ...
              <div className="px-4 py-3 border-b border-[rgba(153,197,255,0.06)]">
                ...
                <div className="space-y-1">
                  {SA103_FIELDS.map(({ box, label, field }) => {
                    ...
                    return (
                      <div key={box} className="flex items-center gap-3 text-xs">
                        <span className="w-14 font-black text-[rgba(153,197,255,0.35)] shrink-0 font-mono text-[10px]">
                          {box}
                        </span>
                  ...
                  <div className="flex items-center gap-3 text-xs pt-2 border-t border-[rgba(153,197,255,0.06)] mt-2">
                    ...
                    <span
                      className={`font-black tabular-nums ${q.net >= 0 ? 'text-white' : 'text-red-400'}`}
                    >
                      {fmt(q.net)}
                    </span>
                  </div>
              ...
              <div className="px-4 py-3 flex gap-2">
                ...
                {isLive && connected && nino && !launchMode ? (
                  ...
                  <button
                    ...
                    title={
                      launchMode
                        ? 'Direct submission arrives later this year — your figures are already prepared'
                        : !isLive
                          ? 'Log in to submit'
                          : !connected
                            ? 'Connect HMRC first'
          ...
          onConfirm={async () => {
            ...
            try {
              ...
              const expensesPayload =
                submitModal.byHmrcField && Object.keys(submitModal.byHmrcField).length > 0
                  ? Object.fromEntries(
                      Object.entries(submitModal.byHmrcField).map(([k, v]) => [
                        k,
              ...
              setSubmitResult({
                success: true,
                message: `${submitModal.label} submitted to HMRC successfully.`,
              });
            } catch (ex) {
      ...
      <SectionDivider
        label="End of year · 2026/27"
        right={<GChip color="ghost">Due 31 Jan 2028</GChip>}
      />
      ...
      {eoyResult && (
        <GAlert type={eoyResult.success ? 'green' : 'red'}>
          {eoyResult.message}
          <button onClick={() => setEoyResult(null)} className="ml-2 opacity-50 hover:opacity-100">
            ×
          </button>
        </GAlert>
      )}
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          <div>
            ...
            <p className="text-[11px] text-[rgba(153,197,255,0.45)] mt-1 leading-relaxed max-w-sm">
              After all 4 quarters are submitted, HMRC produces a cumulative summary of your income
              and expenses. Review it before the Final Declaration — this is where you can claim
      ...
      <GCard className="p-4">
        <div className="flex items-start justify-between gap-3 mb-3">
          <div>
            ...
            <p className="text-[11px] text-[rgba(153,197,255,0.45)] mt-1 leading-relaxed max-w-sm">
              Once you're happy with the BSAS figures, Cadi triggers an intent-to-finalise
              calculation then submits the Final Declaration — completing your tax year with HMRC.
            </p>
          </div>
          {declared ? (
        ...
        {isLive && connected && nino ? (
          <button
            onClick={async () => {
              ...
              try {
                ...
                setEoyResult({
                  success: true,
                  message: 'Final Declaration submitted to HMRC. Tax year 2026/27 complete.',
                });
              } catch (ex) {
                setEoyResult({
                  success: false,
                  message: `Final Declaration failed: ${ex.message}`,
                });
              } finally {
                setEoyBusy(false);
              }
          ...
          <button
            disabled
            className="w-full py-2.5 rounded-xl bg-[rgba(153,197,255,0.04)] border border-[rgba(153,197,255,0.08)] text-xs font-black text-[rgba(153,197,255,0.25)] cursor-not-allowed"
          >
      ...
      <GCard className="p-4 text-center">
        <p className="text-xs text-[rgba(153,197,255,0.4)]">
          Prior year HMRC submissions will appear here once loaded. Connect HMRC and use the
          obligations API with a prior-year date range to populate this view.
        </p>
      </GCard>
    </div>
  );
...
function IncomeTab() {
  ...
  const totalReceived = yearInvs
    .filter((i) => i.status === 'paid')
    .reduce((s, i) => s + invTotal(i), 0);
  const totalOutstanding = totalInvoiced - totalReceived;
  const totalOverdue = yearInvs
    .filter((i) => i.status === 'overdue')
    .reduce((s, i) => s + invTotal(i), 0);
  ...
  const aged30 = unpaid.filter((i) => {
    const d = daysLate(i);
    return d > 0 && d <= 30;
  });
  const aged60 = unpaid.filter((i) => {
    const d = daysLate(i);
    return d > 30 && d <= 60;
  });
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-black transition-all ${
                taxYear === y
                  ...
                  : 'text-[rgba(153,197,255,0.55)] hover:text-white'
              }`}
            >
              {y}
            </button>
        ...
        <span className="text-[10px] text-[rgba(153,197,255,0.35)]">
          {ytdIncome.rows.length} entries
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <GStatCard
          label="YTD income"
          value={fmt(liveYtdIncome)}
          valueColor="text-emerald-400"
          sub={`${ytdIncome.rows.length} payments received`}
        />
        <GStatCard
          label="Outstanding"
          value={fmt(totalOutstanding)}
          valueColor={totalOutstanding > 0 ? 'text-amber-400' : 'text-emerald-400'}
          sub={`${unpaid.length} invoices awaiting`}
        />
        <GStatCard
          label="Overdue"
          value={fmt(totalOverdue)}
          valueColor={totalOverdue > 0 ? 'text-red-400' : 'text-emerald-400'}
          sub={totalOverdue > 0 ? 'Chase up' : 'Clear ✓'}
        />
        <GStatCard
          label="Sources"
          value={String(sourceList.length || 0)}
          sub={
            sourceList
              .slice(0, 2)
              .map((s) => s.label)
      ...
      {sourceList.length > 0 && (
        <>
          <SectionDivider
            ...
            right={<GChip color="green">{fmt(liveYtdIncome)} YTD</GChip>}
          />
              ...
              return (
                <div key={key} className="px-4 py-3">
                  <div className="flex items-center gap-3 mb-1.5">
                    ...
                    <p className="text-sm font-black tabular-nums text-white shrink-0">
                      {fmt(total)}
                    </p>
                    <span className="text-[10px] font-black tabular-nums text-[rgba(153,197,255,0.5)] w-10 text-right shrink-0">
                      {Math.round(pct)}%
                    </span>
                  </div>
      ...
      {unpaid.length > 0 && (
        <>
          <SectionDivider
            ...
            right={<GChip color="amber">{fmt(agedSum(unpaid))} owed</GChip>}
          />
          <GCard className="overflow-hidden divide-y divide-[rgba(153,197,255,0.05)]">
            {[
              {
                ...
                arr: agedCurrent,
                dot: 'bg-[rgba(153,197,255,0.3)]',
                val: 'text-[rgba(153,197,255,0.6)]',
              },
              {
              ...
              <div key={label} className="flex items-center gap-3 px-4 py-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {arr.length > 0 && (
                    <p className="text-[10px] text-[rgba(153,197,255,0.3)] mt-0.5">
                      {arr.map((i) => invCustomerName(i)).join(', ')}
                    </p>
                  )}
                </div>
                <div className="text-right shrink-0">
                  <p
                    className={`text-xs font-black tabular-nums ${arr.length > 0 ? val : 'text-[rgba(153,197,255,0.2)]'}`}
                  >
                    {arr.length > 0 ? fmt2(agedSum(arr)) : '—'}
                  </p>
                  {arr.length > 0 && (
                    <p className="text-[10px] text-[rgba(153,197,255,0.3)]">{arr.length} inv.</p>
                  )}
                </div>
              </div>
            ))}
      ...
      {Object.entries(incomeByMonth).map(([month, rows]) => (
        <div key={month}>
          <SectionDivider
            label={month}
            right={<GChip color="ghost">{fmt(rows.reduce((s, r) => s + r.amount, 0))}</GChip>}
          />
          <GCard className="overflow-hidden divide-y divide-[rgba(153,197,255,0.05)]">
            {rows.map((row) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-xs font-black tabular-nums text-emerald-400 shrink-0">
                    +{fmt2(row.amount)}
                  </p>
                </div>
              );
      ...
      {!ytdIncome.loading && ytdIncome.rows.length === 0 && (
        <GCard className="p-10 text-center">
          ...
          <p className="text-[11px] text-[rgba(153,197,255,0.4)] mt-1 leading-relaxed">
            Send your first invoice, connect your bank in the Money tab,
            <br />
            or log a cash payment manually.
          </p>
        </GCard>
      )}
      ...
      {liveYtdIncome > 0 && (
        <GAlert type="blue">
          <strong>SA103 Box 15:</strong> Total received <strong>{fmt(liveYtdIncome)}</strong> is
          your declared turnover for {taxYear}. Outstanding <strong>{fmt(totalOutstanding)}</strong>{' '}
          counts once paid.
        </GAlert>
      )}
    </div>
...
function ExpensesTab() {
  ...
  const demoCategories = [
    {
      icon: '🚐',
      name: 'Van & Vehicle Costs',
      sub: 'Fuel · Insurance · MOT · Repairs',
      amount: 2890,
      box: 'Box 17',
    },
    {
      icon: '🚗',
      name: 'Mileage (45p/mile HMRC rate)',
      sub: '4,820 miles logged',
      amount: 2169,
      box: 'Box 17',
    },
    {
      icon: '🧴',
      name: 'Cleaning Supplies & Materials',
      sub: 'Products · chemicals · consumables',
      amount: 1640,
      box: 'Box 16',
    },
    {
      ...
      sub: "Public liability · employer's",
      amount: 1200,
      box: 'Box 20',
    },
    {
      icon: '👕',
      name: 'Uniform, PPE & Workwear',
      sub: "Logo'd clothing · gloves · masks",
      amount: 380,
      box: 'Box 27',
    },
    {
      icon: '📱',
      name: 'Phone & Software (business %)',
      sub: 'Mobile plan · Cadi subscription',
      amount: 720,
      box: 'Box 21',
    },
    {
    ...
    },
    {
      ...
      sub: 'Leaflets · Google Ads · website',
      amount: 540,
      box: 'Box 22',
    },
    {
      icon: '🏦',
      name: 'Bank Charges & Professional',
      sub: 'Business account · accountant',
      amount: 358,
      box: 'Box 24',
    },
    {
  ...
  const liveCategories = Object.entries(ytdExpData.byCategory)
    .map(([catKey, total]) => {
      ...
      const bank = ytdExpData.rows.filter(
        (r) => r.category === catKey && r.source === 'bank'
      ).length;
      return {
        icon: display.icon,
        name: display.name,
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-xs text-[rgba(153,197,255,0.45)] mt-0.5">
          Auto-mapped to SA103 boxes on your MTD return
        </p>
      </div>
      ...
      {ytdExpData.loading && !isDemo ? (
        ...
        <GAlert type="amber">
          <strong>No expenses logged yet.</strong> Connect your bank in the Money tab to auto-import
          business spending, or add expenses manually. They'll appear here mapped to the right HMRC
          boxes.
        </GAlert>
      ) : (
        <>
          <GAlert type="green">
            <strong>Total allowable expenses YTD: {fmtNum(ytdTotal)}</strong> · saving approx.{' '}
            <strong>{fmtSave(ytdTotal).replace('~', '')} in tax</strong>. Each row maps to the exact
            SA103 box HMRC uses.
          </GAlert>
          ...
          <GCard className="overflow-hidden divide-y divide-[rgba(153,197,255,0.05)]">
            {showRows.map(({ icon, name, sub, amount, box }) => (
              <div
                ...
                className="flex items-center gap-3 px-4 py-3 hover:bg-[rgba(153,197,255,0.02)] transition-colors"
              >
                <div className="w-8 h-8 rounded-xl bg-[rgba(153,197,255,0.06)] flex items-center justify-center text-base shrink-0">
                  {icon}
                </div>
                <div className="flex-1 min-w-0">
                ...
                <span className="text-[10px] font-black font-mono text-[rgba(153,197,255,0.3)] w-12 text-right shrink-0">
                  {box}
                </span>
              </div>
            ))}
      ...
      {!isDemo && ytdTotal > 0 && (
        <GAlert type="amber">
          <strong>Tip:</strong> Anything uncategorised? Head to the Money tab and tap the
          transaction — Cadi will remember the rule for next time.
        </GAlert>
      )}
    </div>
  );
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-xs text-[rgba(153,197,255,0.45)] mt-0.5">
          Threshold tracker · Flat Rate Scheme comparison · FRS rate: 12%
        </p>
      </div>
      ...
      <GCard className="p-4 space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div>
            <GSelect
              ...
              value={vatScheme}
              onChange={(e) => {
                setVatScheme(e.target.value);
                persistVatFields({ vat_scheme: e.target.value });
              }}
            >
        ...
        {!saveError && vatScheme === 'none' && (
          <GAlert type="blue">
            VAT API support coming — when you register, Cadi will be able to prepare and submit your
            VAT returns directly to HMRC.
          </GAlert>
        )}
        {!saveError && vatScheme !== 'none' && (
          <GAlert type="green">
            VAT scheme saved. Direct VAT return submission to HMRC is coming soon — Cadi will
            connect via the <strong>read:vat write:vat</strong> MTD scope.
          </GAlert>
        )}
      </GCard>
      ...
      <GCard className="p-4 space-y-3">
        ...
        <div>
          <p className="text-2xl font-black text-white tabular-nums mb-2">
            {fmtV(liveTotal)}{' '}
            <span className="text-sm text-[rgba(153,197,255,0.4)] font-normal">
              rolling 12 months
            </span>
          </p>
        ...
        {threshPct < 100 && monthsToThreshold !== null && (
          <div className="flex items-center gap-2 pt-1 border-t border-[rgba(153,197,255,0.06)]">
            <div
              className={`w-1.5 h-1.5 rounded-full shrink-0 ${monthsToThreshold <= 3 ? 'bg-red-400' : monthsToThreshold <= 6 ? 'bg-amber-400' : 'bg-emerald-400'}`}
            />
            <p className="text-[11px] text-[rgba(153,197,255,0.5)]">
              At your current run rate of {fmtV(runRate)}/mo, you'd cross the threshold in{' '}
              <span
                className={`font-black ${monthsToThreshold <= 3 ? 'text-red-400' : monthsToThreshold <= 6 ? 'text-amber-400' : 'text-emerald-400'}`}
              >
        ...
        {threshPct >= 100 && (
          <GAlert type="red">
            <strong>Your rolling 12-month turnover has exceeded £90,000.</strong> You must register
            for VAT within 30 days of the month end when you crossed the threshold.
          </GAlert>
        )}
        {threshPct > 85 && threshPct < 100 && (
          <GAlert type="warn">
            You're close to the VAT threshold. Consider registering voluntarily now — it avoids a
            rushed registration and lets you reclaim input VAT from the registration date.
          </GAlert>
        )}
        {!isDemo && liveTotal === 0 && !rolling.loading && (
          <p className="text-[11px] text-[rgba(153,197,255,0.35)]">
            No income found in the last 12 months — connect your bank or log income to track your
            threshold.
          </p>
        )}
      </GCard>
      ...
      <GCard className="p-4 space-y-4">
        <div className="flex items-center justify-between">
          ...
          {runRate > 0 && !isDemo && (
            <span className="text-[10px] text-[rgba(153,197,255,0.35)]">
              seeded from your avg monthly income
            </span>
          )}
        </div>
        <div className="grid grid-cols-2 gap-3">
          ...
          <GInput
            ...
            type="number"
            value={goods}
            onChange={(e) => setGoods(parseFloat(e.target.value) || 0)}
          />
          <GInput
            label="Other input VAT (£)"
            type="number"
            value={otherInput}
            onChange={(e) => setOtherInput(parseFloat(e.target.value) || 0)}
          />
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <GCard className="p-4">
          ...
          {[
            ...
            <div
              key={l}
              className="flex justify-between text-xs py-1.5 border-b border-[rgba(153,197,255,0.06)]"
            >
      ...
      {frsWins ? (
        <GAlert type="green">
          <strong>
            FRS saves you {fmtV(r.saving)}/quarter ({fmtV(r.annualSaving)}/year).
          </strong>{' '}
          As a cleaning business your FRS rate is 12% — you keep the difference between what you
          charge clients and what you pay HMRC.
        </GAlert>
      ) : (
        <GAlert type="blue">
      ...
      {r.isLimited && (
        <GAlert type="amber">
          <strong>Limited Cost Trader test:</strong> Your goods spend ({r.goodsPct.toFixed(1)}% of
          gross) is below the 2% threshold. HMRC applies the 16.5% limited cost rate — FRS may not
          be beneficial.
        </GAlert>
      )}
    </div>
...
function TaxToolsTab({ setActiveTab: _setActiveTab, isDemo = false }) {
  const { user } = useAuth();
  ...
  const mileageRows = showDemoData
    ? [
        ...
        {
          date: '6 Apr',
          journey: 'Johnson → Greenfield',
          purpose: 'Client to client',
          miles: '4.1',
          claim: '1.85',
        },
        {
          date: '7 Apr',
          journey: 'Home → Supply depot',
          purpose: 'Supply run',
          miles: '12.2',
          claim: '5.49',
        },
  ...
  return (
    <div className="space-y-5">
          ...
          <GCard className="p-4 flex gap-4 items-start">
            ...
            <div className="flex-1">
              {showDemoData ? (
                <>
                  <p className="text-sm font-black text-white mb-1">
                    Good — but £1,847 of relief still unclaimed
                  </p>
                  <p className="text-[11px] text-[rgba(153,197,255,0.45)] leading-relaxed mb-3">
                    Fix all 5 actions below and you'll pay roughly £1,847 less tax this year.
                  </p>
                ...
                <>
                  <p className="text-sm font-black text-white mb-1">
                    {score >= 80
                      ? 'Excellent — most allowances claimed.'
                      : score >= 50
                        ? 'Good start — keep going.'
                        : score > 0
                          ? 'Just getting started — log expenses to grow your score.'
                          : 'Connect your bank and log expenses to build your tax efficiency score.'}
                  </p>
                  <p className="text-[11px] text-[rgba(153,197,255,0.45)] leading-relaxed mb-3">
                    Your score rises as Cadi sees the deductions you're claiming. Common categories:
                    vehicle, insurance, supplies, phone & internet, marketing, premises.
                  </p>
                  <div className="flex flex-wrap gap-1.5">
                    {Object.keys(ytdExpData.byCategory ?? {})
                      .slice(0, 5)
                      .map((c) => (
                        <GChip key={c} color="green">
                          ✓ {CAT_DISPLAY[c]?.name ?? c}
                        </GChip>
                      ))}
            ...
            {(showDemoData
              ? [
                  {
                    emoji: '🚐',
                    title: 'Log 1,620 unlogged miles — save £729',
                    body: "At 45p/mile, 1,620 unlogged miles = £729 tax relief you're not claiming.",
                    action: () => setTool('mileage'),
                    color: 'border-emerald-500/30',
                  },
                  {
                    emoji: '🎯',
                    title: 'Pension — save up to £820',
                    body: '£4,100 into a SIPP = £820 tax relief PLUS the money is in your pension.',
                    action: () => setTool('pension'),
                    color: 'border-[#1f48ff]/30',
                  },
                  {
                    emoji: '⚙️',
                    title: 'AIA on equipment — up to £1,000 tax off',
                    body: 'WFP brush, carpet cleaner, wet vac — Annual Investment Allowance = 100% first-year relief.',
                    action: undefined,
                    color: 'border-[#1f48ff]/30',
                  },
                  {
                    emoji: '🏠',
                    title: 'Claim use of home as office — £312/yr',
                    body: '25+ hrs admin at home? £26/mo HMRC flat rate, no receipts needed.',
                    action: undefined,
                    color: 'border-amber-500/30',
                  },
                  {
                    emoji: '📚',
                    title: 'Training & CPD — ~£300 unclaimed',
                    body: 'NCCA membership, cleaning courses, H&S training are all allowable.',
                    action: undefined,
                    color: 'border-amber-500/30',
                  },
                  {
                    emoji: '🏦',
                    title: 'Your accountant fees are deductible',
                    body: 'If Cadi replaces part of your accountant, that saving goes straight to profit.',
                    action: undefined,
                    color: 'border-emerald-500/30',
                  },
                ]
              : [
                  // Generic, evergreen advice — no fabricated personal numbers
                  {
                    ...
                    body: "HMRC's 45p/mile rate applies to the first 10,000 business miles. Log them in Routes — Cadi does the maths.",
                    action: () => setTool('mileage'),
                    color: 'border-emerald-500/30',
                  },
                  {
                    ...
                    body: 'Personal pension contributions attract 20% basic-rate top-up at source. Use the simulator to size it.',
                    action: () => setTool('pension'),
                    color: 'border-[#1f48ff]/30',
                  },
                  {
                    emoji: '⚙️',
                    title: 'AIA on capital equipment',
                    body: 'WFP brush head, carpet cleaner, wet vac — Annual Investment Allowance = 100% first-year relief on up to £1m/yr.',
                    action: undefined,
                    color: 'border-[#1f48ff]/30',
                  },
                  {
                    ...
                    body: "If you spend 25+ hours/month on admin from home, you can claim HMRC's flat-rate of £26/mo with no receipts.",
                    action: undefined,
                    color: 'border-amber-500/30',
                  },
                  {
                    emoji: '📚',
                    title: 'Training & professional fees',
                    body: 'NCCA / CSSA membership, cleaning courses, H&S training, accountant fees — all allowable expenses.',
                    action: undefined,
                    color: 'border-amber-500/30',
                  },
                  {
                    emoji: '🏦',
                    title: 'Insurance & business banking',
                    body: "Public liability, employer's, vehicle, premises insurance — all deductible. Same for your business account fees.",
                    action: undefined,
                    color: 'border-emerald-500/30',
                  },
                ]
              ...
              <GCard
                key={title}
                className={`p-4 border-t-2 ${color} cursor-pointer hover:bg-[rgba(153,197,255,0.03)] transition-colors`}
                onClick={action}
              >
                <span className="text-xl mb-2 block">{emoji}</span>
                <p className="text-xs font-black text-white mb-1">{title}</p>
      ...
      {tool === 'pension' && (
        <>
          <GAlert type="gold">
            <strong>Key fact:</strong> Every £1 into a pension receives 20p government top-up at
            source. At your income level it also reduces your Payment on Account the following
            January.
          </GAlert>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <GCard className="p-4 space-y-4">
              ...
              <div>
                <SL className="mb-2">
                  Monthly contribution: <span className="text-white">{fmtP(monthly)}</span>
                </SL>
                <input
                  type="range"
                  min={0}
                ...
                />
                <div className="flex justify-between text-[10px] text-[rgba(153,197,255,0.35)] mt-1">
                  <span>£0</span>
                  <span>£1,000/mo</span>
                </div>
              </div>
                ...
                {[
                  {
                    ...
                    desc: 'Max flexibility. Vanguard, AJ Bell, Hargreaves Lansdown. Contribute up to £60k/yr. Best for investment control.',
                  },
                  {
                    name: 'Nest Pension',
                    desc: 'Government-backed. 0.3% fees. Simple. Good if you employ staff — Nest covers both you and them.',
                  },
                  {
                    name: 'Lifetime ISA (under 40 only)',
                    desc: '25% govt bonus on up to £4k/yr. First home OR retirement. 25% penalty if accessed before 60.',
                  },
                ].map(({ name, desc }) => (
                  <div
                    key={name}
                    className="p-3 rounded-xl bg-[rgba(153,197,255,0.04)] border border-[rgba(153,197,255,0.1)]"
                  >
            ...
            {[
              {
                ...
                tax: '£0',
                save: 'n/a',
                color: 'text-[rgba(153,197,255,0.4)]',
              },
              {
                band: 'Basic Rate',
                range: '£12,571 – £50,270',
              ...
              <div
                key={band}
                className={`grid grid-cols-5 px-4 py-2.5 text-xs border-b border-[rgba(153,197,255,0.05)] ${hl ? 'bg-[#1f48ff]/05' : ''}`}
              >
                ...
                <span
                  className={`font-mono font-black ${tax !== '£0' ? 'text-amber-400' : 'text-[rgba(153,197,255,0.2)]'}`}
                >
                  {tax}
                </span>
                <span
                  className={`font-mono text-[10px] ${save !== 'n/a' && save !== 'indir' ? 'text-emerald-400 font-black' : 'text-[rgba(153,197,255,0.2)]'}`}
                >
                  {save}
                </span>
              </div>
      ...
      {tool === 'mileage' && (
        <>
          {showDemoData ? (
            <div className="grid grid-cols-2 gap-3">
              ...
              <GStatCard
                ...
                sub="~£729 unclaimed · Log now"
              />
              <GStatCard label="Remaining at 45p" value="5,180" sub="Before 25p rate kicks in" />
            </div>
          ) : (
            <GAlert type="blue">
              Log every business mile in the Money tab's Mileage card. Each entry is mapped to the
              HMRC 45p/25p rates automatically and appears in your SA103 export.
            </GAlert>
          )}
            ...
            <GCard className="overflow-hidden">
              ...
              <div className="divide-y divide-[rgba(153,197,255,0.05)]">
                {mileageRows.map((row) => (
                  <div
                    key={row.journey}
                    className="grid grid-cols-5 px-4 py-3 text-xs hover:bg-[rgba(153,197,255,0.02)] transition-colors"
                  >
          ...
          <GAlert type="amber">
            <strong>Important:</strong> You cannot claim mileage rate AND actual vehicle costs for
            the same vehicle. Choose one method at tax year start and stick with it. Cadi compares
...
function YearEndTab({ entityType = 'sole_trader', bizSettings = {}, isDemo = false }) {
  ...
  const handleExport = (format) => {
    setExportError(null);
    try {
      const pack = buildSaPack({
        taxYearNum: packYearNum,
        income: packIncome,
        expenses: packExpenses,
        entityType,
      });
      const bizName = profile?.business_name || '';
      if (format === 'csv') {
        downloadCsv(
          `cadi-${isLtd ? 'pl' : 'sa103'}-pack-${packYearLabel.replace('/', '-')}.csv`,
          saPackToCsv(pack)
        );
      } else {
  ...
  const exportButtons = (
    <>
      <div className="flex items-center gap-2 mb-3">
        ...
        <select
          value={packYearLabel}
          onChange={(e) => setPackYearLabel(e.target.value)}
        ...
        >
          {recentTaxYears(3).map((y) => (
            <option key={y} value={y} className="bg-[#010a4f]">
              {y}
            </option>
          ))}
        </select>
        {packLoading && (
          <span className="text-[10px] text-[rgba(153,197,255,0.4)]">loading figures…</span>
        )}
      </div>
      <div className="space-y-2">
        {[
          {
            ...
            desc: 'Xero & QuickBooks compatible',
            format: 'csv',
          },
          {
            icon: '📄',
            label: 'Export PDF summary pack',
            desc: isLtd ? 'Print-ready P&L + CT estimate' : 'Print-ready, SA103 boxes on one page',
            format: 'pdf',
          },
          ...
          <button
            ...
            disabled={packLoading || showDemoData}
        ...
        >
          ...
          <div className="flex-1">
            ...
            <p className="text-[10px] text-[rgba(153,197,255,0.3)]">
              Read-only live access — no files
            </p>
          </div>
          <span className="text-[9px] font-black px-2 py-0.5 rounded-full bg-[rgba(153,197,255,0.08)] text-[rgba(153,197,255,0.5)] shrink-0">
            COMING SOON
          </span>
        </button>
      </div>
      {showDemoData && (
        <p className="mt-2 text-[10px] text-[rgba(153,197,255,0.4)]">
          Sign in to export your real figures.
        </p>
      )}
  ...
  const demoTimeline = [
    {
      date: '7 Aug 2025',
      title: 'Q1 MTD Update — Submitted ✓',
      desc: '6 Apr – 5 Jul 2025. £14,210 income · £2,840 expenses.',
      status: 'done',
    },
    {
      date: '7 Nov 2025',
      title: 'Q2 MTD Update — Submitted ✓',
      desc: '6 Jul – 5 Oct 2025. £13,940 income · £2,780 expenses.',
      status: 'done',
    },
    {
      date: '5 Feb 2026',
      title: 'Q3 MTD Update — Submitted ✓',
      desc: '6 Oct – 5 Jan 2026. £13,670 income · £2,720 expenses.',
      status: 'done',
    },
    {
      date: '31 Jan 2026',
      title: '2024/25 Final Paper SA — Submitted ✓',
      desc: 'Last traditional Self Assessment. Tax payment + Payment on Account paid.',
      status: 'done',
    },
    {
      date: '6 Apr 2026',
      title: 'Tax Year 2026/27 Begins',
      desc: 'MTD ITSA mandatory from April 2027 (income > £30k). Voluntary from now.',
      status: 'done',
    },
    {
      date: '5 Aug 2026 · 115 days',
      title: 'Q1 2026/27 MTD Update',
      desc: '6 Apr – 5 Jul 2026. In progress — 6 days of data logged.',
      status: 'upcoming',
    },
    {
      date: '5 May 2027',
      title: 'Q4 2026/27 MTD Update',
      desc: 'Final quarterly update for the 2026/27 tax year.',
      status: 'future',
    },
    {
      date: '31 Jan 2028',
      title: 'Final Declaration 2026/27',
      desc: 'Replaces Self Assessment. Tax payment + 1st Payment on Account due.',
      status: 'future',
    },
  ];
  ...
  const ltdTimeline = [
    {
      date: `31 ${yearEndLabel}`,
      title: 'Accounting year end',
      desc: `Your company's accounting period closes. Start gathering P&L, payroll, and asset records.`,
      status: 'upcoming',
    },
    {
      date: `${ctPayMonth} (9mo + 1 day)`,
      title: 'Corporation Tax payment due',
      desc: 'Pay CT to HMRC online. If you miss this, interest starts accruing immediately.',
      status: 'future',
    },
    {
      date: `${chFilingMonth} (9 months)`,
      title: 'Companies House — annual accounts',
      desc: 'File full or abbreviated accounts. Private companies have 9 months from year end.',
      status: 'future',
    },
    {
      date: `${ct600Month} (12 months)`,
      title: 'CT600 filing deadline',
      desc: 'Corporation Tax return must be filed with HMRC within 12 months of period end.',
      status: 'future',
    },
    {
      date: 'Annual',
      title: 'Confirmation Statement',
      desc: 'File at Companies House (due on your incorporation anniversary each year). £13 filing fee.',
      status: 'future',
    },
    {
      date: 'Each April',
      title: 'PAYE year-end (P60s)',
      desc: 'Issue P60 to each employee/director. RTI submissions continue monthly throughout the year.',
      status: 'future',
    },
  ];
            ...
            <div className="relative pl-4">
              ...
              {ltdTimeline.map(({ date, title, desc, status }) => (
                <div key={title} className="relative mb-4 ml-4">
                  <div
                    className={`absolute -left-[1.4rem] top-1 w-3 h-3 rounded-full border-2 ${
                      status === 'done'
                  ...
                  />
                  <p
                    className={`text-[10px] font-mono mb-0.5 ${status === 'upcoming' ? 'text-amber-400 font-bold' : 'text-[rgba(153,197,255,0.35)]'}`}
                  >
                    {date}
                  </p>
                  <p
                    className={`text-xs font-black mb-0.5 ${status === 'done' ? 'text-emerald-400' : status === 'upcoming' ? 'text-amber-300' : 'text-[rgba(153,197,255,0.6)]'}`}
                  >
                    {title}
                  </p>
                ...
                {ltdChecklist.map(({ key, label }) => (
                  <label key={key} className="flex items-center gap-2.5 text-xs cursor-pointer">
                    <input
                      ...
                      onChange={() => toggle(key)}
                      checked={!!checked[key]}
                      className="accent-[#1f48ff] w-4 h-4 shrink-0 rounded"
                    />
                    <span
                      className={
                        checked[key] ? 'text-[rgba(153,197,255,0.5)] line-through' : 'text-white'
                      }
                    >
                      {label}
                    </span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          {timeline.length === 0 ? (
            <div className="py-6 text-center">
              ...
              {!hmrcConnected && (
                <a
                  href="#hmrc"
                  className="inline-block px-4 py-2 rounded-xl bg-[#1f48ff] text-white text-xs font-black hover:bg-[#3a5eff] transition-colors"
                >
                  Connect HMRC →
                </a>
              )}
        ...
        <div className="space-y-4">
            ...
            <div className="space-y-2">
              {checklist.map(({ key, label, done }) => (
                <label key={key} className="flex items-center gap-2.5 text-xs cursor-pointer group">
                  ...
                  />
                  <span
                    className={`${checked[key] || done ? 'text-[rgba(153,197,255,0.6)]' : 'text-white'}`}
                  >
                    {label}
                  </span>
                </label>
          ...
          <GCard className="p-4">
            ...
            <p className="text-[11px] text-[rgba(153,197,255,0.45)] leading-relaxed mb-3">
              If your tax bill exceeds £1,000, HMRC requires advance payments. Many first-time
              filers are caught off-guard — you pay 150% in January.
            </p>
            {showDemoData ? (
              ...
              <p className="text-xs text-[rgba(153,197,255,0.55)] mb-3">
                Your payments-on-account will appear here after you file your first Final
                Declaration via HMRC.
              </p>
            )}
            <GAlert type="gold">
              Set aside <strong>25% of every invoice</strong> from day one.
            </GAlert>
          </GCard>
          ...
          <GCard className="p-4">
            ...
            <div className="space-y-2 mb-4">
              {[
                'Full P&L — income by type, all expenses',
                'SA103 box-by-box breakdown (ready to file)',
                'All 4 MTD quarterly figures',
                'Mileage log (HMRC-compliant format)',
                'Capital items & AIA calculations',
                'VAT return workings (if applicable)',
              ].map((item) => (
                <div key={item} className="flex items-start gap-2 text-xs">
                  <span className="text-emerald-400 shrink-0">✓</span>
...
function AccountsSetupCoach({ user, onSaved }) {
  ...
  const persist = async () => {
    ...
    try {
      ...
      await supabase
        .from('profiles')
        .upsert({ id: user.id, biz_structure: bizStructure }, { onConflict: 'id' });
...
function LtdFirstUseGuide({ showCt, showDiv, showDla }) {
  const [dismissed, setDismissed] = useState(() => {
    try {
      return localStorage.getItem(LTD_GUIDE_DISMISS_KEY) === '1';
    } catch {
      return false;
    }
  });
  ...
  const dismiss = () => {
    try {
      localStorage.setItem(LTD_GUIDE_DISMISS_KEY, '1');
    } catch {
      /* ignore */
    }
    setDismissed(true);
  ...
  const steps = [
    showCt && {
      icon: '🧮',
      title: 'Set your estimated profit',
    ...
    showDla && {
      icon: '🔁',
      title: 'Track director’s loan account',
  ...
  return (
    <GCard className="p-4 relative">
      ...
      <SL className="mb-2">
        Getting started · {steps.length} step{steps.length === 1 ? '' : 's'} left
      </SL>
      <p className="text-xs text-[rgba(153,197,255,0.55)] mb-4 leading-relaxed">
        First time here? These three tools work together to give you total visibility on what your
        Ltd owes HMRC and Companies House. Tap any step below to see what it does and how to use it.
      </p>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {steps.map((s) => (
          <details
          ...
          >
            <summary className="cursor-pointer list-none px-3 py-2.5 flex items-start gap-2 select-none">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-[10px] text-[rgba(153,197,255,0.45)] mt-0.5 group-open:hidden">
                  Tap to expand
                </p>
              </div>
              <span className="text-[10px] text-[rgba(153,197,255,0.5)] shrink-0 group-open:rotate-180 transition-transform">
                ▾
              </span>
            </summary>
...
function SalaryDividendCard({ salary, taxYearDividends, ctDue, ctPaid, isDemo, onSaveSalary }) {
  ...
  useEffect(() => {
    setSalaryDraft(String(salary || 0));
  }, [salary]);
  ...
  const persistSalary = async () => {
    ...
    setSaveErr(null);
    try {
      await onSaveSalary(Number(salaryDraft) || 0);
    } catch (e) {
  ...
  return (
    <GCard className="p-4">
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-[10px] text-amber-300/60 mt-0.5">
              {fmt(ctToFund)} Corporation Tax still to fund · {fmt(personalTax.total)} personal tax
              on salary + dividends
            </p>
          </div>
      ...
      {paHeadroom > 0 && (
        <GAlert type="blue">
          Your salary is <strong>{fmt(paHeadroom)}</strong> below the £
          {PERSONAL_ALLOWANCE.toLocaleString()} personal allowance — salary up to that level is
          income-tax-free and reduces the company's Corporation Tax bill. Worth reviewing with your
          accountant.
        </GAlert>
      )}
...
function LtdTab({ bizSettings = {}, saveSettings, isDemo = false }) {
  ...
  const reload = useCallback(async () => {
    if (isDemo) {
      setDividends([
        {
          id: 'd1',
          declared_on: '2026-04-30',
          paid_on: '2026-04-30',
          amount: 5000,
          shareholder: 'Director',
          tax_year: currentTaxYearLabel,
          voucher_ref: 'DV-001',
        },
        {
          id: 'd2',
          declared_on: '2026-05-31',
          paid_on: '2026-05-31',
          amount: 3000,
          shareholder: 'Director',
          tax_year: currentTaxYearLabel,
      ...
      setDla([
        {
          ...
          description: 'Personal expense reimbursed',
          amount: -250,
          category: 'repaid',
        },
        {
      ...
      setAccruals([
        {
          ...
          ct_paid: 0,
          status: 'open',
          ...deriveCtDeadlines(currentPeriodEnd),
        },
      ]);
      setLoading(false);
      return;
  ...
  return (
    <div className="space-y-5">
      ...
      <GAlert type="blue">
        <strong>Money confidence:</strong> CT accrual updates every time you change your estimated
        profit. Dividends are logged with vouchers for HMRC compliance. Director's loan balance
        warns you above the £10,000 benefit-in-kind threshold (s.455).
        {(() => {
          let dismissed = false;
          try {
            dismissed = localStorage.getItem(LTD_GUIDE_DISMISS_KEY) === '1';
          } catch {
            /* ignore */
          }
          return dismissed ? (
            <button
              onClick={() => {
                try {
                  localStorage.removeItem(LTD_GUIDE_DISMISS_KEY);
                } catch {
                  /* ignore */
                }
...
function CtAccrualCard({ accrual, periodStart, periodEnd, onSave, ctFunded }) {
  ...
  return (
    <GCard className="p-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        ...
        <GStatCard
          label="CT due (estimated)"
          value={fmt(liveCT)}
          sub={liveCT > 50000 ? 'Marginal-relief band' : 'Small-profits rate'}
      ...
      <div className="mb-4">
        <div className="flex justify-between text-[10px] mb-1.5">
          <span className="text-[rgba(153,197,255,0.5)] font-black uppercase tracking-wider">
            Funded
          </span>
        ...
        <div className="h-2 bg-[rgba(153,197,255,0.08)] rounded-full overflow-hidden">
          <div
            className={`h-full transition-all ${ctFunded >= 100 ? 'bg-emerald-500' : ctFunded >= 50 ? 'bg-amber-400' : 'bg-red-500'}`}
            style={{ width: `${ctFunded}%` }}
          />
        </div>
      </div>
...
function DividendsCard({ dividends, ytd, taxYear, onAdd, onDelete }) {
  ...
  const submit = async (e) => {
    ...
    try {
      await onAdd({
        declared_on: form.declared_on,
        paid_on: form.declared_on,
  ...
  return (
    <GCard className="p-4">
      ...
      {ytd > 500 && (
        <GAlert type="warn">
          £500 dividend allowance used — anything above is taxable (8.75% basic / 33.75% higher).
        </GAlert>
      )}
      ...
      {open && (
        <form
          onSubmit={submit}
          className="space-y-2 mt-3 p-3 rounded-xl bg-[rgba(153,197,255,0.04)] border border-[rgba(153,197,255,0.1)]"
        >
          <div className="grid grid-cols-2 gap-2">
            <GInput
              type="date"
              ...
              onChange={(e) => setForm((f) => ({ ...f, declared_on: e.target.value }))}
              required
            />
            <GInput
              type="number"
            ...
            <GInput
              ...
              onChange={(e) => setForm((f) => ({ ...f, shareholder: e.target.value }))}
            />
            <GInput
              label="Voucher ref"
              value={form.voucher_ref}
              onChange={(e) => setForm((f) => ({ ...f, voucher_ref: e.target.value }))}
            />
          </div>
          <button
            type="submit"
            disabled={busy}
            className="w-full px-3 py-2 rounded-xl bg-emerald-500 text-white text-[11px] font-black disabled:opacity-50"
          >
        ...
        {dividends.length === 0 && (
          <div className="text-center py-4 px-3">
            ...
            <p className="text-[10px] text-[rgba(153,197,255,0.35)] leading-relaxed">
              Each dividend needs a board minute and a voucher. Cadi stores them automatically when
              you click <strong>+ Log dividend</strong>.
            </p>
          </div>
        )}
        {dividends.map((d) => (
          <div
            key={d.id}
            className="flex items-center justify-between gap-2 px-3 py-2 rounded-lg bg-[rgba(153,197,255,0.03)]"
          >
            <div className="min-w-0">
              <p className="text-xs font-black text-white truncate">
                {fmt(d.amount)} · {d.shareholder}
              </p>
              <p className="text-[10px] text-[rgba(153,197,255,0.45)]">
                {d.declared_on}
                {d.voucher_ref && ` · ${d.voucher_ref}`}
              </p>
            </div>
            <button
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      await onAdd({
        ...
        amount: sign * Math.abs(Number(form.amount)),
    ...
    <GCard className="p-4">
      <div className="flex items-start justify-between mb-3">
        <div>
          ...
          <p
            className={`text-xl font-black tabular-nums ${balance > 0 ? 'text-amber-300' : 'text-emerald-400'}`}
          >
            {balance >= 0 ? fmt(balance) : `(${fmt(Math.abs(balance))})`}
          </p>
          <p className="text-[10px] text-[rgba(153,197,255,0.45)]">
            {balance > 0 ? 'Director owes company' : 'Company owes director'}
          </p>
        </div>
        <button
          onClick={() => setOpen((o) => !o)}
      ...
      <div className="mt-3 space-y-1.5 max-h-72 overflow-y-auto">
        {entries.length === 0 && (
          <div className="text-center py-4 px-3">
            ...
            <p className="text-[10px] text-[rgba(153,197,255,0.35)] leading-relaxed">
              Log any cash, expense, or loan movement between you and the company. Positive amounts
              mean the director owes the company.
            </p>
          </div>
        )}
        {entries.map((e) => (
...
export default function AccountsTab() {
  const { user } = useAuth();
  const { invoices } = useInvoices();
  const _businessId = useBusinessId();
                  ...
                  <span
                    className={`text-[9px] px-1.5 py-0.5 rounded-full font-black ${
                      activeTab === id
                        ...
                        : 'bg-[rgba(153,197,255,0.1)] text-[rgba(153,197,255,0.6)]'
                    }`}
                  >
                    {badge}
                  </span>
