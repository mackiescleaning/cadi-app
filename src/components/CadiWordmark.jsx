...
export default function CadiWordmark({ height = 28, dark = false, className = "" }) {
  ...
  const textColor  = dark ? '#010828' : 'white';
  ...
  return (
    <span
      ...
      style={{ fontFamily: "'Plus Jakarta Sans','Inter',sans-serif", fontWeight: 600, fontSize: height, lineHeight: 1, letterSpacing: '-0.04em' }}
    >
      {/* C */}
      <span style={{ color: dark ? 'rgba(1,8,40,0.55)' : 'rgba(196,211,255,0.65)' }}>C</span>

      {/* a */}
      <span style={{ background: accentGrad, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', filter: 'drop-shadow(0 0 5px rgba(79,120,255,0.4))' }}>a</span>

      {/* d */}
      <span style={{ background: accentGrad, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', filter: 'drop-shadow(0 0 5px rgba(79,120,255,0.4))' }}>d</span>

      {/* i — custom dot replaces default tittle */}
      <span style={{ position: 'relative', color: dark ? 'rgba(1,8,40,0.3)' : 'rgba(255,255,255,0.3)' }}>
