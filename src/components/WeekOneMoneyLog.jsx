...
export default function WeekOneMoneyLog({ onClose, onLogged }) {
      ...
      setLogged((prev) => [
        ...prev,
        {
          ...
          label:
            ...
            (kind === 'income'
              ? 'Cash job'
              : QUICK_EXPENSE_CATS.find((c) => c.id === category)?.label),
          amount: amt,
        },
      ]);
      setLabel('');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div
        className="relative w-full max-w-md rounded-2xl border border-[rgba(153,197,255,0.15)] overflow-hidden shadow-2xl max-h-[90vh] overflow-y-auto"
      ...
      >
        ...
        <div className="px-5 py-4 border-b border-[rgba(153,197,255,0.1)] flex items-start justify-between gap-3">
          ...
          <button
            onClick={onClose}
            className="text-[rgba(153,197,255,0.4)] hover:text-white text-xl leading-none shrink-0"
          >
            ×
          </button>
        </div>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="space-y-3">
            <input
              ...
              placeholder={
                kind === 'income'
                  ? 'Who paid you? (e.g. Mrs Johnson)'
                  : 'What was it? (e.g. Shell garage)'
              }
            ...
            <div className="rounded-xl border border-[rgba(153,197,255,0.1)] divide-y divide-[rgba(153,197,255,0.06)]">
              {logged.map((e, i) => (
                <div key={i} className="flex justify-between px-3 py-2 text-xs">
                  <span className="text-[rgba(153,197,255,0.6)] truncate">
                    {e.kind === 'income' ? '💷' : '💸'} {e.label}
                  </span>
                  <span
                    className={`font-black tabular-nums shrink-0 ${e.kind === 'income' ? 'text-emerald-400' : 'text-amber-300'}`}
                  >
          ...
          {enough ? (
            <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/25">
              <p className="text-xs font-black text-emerald-300">
                ✓ That's enough for your first report
              </p>
              <p className="text-[11px] text-[rgba(153,197,255,0.55)] mt-0.5">
                Add more if you like — the fuller the picture, the sharper the report. Your
                walkthrough is now unlocked.
              </p>
            </div>
          ) : (
          ...
          >
            <p className="text-[10px] font-black uppercase tracking-widest text-amber-300 mb-0.5">
              ⚡ Cadi can do this for you
            </p>
            <p className="text-[11px] text-white leading-snug">
              On Pro, Cadi connects securely to your bank and logs every transaction automatically —
              no typing, ever. <span className="font-black text-amber-300">See Pro →</span>
            </p>
          </button>
