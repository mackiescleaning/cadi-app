...
let _ackIdx = 0;
function ack() {
  return ACKS[_ackIdx++ % ACKS.length];
}
...
function parsePricing(input) {
  ...
  if (
    /\b(varies|vary|depends|depend|quote each|quote every|custom|bespoke|each job|case.by.case|i['']?ll quote|you tell me|not sure|no fixed)\b/.test(
      t
    )
  ) {
    return {
  ...
  if (
    /\b(per\s*(?:square\s*me?t(?:re|er)|sq\.?\s*m(?:e?t(?:re|er))?|m²|sqm)|\/\s*m²|per\s*sq)\b/.test(
      t
    )
  ) {
  ...
  if (/\b(per\s*hour|an\s*hour|\/\s*hr|\/\s*hour|hourly|by\s*the\s*hour)\b/.test(t)) {
    const rate = extractGBP(t);
    const minHM =
      t.match(/(\d+(?:\.\d+)?)\s*(?:hour|hr)s?\s*min(?:imum)?/i) ||
  ...
  // Per bed / per size
  if (
    /\b(per\s*bed|by\s*(?:bed|size|bedroom|room)|1\s*bed|2\s*bed|3\s*bed|different\s*(?:prices?|rates?)[\s\w]*(?:for|per|by)|size|bedroom|pricing\s*matrix)\b/.test(
      t
    )
  ) {
  ...
  if (
    price &&
    /\b(per\s*job|flat|fixed|each|a\s*job|for\s*a|for\s*the|per\s*visit|per\s*clean)\b/.test(t)
  ) {
    return {
      pricing_type: 'fixed',
      confidence: 0.9,
  ...
  return {
    ...
    extracted_values: {},
    follow_up_question:
      'How do you usually price this — by the hour, a fixed price per job, or does it vary?',
  };
}
...
function extractBedPrices(text) {
  ...
  SIZES.forEach(({ label, patterns }) => {
    for (const pat of patterns) {
      ...
      if (match) {
        found.push({ size_label: label, price: parseFloat(match[1]) });
        break;
      }
    }
  });
...
function parseDuration(input) {
  ...
  if (rangeM)
    return {
      duration_value: parseFloat(rangeM[1]),
      range_max: parseFloat(rangeM[2]),
      duration_unit: 'hours',
      is_range: true,
    };
...
function parseIncEx(input) {
  ...
  const included = [],
    excluded = [];
  const NOT_RE =
    /\b(not\s+includ|don'?t\s+do|no\s+(?:oven|iron|carpet|outside|window|conserv)|doesn'?t\s+includ|not\s+cover|exclud|except)\b/i;
...
async function callNlp(step, input) {
  try {
    ...
    const res = await fetch(`${SUPABASE_URL}/functions/v1/parse-service-nlp`, {
      method: 'POST',
      headers: {
...
function Bubble({ role, children }) {
  ...
  return role === 'cadi' ? (
    <div
      className={`${base} rounded-tl-sm bg-[#05124a] text-white border border-[rgba(153,197,255,0.12)]`}
    >
      {children}
    </div>
  ) : (
...
export default function ServiceChat({
  ...
  const [, setPendingFollowUp] = useState(null);
  ...
  function skipStep(nextStep) {
    ...
    setTextInput('');
    cadiSay(
      "No worries, leaving that blank. Front Desk will work with what we've got.",
      () => {
        setStep(nextStep);
        scrollDown();
      },
  ...
  function nextAfterDuration(currentCollected) {
    if (chatMode === 'quick' && !quickDoneShown) {
      ...
      cadiSay(
        "Last few questions, then we're done.\n\nWhat's normally included in a " +
          (currentCollected?.name || collected.name) +
          "? And what's NOT included that customers sometimes ask about? You can answer both together — I'll sort it out.",
  ...
  function nextAfterInclusions() {
    cadiSay('How often do customers usually want this? Weekly, monthly, one-off?', () =>
      setStep(S.FREQUENCY)
    );
  }
  ...
  function saveAndContinue(extraCollected) {
    ...
    const doSave = editService?.id ? updateService(editService.id, record) : createService(record);
      ...
      .then(() => {
        ...
        setTimeout(() => {
          ...
          if (remaining.length) {
            ...
            cadiSay(
              'All done for now. Head to your services menu whenever you want to add more.',
              () => setStep(S.DONE),
              400
            );
          }
    ...
    if (context === 'edit' && editField) {
      ...
      const initial = editService
        ? {
            ...
            duration: editService.duration_value
              ? {
                  duration_value: editService.duration_value,
                  duration_unit: editService.duration_unit,
                }
              : { varies: true },
      ...
      cadiSay(
        fieldMessages[editField] || `Editing *${editService?.name}*.`,
        () => setStep(fieldSteps[editField] || S.PRICING),
        400
      );
      return;
    }
  ...
  function handleName() {
    ...
    if (tooVague) {
      ...
      setTextInput('');
      cadiSay(
        `Got it — '${val}'. Anything more specific? Like 'weekly clean' or 'one-off deep clean'?`,
        () => setStep(S.NAME_CLARIFY)
      );
      return;
    }
    userReply(val, `Got it — *${val}*. Is this residential, exterior, or commercial?`, S.CATEGORY, {
      name: val,
    });
  }
  ...
  function handleCategory(cat) {
    ...
    cadiSay(
      `How do you usually price a ${name}? Tell me how you'd explain it to a customer — by the hour, by the size of the place, fixed price, however you naturally do it.`,
      () => setStep(S.PRICING)
    );
  }
  ...
  async function handlePricing(inputOverride) {
    ...
    if (parsed.confidence < 0.7) {
      ...
      cadiSay(`I want to make sure I get this right — ${parsed.follow_up_question}`, () =>
        setStep(S.PRICING_CLARIFY)
      );
      return;
    }
    ...
    if (parsed.pricing_type === 'custom') {
      ...
      cadiSay(
        `Got it — you quote each job. That works fine. When customers ask through Front Desk, I'll gather the details and flag it for you to quote. Sound good?`,
        () => setStep(S.PRICING_CUSTOM)
      );
      return;
    }
    ...
    if (parsed.pricing_type === 'per_size') {
      if (parsed.extracted_values.pricing_matrix?.length) {
        ...
        cadiSay(
          `Got it — you price by size. Tell me what you charge for each — just type them out, like '£40 for 1 bed, £50 for 2 bed'.`,
          () => setStep(S.PRICING_PERBED)
        );
      }
      return;
    }
  ...
  function handlePricingPerBed() {
    ...
    if (!rows.length) {
      cadiSay(
        'Hmm, I didn\'t quite catch that. Try something like: "1 bed £45, 2 bed £60, 3 bed £80"',
        () => setStep(S.PRICING_PERBED)
      );
      return;
    }
    ...
    cadiSay(`Got it. Here's what I've captured:\n${summary}\n\nLook right?`, () =>
      setStep('pricing_perbed_confirm')
    );
  }
  ...
  function handlePricingCustom(answer) {
    if (answer === 'perfect') {
      ...
      cadiSay(`Roughly how long does a ${collected.name} take you? A range is fine.`, () =>
        setStep(S.DURATION)
      );
    } else {
  ...
  async function handleNotesText() {
    ...
    patch({ [category]: val });
    cadiSay(
      `Got it — saved that as a ${noteLabels[category]} for Front Desk to use when quoting.`,
      () => setStep('notes_saved')
    );
  }
        ...
        <div className="flex gap-2 flex-wrap">
          <QuickBtn
            onClick={() => {
              ...
              cadiSay(
                `How do you usually price a ${collected.name}? Tell me how you'd explain it to a customer — by the hour, by the size of the place, fixed price, however you naturally do it.`,
                () => setStep(S.PRICING)
              );
            }}
          >
    ...
    if (step === S.NAME) {
      return (
        <ChatInput
          ...
          onSubmit={handleName}
          placeholder="e.g. Weekly clean, Gutter clearance…"
        />
      );
    }
    if (step === S.NAME_CLARIFY) {
      return (
        <ChatInput
          value={textInput}
          onChange={setTextInput}
          onSubmit={handleNameClarify}
          placeholder="e.g. Weekly domestic clean…"
        />
      );
    }
        ...
        <div className="flex gap-2">
          {['Residential', 'Exterior', 'Commercial'].map((label) => (
            <CategoryBtn
              key={label}
              label={label}
              active={collected.category === label.toLowerCase()}
      ...
      return (
        <div className="space-y-2">
          <ChatInput
            ...
            placeholder={ph}
          />
          <p className="text-xs text-[rgba(153,197,255,0.35)] px-1">
            Answer naturally — I'll sort out the details.
          </p>
        </div>
      );
    }
    ...
    if (step === S.PRICING_CLARIFY) {
      return (
        <ChatInput
          value={textInput}
          onChange={setTextInput}
          onSubmit={handlePricingClarify}
          placeholder="Clarify your pricing…"
        />
      );
    }
          ...
          <div className="flex gap-2">
            <QuickBtn
              onClick={() => {
                ...
                cadiSay(
                  `Roughly how long does a ${collected.name} take you? A range is fine.`,
                  () => setStep(S.DURATION)
                );
              }}
            >
      ...
      return (
        <div className="space-y-2">
          <ChatInput
            value={textInput}
            onChange={setTextInput}
            onSubmit={handlePricingPerBed}
            placeholder="e.g. 1 bed £40, 2 bed £55, 3 bed £70…"
          />
          <p className="text-xs text-[rgba(153,197,255,0.35)] px-1">
            List each size and price together.
          </p>
        </div>
      );
    }
        ...
        <div className="space-y-2">
          <div className="flex gap-2 flex-wrap">
            <QuickBtn
              onClick={() => {
                addMessage('user', 'Yes');
                cadiSay(`Roughly how long does a ${collected.name} take you?`, () =>
                  setStep(S.DURATION)
                );
              }}
            >
            ...
            <QuickBtn
              ...
              onClick={() => {
                ...
                cadiSay(
                  "Go ahead — give me the full list again and I'll replace what I have.",
                  () => setStep(S.PRICING_PERBED)
                );
              }}
            >
        ...
        <ChatInput
          ...
          onSubmit={() => {
            ...
            if (newRows.length) {
              ...
              cadiSay(
                'Sorry, I didn\'t catch that. Try "4 bed £90".',
                () => setStep('pricing_perbed_addrow'),
                400
              );
            }
          }}
    ...
    if (step === S.PRICING_CUSTOM) {
      return (
        <div className="flex gap-2 flex-wrap">
          ...
          <QuickBtn secondary onClick={() => handlePricingCustom('rough_idea')}>
            Actually let me give you a rough idea
          </QuickBtn>
        </div>
      );
    }
      ...
      return (
        <div className="space-y-2">
          ...
          <button
            ...
            className="text-xs text-[rgba(153,197,255,0.4)] hover:text-[rgba(153,197,255,0.7)] px-1 transition"
          >
            Skip
          </button>
        </div>
        ...
        <div className="flex gap-2 flex-wrap">
          <QuickBtn
            onClick={() => {
              ...
              cadiSay(
                `What's normally included in a ${collected.name}? And what's NOT included?`,
                () => {
          ...
          <QuickBtn secondary onClick={() => saveAndContinue()}>
            Leave it for now
          </QuickBtn>
        </div>
      );
      ...
      return (
        <div className="space-y-2">
          <ChatInput
            value={textInput}
            onChange={setTextInput}
            onSubmit={handleFrequency}
            placeholder="e.g. Weekly or fortnightly / always one-off…"
          />
          <button
            type="button"
            onClick={() => {
    ...
    if (step === S.NOTES_TEXT) {
      return (
        <ChatInput
          value={textInput}
          onChange={setTextInput}
          onSubmit={handleNotesText}
          placeholder="e.g. Add £15 for parking in central London…"
        />
      );
    }
          ...
          <QuickBtn
            ...
            onClick={() => {
              ...
              if (rem.length) {
                addMessage(
                  'cadi',
                  `All saved. You've still got ${rem.join(', ')} to add when you're ready — they're waiting on your menu page.`
                );
              }
    ...
    <div className="fixed inset-0 bg-black/75 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-4">
      ...
      >
        ...
        <div className="relative flex items-center justify-between px-5 py-4 border-b border-[rgba(153,197,255,0.12)] shrink-0">
          ...
          <div className="flex items-center gap-2">
            {onUseForm && (
              <button
                onClick={onUseForm}
                className="text-xs text-[rgba(153,197,255,0.4)] hover:text-[#99c5ff] transition"
              >
                Use form instead
              </button>
            )}
        ...
        <p className="relative px-5 pt-3 text-[10px] text-[rgba(153,197,255,0.4)] shrink-0">
          Cadi uses AI (Anthropic) to help build your service menu. Your answers aren't trained on.{' '}
          <a
            href="/privacy#ai"
            target="_blank"
        ...
        >
          ...
          {messages.map((msg) => (
            <div
              key={msg.id}
              className={`sc-bubble flex ${msg.role === 'cadi' ? 'justify-start' : 'justify-end'}`}
            >
              ...
              <div className="rounded-[18px] rounded-tl-sm border border-[rgba(153,197,255,0.12)] bg-[#05124a] px-4 py-3">
                <div className="flex gap-1.5">
                  {[0, 1, 2].map((i) => (
                    <span
                      key={i}
                      className="h-2 w-2 rounded-full bg-[rgba(153,197,255,0.55)]"
        ...
        {!isTyping && step && (
          <div className="relative px-5 py-4 border-t border-[rgba(153,197,255,0.12)] shrink-0">
            <div className="sc-bubble">{renderInput()}</div>
            {saving && (
