...
const SERVICE_OPTIONS = [
  'Regular Cleaning', 'Deep Clean', 'End of Tenancy', 'Office Clean',
  'School / Education', 'Healthcare / Medical', 'Retail Clean',
  'Window Cleaning', 'Carpet Clean', 'Post-Construction',
  'Industrial / Warehouse', 'Hotel / Hospitality',
];
      ...
      <div className="w-full max-w-lg bg-[#010a4f] rounded-2xl border border-white/10 shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-white/10">
          <div>
            ...
            <p className="text-[#99c5ff]/60 text-xs mt-0.5">Founding 200 members get first access</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-5 max-h-[70vh] overflow-y-auto">
            ...
            <div className="flex flex-wrap gap-2">
              {SERVICE_OPTIONS.map(s => (
                <button
                  ...
                  style={services.includes(s) ? { backgroundColor: EARN_ORANGE + '22', borderColor: EARN_ORANGE } : {}}
                >
                  {s}
                </button>
              ))}
            ...
            <div className="grid grid-cols-2 gap-2">
              {RADIUS_OPTIONS.map(({ value, label }) => (
                <button
                  ...
                  style={radius === value ? { backgroundColor: EARN_ORANGE + '22', borderColor: EARN_ORANGE, color: 'white' } : {}}
                >
                  {label}
                </button>
              ))}
          ...
          <div>
            ...
            <div className="flex gap-3">
              {[{ v: true, l: 'Yes, occasionally' }, { v: false, l: 'No, stay local' }].map(({ v, l }) => (
                <button
                  key={String(v)} type="button"
                  onClick={() => setFlexible(v)}
                  className={`flex-1 py-2.5 rounded-xl border text-sm font-medium transition-all ${
                    flexible === v ? 'text-white' : 'border-white/10 text-[#99c5ff]/60 hover:border-white/20'
                  }`}
                  style={flexible === v ? { backgroundColor: EARN_ORANGE + '22', borderColor: EARN_ORANGE, color: 'white' } : {}}
                >
                  {l}
                </button>
              ))}
          ...
          <div>
            ...
            <div className="relative">
              ...
              />
              <span className="absolute right-4 top-1/2 -translate-y-1/2 text-xs text-[#99c5ff]/40">hours</span>
            </div>
          </div>
