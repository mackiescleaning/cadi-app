    ...
    <div className="border-b border-[rgba(153,197,255,0.06)] last:border-0">
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[10px] text-[rgba(153,197,255,0.4)] mt-0.5">{SOURCE_LABELS[component.source] ?? component.source}</p>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          {awaitingUpload && (
            <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-amber-500/15 text-amber-400 border border-amber-500/20">Upload needed</span>
          )}
          {text && !awaitingUpload && (
            <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-emerald-500/15 text-emerald-400 border border-emerald-500/20">Ready</span>
          )}
          <span className={`text-xs text-[rgba(153,197,255,0.4)] transition-transform ${open ? 'rotate-180' : ''}`}>▾</span>
        </div>
      </button>
        ...
        <div className="px-4 pb-4">
          {text ? (
            ...
            <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-amber-500/8 border border-amber-500/20">
              <span className="text-amber-400 text-xs">Upload the document in Settings → Compliance documents to include in this pack.</span>
            </div>
          ) : (
...
export default function OnboardingPack({ survey, quote, pack: initialPack, onPackCreated, onSigned }) {
  ...
  const handleAssemble = async () => {
    ...
    try {
      ...
      setPack(fresh);
    } catch (err) { alert(`Assembly failed: ${err.message}`); }
  ...
  const readyCount = components.filter(c => c.content?.text || c.content?.status !== 'awaiting_upload').length;
      ...
      <div className="flex flex-col gap-4">
        <div className="p-5 rounded-xl border border-[rgba(153,197,255,0.12)] bg-[rgba(255,255,255,0.03)] text-center">
          ...
          <p className="text-xs text-[rgba(153,197,255,0.55)] mb-4 max-w-xs mx-auto">
            Cadi will assemble RAMS, welcome letter, and credential slots from your survey data and business settings.
          </p>
          <button
          ...
          >
            {assembling
              ? <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Assembling…</>
              : 'Assemble pack'
            }
          </button>
        </div>
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between px-4 py-3 rounded-xl border border-[rgba(153,197,255,0.12)] bg-[rgba(255,255,255,0.03)]">
        ...
        <div className="text-right">
          ...
          <p className="text-sm font-bold text-white">{readyCount}/{components.length} ready</p>
        </div>
      </div>
      ...
      {components.length > 0 && (
        <div className="rounded-xl border border-[rgba(153,197,255,0.12)] bg-[rgba(255,255,255,0.03)] overflow-hidden">
          {components
            ...
            .map(c => <ComponentRow key={c.id} component={c} />)
          }
        </div>
      )}
      ...
      <div className="px-4 py-3 rounded-xl bg-amber-500/8 border border-amber-500/15">
        ...
        <p className="text-xs text-amber-300/70">
          Check Operations Manager for any certification expiry alerts on proposed crew before signing off.
        </p>
      </div>
      ...
      {pack.status === 'awaiting_signoff' && (
        <div className="p-4 rounded-xl border border-[rgba(153,197,255,0.15)] bg-[rgba(255,255,255,0.03)]">
          ...
          <p className="text-xs text-[rgba(153,197,255,0.55)] mb-4">
            By signing off this pack you confirm, as the competent person, that the RAMS is accurate,
            the hazards are complete, and all credentials are current. You are responsible for this assessment.
          </p>
          <button
          ...
          <div>
            ...
            {pack.signed_off_at && (
              <p className="text-xs text-emerald-400/70">
                Signed off {new Date(pack.signed_off_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
              </p>
            )}
          </div>
        </div>
