...
import { updateQuoteStatus, updateSurveyStatus, createPack, logComparable } from '../../lib/db/surveyDb';
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="rounded-xl border border-[rgba(153,197,255,0.15)] bg-[rgba(255,255,255,0.03)] overflow-hidden">
        <div className="px-4 pt-4 pb-3 border-b border-[rgba(153,197,255,0.08)] flex items-center justify-between">
          <div>
            <p className="text-[10px] font-bold tracking-widest uppercase text-[rgba(153,197,255,0.45)] mb-0.5">Quote</p>
            <p className="text-white font-bold">{survey.customers?.name ?? '—'}</p>
          </div>
          <span className={`px-2 py-0.5 rounded-lg border text-xs font-bold capitalize ${statusColour[status] ?? statusColour.draft}`}>
            {status}
          </span>
        </div>
        ...
        <div className="px-4 py-4">
          ...
          {services.length > 0 && (
            <div className="mb-4">
              {services.map((svc, i) => (
                <div key={i} className="flex items-center justify-between py-1.5 border-b border-[rgba(153,197,255,0.06)] last:border-0">
                  <div>
                    <p className="text-sm text-white">{svc.name}</p>
                    {svc.frequency && <p className="text-[10px] text-[rgba(153,197,255,0.4)]">{svc.frequency}</p>}
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="flex items-end justify-between pt-2">
            <div>
              <p className="text-[10px] text-[rgba(153,197,255,0.4)]">
                {plan.schedule?.frequency === 'one_off' ? 'Total' : `Per visit (${plan.schedule?.frequency ?? ''})`}
              </p>
              {plan.schedule?.first_visit && (
                <p className="text-[10px] text-[rgba(153,197,255,0.4)]">
                  First visit: {new Date(plan.schedule.first_visit).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
                </p>
              )}
            </div>
            <p className="text-3xl font-black text-white">£{Number(quote?.price ?? 0).toFixed(2)}</p>
          </div>
        </div>
      </div>
      ...
      {status === 'accepted' && (
        <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
          ...
          <div>
            ...
            <p className="text-xs text-emerald-400/70">Scroll down to assemble the onboarding pack</p>
          </div>
        </div>
      )}
