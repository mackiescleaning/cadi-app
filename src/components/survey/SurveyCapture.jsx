...
import { updateSurveyNotes, uploadSurveyPhoto, addSurveyMedia, listSurveyMedia, deleteSurveyMedia, listChecklistsForSurvey } from '../../lib/db/surveyDb';
...
function isCovered(label, notes) {
  const keywords = label.toLowerCase().replace(/[^a-z0-9 ]/g, '').split(' ').filter(w => w.length > 3);
...
export default function SurveyCapture({ survey, onNotesChange, onPhotoAdded, onDone }) {
  ...
  const triggerClarify = async (currentNotes) => {
    ...
    } catch { /* non-fatal — clarify is best-effort */ }
  };
  ...
  const handleStructure = async () => {
    ...
    try {
      ...
      onDone?.();
    } catch (err) { alert(`Structuring failed: ${err.message}`); setStructuring(false); }
  };
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="px-4 py-3 rounded-xl border border-[rgba(153,197,255,0.12)] bg-[rgba(255,255,255,0.03)]">
        <p className="text-[10px] font-bold tracking-widest uppercase text-[rgba(153,197,255,0.45)] mb-0.5">Site</p>
        <p className="text-white font-bold">{survey.customers?.name ?? '—'}</p>
        {survey.customers?.address_line1 && (
          <p className="text-xs text-[rgba(153,197,255,0.6)]">{survey.customers.address_line1}{survey.customers.town ? `, ${survey.customers.town}` : ''}</p>
        )}
      </div>
      ...
      {clarifyQ && (
        <div className="flex items-start gap-3 px-4 py-3 rounded-xl bg-amber-500/10 border border-amber-500/25">
          <span className="shrink-0 w-5 h-5 rounded-full bg-amber-500/20 flex items-center justify-center text-amber-400 text-xs font-bold mt-0.5">?</span>
      ...
      <div>
        <div className="flex items-center justify-between mb-1.5">
          <p className="text-[10px] font-bold tracking-widest uppercase text-[rgba(153,197,255,0.5)]">Site notes</p>
          <span className={`text-[10px] font-medium ${saveStatus === 'saving' ? 'text-amber-400' : saveStatus === 'error' ? 'text-red-400' : 'text-[rgba(153,197,255,0.35)]'}`}>
        ...
        />
        <p className="text-[10px] text-[rgba(153,197,255,0.3)] mt-1">{notes.length} characters · raw notes are never overwritten</p>
      </div>

      {/* Photos */}
      <div>
        <div className="flex items-center justify-between mb-2">
          <p className="text-[10px] font-bold tracking-widest uppercase text-[rgba(153,197,255,0.5)]">Photos</p>
          <label className={`flex items-center gap-1.5 px-3 py-1 rounded-lg border text-xs font-bold cursor-pointer transition-all ${
            uploading ? 'border-[rgba(153,197,255,0.12)] text-[rgba(153,197,255,0.3)]' : 'border-[rgba(153,197,255,0.2)] text-[rgba(153,197,255,0.6)] hover:border-[rgba(153,197,255,0.4)] hover:text-[#99c5ff]'
          }`}>
        ...
        {media.length > 0 ? (
          <div className="grid grid-cols-4 gap-2">
            {media.map(m => (
              <div key={m.id} className="relative group rounded-lg overflow-hidden aspect-square bg-white/5 border border-[rgba(153,197,255,0.12)]">
          ...
          <p className="text-xs text-[rgba(153,197,255,0.3)]">No photos yet — tap to add from camera or files</p>
        )}
      </div>
      ...
      {checklists.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-2">
            <p className="text-[10px] font-bold tracking-widest uppercase text-[rgba(153,197,255,0.5)]">Still to confirm</p>
          ...
          {uncoveredItems.length > 0 && (
            <div className="space-y-1.5">
              ...
              {uncoveredItems.length > 8 && (
                <p className="text-[10px] text-[rgba(153,197,255,0.3)] pl-1">+{uncoveredItems.length - 8} more…</p>
              )}
            </div>
          )}

          {uncoveredItems.length === 0 && coveredCount > 0 && (
            <p className="text-xs text-emerald-400">All {coveredCount} checklist items covered in your notes.</p>
          )}
        </div>
      )}
      ...
      <div className="pt-2 border-t border-[rgba(153,197,255,0.08)]">
        {uncoveredItems.length > 0 && (
          <p className="text-xs text-[rgba(153,197,255,0.5)] mb-3">
            {uncoveredItems.length} checklist item{uncoveredItems.length !== 1 ? 's' : ''} still uncovered. You can continue — Cadi will flag them in the review.
          </p>
        )}
        <button
        ...
        >
          {structuring
            ? <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Cadi is reading your notes…</>
            : 'Structure my notes →'
          }
        </button>
        <p className="text-[10px] text-[rgba(153,197,255,0.3)] text-center mt-2">Your raw notes are kept exactly as typed. Cadi proposes — you confirm.</p>
      </div>
    </div>
  );
}
...
function PhotoThumb({ storagePath, objectUrl }) {
  ...
  useEffect(() => {
    if (!url && storagePath) {
      supabase.storage.from('survey-media').createSignedUrl(storagePath, 3600)
