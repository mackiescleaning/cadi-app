...
export function saTaxYearForDate(date = new Date()) {
  ...
  const y = d.getMonth() < 3 || (d.getMonth() === 3 && d.getDate() < 6)
    ? d.getFullYear() - 1
    : d.getFullYear();
...
function monthlySeries(period, entries) {
  ...
  return buckets.map(({ m, income, exp }) => ({ m, income: Math.round(income), exp: Math.round(exp) }));
}
...
function estimateTaxSole(profit) { return calcSelfEmployedTax(profit).total; }
function estimateTaxLtd(profit)  { return calculateCT(profit); }
...
async function aggregateOnePeriod(period, opts = {}) {
  const [moneyEntries, jobs, mileage, customers, invoices] = await Promise.all([
    ...
    opts.includeInvoices
      ? listInvoices({ pageSize: 1000 }).catch(() => [])
      : Promise.resolve(null),
  ]);
  ...
  const bestMonth  = monthly.length ? monthly.reduce((a, b) => a.income >= b.income ? a : b) : null;
  const worstMonth = nonZero.length ? nonZero.reduce((a, b) => a.income <= b.income ? a : b) : (monthly[0] ?? null);
  ...
  if (customers) {
    ...
    activeCustomers = customers.filter(c => (c.account_status ?? c.status ?? 'active') === 'active').length;
    ...
    clientsLost     = customers.filter(c => (
      c.archived_at && c.archived_at >= periodStart && c.archived_at <= periodEnd
    )).length;

    // Simple retention: how many customers who existed at start are still
    // active at end. Rate = (start - lost during period) / start.
    if (clientsStart > 0) {
      retentionRate = Math.max(0, Math.min(100, ((clientsStart - clientsLost) / clientsStart) * 100));
    }
    ...
    const totalAttributedRevenue = Array.from(revenueByCustomer.values()).reduce((a, b) => a + b, 0);
    topClients = Array.from(revenueByCustomer.entries())
      .map(([id, revenue]) => {
        ...
        return {
          ...
          share:    totalAttributedRevenue > 0 ? Math.round((revenue / totalAttributedRevenue) * 1000) / 10 : 0,
        };
      })
  ...
  if (services) {
    ...
    for (const j of jobs) {
      ...
      const perJobMargin = Number.isFinite(Number(j.margin))
        ? Number(j.margin)
        : (price - cost);
  ...
  const {
    invoicesIssued, invoicesPaid, outstandingAtEnd, badDebtWrittenOff,
    debtorDays, onTimePaymentRate, invoiceValueIssued, invoiceValuePaid,
  ...
  return {
    ...
    jobsCompleted, avgJobValue,
    mileageClaimed, totalMiles,
    activeCustomers,
    clientsStart, clientsEnd, clientsNew, clientsLost,
...
function buildOperations(jobs, period) {
  const scheduledInPeriod = jobs.filter(j =>
    j.date && j.date >= period.start && j.date <= period.end
  );
  ...
  const completedInPeriod = jobs.filter(j => {
    ...
    const iso = j.completion_marked_at
      ? j.completion_marked_at.slice(0, 10)
      : (j.date ?? null);
    return iso && iso >= period.start && iso <= period.end;
  });

  const cancelled = scheduledInPeriod.filter(j =>
    j.status === 'cancelled' || j.status === 'canceled' || j.deleted_at
  ).length;
  const noShow = scheduledInPeriod.filter(j => j.status === 'no_show' || j.status === 'noshow').length;
  ...
  let onTimeCount = 0, onTimeDenominator = 0;
  ...
  let actualMinsSum = 0, scheduledMinsSum = 0, durationDenom = 0;
  for (const j of completedInPeriod) {
  ...
  const avgActualDurationMins    = durationDenom > 0 ? Math.round(actualMinsSum    / durationDenom) : null;
  const avgScheduledDurationMins = durationDenom > 0 ? Math.round(scheduledMinsSum / durationDenom) : null;
  const durationVariancePct      = scheduledMinsSum > 0
  ...
  const captureRatePct = scheduledCount > 0
    ? Math.round((completedInPeriod.length / scheduledCount) * 1000) / 10
    : null;
...
async function fetchLtdEntries(period, priorPeriod) {
  const ownerId = await getCurrentUserId();
  if (!ownerId) return null;

  const [{ data: dividendsRows }, { data: dlaRows }, { data: ctRows }] = await Promise.all([
    supabase.from('ltd_dividends')
      .select('paid_on, amount')
      .eq('owner_id', ownerId),
    supabase.from('ltd_director_loan_entries')
      .select('entry_date, amount, category')
      .eq('owner_id', ownerId),
    supabase.from('ltd_ct_accrual')
...
async function fetchTeamBits(period) {
  ...
  const [
    ...
    supabase.from('team_members')
      .select('id, first_name, last_name, role, is_active, is_director, hourly_rate, contracted_hours, holiday_entitlement_days, contract_start_date, contract_type, left_at, dbs_check_date, dbs_expiry_date, dbs_type, rtw_check_date, rtw_expiry_date'),
    supabase.from('payslips')
      .select('staff_id, hours_worked, gross_pay, tax_period, ni_employee_period, ni_employer_period, net_pay, created_at, status'),
    supabase.from('staff_absences')
      .select('staff_id, start_date, end_date, type, status, hours_taken')
      .eq('owner_id', ownerId),
    supabase.from('hr_settings').select('holiday_policy_weeks, bank_holidays_on_top, bank_holidays_per_year').maybeSingle(),
  ]);
  ...
  const leavers        = M.filter(m => m.left_at && m.left_at.slice(0, 10) >= period.start && m.left_at.slice(0, 10) <= period.end).length;
  const joiners        = M.filter(m => m.contract_start_date && m.contract_start_date >= period.start && m.contract_start_date <= period.end).length;
  ...
  const totalWeeklyHours = M.filter(m => m.is_active).reduce((s, m) => s + (Number(m.contracted_hours) || 0), 0);
  ...
  const dbsCovered = M.filter(m => m.is_active && m.dbs_check_date && isLive(m.dbs_expiry_date)).length;
  const rtwCovered = M.filter(m => m.is_active && m.rtw_check_date && isLive(m.rtw_expiry_date)).length;

  return {
    headcountStart, headcountEnd, joiners, leavers,
    turnoverRate:       Math.round(turnoverRate * 10) / 10,
    directors, staffActive,
...
async function sumSubcontractorSpend(period) {
  ...
  return (data ?? [])
    .filter(r => {
      ...
      return c.includes('subcontract') || c.includes('contractor') || c.includes('freelance')
          || n.includes('subcontractor') || n.includes('freelance');
    })
    .reduce((s, r) => s + (Number(r.amount) || 0), 0);
}
...
async function fetchTaxContext(period, entityType, businessSettings) {
  ...
  const [
    { data: hmrcStatus },
    { data: hmrcSubmissions },
    payrollRow,
  ] = await Promise.all([
    supabase.from('hmrc_status').select('*').maybeSingle(),
    supabase.from('hmrc_submissions')
      .select('period_start, period_end, income, expenses, submitted_at')
      .eq('owner_id', ownerId)
      .order('submitted_at', { ascending: false }),
    entityType === 'ltd'
      ? supabase.from('payroll_settings').select('paye_ref, employment_allowance, sandbox_mode').maybeSingle()
...
export function buildRiskRegister(snapshot, entityType) {
  ...
  // ── Customer concentration (from §3) ────────────────────────────────
  {
    ...
    let severity = 'unknown', headline = '';
    if (snapshot.topClients?.length > 0) {
      severity = top3 >= 50 ? 'high' : top3 >= 30 ? 'medium' : 'low';
      headline = top3 >= 50 ? `${Math.round(top3)}% of revenue depends on 3 customers — material single-customer exposure`
              : top3 >= 30 ? `${Math.round(top3)}% top-3 concentration — worth watching`
  ...
  // ── Cash runway (from §5) — crude until BL Financial Resilience ─────
  {
    ...
    risks.push({
      ...
      category: 'financial',
      severity, headline,
      evidence: 'Approximation: outstanding receivables ÷ monthly burn. BL Financial Resilience module refines with cash-balance input.',
  ...
  if ((snapshot.badDebtWrittenOff ?? 0) > 0) {
    risks.push({
      ...
      evidence: 'Voided or written-off invoices. Review chase cadence + credit checks on high-value customers.',
      source: 'computed',
    });
  }
  ...
  if (snapshot.tax?.vatProximity) {
    ...
    if (vat.status === 'over_threshold_unregistered') {
      ...
      risks.push({
        ...
        evidence: 'Voluntary deregistration is optional. Weigh admin savings vs input-VAT reclaim you\'d lose.',
        source: 'computed',
      });
    }
  }
  ...
  risks.push({
    ...
    evidence: 'The BL Compliance & Risk module will capture policies, expiry dates and cover amounts.',
  ...
  risks.push({
    ...
    category: 'market',
    severity: 'unknown',
    headline: 'Suppliers (cleaning products, van finance, fuel card, insurance broker) not yet tracked',
  ...
  const overall = risks.reduce((acc, r) => SEV_RANK[r.severity] > SEV_RANK[acc] ? r.severity : acc, 'unknown');
...
function buildTaxPosition({
  entityType, period, income, expenses, profit,
  ltdBits, taxCtx, businessSettings, priorProfit,
}) {
  ...
  if (entityType === 'ltd') {
    ...
    return {
      ...
      ltd: {
        ...
        ctSavingFromOptimal,
        rateBand:
          profit <= 50000  ? 'small_profits'  :
          profit >= 250000 ? 'main_rate'      :
                             'marginal_relief',
      },
    };
  }
  ...
  return {
    ...
    sole: {
      ...
      yoyDeltaTotal,
      band:
        profit <= 12570  ? 'personal_allowance' :
        profit <= 50270  ? 'basic'              :
        profit <= 125140 ? 'higher'             :
                           'additional',
    },
  };
}
...
function summariseInvoices(invoices, period) {
  ...
  let onTimeCount = 0, dueDateCount = 0;
  ...
  for (const inv of invoices) {
    ...
    if (inv.paid_at) {
      ...
      if (paidDate >= period.start && paidDate <= period.end) {
        ...
        if (inv.sent_at) {
          const days = (new Date(paidDate) - new Date(inv.sent_at.slice(0, 10))) / (1000 * 60 * 60 * 24);
  ...
  out.onTimePaymentRate  = dueDateCount > 0 ? Math.round((onTimeCount / dueDateCount) * 1000) / 10 : null;
  return out;
}
...
export async function aggregateForYear({ period, priorPeriod, entityType, businessSettings = null }) {
  ...
  const soleTraderDrawings = entityType !== 'ltd'
    ? Math.round((await sumSoleTraderDrawings(period)) || 0)
    : 0;
  ...
  const taxEst = entityType === 'ltd'
    ? estimateTaxLtd(current.profit)
    : estimateTaxSole(current.profit);

  const netAfterTax = current.profit - taxEst;
  const yoyIncome = prior.income > 0
    ? ((current.income - prior.income) / prior.income) * 100
    : 0;
  const yoyProfit = prior.profit !== 0
    ? ((current.profit - prior.profit) / Math.abs(prior.profit)) * 100
    : 0;
  ...
  return {
    ...
    cashRunwayMonths:  (current.expenses > 0)
    ...
    team: teamBits ? {
      ...
      revenuePerHead:   teamBits.staffActive + teamBits.directors > 0
        ? Math.round(current.income / (teamBits.staffActive + teamBits.directors))
        : null,
      avgCostPerHead:   teamBits.staffActive + teamBits.directors > 0
...
export function composeExecSummary({ snapshot, toneMode = 'internal', businessName = 'The business', entityYear = null, vatRegistered = false }) {
  if (!snapshot) return '';
...
export function execSummaryKpis(snapshot) {
  ...
  return [
    ...
    { key: 'yoy',          label: 'YoY change',      value: signedPct(snapshot.yoyIncome),  sub: 'vs prior period', tone: (snapshot.yoyIncome ?? 0) >= 0 ? 'up' : 'down' },
    { key: 'profit',       label: 'Net profit',      value: fmtGBP(snapshot.profit),        sub: `${round1(snapshot.margin)}% margin`, tone: (snapshot.profit ?? 0) >= 0 ? 'up' : 'down' },
    { key: 'margin',       label: 'Margin',          value: `${round1(snapshot.margin)}%`,  sub: 'industry ~55–65%' },
    { key: 'customers',    label: 'Active customers', value: String(snapshot.activeCustomers ?? 0), sub: 'on the books today' },
    { key: 'jobs',         label: 'Jobs delivered',  value: String(snapshot.jobsCompleted ?? 0), sub: `avg ${fmtGBP(snapshot.avgJobValue)}` },
  ];
}
...
export async function fileAnnualReview({
  taxYear, isInterim = false, interimLabel = null,
  snapshot, execSummaryOverride = null,
  ratings = null, plan = null, overrides = {}, toneMode = 'internal',
}) {
