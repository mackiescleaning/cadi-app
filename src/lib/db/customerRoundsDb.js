...
export async function bulkInsertRounds(rounds) {
  ...
  const { data, error } = await supabase.from('customer_rounds').insert(rows).select('id');
  if (error) throw error;
  return data ?? [];
}

export async function deleteRoundsForCustomer(customerId) {
  const { error } = await supabase.from('customer_rounds').delete().eq('customer_id', customerId);
  if (error) throw error;
}
...
export async function setRoundOrder(items) {
  ...
  await Promise.all(
    items.map(({ id, displayOrder, roundName }) => {
