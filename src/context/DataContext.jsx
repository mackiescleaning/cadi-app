...
export function DataProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          setJobs((prev) =>
            prev.map((j) => (j.id === payload.new.id ? mapJobRow(payload.new) : j))
          );
        }
      )
      .subscribe();
      ...
      prev.map((c) => {
        const customerJobs = jobs.filter(
          (j) =>
            ...
            (j.customer &&
              c.name &&
              j.customer.toLowerCase().trim() === c.name.toLowerCase().trim())
        );
        ...
        return {
          ...
          lastJobDate:
            completed.reduce((latest, j) => (j.date && j.date > latest ? j.date : latest), '') ||
            null,
          nextJobDate: scheduled.reduce(
            (earliest, j) => (!earliest || (j.date && j.date < earliest) ? j.date : earliest),
            null
          ),
        };
      })
  ...
  const findCustomerForJob = useCallback(
    (job) => {
      if (!job) return null;
      if (job.customerId) {
  ...
  const computeCustomerSync = useCallback(
    (prior, next) => {
      ...
      const completedDelta =
        (isJobCompleted(next, today) ? 1 : 0) - (isJobCompleted(prior, today) ? 1 : 0);
      ...
      const completedDates = all
        .filter((j) => isJobCompleted(j, today))
        .map((j) => j.date)
        .filter(Boolean)
        .sort();
      const upcomingDates = all
        .filter((j) => !isJobCompleted(j, today) && j.date && j.date >= today)
        .map((j) => j.date)
        .filter(Boolean)
        .sort();
  ...
  const persistCustomerSync = useCallback(
    (sync) => {
      if (!sync) return;
  ...
  const addJobAndSyncCustomer = useCallback(
    async (job) => {
      let savedJob = job;
      try {
  ...
  const updateJob = useCallback(
    async (id, updates) => {
      ...
      const local = { ...updates };
      if ('start_hour' in local) {
        local.startHour = local.start_hour;
        delete local.start_hour;
      }
      if ('duration_hrs' in local) {
        local.durationHrs = local.duration_hrs;
        delete local.duration_hrs;
      }
      if ('display_order' in local) {
        local.displayOrder = local.display_order;
        delete local.display_order;
      }
      if ('customer_id' in local) {
        local.customerId = local.customer_id;
        delete local.customer_id;
      }
      if ('assignee_ids' in local) {
        local.assigneeIds = local.assignee_ids;
        delete local.assignee_ids;
      }
      if ('address_line1' in local) {
        local.addressLine1 = local.address_line1;
        delete local.address_line1;
      }
      if ('address_line2' in local) {
        local.addressLine2 = local.address_line2;
        delete local.address_line2;
      }
