...
function TeamStructureCoach({ user }) {
  ...
  return (
    ...
    >
      <div>
        <p className="text-[11px] font-bold tracking-wide uppercase text-[#010a4f]/70 mb-2">
          Team structure
        </p>
              ...
              <button
                ...
                className={`text-[12px] font-semibold px-3 py-1.5 rounded-xl border border-[#1f48ff]/15 transition-colors ${
                  selected
                    ...
                    : 'bg-[#f0f4ff] text-[#010a4f] hover:bg-[#e3eaff]'
                }`}
              >
                {opt.label}
              </button>
...
const STATUS_COLOR = {
  ...
  unassigned: {
    bg: 'rgba(153,197,255,0.08)',
    text: 'rgba(153,197,255,0.4)',
    dot: 'rgba(153,197,255,0.3)',
  },
};
...
function RotaStat({ value, sub, color = 'normal' }) {
  const bg = {
    normal: 'bg-[rgba(153,197,255,0.04)] border-[rgba(153,197,255,0.12)]',
  ...
  const sub_ = {
    normal: 'text-[rgba(153,197,255,0.5)]',
...
function StaffHub({ staff, jobs, training, loading, onTabChange }) {
  ...
  const expiringSoon = training.filter(
    (t) => t.expiry_date && t.expiry_date >= today && t.expiry_date <= in30
  );
  ...
  const stats = [
    ...
    },
    {
      ...
      icon: AlertTriangle,
      color: unassigned.length > 0 ? '#ef4444' : '#6b7280',
      bg: unassigned.length > 0 ? '#fef2f2' : '#f9fafb',
    },
  ];
  ...
  if (unassigned.length > 0)
    actionCards.push({
      ...
      color: '#ef4444',
      bg: '#fef2f2',
      text: `${unassigned.length} unassigned job${unassigned.length > 1 ? 's' : ''} — post to Connect or assign staff`,
      tab: 'rota',
    });
  ...
  if (noRate.length > 0)
    actionCards.push({
      ...
      color: '#f59e0b',
      bg: '#fef9ec',
      text: `${noRate.length} staff member${noRate.length > 1 ? 's' : ''} missing hourly rate — needed for payroll`,
      tab: 'people',
    });
  if (expiredStaff > 0)
    actionCards.push({
      ...
      color: '#ef4444',
      bg: '#fef2f2',
      text: `${expiredStaff} staff with expired training certification${expiredStaff > 1 ? 's' : ''}`,
      tab: 'people',
    });
  if (expiringStaff > 0)
    actionCards.push({
      ...
      color: '#f59e0b',
      bg: '#fef9ec',
      text: `${expiringStaff} staff with training certification${expiringStaff > 1 ? 's' : ''} expiring within 30 days`,
      tab: 'people',
    });
          ...
          <Card key={s.label}>
            <div className="p-4 flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-[11px] text-[rgba(153,197,255,0.5)] leading-tight">
                  {s.label}
                </div>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <Card>
          ...
          {loading ? (
            ...
            <div className="px-5 py-6 text-xs text-[rgba(153,197,255,0.5)]">
            ...
            <div className="divide-y divide-[rgba(153,197,255,0.08)]">
              {todayJobs.slice(0, 6).map((j) => {
                ...
                const names =
                  j.assignees?.length > 0 ? j.assignees.join(', ') : j.assignee || 'Unassigned';
                return (
                  <div key={j.id} className="px-5 py-3 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[11px] text-[rgba(153,197,255,0.5)] truncate">
                        {names} · {fmtHour(j.start_hour)}
                      </div>
                    </div>
                    <span
              ...
              {todayJobs.length > 6 && (
                <button
                  ...
                  className="w-full px-5 py-3 text-xs text-[#99c5ff] hover:bg-[rgba(153,197,255,0.06)] text-left"
                >
        ...
        <Card>
          <CardHeader
            ...
            action={
              <button
                ...
                className="text-xs text-[#1f48ff] font-semibold hover:underline"
              >
                View all
              </button>
            }
          ...
          {loading ? (
            ...
            <div className="px-5 py-6 text-center">
              ...
              <button
                onClick={() => onTabChange('people')}
                className="text-xs text-[#99c5ff] font-semibold"
              >
            ...
            <div className="divide-y divide-[rgba(153,197,255,0.08)]">
              {activeStaff.slice(0, 5).map((s) => (
                <div key={s.id} className="px-5 py-3 flex items-center gap-3">
                  ...
                  {s.hourly_rate && (
                    <span className="text-[11px] text-[rgba(153,197,255,0.5)] flex-shrink-0">
                      £{Number(s.hourly_rate).toFixed(0)}/hr
                    </span>
                  )}
                </div>
              ))}
...
function complianceStatus(s, trainingForStaff = []) {
  ...
  const trainExpired = trainingForStaff.some(
    (t) => t.expiry_date && new Date(t.expiry_date) < today
  );
  ...
  const trainSoon = trainingForStaff.some(
    (t) => t.expiry_date && new Date(t.expiry_date) > today && new Date(t.expiry_date) <= in30
  );
...
const COMP_CFG = {
  ...
  not_set: {
    label: 'Not recorded',
    bg: 'bg-white/[0.08]',
    text: 'text-[rgba(153,197,255,0.4)]',
    dot: 'bg-[rgba(153,197,255,0.3)]',
    border: 'border border-[rgba(153,197,255,0.12)]',
  },
};
...
function ComplianceBadge({ status }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 text-[10px] font-medium px-2 py-0.5 rounded-full ${c.bg} ${c.text} ${c.border}`}
    >
...
function ExpiryRow({ label, date }) {
  ...
  return (
    <div className="flex items-start justify-between gap-2 text-xs">
      <span className="text-[rgba(153,197,255,0.5)] flex-shrink-0">{label}</span>
      <span
        className={`text-right font-semibold ${expired ? 'text-red-300' : soon ? 'text-amber-300' : 'text-white'}`}
      >
        {fmtDateGB(date)}
        {expired && ' (expired)'}
...
function TrainingModal({ staffMember, onSave, onClose }) {
  const inp =
    'w-full bg-[rgba(153,197,255,0.06)] border border-[rgba(153,197,255,0.15)] rounded-xl px-3 py-2 text-sm text-white placeholder-[rgba(153,197,255,0.35)] focus:outline-none focus:border-[#99c5ff] transition-colors';
  const lbl =
    'block text-xs font-bold tracking-[0.15em] uppercase text-[rgba(153,197,255,0.5)] mb-1';
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-4">
      <div className="bg-[#05124a] border border-[rgba(153,197,255,0.15)] rounded-2xl w-full max-w-sm">
        <div className="flex items-center justify-between p-5 border-b border-[rgba(153,197,255,0.08)]">
          ...
          <button
            onClick={onClose}
            className="p-1.5 hover:bg-[rgba(153,197,255,0.08)] rounded-lg transition-colors"
          >
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
            ...
            >
              ...
              {CERT_TYPES.map((c) => (
                <option key={c.key} value={c.key}>
                  {c.label}
                  {c.years ? ` (renews every ${c.years}yr${c.years > 1 ? 's' : ''})` : ''}
                </option>
              ))}
            </select>
          </div>
          ...
          <div>
            ...
            <input
              ...
              className={inp}
              placeholder="e.g. certificate number, provider"
            />
          </div>
...
function StaffPeople({ staff, training, loading, onStaffChange, onTrainingChange }) {
  ...
  const compCounts = useMemo(
    () =>
      baseFiltered.reduce((acc, s) => {
        const st = complianceStatus(
          s,
          (training ?? []).filter((t) => t.staff_id === s.id)
        );
        acc[st] = (acc[st] || 0) + 1;
        return acc;
      }, {}),
  ...
  }, [baseFiltered, compFilter, search, training]);
  ...
  async function handleAdd(e) {
    ...
    if (!form.first_name.trim()) {
      setFormErr('First name required');
      return;
    }
    if (form.pin_hash && !/^\d{4}$/.test(form.pin_hash)) {
  ...
  const inp =
    'w-full bg-[rgba(153,197,255,0.06)] border border-[rgba(153,197,255,0.15)] rounded-xl px-3 py-2 text-sm text-white placeholder-[rgba(153,197,255,0.35)] focus:outline-none focus:border-[#99c5ff] transition-colors';
  const lbl =
    'block text-[10px] font-bold tracking-[0.15em] uppercase text-[rgba(153,197,255,0.5)] mb-1';
  ...
  return (
    <div className="flex gap-4 h-full">
      ...
      <div className="flex-1 min-w-0 space-y-3">
            ...
            {[
              {
                ...
                activeBg: 'bg-[#1f48ff]',
                inactiveBg: 'bg-[rgba(153,197,255,0.06)] border border-[rgba(153,197,255,0.12)]',
                countColor: 'text-white',
              },
              {
                key: 'compliant',
                label: 'Compliant',
                count: compCounts.compliant || 0,
                activeBg: 'bg-emerald-600',
                inactiveBg: 'bg-emerald-500/10 border border-emerald-500/20',
                countColor: 'text-emerald-300',
              },
              {
                key: 'expiring',
                label: 'Expiring soon',
                count: compCounts.expiring || 0,
                activeBg: 'bg-amber-500',
                inactiveBg: 'bg-amber-500/10 border border-amber-500/20',
              ...
              },
              {
                ...
                activeBg: 'bg-red-600',
                inactiveBg: 'bg-red-500/10 border border-red-500/20',
              ...
              return (
                ...
                >
                  <div className={`text-xl font-bold ${active ? 'text-white' : countColor}`}>
                    {count}
                  </div>
                  <div
                    className={`text-[11px] mt-0.5 ${active ? 'text-white/80' : 'text-[rgba(153,197,255,0.5)]'}`}
                  >
                    {label}
                  </div>
                </button>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <div className="relative flex-1 min-w-[180px]">
            <Search
              size={13}
              className="absolute left-3 top-1/2 -translate-y-1/2 text-[rgba(153,197,255,0.4)]"
            />
            <input
              value={search}
        ...
        <Card>
          {loading ? (
            <div className="px-5 py-8 text-center text-xs text-[rgba(153,197,255,0.5)]">
              Loading staff…
            </div>
          ) : displayed.length === 0 ? (
            <div className="px-5 py-8 text-center">
              <p className="text-sm text-[rgba(153,197,255,0.5)] mb-3">
                {search
                  ...
                  : compFilter !== 'all'
                    ? 'No staff in this compliance group'
                    : filter === 'active'
                      ? 'No active staff yet'
                      : 'No staff'}
              </p>
            ...
            <div className="divide-y divide-[rgba(153,197,255,0.08)]">
              ...
              {displayed.map((s) => {
                ...
                const cStatus = complianceStatus(
                  s,
                  (training ?? []).filter((t) => t.staff_id === s.id)
                );
                return (
                  ...
                  >
                    <div className="col-span-4 flex items-center gap-2.5 min-w-0">
                      <div
                        className={`w-7 h-7 rounded-full flex items-center justify-center text-white text-[11px] font-bold flex-shrink-0 ${s.is_active ? 'bg-[rgba(31,72,255,0.4)]' : 'bg-[rgba(153,197,255,0.12)]'}`}
                      >
                        {initials}
                      </div>
                      <div className="min-w-0">
                        ...
                        {s.email && (
                          <div className="text-[10px] text-[rgba(153,197,255,0.5)] truncate">
                            {s.email}
                          </div>
                        )}
                        {!s.is_active && (
                          <span className="text-[10px] text-[rgba(153,197,255,0.4)]">Inactive</span>
                        )}
                      </div>
                    </div>
                    <div className="col-span-2 flex items-center">
                      <span className="text-[11px] text-[rgba(153,197,255,0.6)] capitalize">
                        {ROLE_LABELS[s.role] || s.role}
                      </span>
                    </div>
                    <div className="col-span-2 flex items-center">
                    ...
                    <div className="col-span-2 flex items-center">
                      <span className="text-[11px] text-[rgba(153,197,255,0.6)]">
                        {CONTRACT_TYPES[s.contract_type] || '—'}
                      </span>
                    </div>
                    <div className="col-span-2 flex items-center">
          ...
          <Card>
            <div className="px-5 py-3.5 border-b border-[rgba(153,197,255,0.08)] flex items-center justify-between">
              ...
              <button
                ...
                className="text-[rgba(153,197,255,0.5)] hover:text-white"
              >
                <X size={14} />
              </button>
            </div>
            <div className="p-5 space-y-5 overflow-y-auto max-h-[calc(100vh-200px)]">
              ...
              <div className="flex items-center gap-3">
                <div
                  className={`w-12 h-12 rounded-2xl flex items-center justify-center text-white text-base font-bold flex-shrink-0 ${selected.is_active ? 'bg-[rgba(31,72,255,0.4)]' : 'bg-[rgba(153,197,255,0.12)]'}`}
                >
                ...
                <div>
                  ...
                  <div className="text-xs text-[rgba(153,197,255,0.5)] capitalize">
                  ...
                  <div className="mt-1">
                    <ComplianceBadge
                      status={complianceStatus(
                        selected,
                        (training ?? []).filter((t) => t.staff_id === selected.id)
                      )}
                    />
                  </div>
                </div>
              ...
              <section className="space-y-2">
                <div className="text-[10px] font-bold tracking-[0.15em] uppercase text-[rgba(153,197,255,0.5)]">
                  Contact
                </div>
                {selected.phone ? (
                  <div className="flex items-center gap-2 text-xs text-[rgba(153,197,255,0.8)]">
                    <Phone size={11} className="text-[rgba(153,197,255,0.4)] flex-shrink-0" />
                    {selected.phone}
                  </div>
                ) : (
                ...
                {selected.email ? (
                  <div className="flex items-center gap-2 text-xs text-[rgba(153,197,255,0.8)] break-all">
                    <Mail size={11} className="text-[rgba(153,197,255,0.4)] flex-shrink-0" />
                    {selected.email}
                  </div>
                ) : (
              ...
              <section className="space-y-2">
                ...
                <DrawerRow
                  label="Rate"
                  value={
                    selected.hourly_rate ? `£${Number(selected.hourly_rate).toFixed(2)}/hr` : '—'
                  }
                />
                <DrawerRow
                  label="Contracted hrs"
                  value={selected.contracted_hours ? `${selected.contracted_hours} hrs/wk` : '—'}
                />
                <DrawerRow
                  label="Status"
                  value={selected.is_active ? 'Active' : 'Inactive'}
                  valueClass={`font-semibold ${selected.is_active ? 'text-emerald-400' : 'text-[rgba(153,197,255,0.4)]'}`}
                />
                {selected.has_pin && (
                  <DrawerRow
                    label="Staff app PIN"
                    value="Set ✓"
                    valueClass="font-semibold text-emerald-400"
                  />
                )}
              </section>
              ...
              <section className="space-y-2">
                ...
                {selected.rtw_check_date ? (
                  <>
                    <DrawerRow
                      label="Document"
                      value={RTW_DOC_TYPES[selected.rtw_doc_type] || selected.rtw_doc_type || '—'}
                    />
                    ...
                    {selected.rtw_doc_path && (
                      <button
                        onClick={() => handleViewDoc(selected.rtw_doc_path)}
              ...
              <section className="space-y-2">
                ...
                {selected.dbs_check_date ? (
                  <>
                    <DrawerRow
                      label="Type"
                      value={DBS_TYPES[selected.dbs_type] || selected.dbs_type || '—'}
                    />
                    ...
                    {selected.dbs_expiry_date ? (
                      ...
                      <DrawerRow
                        label="Review by"
                        value="Not set"
                        valueClass="text-[rgba(153,197,255,0.4)]"
                      />
                    )}
                    {selected.dbs_doc_path && (
                      <button
                        onClick={() => handleViewDoc(selected.dbs_doc_path)}
                        className="flex items-center gap-1.5 text-[11px] text-[#99c5ff] font-medium hover:underline"
                      >
                  ...
                  <p className="text-[11px] text-[rgba(153,197,255,0.4)] italic">
                    Not recorded — add if required for this role
                  </p>
                )}
              </section>
              ...
              {(selected.emergency_contact_name || selected.emergency_contact_phone) && (
                <section className="space-y-2">
                  ...
                  {selected.emergency_contact_name && (
                    <DrawerRow label="Name" value={selected.emergency_contact_name} />
                  )}
                  {selected.emergency_contact_phone && (
                    <DrawerRow label="Phone" value={selected.emergency_contact_phone} />
                  )}
                </section>
              )}
              ...
              {(() => {
                ...
                const certs = (training ?? [])
                  .filter((t) => t.staff_id === selected.id)
                      ...
                      <div className="space-y-1.5">
                        {certs.map((t) => {
                          ...
                          return (
                            <div
                              key={t.id}
                              className="flex items-center justify-between gap-2 py-1.5 border-b border-[rgba(153,197,255,0.08)] last:border-0"
                            >
                              <div className="min-w-0 flex-1">
                                ...
                                <p
                                  className={`text-[10px] mt-0.5 ${expired ? 'text-red-400 font-semibold' : soon ? 'text-amber-400 font-medium' : 'text-[rgba(153,197,255,0.5)]'}`}
                                >
                                  {t.expiry_date
              ...
              {selected.notes && (
                <section className="space-y-1">
                  ...
                  <p className="text-xs text-[rgba(153,197,255,0.8)] leading-relaxed">
                    {selected.notes}
                  </p>
                </section>
              )}
              ...
              {(selected.ni_number || selected.tax_code) && (
                <section className="space-y-2">
                  ...
                  {selected.ni_number && (
                    <DrawerRow
                      label="NI Number"
                      value={`${selected.ni_number.slice(0, 2)}•••••${selected.ni_number.slice(-1)}`}
                    />
                  )}
                  <DrawerRow label="Tax code" value={selected.tax_code || '1257L'} />
                  <DrawerRow label="NI category" value={selected.ni_category || 'A'} />
                  {selected.date_of_birth && (
                    <DrawerRow label="Date of birth" value={fmtDateGB(selected.date_of_birth)} />
                  )}
                </section>
              )}
            ...
            <form onSubmit={handleAdd} className="p-6 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm((f) => ({ ...f, contract_start_date: e.target.value }))
                    }
                    className={inp}
                    type="date"
                  />
              ...
              <div>
                ...
                <input
                  ...
                  className={inp}
                  placeholder="e.g. 20 or 37.5"
                  type="number"
                  step="0.5"
                  min="0"
                  max="168"
                />
                <p className="text-[11px] text-[rgba(153,197,255,0.5)] mt-1">
                  Used to auto-calculate holiday entitlement in hours
                </p>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
              ...
              <div>
                ...
                />
                <p className="text-[11px] text-[rgba(153,197,255,0.5)] mt-1">
                  Staff use this to log into the Cadi team app
                </p>
              </div>
      ...
      {showComp && selected && (
        <div
          className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4"
          onClick={(e) => e.target === e.currentTarget && setShowComp(false)}
        >
          <div className="bg-[#05124a] border border-[rgba(153,197,255,0.15)] rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden max-h-[90vh] overflow-y-auto">
            ...
            <div className="px-6 py-3 bg-[rgba(153,197,255,0.04)] border-b border-[rgba(153,197,255,0.08)] flex items-center gap-3 text-[11px]">
              <span
                className={`flex items-center gap-1 font-medium ${compForm.rtw_check_date ? 'text-emerald-400' : 'text-[rgba(153,197,255,0.4)]'}`}
              >
                <span
                  className={`w-1.5 h-1.5 rounded-full ${compForm.rtw_check_date ? 'bg-emerald-400' : 'bg-[rgba(153,197,255,0.2)]'}`}
                />
                RTW checked
              </span>
              <span
                className={`flex items-center gap-1 font-medium ${compForm.dbs_type ? 'text-emerald-400' : 'text-[rgba(153,197,255,0.4)]'}`}
              >
                <span
                  className={`w-1.5 h-1.5 rounded-full ${compForm.dbs_type ? 'bg-emerald-400' : 'bg-[rgba(153,197,255,0.2)]'}`}
                />
                DBS
              </span>
              <span
                className={`flex items-center gap-1 font-medium ${compForm.emergency_contact_name ? 'text-emerald-400' : 'text-[rgba(153,197,255,0.4)]'}`}
              >
                <span
                  className={`w-1.5 h-1.5 rounded-full ${compForm.emergency_contact_name ? 'bg-emerald-400' : 'bg-[rgba(153,197,255,0.2)]'}`}
                />
                Emergency contact
              </span>
              <span className="ml-auto text-[rgba(153,197,255,0.4)]">
                Fill in RTW date to go green
              </span>
            </div>
            ...
            <form onSubmit={handleSaveCompliance} className="p-6 space-y-6">
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] bg-[rgba(31,72,255,0.15)] text-[#99c5ff] px-1.5 py-0.5 rounded-full font-medium border border-[rgba(31,72,255,0.25)]">
                    Legally required
                  </span>
                </div>
                <div>
                  ...
                  <select
                    value={compForm.rtw_doc_type}
                    onChange={(e) => setCompForm((f) => ({ ...f, rtw_doc_type: e.target.value }))}
                    className={inp}
                  >
                    <option value="">Select document…</option>
                    {Object.entries(RTW_DOC_TYPES).map(([k, v]) => (
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      value={compForm.rtw_check_date}
                      onChange={(e) =>
                        setCompForm((f) => ({ ...f, rtw_check_date: e.target.value }))
                      }
                      className={inp}
                      type="date"
                    />
                  ...
                  <div>
                    ...
                    <input
                      value={compForm.rtw_expiry_date}
                      onChange={(e) =>
                        setCompForm((f) => ({ ...f, rtw_expiry_date: e.target.value }))
                      }
                      className={inp}
                      type="date"
                    />
                    <p className="text-[10px] text-[rgba(153,197,255,0.5)] mt-1">
                      UK/Irish passport — leave blank
                    </p>
                  </div>
                </div>
                {/* RTW document upload */}
                  ...
                  {selected.rtw_doc_path ? (
                    <div className="flex items-center gap-2 px-3 py-2.5 bg-emerald-500/10 rounded-xl border border-emerald-500/25">
                      ...
                      <span className="text-xs text-emerald-300 flex-1 truncate">
                        {selected.rtw_doc_path.split('/').pop()}
                      </span>
                      <button
                        type="button"
                        onClick={() => handleViewDoc(selected.rtw_doc_path)}
                        className="text-xs text-[#99c5ff] font-medium"
                      >
                        View
                      </button>
                      <button
                        type="button"
                        onClick={() => handleRemoveDoc('rtw')}
                    ...
                    <label
                      className={`flex items-center gap-2 px-3 py-3 border-2 border-dashed rounded-xl cursor-pointer transition-colors ${uploadingDoc.rtw ? 'border-[rgba(153,197,255,0.15)] bg-[rgba(153,197,255,0.04)]' : 'border-[rgba(153,197,255,0.15)] hover:border-[#99c5ff]/40 hover:bg-[rgba(153,197,255,0.06)]'}`}
                    >
                      {uploadingDoc.rtw ? (
                        <>
                          <RefreshCw
                            size={12}
                            className="text-[rgba(153,197,255,0.4)] animate-spin"
                          />
                          <span className="text-xs text-[rgba(153,197,255,0.4)]">Uploading…</span>
                        </>
                      ) : (
                        <>
                          <Plus size={12} className="text-[rgba(153,197,255,0.4)]" />
                          <span className="text-xs text-[rgba(153,197,255,0.5)]">
                            Upload passport, BRP, or other RTW document
                          </span>
                        </>
                      )}
                      <input
                        ...
                        className="sr-only"
                        accept="image/*,.pdf"
                        disabled={uploadingDoc.rtw}
                        onChange={(e) =>
                          e.target.files?.[0] && handleUploadDoc('rtw', e.target.files[0])
                        }
                      />
                    </label>
                  )}
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] bg-[rgba(153,197,255,0.08)] text-[rgba(153,197,255,0.5)] px-1.5 py-0.5 rounded-full font-medium border border-[rgba(153,197,255,0.15)]">
                    If required for this role
                  </span>
                </div>
                <div>
                  <label className={lbl}>DBS type</label>
                  <select
                    value={compForm.dbs_type}
                    onChange={(e) => setCompForm((f) => ({ ...f, dbs_type: e.target.value }))}
                    className={inp}
                  >
                  ...
                  <>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className={lbl}>Date obtained</label>
                        <input
                          value={compForm.dbs_check_date}
                          onChange={(e) =>
                            setCompForm((f) => ({ ...f, dbs_check_date: e.target.value }))
                          }
                          className={inp}
                          type="date"
                        />
                      ...
                      <div>
                        ...
                        <input
                          value={compForm.dbs_expiry_date}
                          onChange={(e) =>
                            setCompForm((f) => ({ ...f, dbs_expiry_date: e.target.value }))
                          }
                          className={inp}
                          type="date"
                        />
                    ...
                    <div>
                      ...
                      {selected.dbs_doc_path ? (
                        <div className="flex items-center gap-2 px-3 py-2.5 bg-emerald-500/10 rounded-xl border border-emerald-500/25">
                          <CheckCircle size={13} className="text-emerald-400 flex-shrink-0" />
                          <span className="text-xs text-emerald-300 flex-1 truncate">
                            {selected.dbs_doc_path.split('/').pop()}
                          </span>
                          <button
                            type="button"
                            onClick={() => handleViewDoc(selected.dbs_doc_path)}
                        ...
                        <label
                          className={`flex items-center gap-2 px-3 py-3 border-2 border-dashed rounded-xl cursor-pointer transition-colors ${uploadingDoc.dbs ? 'border-[rgba(153,197,255,0.15)] bg-[rgba(153,197,255,0.04)]' : 'border-[rgba(153,197,255,0.15)] hover:border-[#99c5ff]/40 hover:bg-[rgba(153,197,255,0.06)]'}`}
                        >
                          {uploadingDoc.dbs ? (
                            <>
                              <RefreshCw
                          ...
                          <input
                            ...
                            className="sr-only"
                            accept="image/*,.pdf"
                            disabled={uploadingDoc.dbs}
                            onChange={(e) =>
                              e.target.files?.[0] && handleUploadDoc('dbs', e.target.files[0])
                            }
                          />
                        </label>
                      )}
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className={lbl}>Contract type</label>
                    <select
                      value={compForm.contract_type}
                      onChange={(e) =>
                        setCompForm((f) => ({ ...f, contract_type: e.target.value }))
                      }
                      className={inp}
                    >
                      <option value="employed">Employee</option>
                  ...
                  <div>
                    <label className={lbl}>Start date</label>
                    <input
                      value={compForm.contract_start_date}
                      onChange={(e) =>
                        setCompForm((f) => ({ ...f, contract_start_date: e.target.value }))
                      }
                      className={inp}
                      type="date"
                    />
                ...
                <div>
                  <label className={lbl}>Contracted hours / week</label>
                  <input
                    value={compForm.contracted_hours}
                    onChange={(e) =>
                      setCompForm((f) => ({ ...f, contracted_hours: e.target.value }))
                    }
                    className={inp}
                    placeholder="e.g. 20 or 37.5"
                    type="number"
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      value={compForm.emergency_contact_name}
                      onChange={(e) =>
                        setCompForm((f) => ({ ...f, emergency_contact_name: e.target.value }))
                      }
                      className={inp}
                  ...
                  <div>
                    <label className={lbl}>Phone</label>
                    <input
                      value={compForm.emergency_contact_phone}
                      onChange={(e) =>
                        setCompForm((f) => ({ ...f, emergency_contact_phone: e.target.value }))
                      }
                      className={inp}
                      placeholder="07700 900000"
                    />
...
const ABSENCE_TYPES = {
  holiday: {
    label: 'Holiday',
    bg: 'rgba(31,72,255,0.15)',
    text: '#99c5ff',
    border: 'rgba(31,72,255,0.3)',
  },
  sick: {
    label: 'Sick',
    ...
    >
      <div className="bg-[#05124a] border border-[rgba(153,197,255,0.15)] rounded-2xl shadow-2xl w-full max-w-md overflow-hidden max-h-[90vh] overflow-y-auto">
        <div className="px-6 py-4 border-b border-[rgba(153,197,255,0.08)] flex items-center justify-between">
          ...
          <button onClick={onClose} className="text-[rgba(153,197,255,0.5)] hover:text-white">
            <X size={16} />
          </button>
        </div>
        <form onSubmit={handleSave} className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                onChange={(e) => setForm((f) => ({ ...f, ni_number: e.target.value }))}
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setForm((f) => ({ ...f, ni_category: e.target.value }))}
              className={inp}
            >
              {NI_CATEGORIES.map((c) => (
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setForm((f) => ({ ...f, address_postcode: e.target.value }))}
              className={`${inp} uppercase`}
          ...
          <div className="bg-[rgba(31,72,255,0.12)] border border-[rgba(31,72,255,0.25)] rounded-xl p-3 text-[11px] text-[#99c5ff] leading-relaxed">
            NI number and address are required for HMRC RTI payroll submission. Tax code defaults to
...
function WorkPatternModal({ staff, onSave, onClose }) {
  ...
  const tinp =
    'px-2 py-1 bg-[rgba(153,197,255,0.06)] border border-[rgba(153,197,255,0.15)] rounded-lg text-xs text-white focus:outline-none focus:border-[#99c5ff] transition-colors w-[78px]';
    ...
    >
      <div className="bg-[#05124a] border border-[rgba(153,197,255,0.15)] rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[rgba(153,197,255,0.08)]">
          ...
          <button
            onClick={onClose}
            className="text-[rgba(153,197,255,0.5)] hover:text-white transition-colors"
          >
            <X size={18} />
          </button>
        </div>
        <div className="px-5 pt-4 pb-2 space-y-1.5">
          {DAY_ORDER.map((key) => {
            ...
            return (
              <div
                key={key}
                className={`flex items-center gap-3 rounded-xl px-3 py-2.5 transition-all ${works ? 'bg-[rgba(153,197,255,0.06)]' : 'opacity-40'}`}
              >
                <button
                ...
                >
                  <span
                    className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow-sm transition-all ${works ? 'right-0.5' : 'left-0.5'}`}
                  />
                </button>
                <span className="text-xs font-semibold text-white w-[76px] flex-shrink-0">
                  {DAY_FULL[key]}
                </span>
                {works ? (
...
function StaffRota({
  ...
  useEffect(() => {
    ...
    const ch = supabase
      .channel('rota-timesheets-' + user.id)
      .on(
        'postgres_changes',
        {
          event: '*',
        ...
        ({ eventType, new: row }) => {
          ...
          setTodayTimesheets((prev) =>
            eventType === 'INSERT' ? [...prev, row] : prev.map((t) => (t.id === row.id ? row : t))
          );
        }
      )
      .subscribe();
  ...
  const weekStats = useMemo(() => {
    ...
    const gapDays = weekDates.filter((d) =>
      weekJobs.some(
        (j) => j.date === d && !j.assignee_ids?.length && !j.assignees?.length && !j.assignee
      )
    ).length;
    return {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- getWeekHours is redefined each render; inputs weekJobs/weekDates/activeStaff cover it
  ...
  const entitlements = useMemo(() => {
    ...
    return activeStaff.map((s) => {
      ...
      const takenHrs = r2(
        staffYear.filter((a) => a.status === 'approved').reduce((sum, a) => sum + calcHrs(a), 0)
      );
      const pendingHrs = r2(
        staffYear.filter((a) => a.status === 'pending').reduce((sum, a) => sum + calcHrs(a), 0)
      );
  ...
  async function handleSaveAbsence(e) {
    e.preventDefault();
    if (absForm.end_date < absForm.start_date) {
    ...
    try {
      ...
      const hoursTaken =
        absForm.type === 'holiday' && daily > 0 ? Math.round(wkDays * daily * 10) / 10 : null;
  ...
  async function handleRemoveAbsence(absence) {
    ...
    if (
      !window.confirm(
        `Remove ${at?.label || absence.type} (${fmtDateGB(absence.start_date)} – ${fmtDateGB(absence.end_date)})?`
      )
    )
      return;
    try {
  ...
  async function handleLeaveDecision(absence, newStatus) {
    ...
    try {
      ...
      if (error) throw error;
      const patch = (prev) =>
        prev.map((a) => (a.id === absence.id ? { ...a, status: newStatus } : a));
  ...
  function openCellClick(staffMember, dateStr) {
    ...
    if (dayJobs.length === 1) {
      setAssignJob(dayJobs[0]);
      return;
    }
    if (dayJobs.length > 1) {
      setDayPicker({ staff: staffMember, dateStr, dayJobs });
      return;
    }
  ...
  const cellBase =
    'min-h-[52px] p-1 border-r border-[rgba(153,197,255,0.08)] last:border-r-0 align-top';
  const inp =
    'w-full bg-[rgba(153,197,255,0.06)] border border-[rgba(153,197,255,0.15)] rounded-xl px-3 py-2 text-sm text-white placeholder-[rgba(153,197,255,0.35)] focus:outline-none focus:border-[#99c5ff] transition-colors';
  const lbl =
    'block text-[10px] font-bold tracking-[0.15em] uppercase text-[rgba(153,197,255,0.5)] mb-1';
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-1 bg-[rgba(153,197,255,0.06)] border border-[rgba(153,197,255,0.12)] rounded-lg p-0.5 text-xs">
          {[
            ...
            {
              id: 'leave',
              label: `Leave & Entitlement${pendingRequests.length > 0 ? ` (${pendingRequests.length})` : ''}`,
            },
          ].map((v) => (
            <button
              key={v.id}
      ...
      {!loading && activeStaff.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          ...
          <RotaStat
            ...
            sub={
              weekStats.contractedTotal > 0
                ? `of ${weekStats.contractedTotal}h contracted`
                : 'scheduled this week'
            }
            color={
              weekStats.contractedTotal > 0 &&
              weekStats.scheduledHours < weekStats.contractedTotal * 0.75
                ? 'amber'
                : 'normal'
            }
          />
          <RotaStat
            ...
            sub={
              weekStats.gapDays > 0
                ? `Day${weekStats.gapDays > 1 ? 's' : ''} with gaps`
                : 'All jobs covered'
            }
          ...
          <RotaStat
            ...
            sub={
              pendingRequests.length > 0
                ? `Leave request${pendingRequests.length > 1 ? 's' : ''} pending`
                : 'No pending requests'
            }
      ...
      {weekStats.wtrViolations > 0 && (
        <div className="flex items-center gap-2 px-4 py-2.5 bg-red-500/10 border border-red-500/25 rounded-xl text-xs text-red-300">
          ...
          <span>
            <strong>
              {weekStats.wtrViolations} staff member{weekStats.wtrViolations > 1 ? 's are' : ' is'}
            </strong>{' '}
            scheduled over 48 hours this week — Working Time Regulations limit exceeded
          </span>
        </div>
      )}
      ...
      {view === 'schedule' && (
        <>
          <Card>
            {loading ? (
              <div className="p-8 text-center text-sm text-[rgba(153,197,255,0.5)]">
                Loading rota…
              </div>
            ) : activeStaff.length === 0 ? (
              <div className="p-8 text-center">
                <p className="text-sm text-[rgba(153,197,255,0.5)] mb-3">
                  Add staff in the People tab to build your rota
                </p>
                <button
                  onClick={() => onTabChange('people')}
                  className="text-sm text-[#99c5ff] font-semibold"
                >
                  Go to People →
                </button>
              </div>
                ...
                <table className="w-full border-collapse text-xs" style={{ minWidth: 720 }}>
                    ...
                    <tr className="border-b border-[rgba(153,197,255,0.08)] bg-[rgba(153,197,255,0.04)]">
                      ...
                      {days.map((d, i) => {
                        ...
                        return (
                          <th
                            key={i}
                            className={`text-center px-2 py-2.5 text-[10px] font-semibold uppercase tracking-wide border-r border-[rgba(153,197,255,0.08)] last:border-r-0 ${isToday ? 'text-[#99c5ff]' : 'text-[rgba(153,197,255,0.4)]'}`}
                          >
                            <div>{DAY_LABELS[i]}</div>
                            <div
                              className={`text-[9px] font-normal mt-0.5 ${isToday ? 'text-[#99c5ff]' : 'text-[rgba(153,197,255,0.25)]'}`}
                            >
                  ...
                  <tbody>
                    {activeStaff.map((s) => {
                      ...
                      const todayTs = isThisWeek
                        ? todayTimesheets.find((t) => t.staff_id === s.id)
                        : null;
                      const tsOnSite =
                        todayTs?.status === 'clocked_in' || todayTs?.status === 'flagged';
                      const tsDone = todayTs?.status === 'clocked_out';
                      const tsDotCls = tsOnSite
                      ...
                      return (
                        <tr
                          key={s.id}
                          className="border-b border-[rgba(153,197,255,0.06)] last:border-b-0 hover:bg-[rgba(153,197,255,0.04)] transition-colors group"
                        >
                          <td className="px-3 py-2 border-r border-[rgba(153,197,255,0.08)]">
                            <div className="flex items-center gap-2">
                              <div className="relative w-6 h-6 flex-shrink-0">
                                ...
                                {tsDotCls && (
                                  <span
                                    className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-[#05124a] ${tsDotCls}`}
                                  />
                                )}
                              </div>
                              <div className="min-w-0 flex-1">
                                <div className="flex items-center gap-1">
                                  ...
                                  {isWTR && (
                                    <span className="flex-shrink-0 text-[9px] bg-red-500/15 text-red-300 border border-red-500/25 rounded px-1 py-px font-bold leading-none">
                                      WTR
                                    </span>
                                  )}
                                  {todayTs?.status === 'flagged' && (
                                    <span className="flex-shrink-0 text-[9px] bg-amber-500/15 text-amber-300 border border-amber-500/25 rounded px-1 py-px font-bold leading-none">
                                      Away
                                    </span>
                                  )}
                                </div>
                                {tsLabel ? (
                                  <div className="text-[9px] text-[rgba(153,197,255,0.5)] leading-none">
                                    {tsLabel}
                                  </div>
                                ) : (
                                  <div className="text-[10px] text-[rgba(153,197,255,0.5)] capitalize">
                                    {ROLE_LABELS[s.role] || s.role}
                                  </div>
                                )}
                              </div>
                              <button
                                onClick={() => openAbsModal(s)}
                                title="Log absence"
                          ...
                          {days.map((d, i) => {
                            ...
                            const absType = absence
                              ? ABSENCE_TYPES[absence.type] || ABSENCE_TYPES.other
                              : null;
                            ...
                            return (
                              <td
                                ...
                                style={{
                                  ...(absence
                                    ? {
                                        background: absType.bg,
                                        opacity: absence.status === 'pending' ? 0.65 : 1,
                                      }
                                    : {}),
                                  ...(!absence && isOff
                                    ? {
                                        background:
                                          'repeating-linear-gradient(45deg,rgba(153,197,255,0.04),rgba(153,197,255,0.04) 3px,transparent 3px,transparent 10px)',
                                      }
                                    : {}),
                                  ...(!absence && !isOff && isToday
                              ...
                              >
                                {absence ? (
                                  ...
                                  <div className="h-full min-h-[44px] flex items-center justify-center">
                                    <span className="text-[10px] text-[rgba(153,197,255,0.2)] font-semibold select-none tracking-wide">
                                      Off
                                    </span>
                                  </div>
                                ) : cellJobs.length > 0 ? (
                                      ...
                                      return (
                                        ...
                                        >
                                          ...
                                          <div className="opacity-70">
                                            {fmtHour(j.start_hour)}
                                            {j.duration_hrs ? ` · ${j.duration_hrs}h` : ''}
                                          </div>
                                        </button>
                                      );
                                    })}
                          ...
                          <td className="px-2 py-2 text-center align-middle border-l border-[rgba(153,197,255,0.08)]">
                            <div className={`text-[11px] font-bold ${hrsColor}`}>
                              {weekHrs > 0 ? `${weekHrs}h` : '—'}
                            </div>
                            {contracted > 0 && weekHrs > 0 && (
                              <div className="text-[9px] text-[rgba(153,197,255,0.25)]">
                                /{contracted}h
                              </div>
                            )}
                          </td>
                      ...
                      <tr className="border-t-2 border-dashed border-red-500/20">
                        <td className="px-3 py-2 border-r border-[rgba(153,197,255,0.08)]">
                          <div className="flex items-center gap-1.5">
                            ...
                            <div>
                              <div className="font-semibold text-red-400 text-[11px]">
                                Unassigned
                              </div>
                              <div className="text-[10px] text-red-400/60">
                                {unassignedJobs.length} gap{unassignedJobs.length > 1 ? 's' : ''}
                              </div>
                            </div>
                          </div>
                        </td>
                        {days.map((d, i) => {
                          ...
                          return (
                            <td
                              key={i}
                              className={`${cellBase} bg-red-500/5 ${ds === todayStr ? 'bg-red-500/10' : ''}`}
                            >
                              {dayGaps.map((j) => (
                                <div
                                  key={j.id}
                                  className="rounded-md px-1.5 py-1 bg-red-500/15 border border-red-500/25 text-[10px] leading-tight mb-0.5"
                                >
                                  <button
                                    onClick={() => setAssignJob(j)}
          ...
          <div className="flex items-center gap-3 flex-wrap text-[11px] text-[rgba(153,197,255,0.5)] px-1">
            {Object.entries({
              ...
              <div key={k} className="flex items-center gap-1.5">
                <div
                  ...
                  style={{
                    background: STATUS_COLOR[k].bg,
                    border: `1px solid ${STATUS_COLOR[k].dot}`,
                  }}
                />
                {v}
              </div>
            ...
            <div className="flex items-center gap-1.5">
              <span className="text-[9px] bg-red-500/15 text-red-300 border border-red-500/25 rounded px-1 font-bold">
                WTR
              </span>
          ...
          <Card>
            ...
            {editingPolicy ? (
              <form onSubmit={handleSavePolicy} className="p-5 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[10px] font-bold tracking-[0.15em] uppercase text-[rgba(153,197,255,0.5)] mb-1">
                      Entitlement (weeks/year) *
                    </label>
                    <input
                      type="number"
                      step="0.1"
                      ...
                      onChange={(e) =>
                        setPolicyForm((f) => ({ ...f, holiday_policy_weeks: e.target.value }))
                      }
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setPolicyForm((f) => ({ ...f, holiday_year_start: e.target.value }))
                      }
                      className="w-full bg-[rgba(153,197,255,0.06)] border border-[rgba(153,197,255,0.15)] rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-[#99c5ff] transition-colors"
                    >
                ...
                {/* Bank holidays */}
                <div className="space-y-3">
                  <label className="block text-[10px] font-bold tracking-[0.15em] uppercase text-[rgba(153,197,255,0.5)]">
                    Bank holidays
                  </label>
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      {
                        value: false,
                        label: 'Within entitlement',
                        desc: 'Staff use their holiday hours for bank holidays',
                      },
                      {
                        value: true,
                        label: 'On top of entitlement',
                        desc: 'Bank holidays are free days in addition to their allowance',
                      },
                    ].map((opt) => (
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          setPolicyForm((f) => ({ ...f, bank_holidays_on_top: opt.value }))
                        }
                      ...
                      >
                        <p
                          className={`text-xs font-semibold ${policyForm.bank_holidays_on_top === opt.value ? 'text-[#99c5ff]' : 'text-white'}`}
                        >
                          {opt.label}
                        </p>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setPolicyForm((f) => ({ ...f, bank_holidays_per_year: e.target.value }))
                      }
                      className="w-full bg-[rgba(153,197,255,0.06)] border border-[rgba(153,197,255,0.15)] rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-[#99c5ff] transition-colors"
                    />
                    <p className="text-[11px] text-[rgba(153,197,255,0.5)] mt-1">
                      England &amp; Wales = 8 · Scotland = 9
                    </p>
                  </div>
                </div>
              ...
              <div className="px-5 pb-5 grid grid-cols-2 gap-x-6 gap-y-3">
                {[
                  ...
                  [
                    'Bank holidays',
                    hrPolicy.bank_holidays_on_top
                      ? `${hrPolicy.bank_holidays_per_year} days on top`
                      : `${hrPolicy.bank_holidays_per_year} days within entitlement`,
                  ],
                  [
                    'Example',
                    `20 hrs/wk → ${Math.round(20 * hrPolicy.holiday_policy_weeks * 10) / 10} hrs/yr${hrPolicy.bank_holidays_on_top ? ` + ${Math.round(hrPolicy.bank_holidays_per_year * 4 * 10) / 10} hrs BH` : ''}`,
                  ],
                ].map(([label, value]) => (
                  <div key={label}>
                    <p className="text-[10px] font-bold tracking-[0.15em] uppercase text-[rgba(153,197,255,0.5)]">
                      {label}
                    </p>
          ...
          {pendingRequests.length > 0 ? (
            <Card>
              <CardHeader
                ...
                action={
                  <span className="w-5 h-5 rounded-full bg-amber-500/20 text-amber-300 text-[10px] font-bold flex items-center justify-center border border-amber-500/25">
                  ...
                  return (
                    <div key={req.id} className="px-5 py-3 flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                          <span
                            ...
                            style={{ background: at.bg, color: at.text, borderColor: at.border }}
                          >
                            {at.label}
                          </span>
                          <span className="text-[11px] text-[rgba(153,197,255,0.5)]">
                            {fmtDateGB(req.start_date)}
                            {req.end_date !== req.start_date
                              ? ` – ${fmtDateGB(req.end_date)}`
                              : ''}{' '}
                            · {wkDays} working day{wkDays !== 1 ? 's' : ''}
                          </span>
                        </div>
                        {req.notes && (
                          <div className="text-[11px] text-[rgba(153,197,255,0.4)] mt-0.5 truncate">
                            {req.notes}
                          </div>
                        )}
                      </div>
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <button
                          onClick={() => handleLeaveDecision(req, 'declined')}
                          disabled={busy}
                        ...
                        <button
                          onClick={() => handleLeaveDecision(req, 'approved')}
                          disabled={busy}
                ...
                {upcomingLeave.slice(0, 8).map((abs) => {
                  ...
                  return (
                    <div key={abs.id} className="px-5 py-2.5 flex items-center gap-3">
                      ...
                      <span
                        className="text-[11px] px-1.5 py-0.5 rounded font-semibold flex-shrink-0 border"
                        style={{ background: at.bg, color: at.text, borderColor: at.border }}
                      >
                        {at.label}
                      </span>
                      <div className="text-[11px] text-[rgba(153,197,255,0.5)] text-right flex-shrink-0">
                        {fmtDateGB(abs.start_date)}
                        {abs.end_date !== abs.start_date ? ` – ${fmtDateGB(abs.end_date)}` : ''}
                      </div>
                    </div>
                  );
                })}
          ...
          {activeStaff.length > 0 && (
            <Card>
              <CardHeader
                ...
                action={
                  <button
                    onClick={() => activeStaff[0] && openAbsModal(activeStaff[0])}
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-xs" style={{ minWidth: 520 }}>
                  <thead>
                    <tr className="border-b border-[rgba(153,197,255,0.08)] bg-[rgba(153,197,255,0.04)]">
                      <th className="text-left px-5 py-2.5 text-[10px] font-semibold text-[rgba(153,197,255,0.4)] uppercase tracking-wide">
                        Staff
                      </th>
                      <th className="text-right px-4 py-2.5 text-[10px] font-semibold text-[rgba(153,197,255,0.4)] uppercase tracking-wide">
                        Entitlement
                      </th>
                      <th className="text-right px-4 py-2.5 text-[10px] font-semibold text-[rgba(153,197,255,0.4)] uppercase tracking-wide">
                        Taken
                  ...
                  <tbody>
                    {entitlements.map(
                      ({
                        ...
                        const usedPct =
                          entitlementHrs > 0
                            ? Math.min(100, Math.round((takenHrs / entitlementHrs) * 100))
                            : 0;
                        const pendPct =
                          entitlementHrs > 0
                            ? Math.min(
                                100 - usedPct,
                                Math.round((pendingHrs / entitlementHrs) * 100)
                              )
                            : 0;
                        ...
                        return (
                          <tr
                            key={s.id}
                            className="border-b border-[rgba(153,197,255,0.06)] last:border-b-0 hover:bg-[rgba(153,197,255,0.04)] transition-colors"
                          >
                                ...
                                <div>
                                  ...
                                  <div className="text-[10px] text-[rgba(153,197,255,0.5)]">
                                    {contracted > 0 ? (
                                      ...
                                      <span className="text-amber-400">No contracted hrs set</span>
                                    )}
                                  </div>
                                </div>
                              </div>
                            ...
                            <td className="px-4 py-3 text-right font-semibold text-white">
                              {noHours ? (
                                <span className="text-[rgba(153,197,255,0.25)]">—</span>
                              ) : (
                                `${entitlementHrs}h`
                              )}
                            </td>
                            <td className="px-4 py-3 text-right text-[rgba(153,197,255,0.7)]">
                              {noHours ? '—' : fmt(takenHrs)}
                            </td>
                            <td className="px-4 py-3 text-right text-amber-400">
                              {noHours ? '—' : pendingHrs > 0 ? `${pendingHrs}h` : '—'}
                            </td>
                            <td
                              className={`px-4 py-3 text-right font-bold ${noHours ? 'text-[rgba(153,197,255,0.25)]' : lowFlag ? 'text-red-400' : warnFlag ? 'text-amber-400' : 'text-emerald-400'}`}
                            >
                              ...
                              {!noHours && (
                                <>
                                  <div className="h-2 rounded-full bg-[rgba(153,197,255,0.1)] overflow-hidden flex">
                                    <div
                                      className="h-full bg-[#1f48ff] rounded-l-full"
                                      style={{ width: `${usedPct}%` }}
                                    />
                                    {pendPct > 0 && (
                                      <div
                                  ...
                                  <div className="text-[9px] text-[rgba(153,197,255,0.4)] mt-0.5">
                                    {usedPct}% used
                                  </div>
                                </>
                              )}
              ...
              <div className="px-5 py-3 border-t border-[rgba(153,197,255,0.08)] flex items-center gap-4 flex-wrap text-[11px] text-[rgba(153,197,255,0.5)]">
                <div className="flex items-center gap-1.5">
                  <div className="w-3 h-2 rounded-sm bg-[#1f48ff]" /> Taken
                </div>
                <div className="flex items-center gap-1.5">
                  <div className="w-3 h-2 rounded-sm bg-amber-400" /> Pending
                </div>
                <div className="flex items-center gap-1.5">
                  <div className="w-3 h-2 rounded-sm bg-[rgba(153,197,255,0.1)] border border-[rgba(153,197,255,0.15)]" />{' '}
                  Remaining
                </div>
                {entitlements.filter((e) => e.contracted === 0).length > 0 && (
                  <span className="ml-2 text-amber-400 font-medium">
                    {entitlements.filter((e) => e.contracted === 0).length} staff need contracted
                    hours set
                  </span>
                )}
                {entitlements.filter((e) => e.contracted > 0 && e.remainingHrs < e.contracted * 2)
                  .length > 0 && (
                  <span className="ml-auto text-red-400 font-medium">
                    {
                      entitlements.filter(
                        (e) => e.contracted > 0 && e.remainingHrs < e.contracted * 2
                      ).length
                    }{' '}
                    staff running low on leave
                  </span>
                )}
              </div>
            </Card>
          ...
          <div className="bg-[#05124a] border border-[rgba(153,197,255,0.15)] rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-[rgba(153,197,255,0.08)] flex items-center justify-between">
              <div>
                ...
                <div className="text-xs text-[rgba(153,197,255,0.5)] mt-0.5">
                  Tap a job to edit who's assigned
                </div>
              </div>
              <button
            ...
            <div className="p-3 space-y-1.5 max-h-72 overflow-y-auto">
              {dayPicker.dayJobs.map((j) => {
                ...
                const assignedNames = activeStaff
                  .filter((m) =>
                    (Array.isArray(j.assignee_ids) ? j.assignee_ids : []).includes(m.id)
                  )
                  .map((m) => m.first_name)
                  .join(', ');
                return (
          ...
          <div className="bg-[#05124a] border border-[rgba(153,197,255,0.15)] rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-[rgba(153,197,255,0.08)] flex items-center justify-between">
              <div>
                ...
                <div className="text-xs text-[rgba(153,197,255,0.5)] mt-0.5">
                  {fmtDateGB(assignJob.date)}
                  {assignJob.start_hour != null ? ` · ${fmtHour(assignJob.start_hour)}` : ''}
                  {assignJob.duration_hrs ? ` · ${assignJob.duration_hrs}h` : ''}
                </div>
              </div>
              <button
            ...
            <div className="p-5">
              ...
              {activeStaff.length === 0 ? (
                <p className="text-sm text-[rgba(153,197,255,0.5)]">
                  Add staff in the People tab first.
                </p>
              ) : (
                <div className="flex flex-wrap gap-2">
                  {activeStaff.map((s) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`w-4 h-4 rounded-full text-[9px] font-bold flex items-center justify-center flex-shrink-0 ${isOn ? 'bg-white/20 text-white' : 'bg-[rgba(153,197,255,0.1)] text-[rgba(153,197,255,0.5)]'}`}
                        >
                          {(s.first_name?.[0] || '') + (s.last_name?.[0] || '')}
                        </div>
                        {s.first_name} {s.last_name || ''}
                        {isOffDay && !isOn && (
                          <span className="ml-0.5 text-[9px] text-amber-400 font-bold">Off</span>
                        )}
              ...
              <p className="text-[11px] text-[rgba(153,197,255,0.4)] mt-4">
                Changes save instantly and sync to the Scheduler.
              </p>
            </div>
          </div>
        </div>
      ...
      {showAbsModal && absTarget && (
        <div
          className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4"
          onClick={(e) => e.target === e.currentTarget && setShowAbsModal(false)}
        >
          <div className="bg-[#05124a] border border-[rgba(153,197,255,0.15)] rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-[rgba(153,197,255,0.08)] flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-[rgba(153,197,255,0.5)] mt-0.5">
                  {absTarget.first_name} {absTarget.last_name || ''}
                </p>
              </div>
              <button
            ...
            <form onSubmit={handleSaveAbsence} className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {Object.entries(ABSENCE_TYPES).map(([k, v]) => (
                    <button
                      ...
                      style={
                        absForm.type === k
                          ? { background: v.bg, color: v.text, borderColor: v.border }
                          : {}
                      }
                    >
                      {v.label}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    value={absForm.start_date}
                    onChange={(e) => setAbsForm((f) => ({ ...f, start_date: e.target.value }))}
                    className={inp}
                    type="date"
                    required
                  />
                ...
                <div>
                  ...
                  <input
                    value={absForm.end_date}
                    onChange={(e) => setAbsForm((f) => ({ ...f, end_date: e.target.value }))}
                    className={inp}
                    type="date"
                    required
                    min={absForm.start_date}
                  />
                </div>
              </div>
              {/* Hours preview for holiday absences */}
              {absForm.type === 'holiday' &&
                absForm.start_date &&
                absForm.end_date >= absForm.start_date &&
                (() => {
                  const wkDays = countWorkingDays(absForm.start_date, absForm.end_date);
                  const daily = parseFloat(absTarget?.contracted_hours ?? 0) / 5;
                      ...
                      <span className="font-semibold text-white">
                        {hrs != null ? (
                          ...
                          <span className="text-amber-400">
                            Set contracted hrs for hour tracking
                          </span>
                        )}
                      </span>
                    </div>
              ...
              <div>
                ...
                <input
                  value={absForm.notes}
                  onChange={(e) => setAbsForm((f) => ({ ...f, notes: e.target.value }))}
                  className={inp}
                  placeholder="e.g. annual leave, self-certified"
                />
              </div>
              <div>
                ...
                <div className="flex gap-2">
                  {[
                    { v: 'approved', label: 'Approve immediately' },
                    { v: 'pending', label: 'Log as request' },
                  ].map((opt) => (
                    <button
                      key={opt.v}
                      type="button"
                      onClick={() => setAbsForm((f) => ({ ...f, status: opt.v }))}
...
function StaffPay() {
  ...
  async function loadPayRuns() {
    ...
    const { data } = await supabase
      .from('pay_runs')
      .select(
        `*, payslips ( id, staff_id, hours_worked, gross_pay, tax_period, ni_employee_period, ni_employer_period, net_pay, status, team_members ( first_name, last_name ) )`
      )
      .eq('business_id', user.id)
  ...
  async function handleCalculate(payRunId) {
    ...
    try {
      const { error } = await supabase.functions.invoke('payroll-calculate', {
        body: { pay_run_id: payRunId },
      });
      if (error) throw error;
      await loadPayRuns();
    } catch (err) {
  ...
  async function handleSubmitFPS(payRunId) {
    ...
    try {
      const { error } = await supabase.functions.invoke('payroll-submit-fps', {
        body: { pay_run_id: payRunId },
      });
      if (error) throw error;
      await loadPayRuns();
    ...
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader
          ...
          subtitle={
            settings
              ? `${settings.tax_office_no}/${settings.paye_ref} · ${settings.sandbox_mode ? 'Sandbox' : 'Live'}`
              : 'Not configured'
          }
          action={
            !editing && (
        ...
        {!editing && !settings && (
          <div className="p-8 text-center">
            ...
            <p className="text-xs text-[rgba(153,197,255,0.5)] max-w-xs mx-auto mb-4">
              Enter your PAYE reference and Government Gateway credentials to enable RTI payroll
              submissions.
            </p>
            <button
              onClick={() => setEditing(true)}
        ...
        {!editing && settings && (
          <div className="p-5 grid grid-cols-2 gap-x-6 gap-y-3">
            {[
              ...
              [
                'Pay frequency',
                { M1: 'Monthly', W1: 'Weekly', W2: 'Fortnightly', W4: '4-weekly' }[
                  settings.payment_frequency
                ] || settings.payment_frequency,
              ],
              [
                'Gateway User ID',
                settings.gateway_user_id ? `${settings.gateway_user_id.slice(0, 4)}••••••••` : '—',
              ],
              ['Gateway password', settings.gateway_password_enc ? '••••••••' : '—'],
              [
                'Contact',
                [settings.contact_fore, settings.contact_sur].filter(Boolean).join(' ') || '—',
              ],
          ...
          <form onSubmit={handleSaveSettings} className="p-5 space-y-4">
            <div className="grid grid-cols-3 gap-3">
              <div>
                ...
                <input
                  value={form.tax_office_no}
                  onChange={(e) => setForm((f) => ({ ...f, tax_office_no: e.target.value }))}
                  className={inp}
                  placeholder="123"
                  maxLength={3}
                  required
              ...
              <div>
                ...
                <input
                  value={form.paye_ref}
                  onChange={(e) => setForm((f) => ({ ...f, paye_ref: e.target.value }))}
                  className={inp}
                  placeholder="A45678"
                  required
                />
              ...
              <div>
                ...
                <input
                  value={form.ao_ref}
                  onChange={(e) => setForm((f) => ({ ...f, ao_ref: e.target.value }))}
                  className={inp}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setForm((f) => ({ ...f, gateway_user_id: e.target.value }))}
                  className={inp}
              ...
              <div>
                ...
                <input
                  value={form.gateway_password_enc}
                  onChange={(e) => setForm((f) => ({ ...f, gateway_password_enc: e.target.value }))}
                  className={inp}
                  type="password"
                  placeholder="••••••••"
                />
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                ...
                <input
                  value={form.contact_fore}
                  onChange={(e) => setForm((f) => ({ ...f, contact_fore: e.target.value }))}
                  className={inp}
                  placeholder="Rhianna"
                />
              </div>
              <div>
                ...
                <input
                  value={form.contact_sur}
                  onChange={(e) => setForm((f) => ({ ...f, contact_sur: e.target.value }))}
                  className={inp}
                  placeholder="Mackie"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  className={inp}
                  placeholder="rhianna@mackies.cleaning"
                  type="email"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="flex flex-col gap-2 pt-5">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={form.employment_allowance}
                    onChange={(e) =>
                      setForm((f) => ({ ...f, employment_allowance: e.target.checked }))
                    }
                    className="rounded border-[rgba(153,197,255,0.3)] text-[#1f48ff]"
                  />
                  <span className="text-xs font-medium text-[rgba(153,197,255,0.8)]">
            ...
            {form.sandbox_mode && (
              <div className="bg-amber-500/10 border border-amber-500/25 rounded-xl p-3 text-[11px] text-amber-300">
                Sandbox mode is on — submissions go to HMRC's test engine and won't affect real
                payroll. Turn off only after HMRC recognition is confirmed.
              </div>
            )}
      ...
      <Card>
        ...
        {!settings ? (
          <div className="p-8 text-center">
            ...
            <p className="text-xs text-[rgba(153,197,255,0.5)] max-w-xs mx-auto">
              Configure your PAYE scheme above to unlock pay runs.
            </p>
          </div>
        ) : runsLoading ? (
          ...
          <div className="p-8 text-center">
            ...
            <p className="text-xs text-[rgba(153,197,255,0.5)] max-w-xs mx-auto mb-4">
              Create a pay run, calculate PAYE + NI from timesheets, review payslips, then submit
              your FPS to HMRC.
            </p>
            <button
              onClick={() => setCreatingRun(true)}
          ...
          <div className="divide-y divide-[rgba(153,197,255,0.08)] px-2 pb-2">
            {payRuns.map((run) => {
              ...
              const STATUS_STYLE = {
                draft:
                  'bg-[rgba(153,197,255,0.08)] text-[rgba(153,197,255,0.5)] border border-[rgba(153,197,255,0.12)]',
                calculating:
                  'bg-[rgba(31,72,255,0.15)] text-[#99c5ff] border border-[rgba(31,72,255,0.25)]',
                calculated:
                  'bg-[rgba(31,72,255,0.2)] text-[#99c5ff] border border-[rgba(31,72,255,0.3)]',
                  ...
                  >
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={`px-2 py-0.5 rounded-full text-[10px] font-semibold capitalize ${STATUS_STYLE[run.status] ?? STATUS_STYLE.draft}`}
                        >
                          {run.status}
                        </span>
                      </div>
                    ...
                    <ChevronDown
                      size={15}
                      className={`text-[rgba(153,197,255,0.4)] flex-shrink-0 transition-transform ${isExpanded ? 'rotate-180' : ''}`}
                    />
                  </button>
                          ...
                          <div className="flex items-center gap-2 text-xs">
                            <span className="text-[rgba(153,197,255,0.8)] font-medium">
                              {settings.sandbox_mode
                                ? 'Send to HMRC test engine?'
                                : 'Send LIVE FPS to HMRC?'}
                            </span>
                            <button
                          ...
                          <table className="w-full text-xs">
                            <thead>
                              <tr className="bg-[rgba(153,197,255,0.06)] border-b border-[rgba(153,197,255,0.08)]">
                                {['Staff', 'Hrs', 'Gross', 'Tax', 'NI emp', 'NI empr', 'Net'].map(
                                  (h) => (
                                    <th
                                      key={h}
                                      className="text-left px-3 py-2 font-semibold text-[rgba(153,197,255,0.4)] whitespace-nowrap"
                                    >
                                      {h}
                                    </th>
                                  )
                            ...
                            <tbody>
                              {slips.map((p) => (
                                <tr
                                  key={p.id}
                                  className="border-b border-[rgba(153,197,255,0.06)] last:border-0 hover:bg-[rgba(153,197,255,0.04)] transition-colors"
                                >
                                  <td className="px-3 py-2.5 font-medium text-white whitespace-nowrap">
                                    {p.team_members?.first_name} {p.team_members?.last_name}
                                  </td>
                                  <td className="px-3 py-2.5 text-[rgba(153,197,255,0.6)]">
                                    {Number(p.hours_worked).toFixed(1)}
                                  </td>
                                  <td className="px-3 py-2.5 text-[rgba(153,197,255,0.8)]">
                                    £{Number(p.gross_pay).toFixed(2)}
                                  </td>
                                  <td className="px-3 py-2.5 text-red-400">
                                    £{Number(p.tax_period).toFixed(2)}
                                  </td>
                                  <td className="px-3 py-2.5 text-amber-400">
                                    £{Number(p.ni_employee_period).toFixed(2)}
                                  </td>
                                  <td className="px-3 py-2.5 text-amber-400">
                                    £{Number(p.ni_employer_period).toFixed(2)}
                                  </td>
                              ...
                              <tr className="bg-[rgba(153,197,255,0.08)] border-t border-[rgba(153,197,255,0.15)] font-semibold">
                                ...
                                <td className="px-3 py-2.5 text-[rgba(153,197,255,0.6)]">
                                  {slips
                                    .reduce((s, p) => s + Number(p.hours_worked || 0), 0)
                                    .toFixed(1)}
                                </td>
                                ...
                                <td className="px-3 py-2.5 text-red-400">
                                  £
                                  {slips
                                    .reduce((s, p) => s + Number(p.tax_period || 0), 0)
                                    .toFixed(2)}
                                </td>
                                <td className="px-3 py-2.5 text-amber-400">
                                  £
                                  {slips
                                    .reduce((s, p) => s + Number(p.ni_employee_period || 0), 0)
                                    .toFixed(2)}
                                </td>
                                <td className="px-3 py-2.5 text-amber-400">
                                  £
                                  {slips
                                    .reduce((s, p) => s + Number(p.ni_employer_period || 0), 0)
                                    .toFixed(2)}
                                </td>
                                <td className="px-3 py-2.5 text-emerald-400 font-bold">
                                  £
                                  {slips.reduce((s, p) => s + Number(p.net_pay || 0), 0).toFixed(2)}
                                </td>
                              </tr>
                            </tbody>
                          </table>
    ...
    <Card>
      <div className="p-10 text-center">
        ...
        <p className="text-sm text-[rgba(153,197,255,0.5)] max-w-sm mx-auto">
          Post cover gaps to nearby Cadi businesses. They accept, invoice through Cadi, and your
          shift is filled — without the WhatsApp chaos.
        </p>
    ...
    try {
      ...
      const [staffRes, jobsRes, absRes, trainRes] = await Promise.all([
        ...
        supabase
          .from('jobs')
          .select(
            'id,customer,date,start_hour,duration_hrs,type,service,status,assignee,assignees,assignee_ids,price,postcode'
          )
          .eq('owner_id', user.id)
          .gte('date', from)
          .lte('date', to)
