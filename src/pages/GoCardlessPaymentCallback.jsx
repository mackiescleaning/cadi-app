/**
 * src/pages/GoCardlessPaymentCallback.jsx
 * Cadi — GoCardless Payments OAuth callback (Direct Debit, NOT Open Banking)
 *
 * This is separate from GocardlessCallback.jsx which handles the Open Banking
 * (bank transaction import) flow. This handles the GoCardless Partner API
 * OAuth for collecting Direct Debits from customers.
 *
 * Route: /gocardless/payment-callback
 * Registered OUTSIDE <ProtectedRoute> in App.js.
 */
...
function Spinner() {
  return (
    <svg className="animate-spin h-10 w-10 text-[#00B0B0] mx-auto mb-4" fill="none" viewBox="0 0 24 24">
...
export default function GoCardlessPaymentCallback() {
  ...
  useEffect(() => {
    ...
    if (error || (!code && !state)) {
      setPhase('denied');
      setDetail('You cancelled the GoCardless authorisation.');
    ...
    supabase.auth.refreshSession()
      ...
      .then(async (session) => {
        // Calls a dedicated edge function for GoCardless Payments (separate from gocardless-auth
        // which handles Open Banking)
        const { data, error: fnErr } = await supabase.functions.invoke('gocardless-payments-auth', {
          body:    { action: 'callback', code, state },
          headers: { Authorization: `Bearer ${session.access_token}` },
        });
        if (fnErr) throw new Error(fnErr.message ?? 'gocardless-payments-auth error');
    ...
    <div className={BG}>
      <div className={CARD}>
        ...
        {phase === 'loading' && (
          <>
            <Spinner />
            <h2 className="text-white font-semibold text-lg mb-2">Connecting GoCardless…</h2>
        ...
        {phase === 'success' && (
          <>
            <CheckIcon />
            <h2 className="text-white font-semibold text-lg mb-2">GoCardless connected!</h2>
            <p className="text-white/50 text-sm">
              Your GoCardless account is linked. You can now collect Direct Debits from your customers.
            </p>
          </>
        )}
        ...
        <div className="mt-6 h-0.5 bg-white/10 rounded-full overflow-hidden">
          <div className={`h-full rounded-full transition-all duration-[2500ms] ease-linear ${
            ...
            phase === 'loading' ? 'bg-[#00B0B0] w-1/2 animate-pulse' :
            'bg-red-400 w-full'
          }`} />
        </div>
      </div>
