...
const JOBS = [
  ...
  },
  {
    ...
    crew: 'jamie',
    type: 'exterior',
    customer: 'Rowan House',
    service: 'Windows + fascia',
    postcode: 'W2 3FF',
    start: 12.67,
    dur: 1.0,
    price: 110,
...
function jobHeight(dur) {
  return dur * PX_PER_HR;
}
function jobTop(start) {
  return (start - DAY_START) * PX_PER_HR;
}
      ...
      >
        {isTight ? (
          <div className="flex items-center gap-1.5 text-[11px] leading-tight font-medium min-w-0">
            ...
            <span className="ml-auto shrink-0 tabular-nums opacity-60 text-[10px]">
              {job.postcode}
            </span>
          </div>
        ) : (
...
// ─── Crew lane ────────────────────────────────────────────────────────────────
function CrewLane({ crew: _crew, jobs }) {
  ...
  return (
    ...
    >
      ...
      <div aria-hidden className="pointer-events-none fixed inset-0 -z-0">
        <div
          className="absolute top-[8%] left-[10%] w-[520px] h-[520px] rounded-full opacity-70"
          style={{
        ...
        />
        <div
          className="absolute top-[40%] right-[5%] w-[600px] h-[600px] rounded-full opacity-60"
          style={{
            background: 'radial-gradient(circle, #c7b8ff 0%, transparent 70%)',
            filter: 'blur(60px)',
          }}
        />
        <div
          className="absolute bottom-[-10%] left-[30%] w-[500px] h-[500px] rounded-full opacity-55"
          style={{
            background: 'radial-gradient(circle, #ffd1a8 0%, transparent 70%)',
            filter: 'blur(60px)',
          }}
        />
        <div
          className="absolute top-[20%] left-[55%] w-[360px] h-[360px] rounded-full opacity-50"
          style={{
            background: 'radial-gradient(circle, #a8f0d1 0%, transparent 70%)',
            filter: 'blur(55px)',
          }}
        />
      </div>
        ...
        >
          ...
          <div className="flex border-b border-white/60 bg-white/40 backdrop-blur rounded-t-2xl">
            ...
            {visibleCrews.map((c) => {
              ...
              const rev = visibleJobs
                .filter((j) => j.crew === c.id)
                .reduce((s, j) => s + j.price, 0);
              return (
                <div
                  key={c.id}
                  className="flex-1 min-w-[180px] border-r border-gray-200 last:border-r-0 px-3 py-2.5"
                >
                  <div className="flex items-center gap-2">
                    <span
          ...
          <div className="flex">
            <HourRuler />
            {visibleCrews.map((c) => (
              <CrewLane key={c.id} crew={c} jobs={visibleJobs.filter((j) => j.crew === c.id)} />
            ))}
          </div>
        </div>
