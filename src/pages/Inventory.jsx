...
const INITIAL_PRODUCTS = [
  ...
  {
    id: 17,
    name: 'Disposable overshoes (50-pk)',
    category: 'ppe',
    type: 'residential',
    unitCost: 5.4,
    qty: 3,
...
const INITIAL_ORDERS = [
  {
    ...
    supplier: 'Amazon',
    items: [
      { name: 'Microfibre cloths', qty: 2, cost: 6.5 },
      { name: 'Nitrile gloves M', qty: 3, cost: 8.9 },
    ],
    total: 39.7,
    receipt: true,
    logged: true,
  },
  {
    ...
    supplier: 'Cleaning Hub',
    items: [
      { name: 'Flash 5L', qty: 4, cost: 2.4 },
      { name: 'Industrial degreaser', qty: 2, cost: 6.8 },
    ],
    total: 23.2,
    receipt: true,
    logged: true,
  },
  {
    ...
    supplier: 'Costco',
    items: [
      { name: 'Bin bags 200-roll', qty: 2, cost: 9.8 },
      { name: 'Toilet Duck', qty: 6, cost: 1.8 },
    ],
    total: 30.4,
    receipt: true,
    logged: true,
  },
  {
    ...
    supplier: 'WFP Direct',
    items: [
      { name: 'Pure water solution', qty: 3, cost: 5.5 },
      { name: 'Squeegee blades', qty: 6, cost: 2.8 },
    ],
    total: 33.3,
    receipt: false,
    logged: false,
  },
  {
    ...
    supplier: 'Screwfix',
    items: [
      { name: 'Bleach 5L', qty: 4, cost: 4.2 },
      { name: 'Safety goggles', qty: 2, cost: 3.5 },
    ],
    total: 23.8,
    receipt: true,
    logged: true,
  },
  {
    ...
    supplier: 'Amazon',
    items: [
      { name: 'Trigger sprayers', qty: 10, cost: 1.4 },
      { name: 'Overshoes 50pk', qty: 2, cost: 5.4 },
    ],
    total: 24.8,
    receipt: true,
    logged: true,
...
function kitStockOk(kit, products) {
  return kit.items.every((item) => {
    const p = products.find((x) => x.id === item.productId);
    return p && p.qty >= item.qtyPerJob;
  });
}
function kitLowItems(kit, products) {
  return kit.items.filter((item) => {
    const p = products.find((x) => x.id === item.productId);
    return p && (p.qty === 0 || p.qty <= item.qtyPerJob);
  }).length;
}
...
function StatusBadge({ status }) {
  ...
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded-lg text-[10px] font-black border ${map[status] ?? map.good}`}
    >
      {label[status] ?? 'Good'}
    </span>
  );
...
function TypeBadge({ type }) {
  ...
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded-lg text-[10px] font-black border ${map[type] ?? map.all}`}
    >
      {label[type] ?? type}
    </span>
  );
...
function ReorderModal({ product, accounts, onConfirm, onClose }) {
  ...
  const [price, setPrice] = useState(
    (product.unitCost * Math.max(product.minQty * 2, 4)).toFixed(2)
  );
        ...
        <div className="p-5 space-y-4">
            ...
            <div>
              ...
              <GInput
                ...
                onChange={(e) => {
                  ...
                  setQty(q);
                  setPrice((product.unitCost * q).toFixed(2));
                }}
              />
            </div>
            <div>
          ...
          <div className="rounded-xl border border-[rgba(153,197,255,0.1)] divide-y divide-[rgba(153,197,255,0.06)]">
            {[
              ...
              {
                l: 'Tax saving est.',
                v: `+${fmt2(taxSaved)} (${(accounts.taxRate * 100).toFixed(0)}%)`,
                c: 'text-emerald-400',
              },
            ].map(({ l, v, c }) => (
          ...
          <GAlert type="gold">
            This purchase will be logged automatically as an allowable expense — reducing your tax
            bill by approximately <strong className="text-white">{fmt2(taxSaved)}</strong>.
          </GAlert>
          ...
          <div>
            ...
            <GSelect value={form.category} onChange={(e) => set('category', e.target.value)}>
              {['chemicals', 'equipment', 'ppe', 'specialist', 'disposables'].map((c) => (
                <option key={c} value={c} className="bg-[#010a4f]">
                ...
                <button
                  ...
                  className={`py-2 text-xs font-black rounded-xl border transition-all ${
                    form.type === t
                      ...
                      : 'border-[rgba(153,197,255,0.15)] text-[rgba(153,197,255,0.5)] hover:text-white hover:border-[rgba(153,197,255,0.3)]'
                  }`}
                >
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          {
            label: 'Low stock',
            val: low.length + critical.length,
            color: low.length + critical.length > 0 ? 'text-amber-400' : 'text-white',
          },
          {
            label: 'Critical',
            val: critical.length,
            color: critical.length > 0 ? 'text-red-400' : 'text-white',
          },
        ].map(({ label, val, color }) => (
      ...
      {critical.length > 0 && (
        <div className="space-y-2">
          {critical.map((p) => (
            <div
              key={p.id}
              className="flex items-center gap-3 px-4 py-3 rounded-xl bg-red-500/8 border border-red-500/25"
            >
              <span className="text-base shrink-0">{p.icon}</span>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-black text-red-300">{p.name} — out of stock</p>
                <p className="text-xs text-red-400/70">
                  {p.supplier} · £{p.unitCost}/{p.unit}
                </p>
              </div>
              <button
      ...
      {low.length > 0 && (
        <GAlert type="warn">
          ...
          — {low.map((p) => p.name).join(', ')}. Reorder before your next job run.
        </GAlert>
      )}
      ...
      {products.length === 0 && (
        <GCard className="p-8 text-center">
          ...
          <p className="text-xs text-[rgba(153,197,255,0.5)] mb-4">
            Add your cleaning supplies, chemicals, and equipment to track stock levels and costs.
          </p>
          <button
            onClick={() => setAdding(true)}
              ...
              <div className="divide-y divide-[rgba(153,197,255,0.05)]">
                {catProducts.map((p) => {
                  ...
                  return (
                    <div
                      key={p.id}
                      className="flex items-center gap-3 px-4 py-3 hover:bg-[rgba(153,197,255,0.03)] group transition-colors"
                    >
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        <button
                          onClick={() => onUpdateProduct(p.id, { qty: Math.max(0, p.qty - 1) })}
                          className="w-6 h-6 rounded-lg bg-[rgba(153,197,255,0.08)] border border-[rgba(153,197,255,0.12)] text-xs font-black text-[rgba(153,197,255,0.6)] hover:bg-[rgba(153,197,255,0.15)] hover:text-white flex items-center justify-center transition-all"
                        >
                          −
                        </button>
                        <span className="text-sm font-black tabular-nums text-white w-8 text-center">
                          {p.qty}
                        </span>
                        <button
                          onClick={() => onUpdateProduct(p.id, { qty: p.qty + 1 })}
                          className="w-6 h-6 rounded-lg bg-[rgba(153,197,255,0.08)] border border-[rgba(153,197,255,0.12)] text-xs font-black text-[rgba(153,197,255,0.6)] hover:bg-[rgba(153,197,255,0.15)] hover:text-white flex items-center justify-center transition-all"
                        >
                          +
                        </button>
                      ...
                      <div className="text-right shrink-0 w-16">
                        <p className="text-[10px] font-mono text-[rgba(153,197,255,0.45)]">
                          £{p.unitCost}/{p.unit}
                        </p>
      ...
      {reordering && (
        <ReorderModal
          product={reordering}
          accounts={accounts}
          onConfirm={(qty, total) => handleReorderConfirm(reordering, qty, total)}
          onClose={() => setReordering(null)}
        />
      )}
      {adding && (
        <AddProductModal onSave={(p) => onAddProduct(p)} onClose={() => setAdding(false)} />
      )}
    </div>
  );
}
...
function KitsSection({ products, setProducts, accounts, kits, setKits }) {
  ...
  const available = products.filter(
    (p) => draft.type === 'all' || p.type === draft.type || p.type === 'all'
  );

  const addDraftItem = (id) => {
    if (!draft.items.some((i) => i.productId === id))
      setDraft((p) => ({ ...p, items: [...p.items, { productId: id, qtyPerJob: 1 }] }));
  };
  const updateDraftItem = (id, qty) =>
    setDraft((p) => ({
      ...p,
      items: p.items.map((i) =>
        i.productId === id ? { ...i, qtyPerJob: Math.max(0.05, qty) } : i
      ),
    }));
  const removeDraftItem = (id) =>
    setDraft((p) => ({ ...p, items: p.items.filter((i) => i.productId !== id) }));
  const resetDraft = () => {
    setDraft({ label: '', type: 'residential', desc: '', assignedTo: 'You', items: [] });
    setShowCreate(false);
  };
    ...
    setKits((prev) => [
      {
        ...
        label: draft.label,
        type: draft.type,
        icon: draft.type === 'residential' ? '🧺' : draft.type === 'commercial' ? '🧰' : '🚐',
        desc: draft.desc || `Custom ${draft.type} kit`,
  ...
  const handleLoadKit = (kit) => {
    setProducts((prev) =>
      prev.map((p) => {
        const item = kit.items.find((i) => i.productId === p.id);
        if (!item) return p;
        return { ...p, qty: Math.max(0, p.qty - item.qtyPerJob) };
      })
    );
    setLoadedKit(kit.id);
  ...
  return (
    <div className="space-y-4">
      <GAlert type="blue">
        <strong className="text-white">How kits work</strong> — create your own kits, choose exactly
        what goes in each one, assign to a staff member, and keep the{' '}
        <strong className="text-white">cost per job</strong> feeding your pricing calculator.
      </GAlert>
      ...
      {showCreate && (
        <GCard className="p-4 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <SL className="mb-1.5">Kit name</SL>
              <GInput
                type="text"
                value={draft.label}
                onChange={(e) => setDraft((p) => ({ ...p, label: e.target.value }))}
                placeholder="e.g. Emma residential kit"
              />
            </div>
            <div>
              <SL className="mb-1.5">Assign to</SL>
              <GSelect
                value={draft.assignedTo}
                onChange={(e) => setDraft((p) => ({ ...p, assignedTo: e.target.value }))}
              >
                {kitStaffOptions.map((n) => (
                  <option key={n} value={n} className="bg-[#010a4f]">
                    {n}
                  </option>
                ))}
              </GSelect>
            ...
            <div>
              ...
              <GSelect
                value={draft.type}
                onChange={(e) => setDraft((p) => ({ ...p, type: e.target.value, items: [] }))}
              >
                {types
                  .filter((t) => t !== 'all')
                  .map((t) => (
                    <option key={t} value={t} className="bg-[#010a4f]">
            ...
            <div>
              <SL className="mb-1.5">Description</SL>
              <GInput
                type="text"
                value={draft.desc}
            ...
            <div>
              ...
              <div className="max-h-56 overflow-y-auto rounded-xl border border-[rgba(153,197,255,0.12)] divide-y divide-[rgba(153,197,255,0.05)]">
                {available.map((product) => (
                  <button
                    key={product.id}
                    onClick={() => addDraftItem(product.id)}
                  ...
                  >
                    ...
                    <span className="font-mono text-[rgba(153,197,255,0.35)]">
                      {fmt2(product.unitCost)}
                    </span>
                  </button>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="rounded-xl border border-[rgba(153,197,255,0.12)] min-h-24 max-h-56 overflow-y-auto divide-y divide-[rgba(153,197,255,0.05)]">
                {draft.items.length === 0 ? (
                  <div className="px-3 py-6 text-xs text-[rgba(153,197,255,0.3)] text-center">
                    Choose products from the left to build this kit.
                  </div>
                ) : (
                    ...
                    return (
                      <div key={item.productId} className="px-3 py-2.5 flex items-center gap-3">
                        ...
                        <input
                          ...
                          min="0.05"
                          step="0.05"
                          value={item.qtyPerJob}
                          onChange={(e) =>
                            updateDraftItem(item.productId, parseFloat(e.target.value) || 0.05)
                          }
                        ...
                        <button
                          ...
                          className="text-[rgba(153,197,255,0.25)] hover:text-red-400 transition-colors"
                        >
                          ✕
                        </button>
                      </div>
          ...
          {draft.items.length > 0 && (
            <div className="flex justify-between px-4 py-3 rounded-xl bg-[rgba(153,197,255,0.04)] border border-[rgba(153,197,255,0.1)] text-sm">
              ...
              <span className="font-black text-red-400">
                {fmt2(kitCostPerJob(draft, products))}
              </span>
            </div>
          )}
          ...
          <div className="flex gap-2">
            <button
              onClick={saveDraftKit}
              disabled={!draft.label || draft.items.length === 0}
        ...
        {filtered.length === 0 && (
          <GCard className="p-8 text-center">
            ...
            <p className="text-xs text-[rgba(153,197,255,0.4)]">
              Create your first custom kit, choose what goes in it, and assign it to a staff member.
            </p>
          </GCard>
        )}
        {filtered.map((kit) => {
          ...
          return (
            <GCard key={kit.id} className="overflow-hidden">
              <div
                className={`flex items-start gap-3 px-5 py-4 border-b border-[rgba(153,197,255,0.08)] bg-gradient-to-r ${typeGrad} to-transparent`}
              >
                <div className="w-10 h-10 rounded-xl bg-[rgba(153,197,255,0.08)] border border-[rgba(153,197,255,0.12)] flex items-center justify-center text-xl shrink-0">
                  {kit.icon}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    {!stockOk && lowCount > 0 && (
                      <span className="px-2 py-0.5 rounded-lg text-[10px] font-black border bg-amber-500/10 border-amber-500/20 text-amber-400">
                        ⚠ {lowCount} item{lowCount > 1 ? 's' : ''} low
                      </span>
                    )}
                    {justLoaded && (
                      <span className="px-2 py-0.5 rounded-lg text-[10px] font-black border bg-emerald-500/10 border-emerald-500/20 text-emerald-400">
                        ✓ Loaded
                      </span>
                    )}
                  </div>
              ...
              <div className="px-5 py-4 space-y-3">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {kit.items.map((item) => {
                    ...
                    return (
                      <div
                        key={item.productId}
                        className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] border ${ok ? 'bg-[rgba(153,197,255,0.05)] border-[rgba(153,197,255,0.12)] text-[rgba(153,197,255,0.6)]' : 'bg-red-500/8 border-red-500/20 text-red-400'}`}
                      >
                        <span>{p.icon}</span>
                        <span className="font-bold">{p.name.split(' ').slice(0, 3).join(' ')}</span>
                        <span className="font-mono opacity-60">
                          ×{item.qtyPerJob < 1 ? item.qtyPerJob.toFixed(2) : item.qtyPerJob}
                        </span>
                ...
                <div className="flex items-center justify-between text-[10px] text-[rgba(153,197,255,0.4)] pt-2 border-t border-[rgba(153,197,255,0.06)] flex-wrap gap-2">
                  <span>
                    {kit.items.length} products · cost per job:{' '}
                    <strong className="text-red-400">{fmt2(costPerJob)}</strong>
                  </span>
                  <span>
                    Tax saving:{' '}
                    <strong className="text-emerald-400">
                      {fmt2(costPerJob * accounts.taxRate)}
                    </strong>
                  </span>
                </div>
                ...
                <div className="flex gap-2">
                  <GSelect
                    value={kit.assignedTo || 'Unassigned'}
                    onChange={(e) =>
                      setKits((prev) =>
                        prev.map((k) =>
                          k.id === kit.id ? { ...k, assignedTo: e.target.value } : k
                        )
                      )
                    }
                  ...
                  <button
                    onClick={() => setKits((prev) => prev.filter((k) => k.id !== kit.id))}
                ...
                <button
                  onClick={() => (stockOk ? handleLoadKit(kit) : handleLoadKit(kit))}
                  className={`w-full py-2.5 rounded-xl text-xs font-black transition-all ${
                    justLoaded
                      ? 'bg-emerald-500/15 border border-emerald-500/25 text-emerald-400'
                      : stockOk
                        ? 'bg-[#1f48ff]/20 border border-[#1f48ff]/35 text-white hover:bg-[#1f48ff]/35'
                ...
                >
                  {justLoaded
                    ? '✓ Stock deducted'
                    : stockOk
                      ? `Load kit — deduct ${fmt2(costPerJob)} from stock`
                      : `⚠ Load anyway — ${lowCount} item${lowCount > 1 ? 's' : ''} short`}
                </button>
              </div>
            </GCard>
          );
      ...
      <GAlert type="gold">
        <strong className="text-white">Pricing calculator connection</strong> — every custom kit
        keeps a live <strong className="text-white">cost per job</strong> figure so your pricing
        calculator stays accurate as supply costs change.
      </GAlert>
    </div>
  );
}
...
function SpendSection({ accounts, orders }) {
  ...
  const thisMonth = orders
    .filter((o) => o.logged && o.date?.startsWith(currentMonthPrefix))
  ...
  const monthlySpend = Array.from({ length: 6 }, (_, i) => {
    ...
    const spend = orders
      .filter((o) => o.logged && o.date?.startsWith(prefix))
      .reduce((s, o) => s + o.total, 0);
    ...
    <div className="space-y-4">
      ...
      <GCard className="overflow-hidden">
        ...
        <div className="divide-y divide-[rgba(153,197,255,0.06)]">
          {[
            {
              l: 'SA103 box 19 — Cleaning materials',
              v: fmt(ytdSpend),
              sub: 'Cost of goods sold',
              c: 'text-red-400',
            },
            {
              l: 'Income tax saving (approx.)',
              v: fmt(taxSaved),
              sub: `${(accounts.taxRate * 100).toFixed(0)}% of supply spend`,
              c: 'text-emerald-400',
            },
            {
              l: 'VAT goods — FRS Ltd Cost test',
              v: fmt(quarterlySpend),
              sub: 'Per quarter',
              c: 'text-amber-400',
            },
            {
              l: 'Goods as % of VAT-inclusive income',
              v: `${goodsPct.toFixed(1)}%`,
              sub: lctPass ? 'Above 2% — FRS rate safe' : 'Below 2% — Limited Cost Trader warning',
              c: lctPass ? 'text-emerald-400' : 'text-red-400',
            },
          ].map(({ l, v, sub, c }) => (
        ...
        {!lctPass && (
          <div className="px-4 pb-4">
            <GAlert type="warn">
              Goods spend is only <strong className="text-white">{goodsPct.toFixed(1)}%</strong> of
              income this quarter — below the 2% threshold. You'll be classed as a{' '}
              <strong className="text-white">Limited Cost Trader</strong> and must use the 16.5% FRS
              rate. Buy more supplies before quarter end to avoid this.
            </GAlert>
          </div>
        )}
      </GCard>
            ...
            return (
              <div key={m.month} className="flex-1 flex flex-col items-center gap-1 group">
                <div className="relative w-full flex flex-col items-center">
                  ...
                  <div
                    className={`w-full rounded-t-sm transition-all ${m.isCurrent ? 'bg-[#1f48ff] opacity-70' : 'bg-[rgba(153,197,255,0.15)] group-hover:bg-[rgba(153,197,255,0.25)]'}`}
      ...
      <GCard className="overflow-hidden">
        <div className="px-4 py-3 border-b border-[rgba(153,197,255,0.08)]">
          <SL>Spend by category — YTD</SL>
        </div>
        <div className="divide-y divide-[rgba(153,197,255,0.05)]">
          {byCategory.map(({ label, spend, color }) => (
            <div key={label} className="flex items-center gap-3 px-4 py-3">
              <span className="text-xs font-bold text-[rgba(153,197,255,0.5)] w-20 shrink-0">
                {label}
              </span>
              <div className="flex-1 h-3 bg-[rgba(153,197,255,0.06)] rounded-full overflow-hidden">
                <div
                  className={`h-full ${color} rounded-full transition-all`}
                  style={{ width: `${(spend / ytdSpend) * 100}%` }}
                />
              </div>
              <span className="text-xs font-black font-mono text-white w-12 text-right shrink-0">
                {fmt(spend)}
              </span>
              <span className="text-[10px] text-[rgba(153,197,255,0.35)] w-8 text-right shrink-0">
                {Math.round((spend / ytdSpend) * 100)}%
              </span>
            </div>
          ))}
        </div>
...
function OrdersSection({ orders, setOrders, accounts }) {
  ...
  const logOrder = (id) =>
    setOrders((prev) => prev.map((o) => (o.id === id ? { ...o, logged: true } : o)));
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: 'Logged to accounts', val: fmt(totalLogged), color: 'text-emerald-400' },
          {
            ...
            val: fmt(totalUnlogged),
            color: totalUnlogged > 0 ? 'text-amber-400' : 'text-white',
          },
          {
            label: 'Digital receipts',
            val: `${totalReceipts} / ${orders.length}`,
            color: 'text-[#99c5ff]',
          },
        ].map(({ label, val, color }) => (
          <GCard key={label} className="px-4 py-3">
      ...
      {totalUnlogged > 0 && (
        <GAlert type="warn">
          <strong className="text-white">{fmt(totalUnlogged)} in purchases not yet logged</strong> —
          these won't appear in your SA103 expenses or reduce your tax bill until logged.
        </GAlert>
      )}

      <GAlert type="gold">
        <strong className="text-white">HMRC records</strong> — under MTD you must keep digital
        records of every business expense for 6 years. This orders log is your compliant record. No
        paper receipts needed once saved here.
      </GAlert>
      ...
      {showAdd && (
        <GCard className="p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <SL className="mb-1.5">Supplier</SL>
              <GInput
                type="text"
            ...
            <div>
              <SL className="mb-1.5">Total spend (£)</SL>
              <GInput
                type="number"
                placeholder="0.00"
                step="0.01"
          ...
          {parseFloat(newOrder.total) > 0 && (
            <div className="flex justify-between px-4 py-2.5 rounded-xl bg-[rgba(153,197,255,0.04)] border border-[rgba(153,197,255,0.08)] text-sm">
              ...
              <span className="text-xs font-black text-emerald-400">
                +{fmt2(parseFloat(newOrder.total) * accounts.taxRate)}
              </span>
            </div>
          )}
          <button
            ...
            onClick={() => {
              const o = {
                ...
                items: [
                  {
                    name: newOrder.notes || 'Cleaning supplies',
                    qty: 1,
                    cost: parseFloat(newOrder.total),
                  },
                ],
            ...
            <div className="flex items-start gap-3">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  ...
                  {o.receipt ? (
                    <span className="px-2 py-0.5 rounded-lg text-[10px] font-black border bg-[#1f48ff]/10 border-[#1f48ff]/20 text-[#99c5ff]">
                      Receipt saved
                    </span>
                  ) : (
                    <span className="px-2 py-0.5 rounded-lg text-[10px] font-black border bg-[rgba(153,197,255,0.06)] border-[rgba(153,197,255,0.12)] text-[rgba(153,197,255,0.35)]">
                      No receipt
                    </span>
                  )}
                </div>
                ...
                <div className="flex flex-wrap gap-1.5 mt-1.5">
                  {o.items.map((item, i) => (
                    <span
                      key={i}
                      className="text-[10px] text-[rgba(153,197,255,0.5)] bg-[rgba(153,197,255,0.04)] border border-[rgba(153,197,255,0.1)] px-2 py-0.5 rounded-lg"
                    >
                      {item.name}
                      {item.qty > 1 ? ` ×${item.qty}` : ''} — {fmt2(item.cost * (item.qty || 1))}
                    </span>
                  ))}
                </div>
              </div>
              <div className="text-right shrink-0">
                ...
                <p className="text-[10px] text-emerald-400 mt-0.5">
                  saves {fmt2(o.total * accounts.taxRate)} tax
                </p>
              </div>
            </div>
            {!o.logged && (
              <button
                onClick={() => logOrder(o.id)}
...
// ─── ROOT ─────────────────────────────────────────────────────────────────────
export default function InventoryTab({ accountsData: _accountsData, onExpenseLog }) {
  const { user } = useAuth();
  ...
  const deleteProductLocal = useCallback(
    async (id) => {
        ...
        setOrders((prev) => [
          {
            ...
            date: orderData.date,
            supplier: product.supplier,
            items: [{ name: product.name, qty, cost: product.unitCost }],
            total,
            receipt: false,
            logged: true,
          },
  ...
  const TABS = [
    {
      ...
      label: 'Stock',
      emoji: '📦',
      badge: critical + low > 0 ? critical + low : null,
      badgeColor: critical > 0 ? 'bg-red-500/20 text-red-400' : 'bg-amber-500/20 text-amber-400',
    },
    ...
    { id: 'spend', label: 'Spend', emoji: '💷', badge: null },
    {
      ...
      label: 'Orders',
      emoji: '🧾',
      badge: unlogged > 0 ? unlogged : null,
      ...
      <div className="relative max-w-3xl mx-auto px-4 py-6 space-y-4">
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <span className="text-xs font-bold text-[rgba(153,197,255,0.5)]">
                {products.length} products
              </span>
              {critical > 0 && (
                <span className="text-xs font-black text-red-400">🔴 {critical} critical</span>
              )}
              {low > 0 && <span className="text-xs font-black text-amber-400">⚠ {low} low</span>}
              <span className="text-xs text-[rgba(153,197,255,0.4)]">
                YTD spend: <strong className="text-white">{fmt(ytdSpend)}</strong>
              </span>
              <span className="text-xs text-[rgba(153,197,255,0.4)]">
                Tax saved:{' '}
                <strong className="text-emerald-400">{fmt(ytdSpend * accounts.taxRate)}</strong>
              </span>
            </div>
          </div>
          <div className="flex items-center gap-1.5 shrink-0">
            <span
              className={`w-2 h-2 rounded-full shrink-0 ${isLive ? 'bg-emerald-400 animate-pulse' : 'bg-[rgba(153,197,255,0.3)]'}`}
            />
            <span className="text-[10px] font-black text-[rgba(153,197,255,0.45)]">
              {isLive ? 'Live' : 'Demo'}
            </span>
          </div>
        </div>
        ...
        <div className="flex gap-1.5 p-1 bg-[rgba(0,0,0,0.2)] rounded-2xl">
          {TABS.map(({ id, label, emoji, badge, badgeColor }) => (
            ...
            >
              ...
              {badge && (
                <span
                  className={`px-1.5 py-0.5 rounded-full text-[9px] font-black ${tab === id ? 'bg-white/20 text-white' : badgeColor}`}
                >
                  {badge}
                </span>
              )}
        ...
        {tab === 'stock' && (
          <StockSection
            ...
            accounts={accounts}
            onOrderLogged={handleOrderLogged}
            onUpdateProduct={updateProductLocal}
            onAddProduct={addProductLocal}
            onDeleteProduct={deleteProductLocal}
          />
        )}
        {tab === 'kits' && (
          <KitsSection
        ...
        {tab === 'orders' && (
          <OrdersSection orders={orders} setOrders={setOrders} accounts={accounts} />
        )}
      </div>
    </div>
  );
