    ...
    <div className={BG}>
      ...
      <div className="sticky top-0 z-10 bg-[#050c1e]/90 backdrop-blur border-b border-white/5 px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="text-[10px] font-bold tracking-widest uppercase text-[#4f78ff]">Walkthrough</span>
          {screen > 0 && total > 0 && (
            <span className="text-[10px] text-white/30">{screen} / {total}</span>
          )}
        </div>
        {onSave && (
          <button onClick={onSave} className="text-xs text-white/40 hover:text-white/60 transition-colors font-semibold">
            Save &amp; pause
          </button>
        )}
      </div>
  ...
  if (dismissed) {
    return (
      <div className="p-4 rounded-xl bg-white/3 border border-white/6 text-sm text-white/30 italic">
        {dismissed === 'follow_up' ? '✓ Marked for follow-up' : dismissed === 'expected' ? 'Got it.' : 'Ignored.'}
      </div>
    );
  }
    ...
    <div className="p-4 rounded-xl bg-white/4 border border-white/10 space-y-3">
      ...
      <div className="flex flex-wrap gap-2">
        {actions.map(([label, value]) => (
          <button
            key={value}
            onClick={() => { setDismissed(value); onAction?.(hole, value); }}
...
function FocusCard({ area, selected, onSelect }) {
  return (
    ...
    >
      <p className={`text-sm font-bold ${selected ? 'text-[#99c5ff]' : 'text-white'}`}>{area.title}</p>
...
export default function FinancialWalkthrough() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [{ data: walks }, { data: an }] = await Promise.all([
        ...
        supabase.from('walkthrough_analysis')
          .select('*').eq('business_id', biz.id)
  ...
  async function goToScreen(n) {
    ...
    setScreen(n);
    await supabase.from('walkthroughs')
  ...
  async function completeFocusArea() {
    const chosen = focusArea || (customFocus.trim() ? { title: customFocus.trim(), body: '' } : null);
    if (!chosen) return;

    const w = await ensureWalkthrough();
    await supabase.from('walkthroughs').update({
    ...
    return (
      <Layout screen={0} total={5} onSave={saveAndPause}>
        <div className="space-y-6">
          <div>
            <p className="text-[10px] font-bold tracking-widest uppercase text-[#4f78ff] mb-2">Financial walkthrough</p>
            <h1 className="text-2xl font-black text-white leading-tight mb-3">
              Let's go through your finances together.
            </h1>
            <p className="text-white/60 text-sm leading-relaxed mb-4">
              I've had a look at the last {periodDays} days of your bank account. We're going to walk through three things:
            </p>
            <ul className="space-y-2 mb-4">
              {['What\'s coming in', 'What\'s going out', 'A few things I spotted that might be worth a look'].map(item => (
  ...
  if (screen === 1) {
    const profitSign = (health.profit ?? 0) >= 0 ? '+' : '';
    return (
      <Layout screen={1} total={5} onSave={saveAndPause}>
        <div className="space-y-6">
          <div>
            <h2 className="text-2xl font-black text-white mb-4">Here's the picture.</h2>
            <p className="text-white/50 text-xs font-semibold uppercase tracking-widest mb-3">Over the last {periodDays} days:</p>
            <div className="space-y-2 mb-5">
              {[
                { label: 'came in', value: fmt(health.totalIn), positive: true },
                { label: 'went out', value: fmt(health.totalOut), positive: false },
                { label: 'a month in profit', value: fmt(health.monthlyProfit), positive: (health.monthlyProfit ?? 0) >= 0 },
              ].map(({ label, value, positive }) => (
                <div key={label} className="flex items-center justify-between px-4 py-3 rounded-xl bg-white/4 border border-white/8">
                  <span className="text-white/50 text-sm">{label}</span>
                  <span className={`font-black text-lg tabular-nums ${positive ? 'text-white' : 'text-white/70'}`}>{value}</span>
                </div>
              ))}
            </div>
            ...
            <p className="text-sm text-white/50 leading-relaxed italic">
              "That's the headline. We'll dig in next — I'll show you where the money comes from, where it goes, and a few things I noticed that might be worth a look.
            </p>
          ...
          {!responses.picture_reaction && (
            <div className="p-4 rounded-xl bg-white/3 border border-white/8">
              <p className="text-sm text-white/60 mb-3">Does that profit figure look about right to you?</p>
              <div className="flex flex-wrap gap-2">
                {['Yes, sounds about right', 'Lower than I thought', 'Higher than I thought'].map(opt => (
                  <button
                    key={opt}
          ...
          {responses.picture_reaction && (
            <p className="text-xs text-white/30 italic">Got it — {responses.picture_reaction.toLowerCase()}.</p>
          )}
      ...
      <Layout screen={2} total={5} onSave={saveAndPause}>
        <div className="space-y-6">
          <div>
            ...
            <p className="text-white/50 text-sm">Here's where your income came from over the last {periodDays} days.</p>
          </div>

          {customers.length > 0 && (
            <div>
              <p className="text-xs font-bold uppercase tracking-widest text-white/30 mb-2">Your top customers</p>
              <div className="space-y-2">
                {customers.slice(0, 5).map((c, i) => (
                  <div key={i} className="flex items-center justify-between px-4 py-3 rounded-xl bg-white/4 border border-white/8">
                    <span className="text-sm text-white/70 truncate flex-1">{c.name}</span>
                    <span className="text-sm font-black text-white tabular-nums ml-2">{fmt(c.amount)}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {gaps.length > 0 && (
            <div className="p-4 rounded-xl bg-amber-500/8 border border-amber-500/20">
              ...
              <p className="text-sm text-white/60 leading-relaxed">
                One thing I spotted — there was a gap in {gaps.map(g => g.month).join(', ')} where revenue dropped. Was that planned, or worth a look?
              </p>
            </div>
          )}
          ...
          {unreconciled.length > 0 && (
            <div className="p-4 rounded-xl bg-white/4 border border-white/10">
              ...
              <p className="text-sm text-white/60 leading-relaxed mb-2">
                I see {fmt(unreconciled.reduce((s, t) => s + t.amount, 0))} came in that I couldn't match to invoices. Could be customers paying that I haven't connected yet.
              </p>
          ...
          {customers.length === 0 && (
            <div className="p-4 rounded-xl bg-white/4 border border-white/8">
              <p className="text-sm text-white/40 italic">Cadi doesn't have customer payment data yet — once invoices start flowing through, this'll get more interesting.</p>
            </div>
          )}
  ...
  if (screen === 3) {
    const categories = (moneyOut.categories ?? []).filter(c =>
      !['income_customer', 'income_other', 'uncategorised'].includes(c.category)
    );
      ...
      <Layout screen={3} total={5} onSave={saveAndPause}>
        <div className="space-y-6">
          <div>
            ...
            <p className="text-white/50 text-sm">Here's where your money went, biggest categories first.</p>
          </div>

          {categories.length > 0 ? (
            <div className="space-y-2">
              {categories.slice(0, 8).map((cat) => (
                <div key={cat.category} className="px-4 py-3 rounded-xl bg-white/4 border border-white/8">
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-sm font-semibold text-white">{catLabel(cat.category)}</span>
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-white/40">{cat.pct}%</span>
                      <span className="text-sm font-black text-white tabular-nums">{fmt(cat.amount)}</span>
                    </div>
                  </div>
                  {/* Mini bar */}
                  <div className="h-1 bg-white/8 rounded-full">
                    <div className="h-full bg-[#4f78ff]/60 rounded-full" style={{ width: `${Math.min(cat.pct, 100)}%` }} />
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="p-4 rounded-xl bg-white/4 border border-white/8">
              <p className="text-sm text-white/40 italic">Quiet on the spending front. Either you're running lean or there's another account in the mix.</p>
            </div>
          )}
          ...
          {watchOuts.length > 0 && (
            <div>
              <p className="text-xs font-bold uppercase tracking-widest text-amber-400/70 mb-2">Worth a look</p>
              <div className="space-y-2">
                {watchOuts.map((wo, i) => (
      ...
      <Layout screen={4} total={5} onSave={saveAndPause}>
        <div className="space-y-6">
          <div>
            ...
            <p className="text-white/50 text-sm leading-relaxed">
              These are things I noticed that you might want to do something about. Some might be totally fine — let me know which ones to ignore and which to follow up on.
            </p>
          </div>
          ...
          {holes.length > 0 ? (
            ...
            <div className="p-5 rounded-xl bg-emerald-500/8 border border-emerald-500/20 text-center">
              ...
              <p className="text-sm text-white/50">I didn't spot anything that worried me. That's a good sign.</p>
            </div>
          )}
      ...
      <Layout screen={5} total={5} onSave={saveAndPause}>
        <div className="space-y-6">
          <div>
            ...
            <p className="text-white/50 text-sm leading-relaxed">
              Based on what we just went through, these are the highest-impact options. Pick one — or write your own. I'll track it in your weekly reports.
            </p>
          </div>
          ...
          {focusAreas.length > 0 ? (
            ...
            <div className="space-y-2">
              {[
                ...
                { title: 'Review subscriptions', body: 'Cancel anything that\'s not earning its keep.', estimated_saving: null },
                { title: 'Move supplies to wholesale', body: 'Costco, Bookers, B&M — typically 30-40% cheaper than retail.', estimated_saving: null },
              ].map((area, i) => (
                <FocusCard key={i} area={area} selected={focusArea === area} onSelect={setFocusArea} />
              ))}
            </div>
          )}
          ...
          <div>
            ...
            <input
              type="text"
              value={customFocus}
              onChange={e => { setCustomFocus(e.target.value); setFocusArea(null); }}
    ...
    <Layout screen={0} total={0}>
      <div className="space-y-6 text-center">
        ...
        <div>
          ...
          <p className="text-white/50 text-sm">You've gone through your finances, spotted what matters, and picked a focus.</p>
        </div>
        ...
        <div className="space-y-2 text-left">
          {[
            ...
            <div key={i} className="flex items-center gap-3 px-4 py-3 rounded-xl bg-white/4 border border-white/8">
        ...
        <div className="p-4 rounded-xl bg-white/4 border border-white/8">
          <p className="text-sm text-white/60 leading-relaxed">
            "That's the whole picture. Your focus for the next 30 days is sorted — I'll check in on it every Monday in your weekly report.
            You can run this walkthrough again any time you want a fresh look."
          </p>
        </div>
        ...
        <div className="p-4 rounded-xl bg-[rgba(79,120,255,0.08)] border border-[rgba(79,120,255,0.2)]">
          ...
          <p className="text-xs text-white/40">On Pro, Cadi compares each month to the last so you can see your changes land.</p>
        </div>
