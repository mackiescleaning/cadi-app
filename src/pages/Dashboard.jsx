...
const DEMO_FEED = [
  {
    ...
    icon: '✓',
    bg: 'bg-emerald-100',
    title: 'Johnson — regular clean complete',
    sub: '£60 · SW4 · marked done',
    time: '08:42',
  },
  {
    id: 2,
    icon: '📤',
    bg: 'bg-blue-100',
    title: 'Invoice sent — Park View Flats',
    sub: '£95 · due 13 Apr',
    time: '08:00',
  },
  {
    ...
    icon: '💷',
    bg: 'bg-emerald-100',
    title: 'Payment received — Miller',
    sub: '£280 · bank transfer',
    time: '07:15',
  },
  {
    id: 4,
    icon: '!',
    bg: 'bg-red-100',
    title: 'Harrington invoice overdue',
    sub: '£85 · 3 days past due',
    time: 'Auto',
  },
  {
    id: 5,
    icon: '🚐',
    bg: 'bg-amber-100',
    title: 'Route logged — 18.4 miles',
    sub: '£8.28 HMRC claim · 5 stops',
    time: 'Yest.',
  },
];
...
const DEMO_MONEY_ENTRIES = (() => {
  ...
  for (let i = 0; i < 90; i++) {
    ...
    out.push({
      id: `demo-inc-${i}`,
      kind: 'income',
      amount: Math.max(80, base + wobble),
      date: iso,
    });
    if (i % 4 === 0)
      out.push({ id: `demo-exp-${i}`, kind: 'expense', amount: 35 + ((i * 13) % 45), date: iso });
  }
  return out;
})();
...
function calcHealthScore({ accounts, weekJobs, invoices, jobsToday }) {
  ...
  const weekRevenue = weekJobs
    .filter((d) => d.done || d.isToday)
    .reduce((s, d) => s + d.revenue, 0);
  const weekTarget = weekJobs.reduce((s, d) => s + d.revenue, 0);
  const monthProgress =
    accounts.ytdIncome > 0 ? Math.min((accounts.ytdIncome / accounts.annualTarget) * 12, 1) : 0;

  // ── Revenue (25pts) ──────────────────────────────────────────────────────
  const weekOnTrack = weekTarget > 0 ? Math.min(weekRevenue / weekTarget, 1) : 0;
  const ytdOnTrack =
    accounts.annualTarget > 0
      ? Math.min(accounts.ytdIncome / (accounts.annualTarget * (4 / 12)), 1)
      : 0; // 4 months in
  ...
  const opsScore = Math.round(
    jobsLogged * 18 + (unassigned === 0 ? 7 : Math.max(0, 7 - unassigned * 2))
  );
  ...
  // ── Compliance (15pts) ───────────────────────────────────────────────────
  const taxReservePct =
    accounts.taxReserveTarget > 0
      ? Math.min(1, accounts.taxReserve / accounts.taxReserveTarget)
      : 0;
  ...
  const dims = [
    ...
    { label: 'Operations', score: opsScore, max: 25, color: 'bg-emerald-500' },
    {
      ...
      max: 25,
      color: overdueCount > 0 ? 'bg-amber-400' : 'bg-emerald-500',
    },
    {
      label: 'Compliance',
      score: complianceScore,
      max: 15,
      color: complianceScore < 10 ? 'bg-amber-400' : 'bg-emerald-500',
    },
...
function buildActions({ accounts, invoices, jobsToday }) {
  const actions = [];
...
function MoneyConfidenceHero({
  ...
  const delta =
    view.prevIn > 0 ? Math.round(((view.moneyIn - view.prevIn) / view.prevIn) * 100) : null;
  ...
  const taxPct =
    accounts.taxReserveTarget > 0
      ? Math.round((accounts.taxReserve / accounts.taxReserveTarget) * 100)
      : null;
  ...
  const ytdPct =
    accounts.annualTarget > 0
      ? Math.min(100, Math.round((accounts.ytdIncome / accounts.annualTarget) * 100))
      : 0;
  ...
  const chip = (tone) =>
    ({
      ...
      warn: {
        text: 'text-amber-300',
        dot: 'bg-amber-400 shadow-[0_0_8px_2px_rgba(251,191,36,0.6)]',
      },
  ...
  const confidence = [
    ...
    },
    {
      ...
      sub:
        overdue.length > 0
          ? `${overdue.length} need${overdue.length === 1 ? 's' : ''} chasing`
          : 'Nothing past due',
    ...
    },
    {
      ...
      sub:
        taxPct == null
          ...
          : taxPct >= 100
            ? 'Fully covered'
            : `${fmt(Math.max(0, accounts.taxReserveTarget - accounts.taxReserve))} to go`,
    ...
    },
    {
      ...
      sub:
        accounts.annualTarget > 0
          ? paceAhead
            ? `Ahead — target ${fmt(accounts.annualTarget)}`
            : `Target ${fmt(accounts.annualTarget)} · ${ytdPct}% in`
      ...
      <div className="relative px-5 py-5 grid grid-cols-1 md:grid-cols-5 gap-6 flex-1">
        ...
        <div className="md:col-span-2 flex flex-col">
          <p className="text-[10px] font-bold uppercase tracking-widest text-[rgba(153,197,255,0.45)] mb-1.5">
            Money in ·{' '}
            {MONEY_PERIODS.find((p) => p.id === period)?.label.toLowerCase() === 'today'
              ? 'today'
              : period === 'week'
                ? 'this week'
                : 'this month'}
          ...
          <div className="flex items-end gap-3 flex-wrap">
            ...
            {delta != null && (
              <span
                className={`mb-1 text-[11px] font-black px-2 py-1 rounded-full border ${
                  delta >= 0
          ...
          <div className="mt-4 space-y-2">
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="font-black tabular-nums text-amber-300">
                {view.moneyOut > 0 ? `−${fmt(view.moneyOut)}` : '£0'}
              </span>
            </div>
            <div className="flex items-center justify-between text-xs border-t border-white/5 pt-2">
              <span className="text-[rgba(153,197,255,0.5)]">Kept</span>
              <span
                className={`font-black tabular-nums ${kept >= 0 ? 'text-emerald-300' : 'text-red-300'}`}
              >
                {kept < 0 ? `−${fmt(kept)}` : fmt(kept)}
              </span>
            </div>
          </div>
          ...
          {(accounts.ytdIncome ?? 0) > 0 &&
            (() => {
              const trulyYours = Math.max(
                0,
                (accounts.ytdIncome ?? 0) -
                  (accounts.ytdExpenses ?? 0) -
                  (accounts.taxReserveTarget ?? 0)
              );
              return (
                <div
                  className="mt-4 px-3 py-2.5 rounded-xl border border-emerald-400/25"
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      <p className="text-[10px] font-black uppercase tracking-widest text-emerald-300 mb-0.5">
                        Truly yours · this tax year
                      </p>
                      <p className="text-[10px] text-[rgba(153,197,255,0.5)] leading-snug">
                        {fmt(accounts.ytdIncome)} earned − {fmt(accounts.ytdExpenses ?? 0)} costs −{' '}
                        {fmt(accounts.taxReserveTarget ?? 0)} for tax
                      </p>
                    </div>
                    <p
                      className="text-2xl font-black tabular-nums text-emerald-300 shrink-0"
          ...
          {targetPct != null ? (
            <div className="mt-4">
              <div className="flex justify-between text-[10px] font-bold mb-1.5">
                ...
                <span
                  className={targetPct >= 100 ? 'text-emerald-300' : 'text-[rgba(153,197,255,0.6)]'}
                >
            ...
            >
              ...
              <p className="text-[11px] font-bold text-white leading-snug">
                Set your monthly target — Cadi paces every day, week and month against it.
              </p>
            </button>
          )}
          ...
          <div className="flex gap-2 mt-5 md:mt-auto md:pt-5">
            <button
              ...
              style={{
                ...
                boxShadow:
                  '0 8px 20px -6px rgba(4,120,87,0.55), inset 0 1px 0 rgba(255,255,255,0.2)',
              }}
            >
              💷 Log payment
            </button>
        ...
        <div className="md:col-span-3 flex flex-col">
          <div className="flex items-center justify-between mb-2">
            <p className="text-[10px] font-bold uppercase tracking-widest text-[rgba(153,197,255,0.4)]">
              {view.chartLabel}
            </p>
            {!hasAny && isLive && (
              <span className="text-[10px] font-bold text-amber-300">
                Log payments to light this up
              </span>
            )}
          </div>
            ...
            {view.bars.map((b, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={`text-[9px] font-bold ${b.current ? 'text-emerald-300' : 'text-[rgba(153,197,255,0.3)]'}`}
                  >
                    {b.label}
                  </span>
                </div>
          ...
          return (
            ...
            >
              <div className="flex items-center gap-1.5 mb-1">
                ...
                <p className="text-[9px] font-black uppercase tracking-widest text-[rgba(153,197,255,0.4)]">
                  {c.label}
                </p>
              </div>
              <p className={`text-base font-black tabular-nums leading-tight ${t.text}`}>
                {c.value}
              </p>
      ...
      {landed && (
        <div
          ...
          style={{
            ...
            animation: 'scoreLand 0.9s ease-out forwards',
          }}
        />
      )}
...
const SCORE_DIMENSIONS = [
  {
    ...
    color: 'bg-brand-blue',
    glow: {
      bar: 'linear-gradient(90deg, #38bdf8, #1f48ff)',
      shadow: '0 0 10px 2px rgba(56,189,248,0.6)',
    },
  ...
  },
  {
    ...
    glow: {
      ...
      shadow: '0 0 10px 2px rgba(52,211,153,0.6)',
    },
  ...
  },
  {
    ...
    glow: {
      ...
      shadow: '0 0 10px 2px rgba(251,191,36,0.6)',
    },
  ...
  },
  {
    ...
    glow: {
      ...
      shadow: '0 0 10px 2px rgba(167,139,250,0.6)',
    },
  ...
  },
  {
    ...
    glow: {
      ...
      shadow: '0 0 10px 2px rgba(248,113,113,0.6)',
    },
    ...
    >
      ...
      >
        <div className="flex items-start justify-between px-6 py-4 border-b border-gray-100 sticky top-0 bg-white">
          <div>
            <p className="text-[10px] font-bold tracking-widest uppercase text-brand-skyblue/80">
              Cadi Score
            </p>
        ...
        <div className="px-6 py-5 space-y-5">
          <p className="text-sm text-gray-600 leading-relaxed">
            Your score is out of <span className="font-bold text-brand-navy">100</span> across five
            dimensions. Each updates live from your real data — jobs, invoices, income logs, and
            settings.
          </p>
...
function PendingPaymentsCard({ invoices, onNavigate }) {
  ...
  return (
    <ModuleCard
      dot={
        overdueCount > 0
          ? 'bg-red-400 shadow-[0_0_8px_2px_rgba(239,68,68,0.6)]'
      ...
      headerRight={
        <button
          ...
          className="text-[11px] font-bold text-[#4f78ff] hover:text-[#99c5ff] transition-colors whitespace-nowrap"
        >
          All →
        </button>
      }
    >
      <div className="px-4 pt-3.5 pb-2">
        <p
          className="text-3xl font-black tabular-nums text-white leading-none"
          style={{ textShadow: total > 0 ? '0 0 24px rgba(251,191,36,0.3)' : undefined }}
        >
          {fmt(total)}
        </p>
        <p className="text-[11px] text-[rgba(153,197,255,0.45)] mt-1.5">
          {unpaid.length === 0 ? (
            ...
            <>
              {unpaid.length} invoice{unpaid.length === 1 ? '' : 's'} waiting
              {overdueCount > 0 && (
                <span className="text-red-300 font-bold"> · {overdueCount} overdue</span>
              )}
            </>
          )}
        </p>
      ...
      <div className="flex-1 divide-y divide-[rgba(79,120,255,0.08)] border-t border-[rgba(79,120,255,0.1)]">
        {unpaid.slice(0, 4).map((inv) => (
          ...
          >
            <div
              className={`w-1.5 h-1.5 rounded-full shrink-0 ${inv.status === 'overdue' ? 'bg-red-400 shadow-[0_0_6px_1px_rgba(239,68,68,0.6)]' : 'bg-amber-400/60'}`}
            />
            ...
            <span
              className={`text-[9px] font-black px-1.5 py-0.5 rounded-full shrink-0 ${inv.status === 'overdue' ? 'bg-red-500/20 text-red-300' : 'bg-white/5 text-[rgba(153,197,255,0.5)]'}`}
            >
        ...
        {unpaid.length === 0 && (
          <div className="px-4 py-6 text-center">
            ...
            <p className="text-xs text-[rgba(153,197,255,0.4)]">
              Invoices land here when they're sent.
            </p>
          </div>
        )}
      </div>
  ...
  return (
    ...
    >
        ...
        <div className="min-w-0">
          ...
          {tierNext ? (
            <p className="text-[11px] text-[rgba(153,197,255,0.5)] mt-0.5">
              <span className="font-bold text-white">{tierNext - total} pts</span> to {nextTier}
            </p>
          ) : (
          ...
          <div className="flex items-center gap-1 mt-2">
            {TIERS_ORDERED.map((t, i) => (
              <div
                key={t}
                className={`rounded-full transition-all ${t === tier ? 'w-5 h-1.5 bg-brand-skyblue' : tierIdx > i ? 'w-1.5 h-1.5 bg-emerald-400' : 'w-1.5 h-1.5 bg-white/15'}`}
              />
            ))}
          </div>
        </div>
      ...
      {topAction && (
        ...
        >
          <p className="text-[10px] font-black uppercase tracking-widest text-amber-300/80 mb-0.5">
            Next move · +{topAction.pts} pts
          </p>
      ...
      <div className="px-4 pt-3.5">
        <div className="flex items-baseline gap-2">
          <p className="text-3xl font-black tabular-nums text-white leading-none">
            {done}
            <span className="text-[rgba(153,197,255,0.35)]">/{jobsToday.length}</span>
          </p>
        ...
        {nextJob ? (
          <div className="mt-3 px-3 py-2 rounded-xl bg-white/[0.04] border border-white/[0.08]">
            <p className="text-[10px] font-black uppercase tracking-widest text-[rgba(153,197,255,0.4)] mb-0.5">
              Up next
            </p>
            <p className="text-xs font-bold text-white truncate">
              {nextJob.customer} · {fmt(nextJob.price)}
            </p>
            <p className="text-[10px] text-[rgba(153,197,255,0.45)] mt-0.5 truncate">
              {nextJob.time}
              {nextJob.postcode ? ` · ${nextJob.postcode}` : ''} · {nextJob.service}
            </p>
          </div>
        ) : (
          <div className="mt-3 px-3 py-2 rounded-xl bg-emerald-500/[0.08] border border-emerald-500/20">
            <p className="text-xs font-bold text-emerald-300">
              {jobsToday.length > 0 ? 'Day wrapped 🎉' : 'Nothing booked today'}
            </p>
          </div>
        )}
      </div>
        ...
        <div className="flex items-end gap-1 h-9">
          {weekJobs.map((d) => (
            ...
            >
              ...
              <span
                className={`text-[8px] font-bold ${d.isToday ? 'text-[#99c5ff]' : 'text-[rgba(153,197,255,0.3)]'}`}
              >
                {d.day[0]}
              </span>
            </div>
      ...
      headerRight={
        <div className="flex items-center gap-2.5">
          ...
          <button
            ...
            className="text-[11px] font-bold text-orange-300 hover:text-orange-200 transition-colors whitespace-nowrap"
          >
            Open marketplace →
          </button>
        </div>
    ...
    >
      ...
      {listings !== null && live.length === 0 && (
        <div className="flex-1 flex flex-col items-center text-center gap-3 px-5 py-6">
          <div
            className="text-4xl"
            style={{ filter: 'drop-shadow(0 0 16px rgba(251,146,60,0.5))' }}
          >
            🏗️
          </div>
          <div>
            <p className="text-sm font-black text-white">Win commercial contracts near you</p>
            <p className="text-xs text-[rgba(153,197,255,0.5)] mt-1 leading-relaxed">
              FM companies post real cleaning work to Cadi Connect — browse open listings, bid, and
              get paid through Cadi.
            </p>
          </div>
          <button
            onClick={() => onNavigate?.('connect')}
            ...
            style={{
              background: 'linear-gradient(180deg, #d64510 0%, #C2410C 100%)',
              boxShadow:
                '0 8px 20px -6px rgba(194,65,12,0.55), inset 0 1px 0 rgba(255,255,255,0.2)',
            }}
          >
            Explore Connect
          </button>
      ...
      {live.length > 0 && (
        <div className="flex-1 divide-y divide-[rgba(251,146,60,0.08)]">
          {live.slice(0, 3).map((l) => (
            ...
            >
              <div
                className="w-1 self-stretch rounded-full shrink-0 min-h-[30px]"
                style={{
                  background: l.status === 'bidding' ? '#fb923c' : '#4f78ff',
                  boxShadow: `0 0 6px 1px ${l.status === 'bidding' ? 'rgba(251,146,60,0.5)' : 'rgba(79,120,255,0.5)'}`,
                }}
              />
              <div className="flex-1 min-w-0">
                <p className="text-xs font-bold text-white truncate">{l.site}</p>
                <p className="text-[10px] text-[rgba(153,197,255,0.45)] mt-0.5 truncate">
                  {l.postcode} · {l.frequency} · {l.fm}
                </p>
              </div>
              ...
              <span
                className={`text-[9px] font-black px-2 py-0.5 rounded-full uppercase tracking-wide shrink-0 ${
                  l.status === 'bidding'
                    ...
                    : 'bg-[#4f78ff]/15 text-[#99c5ff] border border-[#4f78ff]/30'
                }`}
              >
                {l.status}
              </span>
...
function LogPaymentModal({ onClose, onConfirm }) {
  ...
  const inputCls =
    'w-full rounded-xl px-3 py-2.5 text-sm text-white bg-white/[0.07] border border-white/15 focus:outline-none focus:border-emerald-400/60 placeholder:text-white/25';

  return (
    <div
      className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4"
      style={{
        background: 'rgba(1,3,20,0.65)',
        backdropFilter: 'blur(8px)',
        WebkitBackdropFilter: 'blur(8px)',
      }}
    >
      <div
        className="w-full max-w-md rounded-2xl overflow-hidden border border-[rgba(52,211,153,0.25)]"
        style={{ background: DASH_CARD_BG, boxShadow: '0 40px 90px -30px rgba(0,0,0,0.8)' }}
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/10">
          ...
          <button
            onClick={onClose}
            className="text-[rgba(153,197,255,0.4)] hover:text-white text-lg leading-none"
          >
            ✕
          </button>
        </div>
        <div className="p-5 space-y-3">
          <div>
            <label className="block text-[10px] font-black tracking-widest uppercase text-[rgba(153,197,255,0.45)] mb-1.5">
              Customer
            </label>
            <input
              type="text"
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              >
                {['bank', 'cash', 'card', 'cheque'].map((m) => (
                  <option key={m} value={m} className="text-brand-navy">
          ...
          <div className="flex gap-2">
            <button
              ...
              onClick={() => {
                onConfirm({ customer, amount: parseFloat(amount), method });
                onClose();
              }}
              ...
              style={
                customer && parseFloat(amount)
                  ? {
                      background: 'linear-gradient(180deg, #10b981 0%, #047857 100%)',
                      boxShadow: '0 8px 20px -6px rgba(4,120,87,0.55)',
                    }
                  : undefined
              }
            >
            ...
            <button
              onClick={onClose}
              className="px-4 rounded-xl border border-white/15 text-[rgba(153,197,255,0.6)] text-xs font-bold hover:bg-white/5 transition-colors"
            >
              Cancel
            </button>
          </div>
...
function SetTargetModal({ onClose, onSave }) {
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-md rounded-2xl overflow-hidden border border-[rgba(251,191,36,0.3)]"
        style={{ background: DASH_CARD_BG, boxShadow: '0 40px 90px -30px rgba(0,0,0,0.8)' }}
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/10">
          <div className="flex items-center gap-2.5">
        ...
        <div className="p-5 space-y-4">
          <p className="text-xs text-[rgba(153,197,255,0.5)] leading-relaxed">
            How much do you want the business bringing in each month? Cadi paces your days, weeks
            and year against it — you can change it any time in Settings.
          </p>
          <div>
            <label className="block text-[10px] font-black tracking-widest uppercase text-[rgba(153,197,255,0.45)] mb-1.5">
              Monthly target (£)
          ...
          {error && (
            <div className="px-3 py-2.5 rounded-xl bg-red-500/10 border border-red-500/30 text-xs text-red-300">
              {error}
            </div>
          )}
          <div className="flex gap-2">
    ...
    >
      <div className="text-center px-8 py-12 max-w-sm" onClick={(e) => e.stopPropagation()}>
        <div className="relative inline-block mb-6">
          <div
            className="text-8xl"
            style={{ filter: 'drop-shadow(0 0 48px rgba(167,139,250,0.9))' }}
          >
            🏆
          </div>
          <div
            ...
            style={{
              ...
              animation: 'scoreLand 1.2s ease-out forwards',
            }}
          />
        </div>
        <p className="text-xs font-black uppercase tracking-[0.35em] text-violet-400 mb-3">
          Cadi Score · 90+
        </p>
        <h2 className="text-4xl font-black text-white mb-3 leading-tight">Elite.</h2>
        <p className="text-sm text-white/50 leading-relaxed mb-8">
          You've hit the highest tier on Cadi. Your business is firing on all cylinders — keep it
          there.
        </p>
        <button
          onClick={onClose}
          ...
          style={{
            background: 'linear-gradient(135deg, #7c3aed, #4f78ff)',
...
export default function DashboardTab({
  ...
  const onNavigate = (tab) => {
    ...
    if (!isPro && PRO_TABS.some((p) => path.startsWith(p))) {
      setDashUpgradeReason(
        `Upgrade to Cadi Pro to access ${tab.charAt(0).toUpperCase() + tab.slice(1)}.`
      );
      return;
    }
    if (onNavigateProp) {
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps -- fresh object each render is intentional; score/actions memos re-run cheaply
  const accounts = isLive
    ? { ...EMPTY_ACCOUNTS, ...(resolvedAccountsData ?? {}) }
    : { ...DEMO_ACCOUNTS };
  const weekJobs = resolvedSchedulerData?.weekJobs
    ? resolvedSchedulerData.weekJobs
    : isLive
      ? liveWeek
      : DEMO_WEEK;
  // eslint-disable-next-line react-hooks/exhaustive-deps -- fresh array each render is intentional; score/actions memos re-run cheaply
  ...
  const [_dashVisitCount] = useState(() => {
    try {
  ...
  const [, setShowQuickWins] = useState(true);
  ...
  const [communityOptIn, setCommunityOptIn] = useState(() => {
    ...
    try {
      return localStorage.getItem('cadi_community_opt_in') === '1';
    } catch {
      return false;
    }
  });
  ...
  useEffect(() => {
    if (profile?.community_opt_in === true) {
      ...
      try {
        localStorage.setItem('cadi_community_opt_in', '1');
      } catch {}
    } else if (profile?.community_opt_in === false) {
      setCommunityOptIn(false);
      try {
        localStorage.removeItem('cadi_community_opt_in');
      } catch {}
    }
  ...
  const score = useMemo(
    () => calcHealthScore({ accounts, weekJobs, invoices, jobsToday }),
    [accounts, weekJobs, invoices, jobsToday]
  );
  const actions = useMemo(
    () => buildActions({ accounts, invoices, jobsToday, score }),
    [accounts, invoices, jobsToday, score]
  );
  ...
  useEffect(() => {
    try {
      localStorage.setItem('cadi_prev_health_score', String(score.total));
    } catch {}
  }, [score.total]);
  ...
  useEffect(() => {
    ...
    listLeaderboard(50)
      .then((rows) => {
        if (!cancelled) setLiveBoard(rows);
      })
      .catch((err) => {
  ...
  useEffect(() => {
    ...
    if (!lastLbUpsertRef.current) {
      try {
        lastLbUpsertRef.current = parseInt(localStorage.getItem('cadi_lb_upsert') || '0', 10);
      } catch {}
    }
    ...
    try {
      localStorage.setItem('cadi_lb_upsert', String(now));
    } catch {}
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- keyed on user?.id; the full user object is not needed
  }, [
    user?.id,
    communityOptIn,
    score.total,
    profile?.business_name,
    profile?.cleaner_type,
    profile?.postcode,
    displayName,
  ]);

  const isPreviewBoard = useMemo(
    () => (liveBoard || []).filter((r) => r.owner_id !== user?.id).length < 2,
    [liveBoard, user?.id]
  );
  ...
  const allLeaderboard = useMemo(() => {
    const userEntry = {
      id: 'me',
      name: profile?.business_name || displayName,
      sector: profile?.cleaner_type || 'residential',
      score: score.total,
      delta: +3,
      region: 'You',
      ...
      if (score.tier === 'Elite') {
        ...
        const msgs = {
          Building: {
            ...
            body: `Cadi Score reached ${score.total} — keep pushing!`,
          },
          Solid: {
            emoji: '💪',
            title: 'Solid! Business is on track.',
            body: `Cadi Score hit ${score.total} — your business is getting stronger.`,
          },
          Firing: {
            emoji: '⭐',
            title: "Firing! You're flying!",
            body: `Cadi Score hit ${score.total} — you're in the top 25% of Cadi businesses.`,
          },
        };
  ...
  async function dismissReadyBanner() {
    ...
    try {
      ...
      await supabase
        .from('business_settings')
        .update({ setup_data: { ...existing, welcome_banner_dismissed: true } })
        .eq('owner_id', user.id);
    } catch {
      /* non-critical */
    }
  ...
  useEffect(() => {
    ...
    if (profile?.marketplace_interest !== undefined && profile?.marketplace_interest !== null)
      return;
  ...
  async function handleMarketplaceInterest(interested) {
    ...
    try {
      localStorage.setItem('cadi_mp_banner_dismissed', '1');
    } catch {}
    if (!user || user.id === 'demo-user') return;
    try {
      await supabase
  ...
  const handleSaveTarget = async (monthlyTarget) => {
    ...
    const { error } = await supabase
      .from('business_settings')
      .upsert(
        { owner_id: user.id, annual_target: Math.round(monthlyTarget * 12) },
        { onConflict: 'owner_id' }
      );
  ...
  return (
    <>
      ...
      {showPayModal && (
        <LogPaymentModal onConfirm={handlePaymentLogged} onClose={() => setShowPayModal(false)} />
      )}
      {showTargetModal && (
        <SetTargetModal onSave={handleSaveTarget} onClose={() => setShowTargetModal(false)} />
      )}
      {profile && !profile?.onboarding_complete && (
        <Onboarding isModal onComplete={() => updateProfile({ onboarding_complete: true })} />
      )}
      {showEliteModal && <EliteCelebration onClose={() => setShowEliteModal(false)} />}
      {showShareCard && (
        <ShareCardModal
      ...
      >
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              ...
              <h2
                ...
                style={{ textShadow: '0 0 40px rgba(79,120,255,0.3)' }}
              >
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              {streak > 0 && (
                ...
                >
                  ...
                  <span
                    className={`text-xs font-bold tabular-nums ${streak >= 7 ? 'text-orange-300' : 'text-white/70'}`}
                  >
                    {streak}d
                  </span>
                </div>
              ...
              <button
                ...
                style={{
                  background: 'linear-gradient(135deg, #4f78ff, #1a3de0)',
                  boxShadow: '0 4px 14px -4px rgba(79,120,255,0.6)',
                }}
              >
                ✨ Share
              </button>
              ...
              <div className="flex items-center gap-1.5 px-2.5 py-1.5 bg-white/10 rounded-sm">
                <span
                  className={`w-2 h-2 rounded-full ${isLive ? 'bg-emerald-400 animate-pulse' : 'bg-gray-400'}`}
                />
        ...
        {demoMode && (
          <div className="bg-gradient-to-r from-brand-navy to-brand-blue/90 border-b border-brand-blue/30 px-4 py-2.5 flex items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold px-2 py-0.5 bg-white/20 text-white rounded-sm">
                ✨ GUIDED TOUR
              </span>
              <p className="text-xs font-semibold text-white/80">
                Hover over any section to find out what it does — then follow your 30 Day Plan at
        ...
        {showMarketplaceBanner && (
          <div
            ...
            style={{
              ...
              background: 'linear-gradient(135deg, #1a0800 0%, #05124a 100%)',
            }}
          >
            <div className="flex items-start gap-3">
              <div
                className="w-8 h-8 rounded-xl flex items-center justify-center shrink-0 text-lg"
                style={{ backgroundColor: '#C2410C22' }}
              >
                🏗️
              </div>
              <div className="flex-1">
                <p className="text-sm font-bold text-white">
                  We're building a marketplace for cleaners like you
                </p>
                <p className="text-xs text-[rgba(153,197,255,0.6)] mt-0.5 leading-relaxed">
                  Cadi will connect you to FM aggregators across the UK who need reliable
                  subcontractors. Would you be interested?
                </p>
                <div className="flex gap-2 mt-3">
                  <button
                    onClick={() => handleMarketplaceInterest(true)}
                  ...
                  <button
                    onClick={() => handleMarketplaceInterest(false)}
              ...
              <button
                onClick={() => handleMarketplaceInterest(false)}
                className="text-[rgba(153,197,255,0.3)] hover:text-white text-lg leading-none shrink-0"
              >
                ✕
              </button>
            </div>
        ...
        {readyBanner && !readyBannerDismissed && (
          <div className="mx-4 mt-4 rounded-2xl border border-[rgba(31,72,255,0.3)] bg-gradient-to-br from-[#05124a] to-[#0a1860] p-4 sm:p-5">
            <div className="mb-3 flex items-start justify-between gap-3">
              <div>
                <div className="flex items-center gap-2">
                  <div className="flex h-7 w-7 items-center justify-center rounded-full bg-[#1f48ff] text-xs font-extrabold text-white">
                    C
                  </div>
                  <span className="text-[11px] font-bold uppercase tracking-widest text-[rgba(153,197,255,0.6)]">
                    Cadi · Ready for you
                  </span>
                </div>
                <p className="mt-2 text-[15px] font-bold text-white">
                  Your account is pre-built, {profile?.first_name || 'there'} — here's what's
                  already set up.
                </p>
              </div>
              <button
                onClick={dismissReadyBanner}
                className="mt-1 shrink-0 text-[rgba(153,197,255,0.4)] transition hover:text-white text-lg leading-none"
              >
                ✕
              </button>
            </div>
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                  ...
                  <div>
                    ...
                    <div className="text-[11px] leading-relaxed text-[rgba(153,197,255,0.55)]">
                      {readyBanner.sectors
                        ...
                        .join(', ')}{' '}
                      edition — tools, filters and dashboard are set to your work
                    </div>
                  </div>
                </div>
              )}
              {readyBanner.services?.length > 0 && (
                <div className="flex items-start gap-2.5 rounded-xl border border-[rgba(153,197,255,0.1)] bg-[rgba(153,197,255,0.05)] px-3 py-2.5">
                  ...
                  <div>
                    ...
                    <div className="text-[11px] leading-relaxed text-[rgba(153,197,255,0.55)]">
                      {readyBanner.services.length} services ready as quick-picks on every job card,
                      quote and invoice
                    </div>
                  </div>
                </div>
              ...
              {readyBanner.hourlyRate && (
                <div className="flex items-start gap-2.5 rounded-xl border border-[rgba(153,197,255,0.1)] bg-[rgba(153,197,255,0.05)] px-3 py-2.5">
                  ...
                  <div>
                    ...
                    <div className="text-[11px] leading-relaxed text-[rgba(153,197,255,0.55)]">
                      £{readyBanner.hourlyRate}/hr flows into your pricing calculator, quote builder
                      and job cards
                    </div>
                  </div>
                </div>
              ...
              {readyBanner.targetRevenue && (
                <div className="flex items-start gap-2.5 rounded-xl border border-[rgba(153,197,255,0.1)] bg-[rgba(153,197,255,0.05)] px-3 py-2.5">
                  ...
                  <div>
                    ...
                    <div className="text-[11px] leading-relaxed text-[rgba(153,197,255,0.55)]">
                      £{Number(readyBanner.targetRevenue).toLocaleString()}/mo target is live — Cadi
                      tracks every job against it
                    </div>
                  </div>
                </div>
              )}
              {readyBanner.structure && (
                <div className="flex items-start gap-2.5 rounded-xl border border-[rgba(153,197,255,0.1)] bg-[rgba(153,197,255,0.05)] px-3 py-2.5">
                  <span className="mt-0.5 text-base">📋</span>
                  <div>
                    <div className="text-xs font-bold text-[#99c5ff]">
                      Tax & accounts configured
                    </div>
                    <div className="text-[11px] leading-relaxed text-[rgba(153,197,255,0.55)]">
                      {readyBanner.structure === 'sole_trader'
                        ? 'Self Assessment SA103'
                        : readyBanner.structure === 'limited'
                          ? 'Corporation Tax & Companies House'
                          : 'Accounts'}{' '}
                      tools and defaults set
                    </div>
                  </div>
                </div>
              )}
            ...
            <p className="mt-3 text-[11px] text-[rgba(153,197,255,0.4)]">
              All of this updates automatically as you work. You can change any setting in the
              Settings tab.
            </p>
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto">
          <div
            ...
            style={{ display: isLive && dashboardLoading ? 'none' : undefined }}
          >
            ...
            <div className="flex items-center justify-between pt-4 mt-2 border-t border-white/[0.06]">
              ...
              <button
                ...
                className="text-xs text-[#4f78ff] font-semibold hover:text-[#99c5ff] transition-colors"
              >
                Open accounts →
              </button>
            </div>
            ...
            steps={[
              {
                ...
                body: "Your money at a glance — what came in today, this week or this month, what went out, what you kept, and whether you're on pace for your target. Every tile clicks through to the detail.",
              },
              {
                selector: '#tour-pending',
                emoji: '📨',
                title: 'Pending Payments',
                body: 'Who owes you what, overdue first. Tap any invoice to jump straight in and chase it.',
              },
              {
                selector: '#tour-leaderboard',
                emoji: '🏆',
                title: 'Community Leaderboard',
                body: 'See how your business ranks against other Cadi users. Your Cadi Score determines your position — climb by running a great business.',
              },
              {
                selector: '#tour-score',
                emoji: '📊',
                title: 'Cadi Score',
                body: 'One number for the health of your business, plus the single next move that earns you the most points.',
              },
              {
                selector: '#tour-schedule',
                emoji: '📆',
                title: 'Schedule',
                body: "Today's progress, what's up next, and the shape of your week in miniature. Tap to open the full scheduler.",
              },
              {
                selector: '#tour-marketplace',
                emoji: '🏗️',
                title: 'Marketplace',
                body: 'Live commercial work posted by FM companies on Cadi Connect. Browse listings, bid, and win contracts near you.',
              },
            ]}
            onComplete={async () => {
              setShowTour(false);
