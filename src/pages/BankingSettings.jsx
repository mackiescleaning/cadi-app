    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="w-full max-w-md bg-[#0d1530] border border-white/10 rounded-2xl p-6 shadow-2xl">
        ...
        <p className="text-white/60 text-sm mb-4 leading-relaxed">
          Open Banking is a UK regulation that lets you securely share your bank data with apps you
          trust — without giving them your password.
        </p>
        ...
        <ol className="space-y-2.5 mb-5">
          {[
            ...
            <li key={i} className="flex items-start gap-3">
              <span className="shrink-0 w-5 h-5 rounded-full bg-[#4f78ff]/20 border border-[#4f78ff]/40 flex items-center justify-center text-[10px] font-bold text-[#4f78ff]">
                {i + 1}
              </span>
        ...
        <p className="text-white/50 text-sm leading-relaxed mb-5">
          Cadi only ever has <strong className="text-white/70">read-only access</strong>. Your bank
          verifies that. We literally can't move money or change anything — we can only read what's
          already happened.
        </p>
        <p className="text-white/40 text-xs">
          If you ever want to disconnect, head to Settings → Banking. Takes one click.
        </p>
        <button
          onClick={onClose}
    ...
    <div>
      <div className="mb-4">
        ...
        <h2 className="text-xl font-black text-white leading-tight mb-1">
          Which bank do you want to connect?
        </h2>
        <p className="text-white/50 text-sm">
          {HIDE_SANDBOX_BANKS
            ? 'UK banks supported by Yapily Open Banking.'
            : 'UK banks + sandbox test banks.'}
        </p>
      </div>
                ...
                >
                  {inst.logoUrl ? (
                    <img
                      src={inst.logoUrl}
                      alt=""
                      className="w-8 h-8 rounded-md shrink-0 bg-white/10 object-contain"
                    />
                  ) : (
      ...
      <div className={`${BG} flex items-center justify-center p-4`}>
        <div className={`${CARD} bg-white/4 border border-white/10 rounded-2xl p-8 text-center`}>
          ...
          <p className="text-sm text-white/55 leading-relaxed mb-6">
            On Pro, Cadi connects securely to your bank (read-only, FCA-regulated Open Banking) and
            logs every transaction automatically — income matched to invoices, expenses categorised,
            your books always up to date. No typing, ever.
          </p>
          <button
            ...
            <>
              ...
              <div className="space-y-3 mb-8">
                {[
                  ...
                  <div
                    key={title}
                    className="flex items-start gap-3 p-4 rounded-xl bg-white/4 border border-white/8"
                  >
                ...
                <div className="fs-exclude mb-4 px-4 py-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
                  ...
                  {alreadyConnected.needsReauth && (
                    <p className="text-xs text-amber-300 mt-2">
                      Your bank consent has expired. Click below to reconnect.
                    </p>
                  )}
                  {alreadyConnected.reconsentDaysLeft !== null &&
                    alreadyConnected.reconsentDaysLeft <= 14 &&
                    !alreadyConnected.needsReauth && (
                      <p className="text-xs text-amber-300 mt-2">
                        Bank consent expires in {alreadyConnected.reconsentDaysLeft} days —
                        reconnect to avoid an interruption.
                      </p>
                    )}
                </div>
              )}
              ...
              <div className="space-y-2">
                ...
                <button
                  onClick={() => navigate('/banking/upload')}
                  className="w-full py-2.5 text-sm text-white/40 hover:text-white/60 font-semibold transition-colors"
                >
                  Bank not supported yet? Upload a statement instead →
                </button>
              </div>
            </>
          )}
