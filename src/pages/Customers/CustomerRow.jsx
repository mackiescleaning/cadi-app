...
export default function CustomerRow({
  ...
  const statusColor =
    customer.status === 'active'
      ? {
          ...
          bg: 'from-emerald-400 to-emerald-600 text-white',
          dot: 'bg-emerald-500',
      ...
      : customer.status === 'lapsed'
        ? {
            ...
            bg: 'from-red-400 to-red-600 text-white',
            dot: 'bg-red-500',
            glow: 'shadow-red-500/35',
          }
        : {
            ring: 'ring-amber-500/60',
            bg: 'from-amber-400 to-amber-600 text-white',
            dot: 'bg-amber-500',
  ...
  if (density === 'compact') {
    return (
      <div
        ...
        className={`w-full text-left px-3 py-1.5 transition-all group relative cursor-pointer ${selected ? 'opacity-100' : 'opacity-85 hover:opacity-100'}`}
      >
        <GlassSurface tone={tone} depth="lift" blur={blur} glow={selected} className="px-2.5 py-2">
          <div className="flex items-center gap-2.5">
            <div
              className={`w-7 h-7 rounded-lg flex items-center justify-center text-[10px] font-black shrink-0 ring-1 ${statusColor.ring} bg-gradient-to-br ${statusColor.bg} shadow-md ${statusColor.glow}`}
            >
              {initials}
            </div>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5">
                <p className="text-xs font-bold text-[#010a4f] truncate leading-tight">
                  {customer.name}
                </p>
                {urgent && (
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              <span
                className={`w-2 h-2 rounded-full shrink-0 ${isPending ? 'bg-amber-500 animate-pulse' : statusColor.dot}`}
              />
            </div>
          </div>
        </GlassSurface>
          ...
          >
            {confirmArchive ? (
              ...
              <button
                ...
                className="w-7 h-7 rounded-full bg-[rgba(255,80,80,0.15)] hover:bg-[rgba(255,80,80,0.3)] border border-red-500/20 flex items-center justify-center text-red-400/70 hover:text-red-400 transition-all text-xs font-bold"
                title="Remove"
              >
                ×
              </button>
  ...
  if (density === 'medium') {
    return (
      <div
        ...
        onMouseEnter={() => setHover(true)}
        onMouseLeave={() => setHover(false)}
        className={`w-full text-left px-3 py-1.5 transition-all group relative cursor-pointer ${selected ? 'opacity-100' : 'opacity-90 hover:opacity-100'}`}
      >
        <GlassSurface tone={tone} depth="lift" blur={blur} glow={selected}>
          ...
          <div className="flex items-center gap-2.5 p-2.5">
            <div
              className={`w-9 h-9 rounded-xl flex items-center justify-center text-sm font-black shrink-0 ring-2 ${statusColor.ring} bg-gradient-to-br ${statusColor.bg} shadow-md ${statusColor.glow}`}
            >
              {initials}
            </div>
              ...
              <div className="flex items-center gap-1.5">
                ...
                {customer.frequency && (
                  <>
                    ...
                    <span className="text-[11px] text-[#010a4f]/55 capitalize">
                      {customer.frequency}
                    </span>
                  </>
                )}
                {urgent && topSuggestion && (
                  <>
                    ...
                    <span className="text-[11px] text-amber-600 font-semibold truncate">
                      💡 {topSuggestion.title}
                      {topSuggestion.value > 0 && (
                        <span className="ml-1 text-amber-700/80 tabular-nums">
                          +£{topSuggestion.value}
                        </span>
                      )}
                    </span>
            ...
            <div className="text-right shrink-0 flex flex-col items-end gap-1">
              <p className="text-sm font-black text-emerald-600 tabular-nums">
                ...
                {customer.pricePerVisit != null && (
                  <span className="text-[10px] text-emerald-700/70 font-bold">/visit</span>
                )}
              </p>
              {isPending ? (
                <div className="flex items-center gap-1.5">
        ...
        {onArchive && (
          ...
          >
            {confirmArchive ? (
              ...
              <button
                onClick={() => setConfirmArchive(true)}
                className="w-8 h-8 rounded-full bg-[rgba(255,80,80,0.15)] hover:bg-[rgba(255,80,80,0.3)] border border-red-500/20 hover:border-red-500/50 flex items-center justify-center text-red-400/70 hover:text-red-400 transition-all text-sm font-bold"
  ...
  return (
    <div
      ...
      onMouseEnter={() => setHover(true)}
      onMouseLeave={() => setHover(false)}
      className={`w-full text-left p-3 transition-all group cursor-pointer ${selected ? 'opacity-100' : 'opacity-90 hover:opacity-100'}`}
    >
      <GlassSurface tone={tone} depth="lift" blur={blur} glow={selected}>
        {urgent && <div className="absolute inset-0 bg-amber-500/5 pointer-events-none" />}
        <div className="p-3">
          <div className="flex items-start gap-3">
            <div
              className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm font-black shrink-0 ring-2 ${statusColor.ring} bg-gradient-to-br ${statusColor.bg} shadow-md ${statusColor.glow}`}
            >
              {initials}
            </div>
            ...
            <div className="text-right shrink-0">
              <p className="text-sm font-black text-emerald-600 tabular-nums mb-1">
