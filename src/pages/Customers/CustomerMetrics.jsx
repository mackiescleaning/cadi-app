  ...
  return (
    <div className="flex-1 min-w-[110px]">
      <GlassSurface tone="light" depth="flat" className="px-3 py-2">
        <p className="text-[10px] font-bold tracking-wide uppercase text-[#010a4f]/55 mb-0.5">{label}</p>
...
export default function CustomerMetrics({ customer, jobs }) {
  const completed = useMemo(
    () => (jobs || []).filter(j => j.status === 'complete' || (j.date && j.date < new Date().toISOString().slice(0, 10))),
    [jobs]
  );
  ...
  const data = useMemo(() => {
    ...
    for (const j of completed) {
      ...
      if (b) { b.spend += (j.price || 0); b.count += 1; }
    }
    ...
    const now = Date.now();
    const last12 = completed.filter(j => j.date && (now - new Date(j.date).getTime()) <= 365 * DAY_MS);
    ...
    const delta = prior12Spend > 0 ? Math.round(((last12Spend - prior12Spend) / prior12Spend) * 100) : null;
    ...
    const daysSinceLast = customer.lastJobDate ? Math.floor((now - new Date(customer.lastJobDate).getTime()) / DAY_MS) : null;
    ...
    return {
      series, maxSpend, maxCount,
      last12Spend, prior12Spend, delta,
      last12Count: last12.length, prior12Count: prior12.length,
      mixArr, mixTotal,
      lastJob, daysSinceLast, predicted, gapHealth,
    };
  }, [completed, customer]);
  ...
  const W = 320, H = 110, PAD_L = 4, PAD_R = 4, PAD_T = 8, PAD_B = 18;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        <MilestoneCard icon="🤝" label="Customer since" value={tenureLabel} accent="text-[#99c5ff]" />
        <MilestoneCard
          icon="🎂"
          label="Next birthday"
        ...
        <MilestoneCard
          ...
          label="12mo vs prior"
          value={data.delta == null ? gbp(data.last12Spend) : `${data.delta > 0 ? '+' : ''}${data.delta}%`}
          accent={data.delta == null ? "text-[#010a4f]" : data.delta >= 0 ? "text-emerald-600" : "text-red-600"}
        />
        <MilestoneCard
          icon={data.gapHealth?.accent === 'text-emerald-600' ? "✓" : "⏱"}
          label="Visit cadence"
          value={data.gapHealth?.label || (data.daysSinceLast != null ? `${data.daysSinceLast}d ago` : '—')}
          accent={data.gapHealth?.accent || "text-[#010a4f]"}
        />
      </div>

      {/* Spend timeline */}
      <Section title="Spend — last 12 months" sub={`${gbp(data.last12Spend)} · ${data.last12Count} job${data.last12Count === 1 ? '' : 's'}`}>
        {data.last12Count === 0 ? (
          ...
          <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-[110px]" preserveAspectRatio="none">
            ...
            {data.series.map((s, i) => {
              ...
              return (
                <g key={s.key}>
                  {h > 0 && <rect x={x} y={y} width={barW} height={h} fill="url(#spendBar)" rx="2" />}
                  {h === 0 && <rect x={x} y={H - PAD_B - 2} width={barW} height="2" fill="rgba(153,197,255,0.15)" rx="1" />}
                  <text x={x + barW / 2} y={H - 4} textAnchor="middle" fontSize="9" fill="rgba(153,197,255,0.4)" fontFamily="system-ui">
                    {s.label[0]}
                  </text>
                </g>
              );
      ...
      {/* Service mix */}
      <Section title="Service mix" sub={`${data.mixArr.length} type${data.mixArr.length === 1 ? '' : 's'}`}>
                ...
                <div key={m.type}>
                  ...
                  <div className="h-1.5 bg-white/[0.06] rounded-full overflow-hidden">
                    <div
                      ...
                      style={{ width: `${m.pct * 100}%`, backgroundColor: colors[i % colors.length] }}
                    />
                  </div>
                </div>
              );
      ...
      <div className="grid grid-cols-2 gap-3">
        <Section title="Last visit" sub={data.lastJob?.service || ''}>
          <p className={`text-2xl font-black tabular-nums ${data.daysSinceLast != null && data.daysSinceLast > 60 ? 'text-amber-600' : 'text-[#010a4f]'}`}>
            {data.daysSinceLast == null ? '—' : `${data.daysSinceLast}d`}
          </p>
          <p className="text-[10px] text-[#010a4f]/55 mt-0.5">
            {customer.lastJobDate ? new Date(customer.lastJobDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }) : 'no record'}
          </p>
        </Section>
        <Section title="Next due" sub={data.predicted?.source === 'scheduled' ? 'booked' : data.predicted ? 'predicted' : ''}>
          {data.predicted ? (
            <>
              ...
              <p className="text-[10px] text-[#010a4f]/55 mt-0.5">
                {new Date(data.predicted.iso).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}
              </p>
            </>
          ) : (
      ...
      <Section title="Lifetime" sub={customer.source ? `via ${customer.source}` : 'no source recorded'}>
        <div className="grid grid-cols-3 gap-3">
          <div>
            <p className="text-[10px] font-bold tracking-wide uppercase text-[#010a4f]/55 mb-0.5">Value</p>
            <p className="text-lg font-black text-emerald-600 tabular-nums">{gbp(customer.lifetimeValue || 0)}</p>
          </div>
          <div>
            <p className="text-[10px] font-bold tracking-wide uppercase text-[#010a4f]/55 mb-0.5">Jobs</p>
            <p className="text-lg font-black text-[#010a4f] tabular-nums">{completed.length}</p>
          </div>
          <div>
            <p className="text-[10px] font-bold tracking-wide uppercase text-[#010a4f]/55 mb-0.5">Avg / job</p>
