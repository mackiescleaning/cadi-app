...
export default function CustomerDetail({ customer, onMessage, onClose, onBookJob, onUpdateCustomer, onDeleteCustomer, onEraseCustomer, onExportCustomer, ownerId }) {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .limit(100)
      .then(({ data }) => { if (!cancelled) setAllInvoices(data ?? []); })
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-[#010a4f]">
            ...
            {[
              {
                ...
                sub: customer.outstandingBalance > 0
                  ? `£${customer.outstandingBalance.toLocaleString()} outstanding`
                  : (customer.paidLifetimeValue > 0 ? 'all paid ✓' : null),
              ...
              { label: hasLastJob && daysSince >= 0 ? "Last job" : "Next job", value: lastJobLabel, accent: (hasLastJob && daysSince > 60) ? "text-amber-600" : "text-[#010a4f]" },
              ...
              <GlassSurface key={label} tone="light" depth="lift" className="px-2 py-2 text-center">
                ...
                {sub && <p className={`text-[10px] font-semibold mt-0.5 ${subAccent || 'text-[#010a4f]/55'}`}>{sub}</p>}
              </GlassSurface>
            ))}
          </div>
      ...
      <div className="flex-1 overflow-y-auto bg-[#010a4f] p-4 space-y-4">
        ...
        {activeTab === "overview" && (
          <>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <button
                onClick={() => { setNoteDraft(""); setNoteOpen(o => !o); }}
                className="py-2.5 bg-[rgba(153,197,255,0.06)] border border-[rgba(153,197,255,0.15)] text-[rgba(153,197,255,0.7)] hover:border-[rgba(153,197,255,0.35)] hover:text-white text-xs font-bold uppercase tracking-wide transition-all rounded-xl"
              >
        ...
        {activeTab === "metrics" && (
          <CustomerMetrics customer={customer} jobs={allJobs} />
        )}
        ...
        <GlassCard>
          <div className="px-4 py-3 border-b border-[rgba(153,197,255,0.06)] flex items-center gap-2">
            ...
            <span className="text-[11px] font-bold tracking-widest uppercase text-[rgba(153,197,255,0.7)]">Billing</span>
            <span className="ml-auto text-[10px] text-[rgba(153,197,255,0.45)]">When a job is done…</span>
          </div>
          <div className="px-4 py-3 space-y-2">
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-[12px] font-black ${active ? 'text-white' : 'text-[rgba(153,197,255,0.85)]'}`}>
                      {opt.label}
                    </span>
                  </div>
            ...
            {(customer.billing_mode === 'stripe') && (
              <p className="text-[10.5px] text-[rgba(153,197,255,0.55)] mt-1 pl-1">
                Invoices will be sent with a Stripe payment link when this customer has an email on file.
              </p>
            )}
          </div>
        </GlassCard>
