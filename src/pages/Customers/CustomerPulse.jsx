    ...
    return (
      <button onClick={onClick} className="text-left w-full group">
        <GlassSurface tone="navy" depth="lift" className="px-3 py-2.5">
          <p className="text-[10px] text-[rgba(153,197,255,0.5)] font-bold tracking-wide uppercase mb-0.5">
            {label}
          </p>
          <p className={`text-xl font-black tabular-nums ${accent}`}>{value}</p>
          {sub && (
            <p className="text-[10px] text-[rgba(153,197,255,0.45)] mt-0.5 truncate">{sub}</p>
          )}
        </GlassSurface>
      </button>
    );
    ...
    <GlassSurface tone="navy" depth="flat">
      <div className="flex items-center justify-between px-3 py-2 border-b border-[rgba(153,197,255,0.08)]">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-bold text-[rgba(153,197,255,0.5)] tabular-nums">
            {count}
          </span>
        </div>
        {onSeeAll && count > 0 && (
      ...
      <div className="divide-y divide-[rgba(153,197,255,0.06)]">
        {count === 0 ? (
          <p className="px-3 py-3 text-[11px] text-[rgba(153,197,255,0.4)] leading-snug">
            {emptyText}
          </p>
        ) : (
          children
...
function MiniRow({ name, sub, right, onClick }) {
  return (
    ...
    >
      ...
      {right && (
        <span className="text-[10px] font-bold text-[#99c5ff] tabular-nums shrink-0">{right}</span>
      )}
    </button>
  );
}
...
export default function CustomerPulse({ customers, onSelectCustomer, onMessage, onFilter }) {
  ...
  const data = useMemo(() => {
    ...
    const avgTenureMonths = tenures.length
      ? Math.round(tenures.reduce((s, n) => s + n, 0) / tenures.length)
      : 0;
    ...
    const radar = active
      .map((c) => ({
        c,
        sug: generateSuggestions(c)?.find((s) => s.priority === 'urgent' || s.priority === 'high'),
      }))
      .filter((x) => x.sug)
      .sort((a, b) => {
        const pri = (p) => (p === 'urgent' ? 2 : 1);
        return (
          pri(b.sug.priority) - pri(a.sug.priority) ||
          (b.c.lifetimeValue || 0) - (a.c.lifetimeValue || 0)
        );
      })
      .slice(0, 4);
    ...
    const milestones = active
      .map((c) => {
        ...
        if (since && new Date(since).getMonth() === month) {
          ...
          if (yrs >= 1)
            events.push({ type: 'anniversary', label: `🎉 ${yrs}yr anniversary`, date: since });
        }
        ...
        <>
          ...
          <div className="grid grid-cols-3 lg:grid-cols-7 gap-2 mb-3">
            ...
            <Tile
              ...
              accent={
                data.retentionPct >= 80
                  ? 'text-emerald-300'
                  : data.retentionPct >= 60
                    ? 'text-amber-300'
                    : 'text-red-300'
              }
            />
            <Tile
              ...
              value={
                data.avgTenureMonths >= 12
                  ? `${(data.avgTenureMonths / 12).toFixed(1)}y`
                  : `${data.avgTenureMonths}m`
              }
              sub="as customer"
            />
            <Tile
                ...
                <MiniRow
                  ...
                  right={
                    <span
                      onClick={(e) => {
                        e.stopPropagation();
                        onMessage?.(c, { type: 'winback', title: 'Win back' });
                      }}
                      className="px-1.5 py-0.5 rounded bg-amber-400/15 text-amber-300 border border-amber-400/25 cursor-pointer hover:bg-amber-400/25"
                    >
                      Message
                    </span>
                  }
                ...
                <MiniRow
                  ...
                  right={
                    <span
                      ...
                      className="px-1.5 py-0.5 rounded bg-[#1f48ff]/20 text-[#99c5ff] border border-[#1f48ff]/40 cursor-pointer hover:bg-[#1f48ff]/30"
                    >
                      Pitch
                    </span>
                  }
                ...
                <MiniRow
                  ...
                  right={
                    <span
                      onClick={(e) => {
                        e.stopPropagation();
                        onMessage?.(c, { type: events[0].type, title: events[0].label });
                      }}
                      className="px-1.5 py-0.5 rounded bg-pink-400/15 text-pink-300 border border-pink-400/25 cursor-pointer hover:bg-pink-400/25"
                    >
                      Wish
                    </span>
                  }
