...
const inputCls =
  'w-full rounded-[10px] border border-[rgba(153,197,255,0.15)] bg-[#091660] px-[13px] py-[11px] text-sm text-white outline-none transition focus:border-[#99c5ff] focus:ring-4 focus:ring-[rgba(153,197,255,0.1)] placeholder:text-[rgba(153,197,255,0.3)]';
const labelCls =
  'mb-[5px] block text-[11px] font-semibold uppercase tracking-[0.4px] text-[rgba(153,197,255,0.55)]';
...
function Chip({ selected, onClick, children, color }) {
  ...
  const off =
    'border-[rgba(153,197,255,0.12)] bg-[#091660] text-[rgba(153,197,255,0.6)] hover:border-[rgba(153,197,255,0.3)] hover:text-white';
  return (
...
export default function Onboarding({ isModal = false, onComplete = null }) {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .single()
      .then(({ data }) => {
        if (data) patch({ businessId: data.id });
      });
  ...
  useEffect(() => {
    ...
    setForm((prev) => ({
      ...
      vatRegistered:
        typeof profile.vat_registered === 'boolean' ? profile.vat_registered : prev.vatRegistered,
  ...
  function advance(userLabel, updatedForm) {
    ...
    setTimeout(() => {
      ...
      setMessages((prev) => [
        ...prev,
        { id: `cadi-${next}`, role: 'cadi', text: cadiText(TURNS[next], liveForm) },
      ]);
      setTurnIndex(next);
      scrollDown();
    ...
    try {
      ...
      if (id === 'vatRegistered') {
        await supabase
          .from('business_settings')
          .upsert(
            { owner_id: user.id, vat_registered: form.vatRegistered },
            { onConflict: 'owner_id' }
          );
      ...
      if (id === 'services') {
        await mergeSetupData({
          ...
          custom_service: form.customService.trim(),
        });
      ...
      if (id === 'goals') {
        ...
        if (annual)
          await supabase
            .from('business_settings')
            .upsert({ owner_id: user.id, annual_target: annual }, { onConflict: 'owner_id' });
        await mergeSetupData({
  ...
  async function finishOnboarding() {
    ...
    if (user?.id !== 'demo-user') {
      supabase
        .from('profiles')
        .upsert(
          { id: user.id, onboarding_complete: true, onboarding_step: 13 },
          { onConflict: 'id' }
        )
        .then(() => {})
        .catch(() => {});
  ...
  function renderInput() {
    ...
    if (type === 'text' || type === 'email') {
      const validate = () => {
        ...
        if (type === 'email' && !textValue.includes('@')) {
          ...
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter' && validate()) {
                patch({ [field]: textValue.trim() });
                handleAnswer(id, textValue.trim(), textValue.trim(), {
                  ...form,
            ...
            <>
              <div className="rounded-xl border border-[rgba(153,197,255,0.15)] bg-[#091660] divide-y divide-[rgba(153,197,255,0.08)]">
                <div className="flex items-center justify-between px-4 py-3">
                  <span className="text-xs text-[rgba(153,197,255,0.5)] uppercase tracking-wide">
                    Your name
                  </span>
                  <span className="text-sm font-semibold text-white">
                    {`${form.firstName} ${form.lastName}`.trim() || form.firstName}
                  </span>
                </div>
                <div className="flex items-center justify-between px-4 py-3">
                  <span className="text-xs text-[rgba(153,197,255,0.5)] uppercase tracking-wide">
              ...
              <div className="rounded-xl border border-[rgba(153,197,255,0.2)] bg-[#091660] px-4 py-3 space-y-1">
                ...
                <p className="text-xs text-[rgba(153,197,255,0.4)]">
                  This is your login email. Tap "Edit details" below if it needs changing.
                </p>
              </div>
            </>
          ) : (
            <div className="space-y-2">
              ...
              <div>
                ...
                <input
                  className={inputCls}
                  type="email"
                  value={form.bizEmail || user?.email || ''}
                  onChange={(e) => patch({ bizEmail: e.target.value })}
                  placeholder="hello@yourbusiness.co.uk"
                />
                <p className="mt-1 text-xs text-[rgba(153,197,255,0.35)]">
                  Changing your email here updates it in Settings — you may need to reverify.
                </p>
              </div>
            </div>
          )}
        ...
        <div className="space-y-3">
          {form.logoUrl ? (
            <div className="flex items-center gap-4 rounded-xl border border-[rgba(153,197,255,0.2)] bg-[#091660] p-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-[rgba(153,197,255,0.5)] mt-0.5">
                  Will appear in the sidebar, invoices and quotes
                </p>
              </div>
              <button
                type="button"
                onClick={() => patch({ logoUrl: '' })}
                className="text-xs text-[rgba(153,197,255,0.4)] hover:text-white transition"
              >
                Remove
              </button>
            </div>
            ...
            <label className="block cursor-pointer rounded-xl border-2 border-dashed border-[rgba(153,197,255,0.2)] bg-[#091660] p-6 text-center transition hover:border-[#99c5ff] hover:bg-[rgba(153,197,255,0.05)]">
              ...
              <p className="mt-1 text-xs text-[rgba(153,197,255,0.45)]">
                PNG, JPG or SVG — we'll resize it automatically
              </p>
              <input
                type="file"
                accept="image/*"
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              onClick={() =>
                handleAnswer(
                  id,
                  form.logoUrl ? 'Logo added ✓' : 'Skipped',
                  form.logoUrl ? 'Logo added' : 'Skipped'
                )
              }
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                className={inputCls}
                value={form.bizPhone}
                onChange={(e) => patch({ bizPhone: e.target.value })}
            ...
            <div>
              ...
              <input
                className={inputCls}
                value={form.bizPostcode}
                onChange={(e) => patch({ bizPostcode: e.target.value })}
                placeholder="e.g. RM1 3AB"
              />
            </div>
          </div>
          <div className="flex gap-2">
            {skippable && (
              <button
                type="button"
                onClick={() => advance('Skipped', form)}
                className="rounded-[10px] border border-[rgba(153,197,255,0.15)] px-4 py-[11px] text-sm text-[rgba(153,197,255,0.55)] transition hover:border-[rgba(153,197,255,0.35)] hover:text-white"
              >
                Skip
              </button>
            )}
            <button
              ...
              onClick={() =>
                handleAnswer(
                  id,
                  `${form.bizPhone || '—'} · ${form.bizPostcode || '—'}`,
                  `${form.bizPhone || '—'} · ${form.bizPostcode || '—'}`
                )
              }
              className="flex-1 rounded-[10px] bg-[#1f48ff] py-[11px] text-sm font-bold text-white shadow-[0_4px_16px_rgba(31,72,255,0.4)] transition hover:bg-[#3a5eff] disabled:opacity-60"
      ...
      const sectors = [
        {
          ...
          emoji: '🏠',
          title: 'Residential',
          desc: 'Homes, end of tenancy, Airbnb',
          color: '#10b981',
        ...
        },
        {
          ...
          emoji: '🏢',
          title: 'Commercial',
          desc: 'Offices, schools, hospitality',
          color: '#99c5ff',
          border: 'rgba(153,197,255,0.3)',
        ...
        },
        {
          ...
          emoji: '🪟',
          title: 'Exterior',
          desc: 'Windows, gutters, jet washing',
          color: '#fb923c',
            ...
            {sectors.map((s) => {
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    const updated = sel
                      ? form.cleanerSectors.filter((x) => x !== s.key)
                      : [...form.cleanerSectors, s.key];
                  ...
                  style={{
                    borderColor: sel ? s.color : s.border,
                    background: sel ? s.bg : 'rgba(9,22,96,0.6)',
                  }}
                >
                  <div className="mb-2 text-2xl">{s.emoji}</div>
                  <div className="text-sm font-bold text-white">{s.title}</div>
                  <div className="mt-1 text-xs leading-relaxed text-[rgba(153,197,255,0.55)]">
                    {s.desc}
                  </div>
                  {sel && (
          ...
          {valid && (
            <button
              ...
              onClick={() =>
                handleAnswer(
                  id,
                  form.cleanerSectors.map((s) => s.charAt(0).toUpperCase() + s.slice(1)).join(', '),
                  form.cleanerSectors.map((s) => s.charAt(0).toUpperCase() + s.slice(1)).join(', ')
                )
              }
      ...
      const options = [
        {
          ...
          icon: '👤',
          title: 'Sole Trader',
          desc: 'Self-employed, Self Assessment',
        },
        {
          value: 'limited',
          icon: '🏛️',
          title: 'Limited Company',
          desc: 'Incorporated, Corporation Tax',
        },
        {
          value: 'partnership',
          icon: '🤝',
          title: 'Partnership',
          desc: 'Two or more people, shared profits',
        },
        { value: 'llp', icon: '📑', title: 'LLP', desc: 'Limited Liability Partnership' },
        {
          ...
          desc: "Haven't formally set up yet",
        },
      ];
      return (
        <div className="space-y-3">
            ...
            {options.map((o) => (
              <button
                ...
                onClick={async () => {
                  ...
                  setSaving(true);
                  try {
                    await saveChapter('bizStructure');
                  } catch {
                    setSaving(false);
                    return;
                  }
            ...
            <div className="grid grid-cols-2 gap-3 rounded-xl border border-[rgba(153,197,255,0.12)] bg-[#091660] p-4 max-[480px]:grid-cols-1">
              ...
              <div>
                ...
                <input
                  ...
                  className={inputCls}
                  value={form.fyEnd}
                  onChange={(e) => patch({ fyEnd: e.target.value })}
                />
              </div>
            </div>
          )}
    ...
    if (type === 'yes-no') {
      return (
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-2">
            {[
              { val: true, label: '✅ Yes, VAT registered', color: '#34d399' },
              { val: false, label: '❌ Not VAT registered', color: '#fb923c' },
            ].map((opt) => (
              <button
                key={String(opt.val)}
                type="button"
                onClick={async () => {
                  ...
                  if (!opt.val) {
                    ...
                    setSaving(true);
                    try {
                      await saveChapter('vatRegistered');
                    } catch {
                      setSaving(false);
                      return;
                    }
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => patch({ vatScheme: e.target.value })}
                  >
            ...
            {options.map((o) => (
              <button
                ...
                onClick={async () => {
                  ...
                  setSaving(true);
                  try {
                    await saveChapter('teamStructure');
                  } catch {
                    setSaving(false);
                    return;
                  }
        ...
        <div className="space-y-3">
          ...
          <div className="max-h-[220px] overflow-y-auto space-y-2 pr-1">
            {activeGroups.map((group) => (
              <div
                key={group.label}
                className="rounded-xl border border-[rgba(153,197,255,0.1)] bg-[#091660] p-3"
              >
                <div className="mb-2 text-[10px] font-bold uppercase tracking-widest text-[rgba(153,197,255,0.45)]">
                  {group.label}
                </div>
                <div className="flex flex-wrap gap-1.5">
                  {group.items.map((item) => (
                    <Chip
                      key={item}
                      selected={form.services.includes(item)}
                      onClick={() => toggleArr('services', item)}
                    >
                      {item}
                    </Chip>
                  ))}
          ...
          <input
            ...
            onChange={(e) => patch({ customService: e.target.value })}
            placeholder="Other services not listed (comma-separated)…"
          />
          <div className="flex gap-2">
            {skippable && (
              <button
            ...
            >
              {saving
                ...
                : form.services.length > 0
                  ? `Continue with ${form.services.length} services →`
                  : 'Continue →'}
            </button>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                onChange={(e) => patch({ currentRevenue: e.target.value })}
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => patch({ targetRevenue: e.target.value })}
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => patch({ currentClients: e.target.value })}
                placeholder="e.g. 12"
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => patch({ targetClients: e.target.value })}
                placeholder="e.g. 30"
              />
            </div>
          ...
          <div>
            ...
            <div className="mt-1 flex flex-wrap gap-2">
              {ambitionOptions.map((a) => (
                <Chip
                  key={a.value}
                  selected={form.ambition === a.value}
                  onClick={() => patch({ ambition: a.value })}
                >
                  {a.label}
                </Chip>
              ))}
            ...
            <button
              ...
              onClick={() =>
                handleAnswer(
                  id,
                  form.targetRevenue
                    ? `Target: £${Number(form.targetRevenue).toLocaleString()}/mo`
                    : 'Goals noted',
                  form.targetRevenue ? `£${form.targetRevenue}/mo target` : 'Set'
                )
              }
              className="flex-1 rounded-[10px] bg-[#1f48ff] py-3 text-sm font-bold text-white shadow-[0_4px_16px_rgba(31,72,255,0.4)] hover:bg-[#3a5eff] transition disabled:opacity-60"
            >
    ...
    if (type === 'summary') {
      const readyItems = [
        form.logoUrl && {
          icon: null,
          img: form.logoUrl,
          label: 'Your logo loaded',
          detail:
            'Appears in the sidebar, on invoices and on every quote — the app looks completely yours',
        },
        form.cleanerSectors.length && {
          icon: '🧹',
          label: 'App customised',
          detail: `${form.cleanerSectors.map((s) => s.charAt(0).toUpperCase() + s.slice(1)).join(', ')} edition — dashboard, tools and filters set to match your work`,
        },
        form.services.length && {
          icon: '✅',
          label: 'Service menu built',
          detail: `${form.services.length} service${form.services.length !== 1 ? 's' : ''} ready as quick-picks on every job card, quote and invoice`,
        },
        form.targetRevenue && {
          icon: '🎯',
          label: 'Sprint target set',
          detail: `£${Number(form.targetRevenue).toLocaleString()}/mo target pinned to your dashboard — Cadi tracks every job against it`,
        },
        form.bizStructure && {
          icon: '📋',
          label: 'Tax & accounts configured',
          detail: `${form.bizStructure === 'sole_trader' ? 'Self Assessment SA103' : form.bizStructure === 'limited' ? 'Corporation Tax & Companies House' : 'Accounts'} tools and defaults set for your structure`,
        },
        form.vatRegistered && {
          icon: '🧾',
          label: 'VAT set up',
          detail: `${form.vatScheme || 'Standard'} scheme — invoices, quotes and reports will show VAT correctly`,
        },
        (form.pli || form.dbs || form.ico || form.coshh) && {
          icon: '🛡️',
          label: 'Compliance logged',
          detail: 'Renewal reminders are set — Cadi will alert you before anything lapses',
        },
        form.teamStructure &&
          form.teamStructure !== 'solo' && {
            icon: '👥',
            label: 'Team features on',
            detail: 'Job assignment, staff scheduling and multi-cleaner tracking are active',
          },
      ].filter(Boolean);
        ...
        <div className="space-y-3">
          <div className="space-y-2">
            {readyItems.map((item) => (
              <div
                key={item.label}
                className="flex gap-3 rounded-xl border border-[rgba(153,197,255,0.1)] bg-[#091660] px-4 py-3"
              >
                {item.img ? (
                  <img
                    src={item.img}
                    alt=""
                    className="mt-0.5 h-8 w-8 shrink-0 rounded-lg object-contain bg-white/10 p-0.5"
                  />
                ) : (
                ...
                <div>
                  ...
                  <div className="mt-0.5 text-xs leading-relaxed text-[rgba(153,197,255,0.6)]">
                    {item.detail}
                  </div>
                </div>
              </div>
          ...
          {readyItems.length === 0 && (
            <p className="text-sm text-[rgba(153,197,255,0.6)]">
              Your account is ready — you can fill in the details from inside the app.
            </p>
          )}

          {/* Next 3 actions */}
          <div className="rounded-xl border border-[rgba(153,197,255,0.15)] bg-[#091660] px-4 py-3 space-y-2">
            <p className="text-[10px] font-bold tracking-widest uppercase text-[rgba(153,197,255,0.4)] mb-1">
            ...
            <p className="text-[10px] text-[rgba(153,197,255,0.35)] pt-1">
        ...
        <div className="space-y-3">
          ...
          <div className="rounded-xl overflow-hidden border border-[rgba(153,197,255,0.15)] shadow-lg">
            ...
            <div className="relative bg-gray-50" style={{ height: 200 }}>
              ...
              <div className="bg-white border-b border-gray-100 px-4 py-2.5 flex items-center justify-between">
                ...
                <div className="flex gap-3">
                  {['Services', 'Areas', 'Contact'].map((l) => (
                    <span key={l} className="text-gray-400 text-[10px]">
                      {l}
                    </span>
                  ))}
                </div>
              ...
              {widgetDemoOpen && (
                <div
                  className="absolute bottom-[68px] right-3 w-60 rounded-xl shadow-2xl overflow-hidden border border-gray-200 bg-white"
                ...
                >
                  ...
                  <div className="px-3 py-3 space-y-2.5 bg-gray-50">
                    <div className="flex gap-2 items-start">
                      <div className="w-6 h-6 rounded-full bg-[#1f48ff] shrink-0 flex items-center justify-center text-white text-[8px] font-black mt-0.5">
                        C
                      </div>
                      <div className="bg-white rounded-xl rounded-tl-none px-3 py-2 text-[11px] text-gray-800 shadow-sm leading-relaxed">
                        Hi! 👋 I can get you a quote in under a minute. What type of clean are you
                        looking for?
                      </div>
                    </div>
                    <div className="flex gap-1 flex-wrap pl-8">
                      {['Regular clean ✓', 'Deep clean', 'End of tenancy'].map((opt) => (
                        <span
                          key={opt}
                          className="px-2 py-1 rounded-full border border-[#1f48ff]/40 bg-white text-[#1f48ff] text-[10px] font-semibold"
                        >
                          {opt}
                        </span>
                      ))}
          ...
          {!widgetDemoOpen && (
            <p className="text-center text-xs text-[rgba(153,197,255,0.45)]">
              👆 Tap the blue button to see your customers' experience
            </p>
          )}
          {widgetDemoOpen && (
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                <div
                  key={b.label}
                  className="rounded-xl bg-[#091660] border border-[rgba(153,197,255,0.1)] px-2 py-3 text-center"
                >
          ...
          <p className="text-[11px] text-[rgba(153,197,255,0.4)]">
            Paste this just before &lt;/body&gt; on your site. 30 seconds — or do it later in
            Settings → Front Desk.
          </p>
          ...
          <div className="flex gap-2">
            <button
              type="button"
              onClick={() => advance('Do it later', form)}
              className="rounded-[10px] border border-[rgba(153,197,255,0.15)] px-4 py-[11px] text-sm text-[rgba(153,197,255,0.5)] hover:text-white transition"
            >
              Do it later
            </button>
            ...
            {[
              {
                ...
                hint: "We'll add you to the early-access list",
              },
              {
                val: false,
                label: '⏳ Not now',
                hint: 'No problem — you can join later in the Connect tab',
              },
            ].map((opt) => (
              <button
                key={String(opt.val)}
        ...
        <div className="space-y-3">
          <div className="rounded-xl border border-[rgba(153,197,255,0.12)] bg-[#091660] px-4 py-4 space-y-2">
            {[
              ...
              {
                emoji: '🤖',
                label: 'Powers Front Desk',
                desc: 'Cadi quotes correctly from day one',
              },
            ].map((b) => (
  ...
  const inner = (
    <div className="relative z-10 flex h-full w-full max-w-[680px] flex-col">
          ...
          {CHAPTERS.map((ch, i) => {
            ...
            return (
              <div key={ch.id} className="flex items-center gap-1.5">
                <div
                  className={`flex h-[26px] items-center rounded-full px-3 text-[11px] font-bold transition-all ${active ? 'bg-[#1f48ff] text-white shadow-[0_0_12px_rgba(31,72,255,0.5)]' : done ? 'bg-[rgba(153,197,255,0.15)] text-[#99c5ff]' : 'text-[rgba(153,197,255,0.3)]'}`}
                >
                  {done ? '✓' : null} {ch.label}
                </div>
                {i < CHAPTERS.length - 1 && (
                  <div
                    className={`h-px w-3 ${ch.id < currentChapter ? 'bg-[rgba(153,197,255,0.4)]' : 'bg-[rgba(153,197,255,0.12)]'}`}
                  />
                )}
              </div>
            );
      ...
      <p className="mb-3 text-[10px] text-[rgba(153,197,255,0.4)] text-center">
        Cadi uses AI (Anthropic) to personalise this setup. Your answers are processed to tailor
        your app — never trained on.{' '}
        <a
          href="/privacy#ai"
          target="_blank"
          rel="noopener noreferrer"
          className="text-[rgba(153,197,255,0.6)] hover:text-white underline"
        >
          Read more
        </a>
      </p>
      ...
      >
        ...
        {messages.map((msg) => (
          <div
            key={msg.id}
            className={`bubble-in flex ${msg.role === 'cadi' ? 'justify-start' : 'justify-end'}`}
          >
            {msg.role === 'cadi' && (
              <div className="mr-2 mt-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#1f48ff] text-[13px] font-extrabold text-white shadow-[0_0_12px_rgba(31,72,255,0.4)]">
                C
              </div>
            )}
            <div
        ...
        {isTyping && (
          <div className="bubble-in flex justify-start">
            <div className="mr-2 mt-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#1f48ff] text-[13px] font-extrabold text-white">
              C
            </div>
            <div className="rounded-[18px] rounded-tl-sm border border-[rgba(153,197,255,0.1)] bg-[#05124a] px-4 py-3">
              <div className="flex gap-1.5">
                {[0, 1, 2].map((i) => (
                  <span
                    key={i}
                    className="h-2 w-2 rounded-full bg-[rgba(153,197,255,0.6)]"
  ...
  const toast = notif && (
    <div
      className={`fixed right-5 top-5 z-[300] max-w-[340px] rounded-xl border px-4 py-3 text-[13px] shadow-[0_8px_32px_rgba(0,0,0,0.4)] ${notif.type === 'error' ? 'border-[rgba(248,113,113,0.3)] bg-[#3b0d0d] text-[#f87171]' : 'border-[rgba(52,211,153,0.3)] bg-[#0d3b2a] text-[#34d399]'}`}
    >
      {notif.type === 'error' ? '❌ ' : '✅ '}
      {notif.msg}
    </div>
    ...
    return (
      <div className="fixed inset-0 z-[200] overflow-y-auto">
        ...
        <div
          ...
          style={{
            backgroundImage:
              'linear-gradient(rgba(153,197,255,0.03) 1px,transparent 1px),linear-gradient(90deg,rgba(153,197,255,0.03) 1px,transparent 1px)',
  ...
  return (
    <div className="relative flex min-h-screen flex-col items-center justify-start overflow-x-hidden bg-[#010a4f] px-4 py-8 text-white sm:justify-center sm:py-10">
      ...
      {toast}
      <div
        className="relative z-10 w-full max-w-[680px] flex flex-col"
