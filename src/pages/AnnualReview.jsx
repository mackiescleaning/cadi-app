...
function Alert({ type = "blue", children }) {
  const s = { warn:"bg-amber-500/100/10 border-amber-500/25 text-amber-300", green:"bg-emerald-500/100/10 border-emerald-500/25 text-emerald-300", blue:"bg-[#059669]/10 border-[#059669]/25 text-[#99c5ff]", gold:"bg-yellow-500/10 border-yellow-500/25 text-yellow-300", red:"bg-red-500/100/10 border-red-500/25 text-red-300" };
...
function RStat({ label, value, accent = "text-white", sub, change }) {
  return (
    <div className="rounded-xl border border-[rgba(134,239,172,0.14)] p-4" style={{ background: 'linear-gradient(135deg, #064e3b 0%, #047857 100%)' }}>
  ...
  return (
    <div>
      <div className="flex items-center gap-4 text-xs mb-3">
        <div className="flex items-center gap-1.5"><div className="w-3 h-3 bg-[#10b981] rounded-sm" /><span className="text-[rgba(153,197,255,0.6)]">Income</span></div>
        <div className="flex items-center gap-1.5"><div className="w-3 h-3 bg-[#86efac]/60 rounded-sm" /><span className="text-[rgba(153,197,255,0.6)]">Expenses</span></div>
      </div>
        ...
        {data.map(({ m, income, exp }) => (
          <div key={m} className="flex-1 flex flex-col items-center gap-1 relative">
            ...
            <div className="w-full flex items-end gap-0.5 h-32">
              <div className="flex-1 bg-[#10b981] rounded-sm transition-all" style={{ height:`${(income/maxVal)*100}%` }} />
              <div className="flex-1 bg-[#86efac]/50 rounded-sm transition-all" style={{ height:`${(exp/maxVal)*100}%` }} />
            </div>
    ...
    <div className="fixed inset-0 z-50 bg-[rgba(255,255,255,0.03)] overflow-y-auto">
      <div className="max-w-3xl mx-auto p-8">
        <div className="flex items-center justify-between mb-8 print:hidden">
          ...
          <button onClick={() => window.print()} className="flex items-center gap-2 px-5 py-2.5 bg-[#064e3b] text-white text-sm font-bold rounded-sm hover:bg-[#047857] transition-colors">
            🖨️ Print / Save PDF
          </button>
        </div>
        ...
        <div className="bg-[#064e3b] rounded-sm p-8 text-white mb-6">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-xs font-bold tracking-widest uppercase text-[#86efac] mb-4">Cadi Business Review</p>
...
function ExecSummarySection({
  data, businessName, entityType, vatRegistered,
  toneMode, setToneMode, execSummaryOverride, setExecSummaryOverride,
  isLive, liveLoading,
}) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {execSummaryOverride?.trim() && (
          <span className="ml-auto text-[10px] font-bold uppercase tracking-widest text-amber-300">
            ...
            <button
              type="button"
              onClick={() => { setExecSummaryOverride(''); setEditing(false); }}
      ...
      <Card className="p-6">
        ...
        {!editing && !execSummaryOverride?.trim() && (
          <p className="text-[10px] mt-3 text-[rgba(255,255,255,0.4)]">
            Auto-composed from your snapshot. Click <em>Edit</em> to override the wording — the numbers stay locked to the snapshot either way.
          </p>
        )}
      </Card>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {kpis.map(k => {
          const accent =
            k.tone === 'up'   ? 'text-emerald-400' :
            k.tone === 'down' ? 'text-red-400'     :
                                'text-white';
          return (
            <div key={k.key}
              className="rounded-xl border border-[rgba(134,239,172,0.14)] p-4"
              style={{ background: 'linear-gradient(135deg, #064e3b 0%, #047857 100%)' }}>
              <p className="text-[10px] font-bold tracking-[0.12em] uppercase text-[#86efac]/80 mb-1">{k.label}</p>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-[#064e3b] rounded-sm p-6 text-white">
        <p className="text-xs font-bold tracking-widest uppercase text-[#86efac] mb-3">Tax year {data.yr}</p>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <div key={label} className="bg-[rgba(255,255,255,0.03)]/10 rounded-sm p-4">
              <p className="text-xs text-[#86efac] font-bold uppercase tracking-widest mb-1">{label}</p>
...
function ClientsSection({ data, highlights, setHighlights, toneMode = 'internal' }) {
  const TYPE_DOT = { residential:"bg-emerald-500/100", commercial:"bg-[#10b981]", exterior:"bg-orange-500" };
  ...
  const concentrationTone =
    concentrationLevel === 'high'   ? { fg: '#fca5a5', bg: 'rgba(220,38,38,0.14)',  bd: 'rgba(220,38,38,0.35)',  headline: 'High concentration risk' } :
    concentrationLevel === 'medium' ? { fg: '#fbbf24', bg: 'rgba(251,191,36,0.14)', bd: 'rgba(251,191,36,0.35)', headline: 'Moderate concentration' } :
                                       { fg: '#86efac', bg: 'rgba(34,197,94,0.14)',  bd: 'rgba(34,197,94,0.35)',  headline: 'Healthy spread' };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        ...
        <RStat label="Net growth"   value={`${netGrowth >= 0 ? '+' : ''}${netGrowth}`}                                accent={netGrowth >= 0 ? 'text-emerald-400' : 'text-red-400'} sub={data.clientsStart > 0 ? `${Math.round((netGrowth / data.clientsStart) * 100)}%` : ''} />
        <RStat label="Lost"         value={data.clientsLost ?? 0}                                                     accent={(data.clientsLost ?? 0) > 3 ? 'text-red-400' : 'text-amber-400'} sub={data.clientsLost === 0 ? 'Zero churn' : ''} />
        <RStat label="Retention"    value={`${Math.round(retention)}%`}                                               accent={retention >= 90 ? 'text-emerald-400' : retention >= 75 ? 'text-amber-400' : 'text-red-400'} />
      </div>
        ...
        <div
          ...
          <div className="flex items-start gap-4">
            <div
              ...
              style={{ background: `${concentrationTone.fg}22`, color: concentrationTone.fg, fontSize: 24, fontWeight: 900, lineHeight: 1, minWidth: 68, textAlign: 'center' }}>
              {Math.round(top3)}%
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: concentrationTone.fg }}>
              ...
              <p className="text-sm text-white/70 mt-1 leading-relaxed">
                Top 3 customers = <strong className="text-white">{Math.round(top3)}%</strong> of attributed revenue.
                Top 5 = <strong className="text-white">{Math.round(top5)}%</strong>.
                {concentrationLevel === 'high' && (toneMode === 'funding'
                ...
                {concentrationLevel === 'medium' && ' Reasonable but worth watching — a diversification push next year would strengthen the risk profile.'}
                {concentrationLevel === 'low'    && ' Well-diversified base — no single customer dominates the book.'}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <RStat label="Jobs completed"   value={(data.jobsCompleted ?? 0).toLocaleString()} accent="text-white" />
        <RStat label="Avg job value"    value={fmt(data.avgJobValue)}               accent="text-[#10b981]" />
        <RStat label="Jobs per month"   value={Math.round((data.jobsCompleted ?? 0) / 12)} accent="text-white" sub="average" />
      </div>
      ...
      {showTopTable ? (
        <Card className="overflow-hidden">
          <div className="px-4 py-3 border-b border-[rgba(153,197,255,0.08)] flex items-center justify-between">
            ...
            <span className="text-[10px] text-white/40">Only income linked to a customer is attributed</span>
          </div>
            ...
            {topClients.map((c, i) => {
              ...
              return (
                <div key={c.id ?? c.name} className="flex items-center gap-3 px-4 py-3">
                  <div className="w-7 h-7 rounded-full bg-[#064e3b] text-white flex items-center justify-center text-xs font-bold shrink-0">{i+1}</div>
                  <div className={`w-2 h-2 rounded-full shrink-0 ${dotClass}`} />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-white truncate">{displayName}</p>
                    <p className="text-xs text-[rgba(153,197,255,0.5)]">
                      {c.visits} {c.visits === 1 ? 'visit' : 'visits'}{c.category ? ` · ${c.category}` : ''}
                    </p>
                  </div>
                  ...
                  <div className="w-20 h-2 bg-[rgba(153,197,255,0.06)] rounded-sm overflow-hidden shrink-0">
                    <div className="h-full bg-[#10b981]/60 rounded-sm" style={{ width:`${topClients[0].revenue > 0 ? (c.revenue / topClients[0].revenue) * 100 : 0}%` }} />
                  </div>
                </div>
              );
            })}
        ...
        <div
          ...
          <p className="text-[10px] font-bold uppercase tracking-widest text-[#86efac]">Growing customer base</p>
          ...
          <p className="text-sm text-white/70 mt-2 leading-relaxed">
            The top-10 leaderboard appears once you have 5 or more customers with attributed income. Until then, focus on landing steady work — the story of the year is growth, not concentration.
          </p>
        </div>
      )}
...
function ServicesSection({ data, toneMode = 'internal', vatRegistered }) {
  ...
  const CAT_DOT = { residential: 'bg-emerald-500', commercial: 'bg-[#10b981]', exterior: 'bg-orange-500' };

  // Header KPIs
  const offered      = perf.filter(s => s.isActive).length;
  const delivered    = perf.filter(s => s.jobs > 0).length;
  const needsReview  = perf.filter(s => s.recommendation === 'review_pricing' || s.recommendation === 'declining_demand').length;
  ...
  if (toneMode === 'tender') {
    ...
    return (
      <div className="space-y-5">
        <div className="rounded-xl p-6" style={{ background: 'rgba(126,163,255,0.10)', border: '1px solid rgba(126,163,255,0.28)' }}>
          <p className="text-[10px] font-bold uppercase tracking-widest text-[#7ea3ff]">Service capability matrix</p>
          <p className="text-lg font-bold text-white mt-1">{offered} service{offered === 1 ? '' : 's'} delivered under contract or one-off engagement</p>
          <p className="text-sm text-white/70 mt-2 leading-relaxed">
            The categories and offerings below reflect what the business is set up to deliver. Commercial pricing, margin structure, and reference customers available on request.
          </p>
        </div>
              ...
              <div className="divide-y divide-white/5">
                {items.map(s => (
                  <div key={s.id ?? s.name} className="flex items-center gap-3 px-4 py-3">
                    <div className={`w-2 h-2 rounded-full shrink-0 ${CAT_DOT[s.category] || 'bg-white/30'}`} />
  ...
  if (perf.length === 0) {
    return (
      <div className="text-center py-12 rounded-xl border border-white/10 bg-white/5">
        <p className="text-lg font-bold text-white mb-2">Set up your services to track performance</p>
        <p className="text-sm text-white/60 max-w-md mx-auto leading-relaxed">
          §4 pulls per-service revenue, margin and demand trends from jobs delivered. Head to Services to add what you offer, then this section fills in automatically.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <RStat label="Delivered"      value={delivered}                accent="text-emerald-400" sub={`${offered - delivered} idle`} />
        <RStat label="Needs review"   value={needsReview}              accent={needsReview > 0 ? 'text-amber-400' : 'text-white'} />
        <RStat label="Attributed rev" value={fmt(totalRevenue)}        accent="text-[#10b981]" sub={vatRegistered ? 'ex-VAT figures where applicable' : ''} />
      </div>

      {/* Funding mode — hero'd top 3 */}
      {toneMode === 'funding' && perf.filter(s => s.revenue > 0).length > 0 && (
        <div>
          <p className="text-[10px] font-bold uppercase tracking-widest text-[#86efac] mb-2">Strongest three lines of business</p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {perf.filter(s => s.revenue > 0).slice(0, 3).map(s => (
              <div key={s.id ?? s.name}
      ...
      <Card className="overflow-hidden">
        ...
        <div className="divide-y divide-white/5">
          {perf.map(s => {
            ...
            const yoyStr = s.jobsYoY == null
              ? '—'
              : `${s.jobsYoY >= 0 ? '+' : ''}${Math.round(s.jobsYoY)}%`;
            const yoyColor = s.jobsYoY == null    ? 'text-white/40'
                            : s.jobsYoY >= 0      ? 'text-emerald-400'
                            :                        'text-red-400';
            return (
              <div key={s.id ?? s.name} className="px-4 py-3">
                <div className="flex items-center gap-3 mb-1">
                  ...
                  <p className="text-sm font-semibold text-white flex-1 truncate">
                    {s.name}
                    {s.unmatched && <span className="ml-2 text-[10px] font-normal text-amber-300">unmatched job type</span>}
                    {!s.isActive && !s.unmatched && <span className="ml-2 text-[10px] font-normal text-white/40">inactive</span>}
                  </p>
                  {recTone.label && (
                    <span
                ...
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 text-xs">
                  ...
                  <div>
                    ...
                    <p className={`font-bold tabular-nums ${s.grossMargin >= 60 ? 'text-emerald-400' : s.grossMargin > 0 ? 'text-amber-400' : 'text-white/40'}`}>
      ...
      <p className="text-[10px] text-white/40 text-center">
        Margin uses stored per-job margin where set, else price − labour cost. Services without a matching <code>jobs.type</code> string appear as "unmatched" so revenue reconciles.
      </p>
    </div>
  );
}
...
const RECOMMENDATION_TONE = {
  review_pricing:    { label: 'Review pricing',    fg: '#fbbf24', bg: 'rgba(251,191,36,0.14)', bd: 'rgba(251,191,36,0.35)' },
  declining_demand:  { label: 'Declining demand',  fg: '#fca5a5', bg: 'rgba(220,38,38,0.14)',  bd: 'rgba(220,38,38,0.35)'  },
  new:               { label: 'New',               fg: '#7ea3ff', bg: 'rgba(126,163,255,0.14)',bd: 'rgba(126,163,255,0.35)'},
  no_activity:       { label: 'No jobs',           fg: '#94a3b8', bg: 'rgba(148,163,184,0.14)',bd: 'rgba(148,163,184,0.35)'},
...
function MoneySection({ data, toneMode = 'internal', entityType, vatRegistered }) {
  ...
  if (toneMode === 'tender') {
    return (
      <div className="space-y-5">
        <div className="rounded-xl p-6" style={{ background: 'rgba(126,163,255,0.10)', border: '1px solid rgba(126,163,255,0.28)' }}>
          <p className="text-[10px] font-bold uppercase tracking-widest text-[#7ea3ff]">Financial reliability signals</p>
          <p className="text-lg font-bold text-white mt-1">Public disclosure — high level only</p>
          <p className="text-sm text-white/70 mt-2 leading-relaxed">
            Full management accounts available under NDA. The metrics below are what buyers usually care about at pre-qualification.
          </p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <RStat label="On-time invoice payment" value={onTime != null ? `${Math.round(onTime)}%` : '—'} accent={onTime != null && onTime >= 80 ? 'text-emerald-400' : 'text-white'} sub="paid by due date" />
          <RStat label="Debtor days"             value={debtorDays != null ? `${Math.round(debtorDays)}` : '—'} accent="text-white" sub="avg sent → paid" />
          <RStat label="Business current"        value={onTime != null && onTime >= 60 ? 'Yes' : (data.income > 0 ? 'Yes' : '—')} accent="text-emerald-400" sub="No overdue statutory filings" />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      ...
      {toneMode === 'funding' && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="rounded-xl border p-5"
            ...
            <p className="text-[10px] font-bold uppercase tracking-widest text-[#86efac]">Cash runway (crude)</p>
            <p className="text-3xl font-bold text-white tabular-nums mt-1">{runway != null ? `${runway} mo` : '—'}</p>
          ...
          <div className="rounded-xl border p-5"
            ...
            <p className="text-[10px] font-bold uppercase tracking-widest text-[#fbbf24]">Debtor days</p>
            <p className="text-3xl font-bold text-white tabular-nums mt-1">{debtorDays != null ? Math.round(debtorDays) : '—'}</p>
            <p className="text-xs text-white/60 mt-1">
              {debtorDays != null && debtorDays <= 30 ? 'Well within trade norms'
                : debtorDays != null && debtorDays <= 45 ? 'Normal for the sector'
                : debtorDays != null ? 'Room to improve on collections'
                : 'No paid invoices in period yet'}
            </p>
          </div>
          <div className="rounded-xl border p-5"
            style={{ background: 'rgba(126,163,255,0.10)', borderColor: 'rgba(126,163,255,0.30)' }}>
            <p className="text-[10px] font-bold uppercase tracking-widest text-[#7ea3ff]">On-time payment</p>
            <p className="text-3xl font-bold text-white tabular-nums mt-1">{onTime != null ? `${Math.round(onTime)}%` : '—'}</p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <RStat label="Expenses"           value={fmt(data.expenses)} accent="text-red-400" sub={data.income > 0 ? `${Math.round((data.expenses/data.income)*100)}% of income` : ''} />
        <RStat label="Net profit"         value={fmt(data.profit)}   accent={data.profit >= 0 ? 'text-emerald-400' : 'text-red-400'} sub={`${Math.round(data.margin)}% margin`} />
        <RStat label="Outstanding at end" value={fmt(outstanding)}   accent={outstanding > 0 ? 'text-amber-400' : 'text-white'} sub={`${data.invoicesIssued ?? 0} invoices issued`} />
      </div>
      ...
      <Card className="overflow-hidden">
        ...
        <div className="p-4">
          <div className="grid grid-cols-6 sm:grid-cols-12 gap-1.5">
            {monthlyPL.map((m, i) => {
              const maxAbs = Math.max(...monthlyPL.map(x => Math.max(x.income, x.exp, Math.abs(x.profit ?? 0))), 1);
              ...
              return (
                <div key={m.m + i} className="text-center">
                  <div className="h-16 flex items-end justify-center gap-0.5">
                    <div className="w-1.5 rounded-sm bg-[#10b981]" style={{ height: bar(m.income) }} title={`Income ${fmt(m.income)}`} />
                    <div className="w-1.5 rounded-sm bg-red-500/60" style={{ height: bar(m.exp) }} title={`Expenses ${fmt(m.exp)}`} />
                    <div className={`w-1.5 rounded-sm ${profit >= 0 ? 'bg-[#86efac]' : 'bg-red-400'}`} style={{ height: bar(profit) }} title={`Profit ${fmt(profit)}`} />
                  </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        <Card className="overflow-hidden">
          <div className="px-4 py-3 border-b border-[rgba(153,197,255,0.08)]"><SL>Expense breakdown</SL></div>
          <div className="divide-y divide-white/5">
            {expenseBreakdown.length === 0 ? (
              <div className="px-4 py-8 text-center">
                <p className="text-sm text-white mb-1 font-bold">Categorise your expenses</p>
                <p className="text-xs text-white/60 max-w-sm mx-auto leading-relaxed">
                  No categorised expenses in this period. Tag your expenses on the Money tab for a fuller picture here.
                </p>
              </div>
            ) : (
              expenseBreakdown.map(row => (
                <div key={row.label} className="flex items-center gap-3 px-4 py-2.5">
                  ...
                  <div className="flex-1 h-3 bg-[rgba(153,197,255,0.06)] rounded-sm overflow-hidden">
                    <div className="h-full bg-[#10b981]/60 rounded-sm" style={{ width: `${row.pct}%` }} />
                  </div>
                  <span className="text-xs font-mono font-semibold text-white w-16 text-right">{fmt(row.amount)}</span>
        ...
        <Card className="overflow-hidden">
          <div className="px-4 py-3 border-b border-[rgba(153,197,255,0.08)]"><SL>Top 5 single costs</SL></div>
          <div className="divide-y divide-white/5">
            {topCostLines.length === 0 ? (
              ...
              topCostLines.map(c => (
                <div key={c.id} className="flex items-center gap-3 px-4 py-2.5">
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-white font-semibold truncate">{c.notes || c.category || 'Expense'}</p>
                    <p className="text-[10px] text-white/50">
                      {c.date && new Date(c.date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}
      ...
      {toneMode !== 'funding' && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <RStat label="Invoices issued"  value={data.invoicesIssued ?? 0}                   accent="text-white" sub={fmt(data.invoiceValueIssued)} />
          <RStat label="Invoices paid"    value={data.invoicesPaid ?? 0}                     accent="text-emerald-400" sub={fmt(data.invoiceValuePaid)} />
          <RStat label="Debtor days"      value={debtorDays != null ? Math.round(debtorDays) : '—'} accent={debtorDays != null && debtorDays <= 30 ? 'text-emerald-400' : debtorDays != null && debtorDays <= 45 ? 'text-amber-400' : 'text-red-400'} sub="avg sent → paid" />
          <RStat label="On-time rate"     value={onTime != null ? `${Math.round(onTime)}%` : '—'} accent={onTime != null && onTime >= 80 ? 'text-emerald-400' : 'text-white'} />
        </div>
      )}
      ...
      {(data.badDebtWrittenOff ?? 0) > 0 && (
        <div className="rounded-xl border p-4"
          ...
          <p className="text-[10px] font-bold uppercase tracking-widest text-[#fca5a5]">Bad debt written off</p>
          <p className="text-lg font-bold text-white mt-1">{fmt(data.badDebtWrittenOff)}</p>
          <p className="text-xs text-white/60 mt-1">Voided / written-off invoices in this period. Review chase cadence for next year.</p>
        </div>
      )}
      ...
      {isLtd ? (
        <Card className="overflow-hidden">
          <div className="px-4 py-3 border-b border-[rgba(153,197,255,0.08)]"><SL>Limited company movements</SL></div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-4">
            <RStat label="Dividends paid"   value={fmt(data.ltdDividendsPaid ?? 0)}   accent="text-white" sub="in this period" />
            <RStat label="DLA balance"      value={fmt(data.ltdDlaBalance ?? 0)}      accent={Math.abs(data.ltdDlaBalance ?? 0) > 10000 ? 'text-amber-400' : 'text-white'} sub={`${data.ltdDlaMovement >= 0 ? '+' : ''}${fmt(data.ltdDlaMovement ?? 0)} in period`} />
            <RStat label="CT provision"     value={fmt(data.ltdCtProvision ?? 0)}     accent="text-white" sub={data.ltdCtStatus ?? '—'} />
            <RStat label="CT due on"        value={data.ltdCtDueOn ? new Date(data.ltdCtDueOn).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }) : '—'} accent="text-white" />
          </div>
        </Card>
      ) : (
        (data.soleTraderDrawings ?? 0) > 0 && (
          <Card className="overflow-hidden">
            <div className="px-4 py-3 border-b border-[rgba(153,197,255,0.08)]"><SL>Owner's drawings</SL></div>
            <div className="p-4 grid grid-cols-2 gap-3">
              <RStat label="Total drawings"  value={fmt(data.soleTraderDrawings)} accent="text-white" sub="approximate — category-based signal" />
              <RStat label="% of profit"     value={data.profit > 0 ? `${Math.round((data.soleTraderDrawings / data.profit) * 100)}%` : '—'} accent="text-white" />
            </div>
          </Card>
        )
      )}
      ...
      {vatRegistered && (
        <div className="rounded-xl border p-4"
          ...
          <p className="text-[10px] font-bold uppercase tracking-widest text-[#7ea3ff]">VAT — coming in §6</p>
          <p className="text-sm text-white/70 mt-1">
            Scheme, collected / paid split, and net position land in the Tax Position section (next slice). This section shows the P&amp;L gross of VAT.
          </p>
        </div>
      )}
    </div>
...
function TaxSection({ data, toneMode = 'internal' }) {
  ...
  if (toneMode === 'tender') {
    ...
    return (
      <div className="space-y-5">
        <div className="rounded-xl p-6" style={{ background: 'rgba(126,163,255,0.10)', border: '1px solid rgba(126,163,255,0.28)' }}>
          <p className="text-[10px] font-bold uppercase tracking-widest text-[#7ea3ff]">Tax posture</p>
          <p className="text-lg font-bold text-white mt-1">{vatLine}. Tax-compliant filings up to date.</p>
          <p className="text-sm text-white/70 mt-2 leading-relaxed">
            {mtdConnected
  ...
  if (!tax) {
    return (
      <div className="text-center py-12 rounded-xl border border-white/10 bg-white/5">
        <p className="text-white/60 text-sm">Tax position loads once the aggregator lands data for this period.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      ...
      {toneMode === 'funding' && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="rounded-xl border p-5" style={{ background: 'rgba(34,197,94,0.10)', borderColor: 'rgba(34,197,94,0.30)' }}>
            <p className="text-[10px] font-bold uppercase tracking-widest text-[#86efac]">Estimated tax bill</p>
            <p className="text-3xl font-bold text-white tabular-nums mt-1">{fmt(isLtd ? (tax.ltd?.ctDue ?? 0) : (tax.sole?.total ?? 0))}</p>
            <p className="text-xs text-white/60 mt-1">{isLtd ? 'Corporation Tax on this year\'s profit' : 'Income tax + Class 4 NI'}</p>
          </div>
          <div className="rounded-xl border p-5" style={{ background: 'rgba(126,163,255,0.10)', borderColor: 'rgba(126,163,255,0.30)' }}>
            <p className="text-[10px] font-bold uppercase tracking-widest text-[#7ea3ff]">HMRC connection</p>
            <p className="text-lg font-bold text-white mt-1">{hmrc?.connected ? 'Connected' : 'Not connected'}</p>
            <p className="text-xs text-white/60 mt-1">
              {hmrc?.connected
          ...
          <div className="rounded-xl border p-5" style={{ background: 'rgba(251,191,36,0.10)', borderColor: 'rgba(251,191,36,0.30)' }}>
            <p className="text-[10px] font-bold uppercase tracking-widest text-[#fbbf24]">Compliance</p>
            <p className="text-lg font-bold text-white mt-1">Returns filed on time</p>
            <p className="text-xs text-white/60 mt-1">No outstanding regulatory concerns disclosed</p>
          </div>
        </div>
      )}
      ...
      {isLtd ? (
        <Card className="overflow-hidden">
          <div className="px-4 py-3 border-b border-[rgba(153,197,255,0.08)] flex items-center justify-between">
            ...
            <span className="text-[10px] px-2 py-0.5 rounded-full border border-white/15 text-white/70">{RATE_BAND_LABEL[tax.ltd?.rateBand] ?? '—'}</span>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-4">
            <RStat label="Profit for CT"     value={fmt(tax.ltd?.profit ?? 0)}         accent="text-white" />
            <RStat label="CT due"            value={fmt(tax.ltd?.ctDue ?? 0)}          accent="text-amber-400" sub={tax.ltd?.ctDueOn ? `by ${new Date(tax.ltd.ctDueOn).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}` : ''} />
            <RStat label="CT provisioned"    value={fmt(tax.ltd?.ctProvisioned ?? 0)}  accent="text-white" sub={tax.ltd?.ctStatus ?? '—'} />
            <RStat label="Dividends paid"    value={fmt(tax.ltd?.dividendsPaid ?? 0)}  accent="text-white" sub="in this period" />
          </div>
          <div className="border-t border-white/5 px-4 py-4 bg-white/[0.02]">
            <p className="text-[10px] font-bold uppercase tracking-widest text-[#86efac] mb-2">Salary vs dividend split</p>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              <RStat label="Optimal salary (PA)"           value={fmt(tax.ltd?.optimalSalary ?? 0)} accent="text-white" sub="2025/26 personal allowance" />
              <RStat label="Optimal dividend"              value={fmt(tax.ltd?.dividendsAtOptimalSplit ?? 0)} accent="text-white" sub="after CT" />
              <RStat label="CT saved by taking salary"     value={fmt(tax.ltd?.ctSavingFromOptimal ?? 0)} accent="text-emerald-400" sub="vs no salary" />
            </div>
            <p className="text-[10px] text-white/40 mt-3 leading-relaxed">
              Simple heuristic assuming single director + full dividend take. The BL <em>Entity & Tax Strategy</em> module refines this — attaches your dated decision memo when certified.
            </p>
          </div>
          {(tax.payroll?.payeRef || tax.payroll?.employmentAllowance) && (
            <div className="border-t border-white/5 px-4 py-4">
              <p className="text-[10px] font-bold uppercase tracking-widest text-white/60 mb-2">PAYE / Employment allowance</p>
              <div className="grid grid-cols-2 gap-3">
                <RStat label="PAYE reference"          value={tax.payroll.payeRef || '—'} accent="text-white" />
                <RStat label="Employment allowance"    value={tax.payroll.employmentAllowance ? 'Claimed' : 'Not claimed'} accent={tax.payroll.employmentAllowance ? 'text-emerald-400' : 'text-white/60'} sub="Up to £5,000/year off employer NI" />
              </div>
            </div>
          )}
        </Card>
        ...
        <Card className="overflow-hidden">
          <div className="px-4 py-3 border-b border-[rgba(153,197,255,0.08)] flex items-center justify-between">
            ...
            <span className="text-[10px] px-2 py-0.5 rounded-full border border-white/15 text-white/70">{TAX_BAND_LABEL[tax.sole?.band] ?? '—'}</span>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-4">
            ...
            <RStat label="Total tax"            value={fmt(tax.sole?.total ?? 0)}        accent="text-amber-400" sub={tax.sole?.yoyDeltaTotal != null ? `${tax.sole.yoyDeltaTotal >= 0 ? '+' : ''}${fmt(tax.sole.yoyDeltaTotal)} vs last year` : ''} />
          </div>
          {tax.sole?.effectivePA < PERSONAL_ALLOWANCE_UI && (
            <div className="border-t border-white/5 px-4 py-3 bg-amber-500/5">
              <p className="text-xs text-amber-300">
                Personal allowance tapered to <strong>{fmt(tax.sole.effectivePA)}</strong> at this profit level — every £2 of profit above £100k loses £1 of allowance.
              </p>
            </div>
          )}
        </Card>
      ...
      <Card className="overflow-hidden">
        ...
        <div className="p-4 space-y-3">
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <RStat label="Status"        value={tax.vatRegistered ? 'Registered' : 'Not registered'} accent="text-white" sub={tax.vatNumber ? `VAT # ${tax.vatNumber}` : ''} />
            <RStat label="Scheme"        value={tax.vatScheme || (tax.vatRegistered ? 'Standard (default)' : '—')} accent="text-white" />
            <RStat label="Income in period" value={fmt(vat.incomeInPeriod)} accent="text-white" sub={`£${vat.registrationThreshold.toLocaleString()} threshold`} />
          </div>
      ...
      <Card className="overflow-hidden">
        <div className="px-4 py-3 border-b border-[rgba(153,197,255,0.08)] flex items-center justify-between">
          <SL>HMRC + MTD</SL>
          {hmrc?.tokenExpired && (
            <span className="text-[10px] text-red-300 font-bold uppercase tracking-widest">Token expired — reconnect</span>
          )}
        </div>
        <div className="p-4">
          {hmrc?.connected ? (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <RStat label="Connection"              value="Connected"                  accent="text-emerald-400" sub={hmrc.connectedAt ? `since ${new Date(hmrc.connectedAt).toLocaleDateString('en-GB', { month: 'short', year: 'numeric' })}` : ''} />
              <RStat label="Submissions this period" value={hmrc.mtdSubmissionsInPeriod ?? 0} accent="text-white" />
              <RStat label="Submissions all-time"    value={hmrc.mtdSubmissionsAll ?? 0}     accent="text-white" />
              <RStat label="Last submitted"          value={hmrc.mtdLastSubmittedAt ? new Date(hmrc.mtdLastSubmittedAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }) : '—'} accent="text-white" />
            </div>
          ) : (
            <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-4">
              <p className="text-sm text-white font-bold mb-1">HMRC not connected</p>
              <p className="text-xs text-white/70 leading-relaxed">
                Connect HMRC on the <strong className="text-white">Accounting tab</strong> to import your MTD ITSA submission history into this section. Until then the numbers above are Cadi's own estimate — Cadi cannot see whether you've filed.
              </p>
            </div>
          )}
        </div>
      ...
      <p className="text-[10px] text-white/40 text-center leading-relaxed max-w-2xl mx-auto">
        Tax figures are estimates from live data using the same calculator as the Money tab (personal allowance taper, marginal relief for CT). Not tax advice — final numbers come from your accountant.
      </p>
    </div>
  );
}
...
function VatProximityCallout({ vat, vatRegistered }) {
  if (!vat) return null;
  const tone = {
    below_threshold:                { fg: '#86efac', bg: 'rgba(34,197,94,0.10)',  bd: 'rgba(34,197,94,0.28)',  headline: 'Well below the VAT threshold' },
    approaching_registration:       { fg: '#fbbf24', bg: 'rgba(251,191,36,0.10)', bd: 'rgba(251,191,36,0.28)', headline: `£${vat.distanceToRegistration.toLocaleString()} to the VAT threshold` },
    over_threshold_unregistered:    { fg: '#fca5a5', bg: 'rgba(220,38,38,0.10)',  bd: 'rgba(220,38,38,0.28)',  headline: 'Above the VAT threshold — register within 30 days' },
    above_registered:               { fg: '#86efac', bg: 'rgba(34,197,94,0.10)',  bd: 'rgba(34,197,94,0.28)',  headline: 'Registered and trading comfortably above threshold' },
    near_dereg:                     { fg: '#fbbf24', bg: 'rgba(251,191,36,0.10)', bd: 'rgba(251,191,36,0.28)', headline: 'Trading below deregistration threshold — worth reviewing' },
  }[vat.status];
  if (!tone) return null;

  const copy = {
    below_threshold:             'Turnover is safely below the £90,000 mandatory registration threshold. No VAT action required.',
    approaching_registration:    'If this year\'s trajectory continues, VAT registration is likely inside the next 12 months. Model the pricing impact in Business Lab before it becomes forced.',
    over_threshold_unregistered: 'HMRC requires registration when your rolling 12-month turnover exceeds £90,000. Late registration attracts penalties — priority action.',
    above_registered:            'Full compliance posture — no threshold-adjacent concerns to flag.',
    near_dereg:                  'You can voluntarily deregister when turnover drops below £88,000. Only worth doing if the admin overhead exceeds any input VAT you\'re reclaiming.',
  }[vat.status];

  return (
    <div className="rounded-xl p-4 mt-2" style={{ background: tone.bg, border: `1px solid ${tone.bd}` }}>
      <p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: tone.fg }}>Threshold check</p>
...
function TeamSection({ data, toneMode = 'internal', entityType }) {
  ...
  if (!team || totalHeads === 0) {
    return (
      <div className="space-y-5">
        <div className="rounded-xl p-6" style={{ background: 'rgba(126,163,255,0.10)', border: '1px solid rgba(126,163,255,0.28)' }}>
          <p className="text-[10px] font-bold uppercase tracking-widest text-[#7ea3ff]">Single-operator business</p>
          <p className="text-lg font-bold text-white mt-1">You're the whole team — and that's fine.</p>
          <p className="text-sm text-white/70 mt-2 leading-relaxed">
            Most successful cleaning businesses in the UK are single-operator for their first 2–3 years. The section below gives you capacity insights from the work you've delivered this year.
          </p>
        </div>

        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <RStat label="Jobs delivered"   value={(data.jobsCompleted ?? 0).toLocaleString()} accent="text-white" />
          <RStat label="Revenue per job"  value={fmt(data.avgJobValue)}                       accent="text-[#10b981]" />
          <RStat label="Jobs per month"   value={Math.round((data.jobsCompleted ?? 0) / 12)}  accent="text-white" />
          <RStat label="Revenue solo"     value={fmt(data.income)}                            accent="text-white" sub="single-operator run rate" />
        </div>

        <div className="rounded-xl border border-white/10 bg-white/5 p-5">
          <p className="text-sm font-bold text-white mb-2">Ready to hire?</p>
          <p className="text-sm text-white/70 leading-relaxed">
            Model your first employee in <strong className="text-white">Business Lab → Hire a cleaner</strong>. It answers the only question that matters: does this hire make you money, or cost you money?
          </p>
        </div>
      </div>
    );
  ...
  if (toneMode === 'tender') {
    ...
    return (
      <div className="space-y-5">
        <div className="rounded-xl p-6" style={{ background: 'rgba(126,163,255,0.10)', border: '1px solid rgba(126,163,255,0.28)' }}>
          <p className="text-[10px] font-bold uppercase tracking-widest text-[#7ea3ff]">Workforce & compliance</p>
          <p className="text-lg font-bold text-white mt-1">
            {activeTotal} directly-employed staff{team.directors ? ` (${team.directors} director)` : ''}
          </p>
          <p className="text-sm text-white/70 mt-2 leading-relaxed">
            All staff engaged under UK employment contracts. RTW and DBS check registers maintained and available for audit under NDA.
          </p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <RStat label="Active headcount"       value={activeTotal}         accent="text-white" />
          <RStat label="DBS cover (current)"    value={`${team.dbsCovered}/${activeTotal}`} accent={dbsPct === 100 ? 'text-emerald-400' : 'text-amber-400'} sub={`${dbsPct}%`} />
          <RStat label="Right-to-work verified" value={`${team.rtwCovered}/${activeTotal}`} accent={rtwPct === 100 ? 'text-emerald-400' : 'text-amber-400'} sub={`${rtwPct}%`} />
        </div>
      </div>
    );
  }
  ...
  const holidayPct = team.holidayEntitlementHours > 0
  ...
  return (
    <div className="space-y-5">
      ...
      {toneMode === 'funding' && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="rounded-xl border p-5" style={{ background: 'rgba(34,197,94,0.10)', borderColor: 'rgba(34,197,94,0.30)' }}>
            <p className="text-[10px] font-bold uppercase tracking-widest text-[#86efac]">Revenue per head</p>
            <p className="text-3xl font-bold text-white tabular-nums mt-1">{team.revenuePerHead != null ? fmt(team.revenuePerHead) : '—'}</p>
            <p className="text-xs text-white/60 mt-1">Total revenue ÷ active heads</p>
          </div>
          <div className="rounded-xl border p-5" style={{ background: 'rgba(126,163,255,0.10)', borderColor: 'rgba(126,163,255,0.30)' }}>
            <p className="text-[10px] font-bold uppercase tracking-widest text-[#7ea3ff]">Payroll efficiency</p>
          ...
          <div className="rounded-xl border p-5" style={{ background: 'rgba(251,191,36,0.10)', borderColor: 'rgba(251,191,36,0.30)' }}>
            <p className="text-[10px] font-bold uppercase tracking-widest text-[#fbbf24]">Turnover rate</p>
            <p className="text-3xl font-bold text-white tabular-nums mt-1">{Math.round(team.turnoverRate)}%</p>
            <p className="text-xs text-white/60 mt-1">
              {team.turnoverRate <= 15  ? 'Below sector average'
                : team.turnoverRate <= 25 ? 'In line with sector'
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        ...
        <RStat label="Leavers"        value={team.leavers}        accent={team.leavers > 0 ? 'text-amber-400' : 'text-white'} />
        <RStat label="Turnover"       value={`${Math.round(team.turnoverRate)}%`} accent={team.turnoverRate <= 15 ? 'text-emerald-400' : team.turnoverRate <= 25 ? 'text-amber-400' : 'text-red-400'} />
      </div>
      ...
      <Card className="overflow-hidden">
        <div className="px-4 py-3 border-b border-[rgba(153,197,255,0.08)] flex items-center justify-between">
          ...
          <span className="text-[10px] text-white/40">
            {team.rtiPayslipsCount} payslip{team.rtiPayslipsCount === 1 ? '' : 's'} submitted this period
          </span>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-4">
          <RStat label="Gross pay"          value={fmt(team.payrollGross)}     accent="text-white" />
          <RStat label="Employer NI"        value={fmt(team.niEmployer)}       accent="text-white" sub="cost to the business" />
          <RStat label="Total employer cost" value={fmt(team.payrollTotal)}    accent="text-amber-400" sub="gross + employer NI" />
          <RStat label="PAYE tax deducted"  value={fmt(team.payrollTaxPeriod)} accent="text-white" sub="submitted to HMRC via FPS" />
        </div>
        {isLtd && team.directorPayGross > 0 && (
          <div className="border-t border-white/5 px-4 py-4 bg-white/[0.02]">
            <p className="text-[10px] font-bold uppercase tracking-widest text-[#86efac] mb-2">Director salary</p>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              <RStat label="Director gross pay" value={fmt(team.directorPayGross)}           accent="text-white" sub={`${team.directors} director${team.directors === 1 ? '' : 's'}`} />
              <RStat label="As % of payroll"    value={team.payrollGross > 0 ? `${Math.round((team.directorPayGross/team.payrollGross)*100)}%` : '—'} accent="text-white" />
              <RStat label="Staff gross pay"    value={fmt(team.payrollGross - team.directorPayGross)} accent="text-white" sub={`${team.staffActive} staff`} />
            </div>
          </div>
        )}
      </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        <Card className="overflow-hidden">
          <div className="px-4 py-3 border-b border-[rgba(153,197,255,0.08)]"><SL>Efficiency</SL></div>
          <div className="grid grid-cols-2 gap-3 p-4">
            <RStat label="Revenue per head"   value={team.revenuePerHead != null ? fmt(team.revenuePerHead) : '—'} accent="text-[#10b981]" sub={`${activeTotal} active`} />
            <RStat label="Avg cost per head"  value={team.avgCostPerHead  != null ? fmt(team.avgCostPerHead)  : '—'} accent="text-white" sub="employer-side" />
            <RStat label="Subcontractor spend" value={fmt(team.subcontractorSpend ?? 0)}                            accent="text-white" sub="approximate — category-based" />
            <RStat label="Sub spend as % of payroll" value={team.payrollGross > 0 ? `${Math.round(((team.subcontractorSpend ?? 0) / team.payrollGross) * 100)}%` : '—'} accent="text-white" />
          </div>
        </Card>
        ...
        <Card className="overflow-hidden">
          ...
          <div className="grid grid-cols-2 gap-3 p-4">
            <RStat label="Holiday taken"       value={`${team.holidayHoursTaken} hrs`} accent="text-white" sub={holidayPct != null ? `${holidayPct}% of entitlement` : ''} />
            <RStat label="Total entitlement"   value={`${team.holidayEntitlementHours} hrs`} accent="text-white" sub={`${team.hrPolicyWeeks} weeks/yr${team.bankHolidaysOnTop ? ' + bank hols' : ''}`} />
            <RStat label="Sickness hours"      value={`${team.sicknessHoursTaken} hrs`} accent={team.sicknessHoursTaken > 0 ? 'text-amber-400' : 'text-white'} />
            <RStat label="Sickness as % of contracted" value={team.holidayEntitlementHours > 0 ? `${Math.round((team.sicknessHoursTaken / (team.holidayEntitlementHours / (team.hrPolicyWeeks || 5.6) * 52)) * 100) / 100}%` : '—'} accent="text-white" />
          </div>
        </Card>
      </div>
      ...
      <Card className="overflow-hidden">
        <div className="px-4 py-3 border-b border-[rgba(153,197,255,0.08)]"><SL>Compliance cover</SL></div>
        <div className="grid grid-cols-2 gap-3 p-4">
          <RStat label="DBS check current" value={`${team.dbsCovered}/${activeTotal}`} accent={team.dbsCovered === activeTotal ? 'text-emerald-400' : 'text-amber-400'} sub={activeTotal > 0 ? `${Math.round((team.dbsCovered / activeTotal) * 100)}% coverage` : ''} />
          <RStat label="Right-to-work verified" value={`${team.rtwCovered}/${activeTotal}`} accent={team.rtwCovered === activeTotal ? 'text-emerald-400' : 'text-amber-400'} sub={activeTotal > 0 ? `${Math.round((team.rtwCovered / activeTotal) * 100)}% coverage` : ''} />
        </div>
      </Card>

      <p className="text-[10px] text-white/40 text-center leading-relaxed max-w-2xl mx-auto">
        Payroll figures aggregate finalised payslips only (drafts excluded). Subcontractor spend uses category-text matching on money entries — approximate. Turnover = leavers ÷ average headcount.
      </p>
    </div>
  );
}
...
function OperationsSection({ data, toneMode = 'internal' }) {
  ...
  if (!ops || ops.scheduled === 0) {
    return (
      <div className="space-y-5">
        <div className="rounded-xl p-6" style={{ background: 'rgba(126,163,255,0.10)', border: '1px solid rgba(126,163,255,0.28)' }}>
          <p className="text-[10px] font-bold uppercase tracking-widest text-[#7ea3ff]">Not enough data yet</p>
          <p className="text-lg font-bold text-white mt-1">Log job completions to populate this section</p>
          <p className="text-sm text-white/70 mt-2 leading-relaxed">
            §8 pulls on-time %, photo coverage, cancellations and duration variance from your Scheduler + Work Completion tabs. Nothing here means no scheduled work in the review period — or the jobs weren't marked complete.
          </p>
        </div>
      </div>
    );
  ...
  if (toneMode === 'tender') {
    ...
    return (
      <div className="space-y-5">
        <div className="rounded-xl p-6" style={{ background: 'rgba(126,163,255,0.10)', border: '1px solid rgba(126,163,255,0.28)' }}>
          <p className="text-[10px] font-bold uppercase tracking-widest text-[#7ea3ff]">Delivery reliability</p>
          <p className="text-lg font-bold text-white mt-1">
            {ops.completed.toLocaleString()} engagement{ops.completed === 1 ? '' : 's'} delivered — {onTimeStr} on-time
          </p>
          <p className="text-sm text-white/70 mt-2 leading-relaxed">
            All completions timestamped and evidenced. Full route logs, photo evidence and mobilisation history available under NDA.
          </p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <RStat label="On-time delivery"    value={onTimeStr}                                                       accent={ops.onTimePct != null && ops.onTimePct >= 90 ? 'text-emerald-400' : ops.onTimePct != null && ops.onTimePct >= 80 ? 'text-amber-400' : 'text-white'} sub={ops.onTimeSampleSize ? `${ops.onTimeSampleSize} tracked visits` : ''} />
          <RStat label="Photo evidence"      value={ops.photoCoveragePct != null ? `${Math.round(ops.photoCoveragePct)}%` : '—'} accent={ops.photoCoveragePct != null && ops.photoCoveragePct >= 90 ? 'text-emerald-400' : 'text-white'} sub="jobs with proof-of-work photos" />
          <RStat label="Complaints"          value={complaintStr}                                                    accent={ops.complaintCount == null ? 'text-white/50' : ops.complaintCount === 0 ? 'text-emerald-400' : 'text-amber-400'} sub={ops.complaintCount == null ? 'complaint tracking not enabled' : 'across the review period'} />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      ...
      {toneMode === 'funding' && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="rounded-xl border p-5" style={{ background: 'rgba(34,197,94,0.10)', borderColor: 'rgba(34,197,94,0.30)' }}>
            <p className="text-[10px] font-bold uppercase tracking-widest text-[#86efac]">On-time delivery</p>
            <p className="text-3xl font-bold text-white tabular-nums mt-1">{ops.onTimePct != null ? `${Math.round(ops.onTimePct)}%` : '—'}</p>
            <p className="text-xs text-white/60 mt-1">Completed on the scheduled date{ops.onTimeSampleSize ? ` · n=${ops.onTimeSampleSize}` : ''}</p>
          </div>
          <div className="rounded-xl border p-5" style={{ background: 'rgba(126,163,255,0.10)', borderColor: 'rgba(126,163,255,0.30)' }}>
            <p className="text-[10px] font-bold uppercase tracking-widest text-[#7ea3ff]">Job tracking discipline</p>
            <p className="text-3xl font-bold text-white tabular-nums mt-1">{ops.captureRatePct != null ? `${Math.round(ops.captureRatePct)}%` : '—'}</p>
            <p className="text-xs text-white/60 mt-1">Scheduled jobs marked complete</p>
          </div>
          <div className="rounded-xl border p-5" style={{ background: 'rgba(251,191,36,0.10)', borderColor: 'rgba(251,191,36,0.30)' }}>
            <p className="text-[10px] font-bold uppercase tracking-widest text-[#fbbf24]">Recurring revenue base</p>
            <p className="text-3xl font-bold text-white tabular-nums mt-1">{Math.round(ops.recurringPct)}%</p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <RStat label="Completed"    value={ops.completed.toLocaleString()} accent="text-emerald-400" sub={ops.captureRatePct != null ? `${Math.round(ops.captureRatePct)}% capture` : ''} />
        <RStat label="Cancellations" value={ops.cancelled}                 accent={cancelPct > 5 ? 'text-amber-400' : 'text-white'} sub={ops.scheduled > 0 ? `${cancelPct}%` : ''} />
        <RStat label="No-shows"     value={ops.noShow}                     accent={noShowPct > 2 ? 'text-red-400' : ops.noShow > 0 ? 'text-amber-400' : 'text-white'} sub={ops.scheduled > 0 ? `${noShowPct}%` : ''} />
      </div>

      {/* Delivery quality — on-time + duration + photo */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        <Card className="overflow-hidden">
          <div className="px-4 py-3 border-b border-[rgba(153,197,255,0.08)]"><SL>Delivery quality</SL></div>
          <div className="grid grid-cols-2 gap-3 p-4">
            <RStat label="On-time %"       value={ops.onTimePct != null ? `${Math.round(ops.onTimePct)}%` : '—'} accent={ops.onTimePct != null && ops.onTimePct >= 90 ? 'text-emerald-400' : ops.onTimePct != null && ops.onTimePct >= 80 ? 'text-amber-400' : 'text-red-400'} sub={ops.onTimeSampleSize ? `n=${ops.onTimeSampleSize} tracked` : 'no schedule/complete pair'} />
            <RStat label="Duration variance" value={ops.durationVariancePct != null ? `${ops.durationVariancePct >= 0 ? '+' : ''}${Math.round(ops.durationVariancePct)}%` : '—'} accent={ops.durationVariancePct == null ? 'text-white' : Math.abs(ops.durationVariancePct) <= 10 ? 'text-emerald-400' : 'text-amber-400'} sub="actual vs scheduled" />
            <RStat label="Avg actual duration" value={ops.avgActualDurationMins != null ? `${ops.avgActualDurationMins} min` : '—'} accent="text-white" />
            <RStat label="Avg scheduled duration" value={ops.avgScheduledDurationMins != null ? `${ops.avgScheduledDurationMins} min` : '—'} accent="text-white/70" />
          </div>
        </Card>

        <Card className="overflow-hidden">
          <div className="px-4 py-3 border-b border-[rgba(153,197,255,0.08)]"><SL>Evidence & recurring split</SL></div>
          <div className="grid grid-cols-2 gap-3 p-4">
            <RStat label="Photo coverage"     value={ops.photoCoveragePct != null ? `${Math.round(ops.photoCoveragePct)}%` : '—'} accent={ops.photoCoveragePct != null && ops.photoCoveragePct >= 80 ? 'text-emerald-400' : ops.photoCoveragePct != null && ops.photoCoveragePct >= 50 ? 'text-amber-400' : 'text-white'} sub="jobs with ≥1 photo" />
            <RStat label="Avg photos / job"   value={ops.avgPhotosPerJob}                                                         accent="text-white" sub={`${ops.jobsWithPhotos} evidenced`} />
            <RStat label="Recurring"          value={ops.recurringCount}                                                          accent="text-white" sub={`${Math.round(ops.recurringPct)}% of completions`} />
            <RStat label="One-off"            value={ops.oneOffCount}                                                              accent="text-white" sub={`${Math.round(100 - ops.recurringPct)}% of completions`} />
          </div>
        </Card>
      </div>
      ...
      {(ops.complaintCount == null || ops.npsScore == null) && (
        <div className="rounded-xl border border-white/10 bg-white/[0.04] p-4">
          <p className="text-[10px] font-bold uppercase tracking-widest text-white/60">Customer feedback signal</p>
          <p className="text-sm text-white/70 mt-1 leading-relaxed">
            Complaint count and NPS aren't captured yet. A future <em>Feedback</em> module will let you tag jobs with complaints and send end-of-visit ratings — the numbers will land in this card automatically once it ships.
          </p>
        </div>
      )}

      <p className="text-[10px] text-white/40 text-center leading-relaxed max-w-2xl mx-auto">
        On-time = completion within 1 day of the scheduled date. Duration variance = actual vs scheduled hours. Capture rate = scheduled jobs that received a completion timestamp — low capture makes every other metric less reliable.
      </p>
    </div>
  );
}
...
function RiskSection({ data, toneMode = 'internal', entityType }) {
  const { risks, overall } = useMemo(
    () => buildRiskRegister(data, entityType),
    [data, entityType],
  );
  ...
  if (toneMode === 'tender') {
    return (
      <div className="space-y-5">
        <div className="rounded-xl p-6" style={{ background: 'rgba(126,163,255,0.10)', border: '1px solid rgba(126,163,255,0.28)' }}>
          <p className="text-[10px] font-bold uppercase tracking-widest text-[#7ea3ff]">Compliance & risk posture</p>
          <p className="text-lg font-bold text-white mt-1">All statutory obligations current</p>
          <p className="text-sm text-white/70 mt-2 leading-relaxed">
            The items below are the compliance surfaces buyers usually audit at pre-qualification. Financial / commercial risk register available under NDA.
          </p>
        </div>
        {sortedRisks.length === 0 ? (
          ...
          <div className="space-y-2">
            {sortedRisks.map(r => <RiskRow key={r.id} risk={r} />)}
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="space-y-5">
      {/* Overall banner */}
      <div className="rounded-xl p-5" style={{ background: overallLabel.bg, border: `1px solid ${overallLabel.bd}` }}>
        <div className="flex items-start gap-4">
          <div style={{ background: `${overallLabel.fg}22`, color: overallLabel.fg, fontSize: 22, fontWeight: 900, lineHeight: 1, minWidth: 68, textAlign: 'center', padding: '10px 8px', borderRadius: 10 }}>
            {overallLabel.chip}
          </div>
          <div className="flex-1">
            <p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: overallLabel.fg }}>Overall risk posture</p>
      ...
      {sortedRisks.some(r => r.source.startsWith('lockedByBL:')) && (
        <div className="rounded-xl border border-white/10 bg-white/[0.04] p-4">
          <p className="text-[10px] font-bold uppercase tracking-widest text-white/60">Deeper scoring available via Business Lab</p>
          <p className="text-sm text-white/70 mt-1 leading-relaxed">
            The rows marked "Run BL module" would be scored properly once you complete the relevant Business Lab certification (Financial Resilience for cash / debtor risk; Compliance & Risk for insurance and supplier concentration). Certifying attaches your dated decision memo — adds enormous credibility in a funding pack.
          </p>
        </div>
      )}

      <p className="text-[10px] text-white/40 text-center leading-relaxed max-w-2xl mx-auto">
        This register is a live derivation, not an audit. It's designed to make gaps obvious rather than to reassure — the strongest signal for a funder is that you've thought about what could kill the business.
      </p>
    </div>
  );
}
...
const OVERALL_LABEL = {
  high:    { chip: 'HIGH',   fg: '#fca5a5', bg: 'rgba(220,38,38,0.10)',  bd: 'rgba(220,38,38,0.28)',  headline: 'Active risks need addressing before scaling' },
  medium:  { chip: 'MED',    fg: '#fbbf24', bg: 'rgba(251,191,36,0.10)', bd: 'rgba(251,191,36,0.28)', headline: 'Some risks worth actively managing' },
  low:     { chip: 'LOW',    fg: '#86efac', bg: 'rgba(34,197,94,0.10)',  bd: 'rgba(34,197,94,0.28)',  headline: 'Well-managed risk profile' },
  unknown: { chip: 'TBC',    fg: '#94a3b8', bg: 'rgba(148,163,184,0.10)',bd: 'rgba(148,163,184,0.28)',headline: 'Not enough data to score yet' },
};

const SEVERITY_TONE = {
  high:    { fg: '#fca5a5', bg: 'rgba(220,38,38,0.14)',  bd: 'rgba(220,38,38,0.35)',  label: 'High'    },
  medium:  { fg: '#fbbf24', bg: 'rgba(251,191,36,0.14)', bd: 'rgba(251,191,36,0.35)', label: 'Medium'  },
  low:     { fg: '#86efac', bg: 'rgba(34,197,94,0.14)',  bd: 'rgba(34,197,94,0.35)',  label: 'Low'     },
  unknown: { fg: '#94a3b8', bg: 'rgba(148,163,184,0.14)',bd: 'rgba(148,163,184,0.35)',label: 'Not scored' },
};
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        <span className="text-lg shrink-0" title={risk.category}>{CATEGORY_ICON[risk.category] ?? '•'}</span>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-1">
            ...
            {blLocked && (
              <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full border shrink-0" style={{ color: '#7ea3ff', background: 'rgba(126,163,255,0.14)', borderColor: 'rgba(126,163,255,0.35)' }}>
                Run BL module
              </span>
            )}
          </div>
...
function PlanSection({ plan, setPlan, toneMode = 'internal', data, businessName, entityType }) {
  const updateField = (field, value) => setPlan(p => ({ ...p, [field]: value, authoredAt: new Date().toISOString() }));
  const updateGoal  = (idx, field, value) => setPlan(p => ({
    ...p,
    ...
    return (
      <div className="space-y-5">
        <div
          ...
          style={{ background: 'rgba(126,163,255,0.12)', border: '1px solid rgba(126,163,255,0.32)' }}
        >
          <div className="flex items-start gap-4">
            <div style={{ background: 'rgba(126,163,255,0.25)', color: '#7ea3ff', fontSize: 22, minWidth: 68, textAlign: 'center', padding: '10px 8px', borderRadius: 10 }}>
              🔒
            </div>
            <div className="flex-1">
              <p className="text-[10px] font-bold uppercase tracking-widest text-[#7ea3ff]">Section locked</p>
              <p className="text-xl font-bold text-white mt-1">
                The Plan — Next 12 Months
              </p>
              <p className="text-sm text-white/70 mt-2 leading-relaxed">
                This is the section that turns your review from a report into a planning document. Per the AR framework, a certified <strong className="text-white">Business Lab — Strategy & 12-Month Plan</strong> submission is required to file it.
              </p>
              <p className="text-sm text-white/70 mt-3 leading-relaxed">
                The BL module forces you to model the maths behind each goal (revenue targets, hire economics, capex payback) so nothing here is aspirational. Output is a dated, signed plan you can attach to any funding pack or contract bid.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
          {[1, 2, 3].map(n => (
            <div key={n} className="rounded-xl border border-white/10 p-5 opacity-40">
              <p className="text-[10px] font-bold uppercase tracking-widest text-white/60">Goal {n}</p>
        ...
        <div className="rounded-xl border border-white/10 bg-white/[0.04] p-5">
          <p className="text-[10px] font-bold uppercase tracking-widest text-white/60 mb-2">Also captured</p>
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 pt-2">
          <button
            type="button"
            onClick={() => alert('Business Lab modules ship in a later slice — for now use the bypass to write your plan uncertified.')}
  ...
  if (toneMode === 'tender') {
    return (
      <div className="space-y-5">
        <div className="rounded-xl p-6" style={{ background: 'rgba(126,163,255,0.10)', border: '1px solid rgba(126,163,255,0.28)' }}>
          <p className="text-[10px] font-bold uppercase tracking-widest text-[#7ea3ff]">Forward capability</p>
          <p className="text-lg font-bold text-white mt-1">Where {businessName} is heading in the next 12 months</p>
          <p className="text-sm text-white/70 mt-2 leading-relaxed">
  ...
  return (
    <div className="space-y-5">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-white/70 mt-1 leading-relaxed">
                You bypassed the BL Strategy module — this plan renders in the AR but won't carry BL certification when you file. Certify to unlock the "dated, signed plan" line in your Appendix.
              </p>
            </div>
            <button
              type="button"
      ...
      {toneMode === 'funding' && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div
            ...
            style={{ background: 'rgba(34,197,94,0.10)', borderColor: 'rgba(34,197,94,0.30)' }}
          >
            <p className="text-[10px] font-bold uppercase tracking-widest text-[#86efac]">Income target — next 12 months</p>
            ...
            <p className="text-xs text-white/60 mt-2">
              ...
              {plan.incomeTarget && lastYearIncome > 0 && (
                <> · <span className={Number(plan.incomeTarget) > lastYearIncome ? 'text-emerald-300' : 'text-amber-300'}>
                  {Number(plan.incomeTarget) > 0 ? `+${Math.round((Number(plan.incomeTarget) / lastYearIncome - 1) * 100)}%` : ''}
                </span></>
              )}
            </p>
          </div>
          <div
            ...
            style={{ background: 'rgba(126,163,255,0.10)', borderColor: 'rgba(126,163,255,0.30)' }}
          >
            <p className="text-[10px] font-bold uppercase tracking-widest text-[#7ea3ff]">Expected position — end of next year</p>
            <textarea
      ...
      <div>
        <p className="text-[10px] font-bold uppercase tracking-widest text-white/60 mb-2 pl-1">Three strategic goals</p>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
          {plan.goals.map((g, i) => (
            <div
              key={g.id}
            ...
            >
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 rounded-full bg-[#10b981] text-white flex items-center justify-center text-xs font-black">{i + 1}</div>
                <p className="text-[10px] font-bold uppercase tracking-widest text-white/60">Goal {i + 1}</p>
              </div>
              <input
                type="text"
                value={g.title}
                onChange={e => updateGoal(i, 'title', e.target.value)}
                placeholder={i === 0 ? "e.g. Land 3 recurring commercial contracts" : i === 1 ? "e.g. Hire second cleaner" : "e.g. Launch end-of-tenancy service"}
              ...
              />
              <div>
                <p className="text-[10px] font-bold uppercase tracking-widest text-white/50 mb-1">Math behind it</p>
                <input
                  type="text"
                  value={g.mathLabel}
      ...
      <Card className="overflow-hidden border-t-2 border-t-brand-blue">
        ...
        <div className="p-5">
          ...
          <div className="h-2.5 bg-[rgba(153,197,255,0.06)] rounded-full overflow-hidden mb-2">
            <div className="h-full bg-[#10b981] rounded-full" style={{ width:`${Math.min((data.income/incomeTarget)*100,100)}%` }} />
          </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              >
                ...
                <p className={`text-xs mt-0.5 ${active?"text-[#86efac]":"text-[rgba(153,197,255,0.4)]"}`}>{label}</p>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="space-y-5">
        ...
        {!audit && !auditLoading && (
          <div className="text-center py-10">
            ...
            <button onClick={generateAudit} className="px-8 py-3 bg-[#064e3b] text-white text-xs font-bold uppercase tracking-widest hover:bg-[#047857] transition-colors rounded-sm">
        ...
        {audit && (
          <div className="space-y-4">
              ...
              <div className="bg-[#064e3b] px-5 py-5 text-white">
                <div className="flex items-start gap-5">
                  <div className={`w-20 h-20 rounded-sm flex items-center justify-center text-4xl font-bold shrink-0 ${
                    ...
                    audit.grade?.startsWith("B") ? "bg-[#10b981] text-white" :
                    "bg-amber-500/100 text-white"
                  }`}>{audit.grade}</div>
                  <div>
                    <p className="text-xs font-bold uppercase tracking-widest text-[#86efac] mb-2">AI audit · {audit.overallScore}/100</p>
            ...
            <Card className="overflow-hidden border-t-2 border-t-brand-navy">
              <div className="px-4 py-3 border-b border-[rgba(153,197,255,0.08)] bg-[#064e3b]/5"><SL>Next sprint — focus areas</SL></div>
              <div className="divide-y divide-gray-100">
                {audit.nextSprintFocus?.map((item,i) => (
                  <div key={i} className="flex items-start gap-3 px-4 py-3">
                    <div className="w-6 h-6 rounded-sm bg-[#064e3b] text-white flex items-center justify-center text-xs font-bold shrink-0">{i+1}</div>
                    <p className="text-sm text-white">{item}</p>
                  </div>
                ))}
              </div>
            ...
            <button onClick={() => { setAudit(null); }} className="w-full py-2.5 border border-[rgba(153,197,255,0.12)] text-[rgba(153,197,255,0.6)] text-xs font-bold uppercase rounded-sm hover:border-[#10b981] hover:text-[#10b981] transition-colors">Re-run audit</button>
          </div>
        )}
      </div>
    );
      ...
      <div className="space-y-5">
        <Card className="overflow-hidden border-t-2 border-t-brand-blue">
          <div className="p-8 text-center">
            ...
            <button onClick={() => setSprintActive(true)} className="px-8 py-3 bg-[#064e3b] text-white text-sm font-bold uppercase tracking-widest hover:bg-[#047857] transition-colors rounded-sm">
              🚀 Start sprint
            </button>
          </div>
        </Card>
    ...
    <div className="space-y-5">
      ...
      <Card className="overflow-hidden border-t-2 border-t-brand-blue">
        <div className="bg-[#064e3b] px-5 py-4 flex items-center justify-between text-white">
          <div>
            <p className="text-xs font-bold uppercase tracking-widest text-[#86efac]">90-day sprint · active</p>
        ...
        <div className="h-2 bg-[rgba(153,197,255,0.06)]">
          <div className="h-full bg-[#10b981] transition-all duration-500" style={{ width:`${sprintPct}%` }} />
        </div>
            ...
            return (
              <div key={m.week} className={`p-4 ${!unlocked?"opacity-40":""}`}>
                <div className="flex items-center gap-3 mb-2">
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${m.done?"bg-emerald-500/100 text-white":unlocked?"bg-[#10b981] text-white":"bg-gray-200 text-[rgba(153,197,255,0.4)]"}`}>
                    {m.done?"✓":`W${m.week}`}
                  </div>
...
const EMPTY_YEAR_DATA = {
  income: 0, prevIncome: 0, expenses: 0, prevExpenses: 0,
  jobsCompleted: 0, avgJobValue: 0,
  clientsStart: 0, clientsEnd: 0, clientsLost: 0,
  bestMonth: { name: "N/A", value: 0 }, worstMonth: { name: "N/A", value: 0 },
  topClients: [], expenseBreakdown: [],
  monthlyData: [
...
export default function AnnualReviewTab({ accountsData, onNavigate }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setBusinessName(profileRow?.data?.business_name || profileRow?.data?.first_name || 'The business');
  ...
  useEffect(() => {
    ...
    let alive = true;
    getFiledReview(selectedYear).then(row => {
  ...
  return (
    <div className="-mx-4 md:-mx-8 -my-6 flex" style={greenCanvas()}>
      ...
      {/* Sidebar */}
      <aside className="w-56 shrink-0 flex flex-col py-4 overflow-y-auto"
        style={{
        ...
        <div className="px-4 mb-5">
          <p className="text-xs font-bold tracking-widest uppercase text-[#86efac] mb-2">Tax year</p>
          <select
            value={selectedYear}
          ...
          >
            {availableYears.map(y => <option key={y} value={y} className="bg-[#064e3b]">{y}</option>)}
          </select>
        </div>
        ...
        {NAV.map(n => (
          <button key={n.id} onClick={() => setSection(n.id)}
            ...
            <div>
              ...
              <p className={`text-xs mt-0.5 ${section===n.id?"text-[#86efac]":"text-[#86efac]/40"}`}>{n.sub}</p>
            </div>
          </button>
        ))}
      ...
      <main className="flex-1 overflow-y-auto p-6 pb-28 space-y-5">
          ...
          <div className="relative px-6 md:px-9 py-7 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <div className="inline-flex items-center gap-2 mb-3">
                ...
                <span className="text-[10px] font-black tracking-[0.28em] text-white/70 uppercase">Annual Review · {selectedYear}</span>
              </div>
              <h1 className="text-white mb-2"
                style={{ fontSize: 26, fontWeight: 900, letterSpacing: '-0.02em', lineHeight: 1.15 }}>
                Look back,{' '}
                <span style={{ color: '#86efac' }}>plan forward.</span>
              </h1>
              <p className="text-white/70 text-[14px] leading-relaxed max-w-2xl">
                Numbers, clients, goals, self-assessment — and a 90-day sprint plan for the next chapter.
              </p>
            </div>
            <div className="shrink-0 grid grid-cols-2 gap-2" style={{ minWidth: 220 }}>
              <div style={{ ...glassDark({ padding: 12, radius: 12, strong: true }), textAlign: 'center' }}>
                <div style={{ fontSize: 20, fontWeight: 900, color: '#86efac', lineHeight: 1, letterSpacing: '-0.02em', fontVariantNumeric: 'tabular-nums' }}>
                  {liveLoading && isLive ? '—' : fmt(data.income)}
                </div>
                <div style={{ fontSize: 9, fontWeight: 800, color: 'rgba(255,255,255,0.55)', letterSpacing: '0.12em', textTransform: 'uppercase', marginTop: 5 }}>Income</div>
              </div>
              <div style={{ ...glassDark({ padding: 12, radius: 12, strong: true }), textAlign: 'center' }}>
                <div style={{ fontSize: 20, fontWeight: 900, color: '#ffffff', lineHeight: 1, letterSpacing: '-0.02em', fontVariantNumeric: 'tabular-nums' }}>
                  {liveLoading && isLive ? '—' : `${Math.round(data.margin)}%`}
                </div>
                <div style={{ fontSize: 9, fontWeight: 800, color: 'rgba(255,255,255,0.55)', letterSpacing: '0.12em', textTransform: 'uppercase', marginTop: 5 }}>Margin</div>
              </div>
            </div>
          </div>
          {isLive && (liveLoading || liveError) && (
            <div className="relative px-6 md:px-9 pb-4 flex items-center gap-2 text-xs" style={{ color: 'rgba(255,255,255,0.65)' }}>
              {liveLoading && (
                <>
                  <Loader2 size={12} style={{ animation: 'annualSpin 0.8s linear infinite' }} />
                  <span>Aggregating {currentPeriod?.label ?? 'this year'} from your Money, Jobs and Mileage…</span>
        ...
        {section==="clients"  && <ClientsSection data={data} highlights={highlights} setHighlights={setHighlights} toneMode={toneMode} />}
        {section==="services" && <ServicesSection data={data} toneMode={toneMode} vatRegistered={vatRegistered} />}
        {section==="money"    && <MoneySection    data={data} toneMode={toneMode} entityType={entityType} vatRegistered={vatRegistered} />}
        {section==="tax"      && <TaxSection      data={data} toneMode={toneMode} />}
        {section==="team"     && <TeamSection     data={data} toneMode={toneMode} entityType={entityType} />}
        {section==="ops"      && <OperationsSection data={data} toneMode={toneMode} />}
        {section==="risk"     && <RiskSection       data={data} toneMode={toneMode} entityType={entityType} />}
        {section==="plan"     && <PlanSection       plan={plan} setPlan={setPlan} toneMode={toneMode} data={data} businessName={businessName} entityType={entityType} />}
