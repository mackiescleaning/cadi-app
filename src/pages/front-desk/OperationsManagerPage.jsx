...
export default function OperationsManagerPage() {
  ...
  const { canUseOperationsManager, priceMonthly } = usePlan();
  ...
  useEffect(() => {
    ...
    Promise.all([
      supabase
        ...
        .maybeSingle(),
      supabase.from('autobooking_settings').select('*').eq('business_id', businessId).maybeSingle(),
    ])
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      {fromPhase3 && !introDismissed && !showFreePreview && (
        ...
        >
          <p className="text-[10px] font-bold tracking-[0.2em] uppercase text-amber-300/50 mb-2">
            Phase 3 · Step 3
          </p>
          <h2 className="text-xl font-black text-white mb-3 leading-snug">
            Meet your Operations Manager.
          </h2>
          <p className="text-sm text-white/60 leading-relaxed mb-3">
            The third agent at your Front Desk — and probably the one that'll change your week the
            most.
          </p>
          ...
          <p className="text-sm text-white/60 leading-relaxed mb-4">
            Most cleaning software makes you tick off every completed job by hand. Your Operations
            Manager assumes jobs happened and only flags you when something didn't go to plan.
          </p>
        ...
        <div className="rounded-2xl border border-amber-200/30 bg-white overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            <p className="text-sm font-bold text-[#010a4f]">
              What Pro's Operations Manager looks like in practice
            </p>
            <button
          ...
          <div className="p-6 space-y-4">
            ...
            <p className="text-sm text-gray-600 leading-relaxed">
              That's Operations Manager working. The system that finally makes "set and forget"
              actually true for your cleaning business.
            </p>
      ...
      <div className="rounded-2xl overflow-hidden border border-[#99c5ff]/20">
        <div
          className="px-6 py-5"
          style={{ background: 'linear-gradient(135deg, #78350f 0%, #C2410C 100%)' }}
        >
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[10px] font-bold tracking-[0.2em] uppercase text-orange-200/70 mb-1">
                Front Desk · Agent
              </p>
              <h1 className="text-xl font-black text-white">Operations Manager</h1>
              <p className="text-sm text-white/60 mt-1">
                Runs your schedule, reminders, and payment matching — so you stop manually ticking
                off jobs.
              </p>
            </div>
            {!canUseOperationsManager && (
        ...
        <div className="bg-white px-6 py-4 flex items-center justify-between border-t border-[#99c5ff]/20">
          {loading ? (
            ...
            <>
              <div className="flex items-center gap-2">
                <span
                  className={`w-2 h-2 rounded-full ${omEnabled ? 'bg-green-500' : 'bg-gray-300'}`}
                />
                <span
                  className={`text-sm font-semibold ${omEnabled ? 'text-green-700' : 'text-gray-500'}`}
                >
      ...
      <div>
        <p className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-3">
          What your Operations Manager does
        </p>
