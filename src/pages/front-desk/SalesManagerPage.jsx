...
function SetupChecklist({ setupStep, hasChats, navigate }) {
  ...
  return (
    <div
      ...
      style={fdCard({ radius: 18, gold: !allStepsDone })}
    >
      <div
        className="px-5 py-4 flex items-center justify-between gap-3"
        style={{ borderBottom: `1px solid ${ON_DARK.line}` }}
      >
        <div className="flex items-center gap-2.5">
          <div
            className="w-1.5 h-4 rounded-full"
            style={{ background: FD_GOLD, boxShadow: `0 0 8px 2px ${FD_GOLD}88` }}
          />
                    ...
                    : done
                      ...
                      : isNext
                        ...
                        : {
                            ...
                            border: `1.5px solid ${ON_DARK.lineHi}`,
                          }
                }
              >
                {isFinalAndUnconfirmed ? (
                  ...
                  <Circle
                    size={7}
                    fill={isNext ? FD_SKY : 'transparent'}
                    style={{ color: isNext ? FD_SKY : ON_DARK.faint }}
                  />
                )}
              </div>
              <div className="flex-1 min-w-0">
                <p
                  className={`text-sm font-bold ${done && !isFinalAndUnconfirmed ? 'text-white/70' : 'text-white'}`}
                >
                  {step.label}
                </p>
                <p
                  className="text-xs mt-0.5"
                  style={{ color: isFinalAndUnconfirmed ? FD_GOLD : ON_DARK.muted }}
                >
                  {isFinalAndUnconfirmed
                    ? "Snippet not detected yet — we haven't seen a chat come through"
                    : step.desc}
                </p>
              </div>
              {(isNext || isFinalAndUnconfirmed) && (
                <ArrowRight
                  size={13}
                  style={{ color: isFinalAndUnconfirmed ? FD_GOLD : FD_SKY }}
                  className="shrink-0"
                />
              )}
            </button>
      ...
      {!allStepsDone && (
        <div className="px-5 pb-4">
          ...
          >
            {setupStep === 0 ? 'Get started — 2 minutes' : 'Continue setup'}{' '}
            <ArrowRight size={14} />
          </button>
        </div>
      )}
    ...
    <div style={fdCanvas()} className="-mx-4 md:-mx-8 -mt-6 -mb-24 md:-mb-6">
      <div className="max-w-3xl mx-auto px-4 md:px-8 py-8 space-y-6">
        ...
        {fromPhase3 && !introDismissed && (
          ...
          >
            <p className="text-[10px] font-bold tracking-[0.2em] uppercase text-[#99c5ff]/50 mb-2">
              Phase 3 · Step 1
            </p>
            ...
            <p className="text-sm text-white/60 leading-relaxed mb-4">
              This is the agent at your Front Desk who handles incoming enquiries — quoting,
              booking, answering questions while you're working.
            </p>
            <p className="text-sm text-white/60 leading-relaxed mb-5">
              Front Desk gets the enquiry. Your Sales Manager replies using your services menu and
              your tone of voice. New work books itself in without you lifting a finger.
            </p>
        ...
        {setupStep !== null && (
          <SetupChecklist
            setupStep={setupStep}
            hasChats={(stats.conversations ?? 0) > 0}
            navigate={navigate}
          />
        )}
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <p className="text-[10px] font-bold tracking-[0.2em] uppercase text-[#99c5ff]/70">
                    Front Desk · Agent
                  </p>
                </div>
