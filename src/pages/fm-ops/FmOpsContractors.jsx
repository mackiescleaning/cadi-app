        ...
        >
          <div>
            ...
            <div style={{ fontSize: 12, color: ON_DARK.secondary, marginTop: 5, lineHeight: 1.5 }}>
              Drop in a CSV from the FM client. Each row becomes a Cadi Connect invite — your subs
              sign up free under your unlock.
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {!parsed && (
          <>
            ...
            >
              ...
              <div
                style={{ fontSize: 14, fontWeight: 800, color: ON_DARK.primary, marginBottom: 4 }}
              >
                Drop CSV here, or click to choose
              </div>
              <div style={{ fontSize: 11, color: ON_DARK.muted }}>
                Columns: company · contact · email · phone · region · trades
              </div>
            </div>
            <input
              ref={fileRef}
            ...
            >
              <strong style={{ color: POP.blue }}>Email is required.</strong> Subs claim their
              account through the link in the invite email — phone-only rows are rejected.
            </div>
          </>
        )}
          ...
          <>
            ...
            >
              ...
              <KpiCard
                label="Skipped"
                value={skipCount}
                pop={skipCount ? POP.amber : ON_DARK.faint}
              />
            </div>
            <div
              style={{
            ...
            >
              ...
              {previewRows.length === 0 && (
                <div style={{ padding: 14, fontSize: 11, color: ON_DARK.muted }}>
                  No rows parsed. Check the CSV headers — first row must contain column names.
                </div>
              )}
              {previewRows.map((r, i) => (
                <div
                  ...
                  style={{
                    ...
                    gridTemplateColumns: '1.4fr 1.4fr 0.8fr 0.6fr',
                    padding: '8px 12px',
                    borderBottom: i < previewRows.length - 1 ? `1px solid ${ON_DARK.line}` : 'none',
                    fontSize: 11,
                ...
                >
                  ...
                  <div
                    style={{
                      color: r.email ? ON_DARK.secondary : ON_DARK.faint,
                      overflow: 'hidden',
                      textOverflow: 'ellipsis',
                      whiteSpace: 'nowrap',
                    }}
            ...
            >
              ...
              <span style={{ color: ON_DARK.muted, fontSize: 11 }}>(recommended)</span>
            </label>
            {error && (
              <div
                style={{
          ...
          <div>
            ...
            >
              ...
              <div>
                <div style={{ fontSize: 14, fontWeight: 800, color: ON_DARK.primary }}>
                  Imported {result.imported ?? 0} contractor
                  {(result.imported ?? 0) === 1 ? '' : 's'}
                </div>
                <div style={{ fontSize: 11, color: ON_DARK.secondary }}>
                  {result.emails_sent ?? 0} invite email{(result.emails_sent ?? 0) === 1 ? '' : 's'}{' '}
                  sent · {result.skipped ?? 0} skipped
                </div>
              </div>
            </div>
            {result.skipped_reasons && (
              ...
              >
                ...
                <KpiCard
                  label="Missing email"
                  value={result.skipped_reasons.missing_contact ?? 0}
                />
    ...
    >
      <span
        style={{
          ...
          borderRadius: 999,
          color,
          background: hex ? `${color}1f` : 'rgba(255,255,255,0.08)',
          border: `1px solid ${hex ? `${color}42` : 'rgba(255,255,255,0.16)'}`,
          whiteSpace: 'nowrap',
        }}
      >
        {label}
...
function ContractorDrawer({ sub, onClose }) {
  ...
  useEffect(() => {
    ...
    import('../../lib/db/fmOpsDb').then((mod) => {
      mod
        .listSubDocsForFm(sub.claimedUserId)
        .then(setDocs)
        .catch(() => setDocs([]))
        .finally(() => setDocsLoading(false));
      mod
        .listSubAvailabilityForFm(sub.claimedUserId)
      ...
      >
        ...
        >
          <div>
            ...
            >
              ...
              {sub.region && (
                <span style={{ fontSize: 10, color: ON_DARK.secondary, fontWeight: 700 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 18 }}>
          <KpiCard
            label="Connect score"
            value={sub.score ?? '—'}
            pop={sub.score ? POP.blue : ON_DARK.faint}
          />
          <KpiCard
            label="Capacity"
            value={sub.capacity ?? '—'}
            pop={sub.capacity ? POP.green : ON_DARK.faint}
          />
        </div>
        ...
        <div style={sectionCard}>
          ...
          {sub.contactName && (
            <div style={{ fontSize: 11, color: ON_DARK.muted, paddingTop: 6 }}>
            ...
            >
              ...
              <div
                style={{
                  fontSize: 15,
                  fontWeight: 900,
                  color: missingCritical
                    ? POP.red
                    : totalOk === FM_DOC_TYPES.length
                      ? POP.green
                      : ON_DARK.primary,
                }}
              >
                {totalOk}
                <span style={{ color: ON_DARK.faint, fontSize: 11, fontWeight: 500 }}>
                  /{FM_DOC_TYPES.length}
                </span>
              </div>
            </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {availability.map((b) => {
                  ...
                  return (
                    ...
                    >
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 12, fontWeight: 700, color: POP.amber }}>
                          {range}
                        </div>
                        {b.reason && (
                          <div
        ...
        >
          {sub.status === 'pending' && (
            <>
              Invited {new Date(sub.invitedAt).toLocaleDateString()} · expires{' '}
              {new Date(sub.expiresAt).toLocaleDateString()}. They'll appear as active once they
              accept the email invite.
            </>
          )}
          {sub.status === 'claimed' && (
            <>
              Joined {sub.claimedAt ? new Date(sub.claimedAt).toLocaleDateString() : '—'}. Use
              Contracts → Allocate to assign this contractor to sites.
            </>
          )}
          {sub.status === 'expired' && (
            <>Invite expired — re-send from the Contractors list to renew the token.</>
          )}
        </div>
      </div>
    </div>
      ...
      <div style={{ maxWidth: 1240, margin: '0 auto' }}>
        ...
        >
          ...
          <KpiCard
            ...
            value={kpis.pending}
            pop={kpis.pending ? POP.orange : ON_DARK.faint}
          />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 22 }}>
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11, color: ON_DARK.muted, marginTop: 2 }}>
                Up to 500 rows. Sends Cadi Connect invite emails automatically.
              </div>
            </div>
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11, color: ON_DARK.muted, marginTop: 2 }}>
                Add a single contractor with an email — same import flow, one row.
              </div>
            </div>
            <ChevronRight size={14} color={ON_DARK.faint} />
          </button>
        ...
        {!loading && !error && contractors.length === 0 && (
          ...
          >
            ...
            >
              Drop in the FM client's sub list and Cadi sends free Connect invites — you'll see them
              ...
              >
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 10, color: ON_DARK.muted, marginTop: 2 }}>
                      {bucket.subs.length} contractor{bucket.subs.length === 1 ? '' : 's'} ·{' '}
                      {bucket.subs.filter((s) => s.status === 'claimed').length} active
                    </div>
                  </div>
                </div>
