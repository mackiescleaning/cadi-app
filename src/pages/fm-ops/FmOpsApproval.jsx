...
import {
  blueCanvas, glassDark, greenButton, ON_DARK, FM_POP as POP,
...
const darkInput = {
  ...
  fontFamily: 'inherit', resize: 'vertical', colorScheme: 'dark',
};
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        {loading && (
          <div style={{ padding: 40, textAlign: 'center', fontSize: 12, color: ON_DARK.muted, fontWeight: 700 }}>
            <Loader2 size={20} color={ON_DARK.secondary} style={{ animation: 'spin 0.8s linear infinite', display: 'block', margin: '0 auto 8px' }} /> Loading job…
        ...
        {!loading && detail && (
          <>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, flexWrap: 'wrap' }}>
                  ...
                  {detail.contract?.name && (
                    ...
                    }}>{detail.contract.name}</span>
                  )}
                </div>
                <div style={{ fontSize: 17, fontWeight: 900, color: ON_DARK.primary }}>{detail.site?.name}</div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8, marginBottom: 14 }}>
              ...
              <Kpi label="Service date"     value={detail.date ? new Date(detail.date).toLocaleDateString() : '—'} pop={POP.blue} />
              <Kpi label="Actual duration"  value={detail.actual_duration_minutes ? `${detail.actual_duration_minutes}m` : '—'} pop={POP.blue} />
              <Kpi label="Completed"        value={detail.completion_marked_at ? new Date(detail.completion_marked_at).toLocaleDateString() : '—'} pop={POP.blue} />
            </div>
            ...
            <div style={{ ...glassDark({ radius: 14, padding: 14 }), marginBottom: 14 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{
                  ...
                  background: 'rgba(79,120,255,0.20)', color: POP.blue, border: '1px solid rgba(79,120,255,0.35)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 900,
                }}>
                  {(detail.subName?.[0] ?? '?').toUpperCase()}
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 13, fontWeight: 800, color: ON_DARK.primary }}>{detail.subName}</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 4 }}>
                    <TierBadge tier={detail.sub?.connect_tier} />
                    <span style={{ fontSize: 10, color: ON_DARK.muted }}>Score <strong style={{ color: ON_DARK.primary }}>{detail.sub?.connect_score ?? '—'}</strong></span>
                    {detail.sub?.connect_region && <span style={{ fontSize: 10, color: ON_DARK.muted }}>· {detail.sub.connect_region}</span>}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div style={{ ...glassDark({ radius: 14, padding: 14 }), marginBottom: 14 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {[['Check-in', checkin], ['Check-out', checkout]].map(([label, c]) => (
                  <div key={label} style={{
                    background: c ? (c.inside_geo_fence ? 'rgba(52,211,153,0.10)' : 'rgba(251,191,36,0.10)') : 'rgba(255,255,255,0.04)',
                    ...
                    {c && (
                      <>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 800, color: c.inside_geo_fence ? POP.green : POP.amber }}>
                          {c.inside_geo_fence
              ...
              {detail.completion_method && (
                <div style={{ marginTop: 10, fontSize: 11, color: ON_DARK.muted, display: 'flex', alignItems: 'center', gap: 6 }}>
                  <Clock size={11} /> Completion method · <strong style={{ color: ON_DARK.primary }}>{detail.completion_method.replace('_', ' ')}</strong>
                </div>
              )}
            </div>
            ...
            {(() => {
              ...
              const colour = wasPresent ? POP.green : wasAbsent ? POP.amber : 'rgba(255,255,255,0.45)';
              const bg     = wasPresent ? 'rgba(52,211,153,0.08)' : wasAbsent ? 'rgba(251,191,36,0.08)' : 'rgba(255,255,255,0.04)';
              const border = wasPresent ? 'rgba(52,211,153,0.30)' : wasAbsent ? 'rgba(251,191,36,0.30)' : ON_DARK.line;
              return (
                <div style={{ background: bg, border: `1px solid ${border}`, borderRadius: 14, padding: 14, marginBottom: 14 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{
                      ...
                      background: colour.startsWith('#') ? `${colour}22` : 'rgba(255,255,255,0.08)',
                      color: colour, display: 'flex', alignItems: 'center', justifyContent: 'center',
                    }}>
                    ...
                    <div style={{ flex: 1 }}>
                      {wasPresent && (
                        <>
                          ...
                          <div style={{ fontSize: 10, color: ON_DARK.muted, marginTop: 2 }}>Witnessed the work at check-out</div>
                        </>
                      )}
                      {wasAbsent && (
                        <>
                          <div style={{ fontSize: 13, fontWeight: 800, color: ON_DARK.primary }}>No site contact present</div>
                          <div style={{ fontSize: 10, color: ON_DARK.muted, marginTop: 2 }}>GPS + photo evidence stand in for sign-off</div>
                        </>
                      )}
                      {presence == null && (
                        <>
                          <div style={{ fontSize: 13, fontWeight: 700, color: ON_DARK.faint }}>Not recorded</div>
                          <div style={{ fontSize: 10, color: ON_DARK.muted, marginTop: 2 }}>Sub didn't tick a site-contact option at check-out</div>
                        </>
                      )}
                    </div>
                  </div>
            ...
            <div style={{
              ...
              border: '1.5px solid rgba(251,146,60,0.55)', marginBottom: 14,
            }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                <div style={{ fontSize: 11, fontWeight: 900, color: POP.orange, letterSpacing: '0.06em', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: 6 }}>
                  <Camera size={13} /> Photo evidence
                </div>
                <span style={{ fontSize: 10, color: ON_DARK.muted, fontWeight: 700 }}>
                  {detail.evidence.filter(e => e.type !== 'note').length} photo{detail.evidence.filter(e => e.type !== 'note').length === 1 ? '' : 's'}
                </span>
              </div>
              {detail.evidence.filter(e => e.type !== 'note').length === 0 && (
                <div style={{
                  padding: 18, borderRadius: 10, fontSize: 12, textAlign: 'center',
                  background: 'rgba(251,191,36,0.10)', border: '1px solid rgba(251,191,36,0.30)', color: POP.amber,
                }}>
              ...
              {detail.evidence.filter(e => e.type !== 'note').length > 0 && (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10 }}>
                  {detail.evidence.filter(e => e.type !== 'note').map(e => {
                    ...
                    return (
                      <a
                        ...
                        style={{
                          borderRadius: 10, background: 'rgba(255,255,255,0.06)', border: `1px solid ${ON_DARK.lineHi}`,
              ...
              {detail.evidence.filter(e => e.type === 'note').map(e => (
                <div key={e.id} style={{
                  ...
                  borderRadius: 10, fontSize: 12, color: ON_DARK.secondary,
                }}>
                  <div style={{ fontSize: 10, fontWeight: 800, color: ON_DARK.muted, marginBottom: 4 }}>Sub's note</div>
                  {e.data?.text ?? ''}
                </div>
              ))}
            </div>
            ...
            {(detail.query_note || detail.messages?.length > 0) && (
              <div style={{ ...glassDark({ radius: 14, padding: 14 }), marginBottom: 14 }}>
                <div style={{ ...sectionLabel, marginBottom: 10, display: 'flex', alignItems: 'center', gap: 6 }}>
                  <MessageCircleQuestion size={11} /> Query thread
                </div>

                {detail.query_note && (
                  <div style={{
                    background: 'rgba(79,120,255,0.12)', border: '1px solid rgba(79,120,255,0.30)',
                    borderRadius: 10, padding: '10px 12px',
                    fontSize: 12, color: ON_DARK.primary, marginBottom: 10, whiteSpace: 'pre-wrap',
                  }}>
                    <div style={{ fontSize: 10, fontWeight: 800, color: POP.blue, letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 4 }}>Original query</div>
                    {detail.query_note}
                  </div>
                )}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 10 }}>
                    ...
                    return (
                      <div key={m.id} style={{ display: 'flex', justifyContent: mine ? 'flex-end' : 'flex-start' }}>
                        <div style={{
                          ...
                          background: mine ? 'linear-gradient(180deg, #d64510 0%, #C2410C 100%)' : 'rgba(255,255,255,0.08)',
                          border: mine ? '1px solid rgba(255,255,255,0.15)' : `1px solid ${ON_DARK.line}`,
                          ...
                          <div style={{ fontSize: 9, opacity: 0.75, marginTop: 4 }}>
                            {mine ? 'You' : 'Contractor'} · {new Date(m.created_at).toLocaleString()}
                          </div>
                        </div>
                      </div>
                    );
                  ...
                  {(!detail.messages || detail.messages.length === 0) && detail.query_note && (
                    <div style={{ fontSize: 11, color: ON_DARK.faint, fontStyle: 'italic', padding: '4px 0' }}>
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  ...
                  <button
                    ...
                    style={{
                      width: 84,
                      background: threadReply.trim() ? 'linear-gradient(180deg, #d64510 0%, #C2410C 100%)' : 'rgba(255,255,255,0.10)',
                      color: threadReply.trim() ? 'white' : ON_DARK.faint,
                      border: '1px solid rgba(255,255,255,0.15)', borderRadius: 10, fontSize: 12, fontWeight: 800,
            ...
            {detail.rejection_note && (
              <div style={{
                background: 'rgba(220,38,38,0.14)', border: '1px solid rgba(248,113,113,0.40)',
                borderRadius: 12, padding: 12, marginBottom: 14, fontSize: 12, color: '#fecaca',
              }}>
            ...
            {detail.approval_status === 'pending' || detail.approval_status === 'queried' ? (
              <>
                ...
                <div style={{
                  background: 'rgba(251,146,60,0.08)', border: '1px solid rgba(251,146,60,0.30)',
                  borderRadius: 12, padding: '10px 12px', marginBottom: 12,
                }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                    <div style={sectionLabel}>
                      Rate this job <span style={{ color: ON_DARK.faint, fontWeight: 700, letterSpacing: 0, textTransform: 'none' }}>· optional, applies on Approve</span>
                    </div>
                    {ratingStars > 0 && (
                      <button
                        onClick={() => { setRatingStars(0); setRatingComment(''); }}
                        style={{ fontSize: 10, color: ON_DARK.muted, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
                        Clear
                      </button>
                    )}
                  </div>
                    ...
                    {[1, 2, 3, 4, 5].map(n => (
                      <button
                        ...
                        style={{
                          ...
                          padding: 2, fontSize: 26, lineHeight: 1,
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                  <button onClick={() => decide('approved')} disabled={!!busy} style={{
                    ...
                    cursor: busy ? 'not-allowed' : 'pointer', opacity: busy === 'approved' ? 0.7 : 1,
                  ...
                  <button onClick={() => decide('queried')} disabled={!!busy} style={{
                    background: 'rgba(79,120,255,0.14)', color: POP.blue, border: '1px solid rgba(79,120,255,0.40)',
                  ...
                  <button onClick={() => decide('rejected')} disabled={!!busy} style={{
                    background: 'rgba(220,38,38,0.14)', color: POP.red, border: '1px solid rgba(248,113,113,0.40)',
                    borderRadius: 10, padding: '10px 12px', fontSize: 12, fontWeight: 800,
                    cursor: busy ? 'not-allowed' : 'pointer',
                    display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
                  }}>
              ...
              <div style={{
                padding: 12,
                background: detail.approval_status === 'approved' ? 'rgba(52,211,153,0.12)' : 'rgba(220,38,38,0.14)',
                border: `1px solid ${detail.approval_status === 'approved' ? 'rgba(52,211,153,0.35)' : 'rgba(248,113,113,0.40)'}`,
                borderRadius: 12,
                fontSize: 12, color: detail.approval_status === 'approved' ? POP.green : '#fecaca',
    ...
    <div style={{ ...blueCanvas(), margin: '-28px -32px', padding: '34px 36px 56px' }}>
      <div style={{ maxWidth: 1240, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginBottom: 16 }}>
          <Kpi label="Awaiting your review" value={counts.pending ?? 0} pop={counts.pending ? POP.orange : ON_DARK.faint} />
          <Kpi label={`In ${TABS.find(t => t.id === tab)?.label.toLowerCase()}`} value={totals.count} />
          <Kpi label="Value in view" value={`£${totals.total.toFixed(0)}`} pop={POP.blue} />
        </div>

        <div style={{ ...glassDark({ radius: 12 }), display: 'flex', gap: 4, padding: 4, marginBottom: 18, width: 'fit-content' }}>
          {TABS.map(t => {
        ...
        {loading && (
          <div style={{ padding: 60, textAlign: 'center', fontSize: 12, color: ON_DARK.muted, fontWeight: 700 }}>
            <Loader2 size={20} color={ON_DARK.secondary} style={{ animation: 'spin 0.8s linear infinite', display: 'block', margin: '0 auto 10px' }} /> Loading jobs…
            <style>{`@keyframes spin { from { transform: rotate(0); } to { transform: rotate(360deg); } }`}</style>
          </div>
        )}
        ...
        {!loading && !error && jobs.length === 0 && (
          <div style={{ padding: '44px 24px', borderRadius: 18, border: '1.5px dashed rgba(255,255,255,0.16)', textAlign: 'center' }}>
            <div style={{
              ...
              background: 'rgba(52,211,153,0.14)', color: POP.green, border: '1px solid rgba(52,211,153,0.35)',
              display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 14px',
            }}>
              <CheckCircle2 size={24} />
            </div>
            <div style={{ fontSize: 15, fontWeight: 800, color: ON_DARK.primary, marginBottom: 6 }}>Nothing to review</div>
          ...
          <div style={{ ...glassDark({ radius: 18 }), overflow: 'hidden' }}>
            ...
            {jobs.map((j, i) => (
              <div
                ...
                style={{
                  display: 'grid', gridTemplateColumns: '2fr 1.4fr 1fr 1fr 1fr 40px',
                  padding: '14px 18px', borderBottom: i < jobs.length - 1 ? `1px solid ${ON_DARK.line}` : 'none',
              ...
              >
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  {j.sub?.connect_score != null && (
                    <div style={{ fontSize: 10, color: ON_DARK.muted, marginTop: 2 }}>Score {j.sub.connect_score}</div>
                  )}
                </div>
