...
import { blueCanvas, glassDark, ghostButton, ON_DARK, FM_POP as POP } from '../../lib/connectTheme';
...
export default function FmOpsSchedule() {
  ...
  const subs = useMemo(() => {
    const set = new Map();
    jobs.forEach((j) => {
      if (j.subName) set.set(j.subName, true);
    });
  ...
  const contracts = useMemo(() => {
    const set = new Map();
    jobs.forEach((j) => {
      if (j.contract?.name) set.set(j.contract.name, true);
    });
    return Array.from(set.keys()).sort();
  }, [jobs]);
  ...
  const days = useMemo(() => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      out.push({
        ...
        jobs: filteredJobs
          .filter((j) => j.date === iso)
          .sort((a, b) => (Number(a.start_hour) || 9) - (Number(b.start_hour) || 9)),
      });
    }
    return out;
      ...
      <div style={{ maxWidth: 1240, margin: '0 auto' }}>
        ...
        >
          ...
          <Kpi
            ...
            pop={totals.inProgress ? POP.orange : ON_DARK.faint}
          />
        ...
        >
          ...
          >
            ...
            {subs.map((s) => (
              <option key={s} value={s} style={{ color: '#010a4f' }}>
                {s}
              </option>
            ))}
          </select>
          ...
          >
            ...
            {contracts.map((c) => (
              <option key={c} value={c} style={{ color: '#010a4f' }}>
                {c}
              </option>
            ))}
          </select>
                ...
                >
                    ...
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                      <span
                        style={{
                          fontSize: 13,
                          fontWeight: 900,
                          color: today ? POP.orange : ON_DARK.primary,
                        }}
                      >
                  ...
                  {day.jobs.map((j, i) => (
                    <div
                      ...
                      style={{
                        display: 'grid',
                        gridTemplateColumns: '90px 2fr 1.4fr 1fr 1fr',
                        gap: 10,
                        alignItems: 'center',
                        padding: '12px 16px',
                    ...
                    >
                      ...
                      >
                        ...
                        {String(Math.round(((Number(j.start_hour) || 9) % 1) * 60)).padStart(
                          2,
                          '0'
                        )}
                      </div>
