...
import {
  ...
  X, Eye, MapPin, FileDown, FileText, PoundSterling,
  Settings,
} from 'lucide-react';
import {
  listFmInvoices,
...
import { greenCanvas, greenButton, ghostButton, ON_DARK } from '../../lib/connectTheme';
...
function Kpi({ label, value, pop }) {
  return (
    <div style={labCard(14, '13px 15px')}>
      <div style={{ fontSize: 22, fontWeight: 900, color: pop || '#ffffff', lineHeight: 1 }}>{value}</div>
      <div style={{ fontSize: 10, color: SKY, fontWeight: 800, letterSpacing: '0.10em', textTransform: 'uppercase', marginTop: 7 }}>{label}</div>
    </div>
  );
}

const sectionLabel = { fontSize: 10, fontWeight: 800, color: SKY, letterSpacing: '0.10em', textTransform: 'uppercase' };
const darkInput = {
  width: '100%', padding: '8px 10px', fontSize: 13,
  border: '1px solid rgba(153,197,255,0.25)', borderRadius: 10,
...
function ExportDrawer({ onClose, onDone }) {
  ...
  return (
    <div style={{
      position: 'fixed', inset: 0, background: 'rgba(1,18,11,0.60)',
      backdropFilter: 'blur(6px)', WebkitBackdropFilter: 'blur(6px)',
      display: 'flex', justifyContent: 'flex-end', zIndex: 50,
    }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{
        width: 560, maxWidth: '92vw', background: DRAWER_BG,
        borderLeft: `1px solid ${CARD_LINE}`, padding: '24px 28px',
        overflowY: 'auto', boxShadow: '-16px 0 60px rgba(0,0,0,0.55)',
      }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 18 }}>
          <div>
            <div style={{ fontSize: 17, fontWeight: 900, color: '#ffffff' }}>Export to accounts</div>
            <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.70)', marginTop: 5, lineHeight: 1.5 }}>
              Pick a period, preview the CSV, then export — submitted invoices in range flip to "exported".
            </div>
          </div>
          <button onClick={onClose} style={{ background: 'rgba(255,255,255,0.08)', border: `1px solid ${CARD_LINE}`, borderRadius: 9, cursor: 'pointer', color: 'rgba(255,255,255,0.7)', padding: 6, display: 'flex' }}>
            <X size={16} />
          </button>
        </div>
        ...
        {preview && (
          <>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10, marginBottom: 14 }}>
              <Kpi label="Rows in period"  value={preview.row_count} pop={preview.row_count ? SKY_POP : 'rgba(255,255,255,0.4)'} />
              <Kpi label="Total value"     value={`£${Number(preview.total_value).toFixed(2)}`} pop={preview.row_count ? EMERALD : 'rgba(255,255,255,0.4)'} />
            </div>
            {preview.row_count === 0 ? (
              <div style={{ padding: 14, background: 'rgba(255,255,255,0.06)', border: `1px solid ${CARD_LINE}`, borderRadius: 12, fontSize: 12, color: 'rgba(255,255,255,0.65)' }}>
                No submitted invoices in this period. Subs need to submit drafts before they appear here.
              </div>
            ) : (
              <div style={{
                background: 'rgba(0,0,0,0.35)', border: `1px solid ${CARD_LINE}`,
                borderRadius: 12, padding: 12,
                fontFamily: 'ui-monospace,Menlo,monospace', fontSize: 11, color: SKY_POP,
...
function InvoiceDetailDrawer({ invoiceId, onClose, onChanged }) {
  ...
  async function load() {
    ...
    try { setDetail(await getFmInvoiceDetail(invoiceId)); }
      ...
      const pdf = generateInvoicePdf(
        {
          ...
          dueDate:  '',
          customer: { name: detail.fm_organisation?.name || 'Facilities Manager', address: '', email: '' },
  ...
  async function handleMarkPaid() {
    setBusy('paid'); setError(null);
    const { ok, error: e } = await markInvoicePaid(invoiceId);
    if (!ok) { setError(e?.message || 'Mark paid failed.'); setBusy(null); return; }
    onChanged?.();
    await load();
    setBusy(null);
  }
  ...
  async function handleMarkExported() {
    setBusy('exported'); setError(null);
    const { ok, error: e } = await markInvoiceExported(invoiceId);
    if (!ok) { setError(e?.message || 'Mark exported failed.'); setBusy(null); return; }
    onChanged?.();
    await load();
    setBusy(null);
  }
  ...
  return (
    ...
    >
      ...
      >
        {loading && (
          <div style={{ padding: 40, textAlign: 'center', fontSize: 12, color: 'rgba(255,255,255,0.6)', fontWeight: 700 }}>
            <Loader2 size={20} color="rgba(255,255,255,0.7)" style={{ animation: 'spin 0.8s linear infinite', display: 'block', margin: '0 auto 8px' }} /> Loading invoice…
          </div>
        )}
        ...
        {!loading && detail && (
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
              <div>
                <div style={{ fontSize: 11, fontWeight: 800, color: SKY, letterSpacing: '0.04em', textTransform: 'uppercase', fontFamily: 'ui-monospace, Menlo, monospace' }}>
                  {detail.reference}
                </div>
                <div style={{ fontSize: 17, fontWeight: 900, color: '#ffffff', marginTop: 3 }}>{detail.subName}</div>
                <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.60)', marginTop: 4 }}>
                  {(detail.lines?.length ?? 0)} line{detail.lines?.length === 1 ? '' : 's'} · submitted {detail.submitted_at ? new Date(detail.submitted_at).toLocaleDateString() : '—'}
                </div>
              </div>
              <button onClick={onClose} style={{ background: 'rgba(255,255,255,0.08)', border: `1px solid ${CARD_LINE}`, borderRadius: 9, cursor: 'pointer', color: 'rgba(255,255,255,0.7)', padding: 6, display: 'flex' }}>
                <X size={16} />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginBottom: 14 }}>
              ...
              <Kpi label="VAT £"   value={`£${Number(detail.vat_value ?? 0).toFixed(2)}`} pop={SKY_POP} />
              <Kpi label="Total £" value={`£${(Number(detail.net_value ?? 0) + Number(detail.vat_value ?? 0)).toFixed(2)}`} pop={EMERALD} />
            </div>

            {/* Status strip */}
            <div style={{
              background: 'rgba(255,255,255,0.05)', border: `1px solid ${CARD_LINE}`, borderRadius: 12,
              padding: '10px 14px', marginBottom: 14, fontSize: 12, color: '#ffffff',
              display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap',
            }}>
              <StatusPill status={detail.status} />
              {detail.submitted_at && <span style={{ color: 'rgba(255,255,255,0.55)' }}>· Submitted {new Date(detail.submitted_at).toLocaleDateString()}</span>}
              {detail.exported_at  && <span style={{ color: 'rgba(255,255,255,0.55)' }}>· Exported {new Date(detail.exported_at).toLocaleDateString()}</span>}
              {detail.paid_at      && <span style={{ color: EMERALD, fontWeight: 800 }}>· Paid {new Date(detail.paid_at).toLocaleDateString()}</span>}
            </div>

            {/* Line items table */}
            <div style={{ ...labCard(14), marginBottom: 14 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr 1fr', padding: '10px 14px', background: 'rgba(0,0,0,0.20)', borderBottom: `1px solid ${CARD_LINE}`, ...sectionLabel }}>
              ...
              {(detail.lines ?? []).map((l, i) => (
                <div key={l.id} style={{
                  ...
                  padding: '10px 14px', borderBottom: i < detail.lines.length - 1 ? `1px solid ${CARD_LINE}` : 'none',
                  alignItems: 'center', fontSize: 12, color: '#ffffff',
                }}>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{l.description}</div>
                    {l.job?.site?.postcode && (
                      <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.55)', marginTop: 2, display: 'flex', alignItems: 'center', gap: 4 }}>
                        <MapPin size={9} /> {l.job.site.postcode}
                      </div>
                    )}
                  </div>
                  <div style={{ color: 'rgba(255,255,255,0.60)' }}>{l.service_date ? new Date(l.service_date).toLocaleDateString() : '—'}</div>
                  <div style={{ fontWeight: 800, textAlign: 'right' }}>£{Number(l.net_value ?? 0).toFixed(2)}</div>
                </div>
              ))}
              {(!detail.lines || detail.lines.length === 0) && (
                <div style={{ padding: 14, fontSize: 11, color: 'rgba(255,255,255,0.45)', fontStyle: 'italic' }}>No line items.</div>
              )}
              <div style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr 1fr', padding: '10px 14px', background: 'rgba(0,0,0,0.20)', borderTop: `2px solid ${CARD_LINE}`, fontSize: 12, fontWeight: 800, color: '#ffffff' }}>
                <div>Net total</div>
                <div></div>
                <div style={{ textAlign: 'right' }}>£{totalNet.toFixed(2)}</div>
              </div>
              {totalVat > 0 && (
                <div style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr 1fr', padding: '8px 14px', background: 'rgba(0,0,0,0.20)', fontSize: 11, color: 'rgba(255,255,255,0.60)' }}>
            ...
            {detail.note && (
              <div style={{
                background: 'rgba(153,197,255,0.10)', border: '1px solid rgba(153,197,255,0.25)',
                borderRadius: 12, padding: '10px 12px', fontSize: 12, color: '#ffffff', marginBottom: 14, whiteSpace: 'pre-wrap',
              }}>
                <div style={{ fontSize: 10, fontWeight: 800, color: SKY_POP, letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 4 }}>Sub's note</div>
                {detail.note}
              </div>
            )}
            ...
            {error && (
              <div style={{
                padding: 10, marginBottom: 10, borderRadius: 10, fontSize: 12,
                background: 'rgba(220,38,38,0.16)', border: '1px solid rgba(248,113,113,0.40)', color: '#fecaca',
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
              ...
              <button
                ...
                title={detail.status === 'paid' ? 'Already paid' : detail.status === 'exported' ? 'Already exported' : 'Mark as pushed to your accounts system'}
                style={{
                  background: 'rgba(153,197,255,0.12)', color: SKY_POP, border: '1px solid rgba(153,197,255,0.35)',
                  borderRadius: 10, padding: '10px 12px', fontSize: 12, fontWeight: 800,
                  cursor: (busy || detail.status === 'exported' || detail.status === 'paid') ? 'not-allowed' : 'pointer',
      ...
      <div style={{ maxWidth: 1240, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap', marginBottom: 22 }}>
          <div>
            ...
            <div style={{ fontSize: 12.5, color: 'rgba(255,255,255,0.72)', marginTop: 6, maxWidth: 560 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 16 }}>
          <Kpi label="With FM (to action)" value={counts.submitted ?? 0} pop={counts.submitted ? AMBER : 'rgba(255,255,255,0.4)'} />
        ...
        {loading && (
          <div style={{ padding: 60, textAlign: 'center', fontSize: 12, color: 'rgba(255,255,255,0.55)', fontWeight: 700 }}>
            <Loader2 size={20} color="rgba(255,255,255,0.7)" style={{ animation: 'spin 0.8s linear infinite', display: 'block', margin: '0 auto 10px' }} /> Loading invoices…
            <style>{`@keyframes spin { from { transform: rotate(0); } to { transform: rotate(360deg); } }`}</style>
          </div>
        )}
        ...
        {!loading && !error && invoices.length === 0 && (
          <div style={{ padding: '44px 24px', borderRadius: 18, border: '1.5px dashed rgba(153,197,255,0.20)', textAlign: 'center' }}>
            <div style={{
              width: 52, height: 52, borderRadius: 14,
              background: 'rgba(52,211,153,0.14)', color: EMERALD, border: '1px solid rgba(52,211,153,0.35)',
              display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 14px',
            }}>
              <Receipt size={24} />
            </div>
            <div style={{ fontSize: 15, fontWeight: 800, color: '#ffffff', marginBottom: 6 }}>No invoices here yet</div>
            <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.60)', maxWidth: 400, margin: '0 auto', lineHeight: 1.6 }}>
              Subs submit drafts after their work is approved — they show up in <strong style={{ color: '#ffffff' }}>With FM</strong> ready for export.
            </div>
          </div>
        )}
        ...
        {!loading && !error && invoices.length > 0 && (
          <div style={{ ...labCard(18), overflow: 'hidden' }}>
            <div style={{
              ...
              padding: '11px 18px', background: 'rgba(0,0,0,0.20)', borderBottom: `1px solid ${CARD_LINE}`,
            ...
            {invoices.map((inv, i) => (
              <div
                ...
                onMouseEnter={e => { if (!selected.has(inv.id)) e.currentTarget.style.background = 'rgba(255,255,255,0.04)'; }}
                onMouseLeave={e => { if (!selected.has(inv.id)) e.currentTarget.style.background = 'transparent'; }}
                style={{
                  display: 'grid', gridTemplateColumns: '32px 1.4fr 1.4fr 1fr 1fr 1fr 1fr',
                  padding: '12px 18px', borderBottom: i < invoices.length - 1 ? `1px solid ${CARD_LINE}` : 'none',
                  alignItems: 'center', fontSize: 12, color: '#ffffff',
              ...
              >
                <input
                  ...
                  style={{ accentColor: '#10b981', cursor: inv.status === 'paid' || inv.status === 'draft' ? 'not-allowed' : 'pointer' }}
                />
                <div style={{ fontWeight: 800, fontFamily: 'ui-monospace,Menlo,monospace', fontSize: 11 }}>{inv.reference ?? '—'}</div>
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{inv.subName}</div>
                  <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.55)', marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {inv.siteName ?? '—'}{inv.sitePostcode ? ` · ${inv.sitePostcode}` : ''}
                  </div>
                </div>
                <div style={{ color: 'rgba(255,255,255,0.60)' }}>{inv.service_date ? new Date(inv.service_date).toLocaleDateString() : '—'}</div>
