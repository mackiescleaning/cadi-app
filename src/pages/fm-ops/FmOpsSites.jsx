...
import {
  Search, MapPin, ChevronRight, Loader2, ClipboardList, X, Send,
  Camera, CheckCircle2, Clock, History,
} from 'lucide-react';
import {
...
import {
  blueCanvas, glassDark, ON_DARK, FM_POP as POP,
...
function SiteDrawer({ site, subs, fmOrg, onClose, onChanged }) {
  ...
  const stats = useMemo(() => {
    if (!history || history.length === 0) return { total: 0, approvalPct: null, thisYear: 0, valueYtd: 0 };
    ...
    for (const j of history) {
      if (j.approval_status === 'approved' || j.approval_status === 'rejected' || j.approval_status === 'queried') {
        reviewed++;
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        {site.notes && (
          <div style={{
            ...
            padding: 12, borderRadius: 12, marginBottom: 14, fontSize: 12, color: ON_DARK.secondary,
          }}>
            <strong style={{ color: ON_DARK.primary }}>Notes · </strong>{site.notes}
          </div>
        )}

        {err && (
          <div style={{
            padding: 10, borderRadius: 10, marginBottom: 12, fontSize: 11,
        ...
        <div style={{ fontSize: 10, fontWeight: 800, color: ON_DARK.muted, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 8 }}>Visit specs</div>
        {site.specs.length === 0 && (
          <div style={{ padding: 16, fontSize: 12, color: ON_DARK.muted, border: '1.5px dashed rgba(255,255,255,0.16)', borderRadius: 12 }}>
          ...
          return (
            <div key={s.id} style={{
              ...
              <div style={{
                ...
                <div style={{ fontSize: 11, fontWeight: 800, color: ON_DARK.primary }}>{FREQ_LABEL[s.frequency] ?? s.frequency}</div>
                <div style={{ fontSize: 11, color: ON_DARK.secondary, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.scope}</div>
                <div style={{ fontSize: 11, fontWeight: 800, color: ON_DARK.primary }}>£{s.price_per_visit}</div>
                <Pill map={SPEC_STATUS_LABEL} status={s.status} />
              </div>
              {assignedSub && (
                <div style={{ fontSize: 10, color: ON_DARK.secondary, marginBottom: 6 }}>→ {assignedSub.name}</div>
              )}
              <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                {isBusy && <Loader2 size={13} color={ON_DARK.secondary} style={{ animation: 'spin 0.8s linear infinite' }} />}
                {!isBusy && (
                  <>
                    <select
                      ...
                      style={{
                        ...
                        border: `1px solid ${locked ? ON_DARK.line : ON_DARK.lineHi}`, borderRadius: 8,
                    ...
                    >
                      <option value="" style={{ color: '#010a4f' }}>{locked ? '(locked while on marketplace / active)' : 'Assign sub…'}</option>
                      {subs.map(sub => (
                        <option key={sub.id} value={sub.id} style={{ color: '#010a4f' }}>{sub.name}{sub.region ? ` · ${sub.region}` : ''}</option>
                      ))}
                    </select>
                    {!locked && (
                      <button
        ...
        <div style={{ marginTop: 22 }}>
          <div style={{ fontSize: 10, fontWeight: 800, color: ON_DARK.muted, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 5 }}>
          ...
          {history && history.length > 0 && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, marginBottom: 12 }}>
              ...
              <Kpi label="Approval rate" value={stats.approvalPct == null ? '—' : `${stats.approvalPct}%`} pop={POP.green} />
              <Kpi label="This year"     value={stats.thisYear} pop={POP.blue} />
              <Kpi label="£ YTD (paid ok)" value={`£${stats.valueYtd.toFixed(0)}`} pop={POP.orange} />
            </div>
          )}

          {historyLoading && (
            <div style={{ padding: 16, fontSize: 11, color: ON_DARK.muted, textAlign: 'center' }}>
              <Loader2 size={14} color={ON_DARK.secondary} style={{ animation: 'spin 0.8s linear infinite', display: 'block', margin: '0 auto 6px' }} />
              Loading history…
            </div>
          )}
          ...
          {!historyLoading && history && history.length > 0 && (
            <div style={{ ...glassDark({ radius: 14 }), overflow: 'hidden' }}>
              {history.map((j, i) => {
                ...
                return (
                  <div key={j.id} style={{
                    display: 'grid', gridTemplateColumns: '90px 1fr 84px 90px',
                    ...
                    <div style={{ fontSize: 11, color: ON_DARK.muted }}>
                      {fmtDate(j.date)}
                      {isCompleted && (
                        <div style={{ fontSize: 10, color: POP.green, marginTop: 2, display: 'flex', alignItems: 'center', gap: 3 }}>
                          <CheckCircle2 size={9} /> {new Date(j.completion_marked_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}
                        </div>
                      )}
                    </div>
                    <div style={{ minWidth: 0 }}>
                      <div style={{ fontSize: 11, fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{j.subName}</div>
                      {j.service && (
                        <div style={{ fontSize: 10, color: ON_DARK.muted, marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{j.service}</div>
                      )}
                        ...
                        <div style={{ display: 'flex', gap: 3, marginTop: 5 }}>
                          ...
                          {j.photoCount > 4 && (
                            <div style={{
                              ...
                              fontSize: 9, fontWeight: 800, color: ON_DARK.secondary,
                            }}>
                              +{j.photoCount - 4}
                            </div>
                          )}
                      ...
                      {(!j.photoUrls || j.photoUrls.length === 0) && j.status === 'complete' && (
                        <div style={{ fontSize: 10, color: ON_DARK.faint, marginTop: 4, display: 'flex', alignItems: 'center', gap: 3 }}>
                    ...
                    <div style={{ fontSize: 11, fontWeight: 800, textAlign: 'right' }}>
                      ...
                      {j.actual_duration_minutes != null && (
                        <div style={{ fontSize: 10, fontWeight: 500, color: ON_DARK.muted, marginTop: 2, display: 'flex', alignItems: 'center', gap: 3, justifyContent: 'flex-end' }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      <span style={{
                        fontSize: 9, fontWeight: 800, color: pillPop,
                        background: pillPop.startsWith('#') ? `${pillPop}1f` : 'rgba(255,255,255,0.08)',
              ...
              {history.length >= 100 && (
                <div style={{ padding: 8, fontSize: 10, color: ON_DARK.muted, textAlign: 'center', background: 'rgba(255,255,255,0.04)' }}>
                  Showing most recent 100 jobs — older history available on request.
                </div>
              )}
              <div style={{ padding: '9px 12px', background: 'rgba(255,255,255,0.04)', borderTop: `1px solid ${ON_DARK.line}` }}>
                <Link to="/fm-ops/approval" style={{
                  fontSize: 11, fontWeight: 800, color: POP.orange, textDecoration: 'none',
    ...
    <div style={{ ...blueCanvas(), margin: '-28px -32px', padding: '34px 36px 56px' }}>
      <div style={{ maxWidth: 1240, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 12, marginBottom: 20 }}>
          ...
          <Kpi label="Unassigned"     value={kpis.unassigned} pop={kpis.unassigned ? POP.amber : ON_DARK.faint} />
          <Kpi label="Pending approval" value={kpis.pendingApprovals} pop={kpis.pendingApprovals ? POP.orange : ON_DARK.faint} />
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
          <div style={{
            ...
            padding: '8px 12px', flex: 1, maxWidth: 380,
          }}>
        ...
        {loading && (
          <div style={{ padding: 60, textAlign: 'center', fontSize: 12, color: ON_DARK.muted, fontWeight: 700 }}>
            <Loader2 size={20} color={ON_DARK.secondary} style={{ animation: 'spin 0.8s linear infinite', display: 'block', margin: '0 auto 10px' }} /> Loading sites…
            <style>{`@keyframes spin { from { transform: rotate(0); } to { transform: rotate(360deg); } }`}</style>
          </div>
        )}
          ...
          <div style={{ ...glassDark({ radius: 18 }), overflow: 'hidden' }}>
            ...
            {filtered.map((s, i) => (
              <div
                ...
                style={{
                  display: 'grid', gridTemplateColumns: '40px 2fr 1.4fr 1fr 1fr 1fr 1fr 40px',
                  padding: '12px 18px', borderBottom: i < filtered.length - 1 ? `1px solid ${ON_DARK.line}` : 'none',
                  alignItems: 'center', fontSize: 12, color: ON_DARK.primary, cursor: 'pointer',
                  transition: 'background 150ms ease',
                }}
              >
                <input type="checkbox" disabled style={{ accentColor: '#fb923c' }} onClick={e => e.stopPropagation()} />
                <div style={{ minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
                  ...
                  {s.specs.length === 0 && <span style={{ fontSize: 10, color: ON_DARK.faint }}>—</span>}
                </div>
                <div style={{ fontWeight: 800 }}>{s.perVisit ? `£${s.perVisit}` : '—'}</div>
                <div style={{ color: ON_DARK.secondary }}>{s.lastVisit ? new Date(s.lastVisit).toLocaleDateString() : '—'}</div>
                <div style={{ color: s.nextVisit ? ON_DARK.primary : ON_DARK.faint, fontWeight: s.nextVisit ? 700 : 500 }}>
