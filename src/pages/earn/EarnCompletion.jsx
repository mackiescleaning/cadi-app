...
import {
  ...
  MessageCircleQuestion, Send, Receipt, ChevronRight, ClipboardCheck,
} from 'lucide-react';
import {
  listMyConnectJobs,
  connectCheckIn,
...
import {
  ...
  glassDark, orangeCanvas, primaryButton, whiteButton, HOVER_LIFT,
    ...
    >
      <div
        ...
        style={{
          ...
          maxHeight: '90vh', overflowY: 'auto', color: '#ffffff',
        }}
      >
        {children}
      </div>
...
function ModalLabel({ children }) {
  return (
    <label style={{
      ...
      color: ON_DARK.muted, letterSpacing: '0.14em',
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      const { ok, data } = await connectCheckOut({
        jobId: job.id, lat: pos.lat, lng: pos.lng,
        note: note || undefined,
        customerOnSite,
  ...
  return (
    <ModalShell onCancel={onCancel}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <div>
          <div style={{ fontSize: 18, fontWeight: 900, color: '#ffffff', lineHeight: 1.2, letterSpacing: '-0.01em' }}>Check out</div>
          <div style={{ fontSize: 11, color: ON_DARK.muted, marginTop: 3 }}>{job.site?.name ?? 'Site'}</div>
        </div>
        <button onClick={onCancel} style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.14)', color: '#ffffff', cursor: 'pointer', padding: 6, borderRadius: 8 }}>
          <X size={16} />
        </button>
      </div>
      ...
      <ModalLabel>Photos ({files.length}/{MAX_PHOTOS})</ModalLabel>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 6, marginBottom: 14 }}>
        {files.map((f, i) => {
          const url = URL.createObjectURL(f);
          return (
            <div key={i} style={{ position: 'relative', aspectRatio: '1', borderRadius: 10, overflow: 'hidden', background: 'rgba(255,255,255,0.06)' }}>
        ...
        {files.length < MAX_PHOTOS && (
          <label style={{
            aspectRatio: '1', borderRadius: 10, border: '1.5px dashed rgba(255,255,255,0.20)',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            color: ON_DARK.muted, cursor: 'pointer',
      ...
      {(() => {
        ...
        else if (files.length === 1 && !hasWitness) need = 'Add 1 more photo — or tick that a site contact was present and add their name.';
        ...
        return (
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button onClick={handleConfirm} disabled={blocked}
              ...
              {busy
                ? <><Loader2 size={13} style={{ animation: 'connectSpin 0.8s linear infinite' }} /> Submitting…</>
      ...
      <p style={{ fontSize: 10, color: ON_DARK.faint, textAlign: 'center', margin: '10px 0 0', lineHeight: 1.4 }}>
...
function QueryReplyModal({ job, onCancel, onResubmitted }) {
  ...
  async function load() {
    ...
    try { setMessages(await listJobMessages(job.id)); }
    catch (e) { setError(e.message || 'Could not load thread.'); }
  ...
  async function handleResubmit() {
    setBusy('resubmit'); setError(null);
    try {
  ...
  return (
    <ModalShell onCancel={onCancel} maxWidth={520}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div style={{ fontSize: 18, fontWeight: 900, color: '#ffffff', lineHeight: 1.2, letterSpacing: '-0.01em' }}>FM queried this job</div>
          </div>
          <div style={{ fontSize: 11, color: ON_DARK.muted, marginTop: 3, marginLeft: 24 }}>{job.site?.name ?? 'Site'}</div>
        </div>
        <button onClick={onCancel} style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.14)', color: '#ffffff', cursor: 'pointer', padding: 6, borderRadius: 8 }}>
          <X size={16} />
        </button>
      ...
      {job.query_note && (
        <div style={{
          background: 'rgba(126,163,255,0.12)', border: '1px solid rgba(126,163,255,0.30)',
          borderRadius: 10, padding: '10px 12px', fontSize: 12, color: '#c7d7ff',
          marginBottom: 14, whiteSpace: 'pre-wrap', lineHeight: 1.5,
        }}>
          <div style={{ fontSize: 10, fontWeight: 800, color: BLUE, letterSpacing: '0.14em', textTransform: 'uppercase', marginBottom: 6 }}>The query</div>
          {job.query_note}
        </div>
      )}

      <ModalLabel>Conversation</ModalLabel>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 14, minHeight: 60 }}>
        {loading && <div style={{ fontSize: 11, color: ON_DARK.muted, textAlign: 'center' }}>Loading…</div>}
        {!loading && messages.length === 0 && (
          <div style={{ fontSize: 11, color: ON_DARK.faint, fontStyle: 'italic', textAlign: 'center', padding: '12px 0' }}>
              ...
              <div key={i} style={{ position: 'relative', aspectRatio: '1', borderRadius: 10, overflow: 'hidden', background: 'rgba(255,255,255,0.06)' }}>
                ...
                <button type="button" onClick={() => removeFile(i)}
                  style={{
                    ...
                    border: 'none', color: 'white', borderRadius: 12, width: 22, height: 22,
                    display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer',
                  }}>
                  <X size={11} />
                </button>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <button
          ...
          style={{
            ...
            fontSize: 12, fontWeight: 900, cursor: busy ? 'not-allowed' : 'pointer',
            display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 6,
          }}>
          {busy === 'reply'
            ? <><Loader2 size={12} style={{ animation: 'connectSpin 0.8s linear infinite' }} /> Sending…</>
        ...
        <button
          ...
          style={{
            ...
            border: '1px solid rgba(255,255,255,0.5)', borderRadius: 10,
            fontSize: 12, fontWeight: 900, cursor: canResubmit ? 'pointer' : 'not-allowed',
            display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 6,
          }}
          title={canResubmit ? 'Send back to FM for approval' : 'Send a reply first'}>
          {busy === 'resubmit'
            ? <><Loader2 size={12} style={{ animation: 'connectSpin 0.8s linear infinite' }} /> Resubmitting…</>
...
function JobCard({ job, onUpdated, onCheckedIn, onCheckedOut }) {
  ...
  async function handleCheckIn() {
    ...
    try {
      ...
      if (!ok) {
        setFenceResult({ ok: false, action: 'checkin', distance_m: data?.distance_m, message: data?.error || 'Check-in rejected.' });
      } else {
        onCheckedIn?.({ jobId: job.id, siteName: job.site?.name ?? 'the site', distance_m: data?.distance_m });
        onUpdated?.();
      }
    } catch (e) {
  ...
  function onCheckoutConfirmed(result) {
    ...
    if (result?.ok) {
      onCheckedOut?.({ jobId: job.id, siteName: job.site?.name ?? 'the site', distance_m: result.distance_m });
    } else {
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
        <span style={{
          ...
          padding: '3px 9px', borderRadius: 999, letterSpacing: '0.10em', textTransform: 'uppercase',
        }}>
          {isQueried ? 'Queried' : status.label}
        </span>
        <span style={{ fontSize: 10, color: ON_DARK.muted }}>{job.fm_organisation?.name ?? '—'}</span>
      </div>

      <div style={{ fontSize: 14, fontWeight: 900, color: '#ffffff', letterSpacing: '-0.01em' }}>{job.site?.name ?? 'Site'}</div>
      ...
      {job.status === 'complete' && job.actual_duration_minutes != null && (
        <div style={{ marginTop: 8, fontSize: 11, color: ON_DARK.muted }}>
          Time on site: <strong style={{ color: '#ffffff' }}>{job.actual_duration_minutes} min</strong>
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 6, marginTop: 14 }}>
        ...
        {job.status === 'complete' && !isQueried && (
          <div style={{
            flex: 1, fontSize: 12, fontWeight: 800, color: GREEN,
        ...
        {isQueried && (
          <button onClick={() => setShowQueryModal(true)} disabled={busy !== null}
            style={{
              flex: 1, fontSize: 12, fontWeight: 900, color: '#01041f',
      ...
      {showQueryModal && (
        <QueryReplyModal
          ...
          onResubmitted={() => { setShowQueryModal(false); onUpdated?.(); }}
        />
      )}
    </div>
  );
      ...
      <div className="relative max-w-5xl mx-auto px-4 md:px-8 pt-6 pb-28 space-y-5 z-10">
          ...
          <div className="relative px-6 md:px-9 py-7 md:py-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <h1 className="text-white mb-2"
                ...
                <span style={{ color: '#ffd7bf' }}>ship the evidence.</span>
              </h1>
              <p className="text-white/70 text-[14px] leading-relaxed max-w-2xl">
                Both check-in and check-out verify you're inside the site fence (80m by default). Photos and notes ship to the FM automatically.
              </p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 shrink-0" style={{ minWidth: 220 }}>
              {[
                ...
                <div key={label} style={{
                  ...
                  <div style={{ fontSize: 9, fontWeight: 800, color: ON_DARK.muted, letterSpacing: '0.12em', textTransform: 'uppercase', marginTop: 4 }}>{label}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {recentEvent && (
          <div style={{
            ...
            <div style={{
              ...
              color: GREEN, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
            }}>
            ...
            <button
              ...
              style={{
                ...
                cursor: 'pointer', color: '#ffffff', padding: 6, borderRadius: 8, flexShrink: 0,
              }}>
              <X size={14} />
            </button>
          </div>
        ...
        {loading ? (
          ...
          <div style={{
            ...
            <div style={{ fontSize: 16, fontWeight: 900, color: '#ffffff', letterSpacing: '-0.01em' }}>No scheduled jobs yet</div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            {queried.length > 0 && (
              <section>
                <SectionHead icon={MessageCircleQuestion} tone="blue">Needs your response</SectionHead>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {queried.map(j => <JobCard key={j.id} job={j} onUpdated={reload}
