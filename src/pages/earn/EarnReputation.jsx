...
import { Star, Shield, Award, FileText, Sparkles, ChevronRight } from 'lucide-react';
import PublicProfilePreview from '../../components/PublicProfilePreview';
import {
  getMyConnectProfile, listMyFmRatings,
  listMySubDocs, uploadSubDoc, deleteSubDoc, signSubDocUrl,
  SUB_DOC_TYPES,
} from '../../lib/db/connectDb';
import {
  CONNECT_COLORS, CONNECT_RADII, TIER_STYLE, ON_DARK,
  glassDark, navyCanvas, whiteButton, primaryButton, HOVER_LIFT,
...
function Stars({ n }) {
  return (
    <span aria-label={`${n} stars`}>
      {[1, 2, 3, 4, 5].map(i => (
        <span key={i} style={{ color: i <= n ? '#fbbf24' : 'rgba(255,255,255,0.20)', fontSize: 14 }}>★</span>
      ))}
    </span>
  );
}
...
function tierMeta(tier) {
  ...
  return map[tier] || { label: 'Building reputation', color: '#a3a3a3', glow: 'rgba(163,163,163,0.35)' };
}
...
function docStatus(doc) {
  ...
  if (doc.expiry_date) {
    ...
    return { tone: 'ok', label: 'Uploaded', sub: `Expires ${new Date(doc.expiry_date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}` };
  }
...
function DocRow({ type, doc, onChanged }) {
  ...
  const toneColor = tone === 'ok'       ? GREEN
                  : tone === 'expiring' ? '#fbbf24'
                  : tone === 'expired'  ? '#f87171'
                                        : ON_DARK.faint;
  const toneBg    = tone === 'ok'       ? 'rgba(34,197,94,0.14)'
                  : tone === 'expiring' ? 'rgba(251,191,36,0.14)'
                  : tone === 'expired'  ? 'rgba(220,38,38,0.14)'
  ...
  async function handleUpload(e) {
    ...
    try {
      ...
      setFile(null); setExpanded(false);
      await onChanged();
    } catch (err) {
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 13, fontWeight: 900, color: '#ffffff', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{type.label}</div>
          <div style={{ fontSize: 11, color: ON_DARK.muted, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {doc
              ? `${status.sub}${doc.provider ? ` · ${doc.provider}` : ''}`
              : type.hint}
          </div>
        </div>
      ...
      {expanded && (
        <form onSubmit={handleUpload} style={{
          ...
          <label style={{ display: 'block' }}>
            ...
            />
            <span style={{ fontSize: 10, color: ON_DARK.faint, marginTop: 4, display: 'block' }}>PDF or image, max 10 MB</span>
          </label>
          ...
          {error && (
            <div style={{
              padding: 10, borderRadius: 10, fontSize: 11, color: '#fca5a5',
              background: 'rgba(220,38,38,0.14)', border: '1px solid rgba(220,38,38,0.30)',
            }}>{error}</div>
          )}
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <button
              type="submit" disabled={saving || !file}
              style={{
...
function ComplianceSection() {
  ...
  async function reload() {
    setError(null);
    try { setDocs(await listMySubDocs()); }
    catch (e) { setError(e.message || 'Failed to load documents'); }
    finally { setLoading(false); }
  }
  useEffect(() => { reload(); }, []);

  const byType = docs.reduce((acc, d) => { acc[d.doc_type] = d; return acc; }, {});
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, gap: 12 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: ON_DARK.muted, marginTop: 6, lineHeight: 1.5 }}>
            FMs see badges based on what you've uploaded. Missing docs = fewer marketplace opportunities.
          </div>
        </div>
        <div style={{ textAlign: 'right', flexShrink: 0 }}>
          <div style={{ fontSize: 22, fontWeight: 900, color: '#ffffff', letterSpacing: '-0.02em' }}>
            {totalOk}<span style={{ color: ON_DARK.faint, fontSize: 14, fontWeight: 500 }}>/{SUB_DOC_TYPES.length}</span>
          </div>
...
export default function EarnReputation() {
  ...
  useEffect(() => {
    getMyConnectProfile().then(setProfile).catch(() => {});
    listMyFmRatings().then(setRatings).catch(() => setRatings([]));
  }, []);

  const avgStars = ratings.length > 0
    ? Math.round((ratings.reduce((s, r) => s + Number(r.stars || 0), 0) / ratings.length) * 10) / 10
    : null;
      ...
      <div className="relative max-w-5xl mx-auto px-4 md:px-8 pt-6 pb-28 space-y-5 z-10">
          ...
          <div className="relative px-6 md:px-9 py-7 md:py-8 flex flex-col md:flex-row md:items-center gap-6">
            <div className="flex-1 min-w-0">
              <div className="inline-flex items-center gap-2 mb-3">
                ...
                <span className="text-[10px] font-black tracking-[0.28em] text-white/60 uppercase">My Profile</span>
              </div>
              <h1 className="text-white mb-2"
                style={{ fontSize: 26, fontWeight: 900, letterSpacing: '-0.02em', lineHeight: 1.15 }}>
                Your portable{' '}
                <span style={{ color: '#ffb08a' }}>reputation.</span>
              </h1>
              <p className="text-white/60 text-[14px] leading-relaxed max-w-2xl">
                This is how FM companies see you on Cadi Connect. Score updates nightly from your last 90 days of work.
              </p>
            </div>
            ...
            <div className="relative shrink-0" style={{ width: 130 }}>
              ...
              <div style={{
                ...
                <div style={{ color: ON_DARK.muted, fontSize: 9, fontWeight: 900, letterSpacing: '0.22em', textTransform: 'uppercase', marginBottom: 4 }}>Cadi Score</div>
                <div style={{ color: '#ffffff', fontSize: 36, fontWeight: 900, lineHeight: 1, letterSpacing: '-0.02em' }}>{score ?? '—'}</div>
                <div style={{
                  marginTop: 6, padding: '3px 10px', borderRadius: 999,
                  fontSize: 10, fontWeight: 900, color: '#ffffff',
                  background: 'rgba(255,255,255,0.14)', letterSpacing: '0.06em', textTransform: 'uppercase',
                }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              <div style={{ fontSize: 14, fontWeight: 900, color: '#ffffff', letterSpacing: '-0.01em' }}>Availability</div>
              <div style={{ fontSize: 11, color: ON_DARK.muted, marginTop: 3 }}>
                {available ? 'Visible to FMs and accepting new work' : 'Hidden from FMs — not taking on new work'}
              </div>
            </div>
          </div>
          <button
        ...
        <div>
          <div style={{ ...SUBLABEL, marginBottom: 10, marginLeft: 4, display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        <div style={{ ...glassDark({ padding: 20, radius: CONNECT_RADII.xl, strong: true }) }}>
          ...
          {status === 'building' && (
            <div style={{
              ...
              <div style={{ fontSize: 13, fontWeight: 900, color: '#fcd34d', marginBottom: 6 }}>Building reputation</div>
              <div style={{ fontSize: 12, color: '#fef3c7', lineHeight: 1.55 }}>
                {breakdown?.status_reason || `Complete at least 5 jobs to unlock a full Connect Score. You're at ${jobsInWindow}.`}
                {' '}Until then your profile is visible in the marketplace at a neutral score so you can earn work.
              </div>
            </div>
          )}
          ...
          {!metrics && status !== 'building' && (
            <div style={{
              ...
              fontSize: 12, color: ON_DARK.muted, lineHeight: 1.55,
            }}>
              Your score will appear here after the daily recompute (overnight) once you've completed a few jobs. Take photos, capture site contacts, respond fast to FM queries — these all push your score up.
            </div>
          )}
                ...
                return (
                  <div key={key}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                      <div>
                        <span style={{ fontSize: 13, fontWeight: 900, color: '#ffffff' }}>{METRIC_LABEL[key] || key}</span>
                        <span style={{ fontSize: 11, color: ON_DARK.muted, marginLeft: 10 }}>{m.desc}</span>
                      </div>
                      <span style={{ fontSize: 14, fontWeight: 900, color }}>
                        {pts}<span style={{ fontSize: 11, fontWeight: 500, color: ON_DARK.faint }}>/{max}</span>
                      </span>
                    </div>
                    <div style={{ width: '100%', background: 'rgba(255,255,255,0.08)', borderRadius: 999, height: 6 }}>
                      <div style={{ height: 6, borderRadius: 999, width: `${pct}%`, backgroundColor: color, transition: 'width 300ms ease' }} />
                    </div>
                  </div>
                );
              })}
          ...
          <div style={{
            ...
            <span style={{ fontWeight: 900, color: '#ffffff' }}>Connect Score</span> is your portable reputation across the Cadi network. It recomputes daily from your last 90 days of work — approval rate, photos, on-time check-ins, query response speed. FMs use it to award new jobs.
          </div>
        </div>
        ...
        {/* ─── FM REVIEWS ────────────────────────────────────────── */}
        <div style={{ ...glassDark({ radius: CONNECT_RADII.xl, strong: true }), overflow: 'hidden' }}>
          <div style={{
            padding: '16px 20px',
            ...
            {avgStars != null && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ fontSize: 14, fontWeight: 900, color: '#ffffff' }}>{avgStars.toFixed(1)}</span>
                <span style={{ fontSize: 11, color: ON_DARK.muted }}>({ratings.length} rating{ratings.length === 1 ? '' : 's'})</span>
              </div>
            )}
          </div>
          ...
          {ratings.length === 0 ? (
            <div style={{ padding: '32px 20px', textAlign: 'center' }}>
              <div style={{ fontSize: 32, marginBottom: 10, color: 'rgba(255,255,255,0.20)' }}>★</div>
              <div style={{ fontSize: 14, fontWeight: 900, color: '#ffffff', marginBottom: 6, letterSpacing: '-0.01em' }}>No written reviews yet</div>
              <div style={{ fontSize: 12, color: ON_DARK.muted, maxWidth: 420, margin: '6px auto 0', lineHeight: 1.55 }}>
                Your <span style={{ fontWeight: 900, color: '#ffffff' }}>Connect Score breakdown</span> above tracks your automated metrics. Written star reviews appear here whenever an FM rates you on approval.
              </div>
            </div>
          ) : (
            <div>
              {ratings.map((r, idx) => (
                <div key={r.job_id} style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 8 }}>
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 900, color: '#ffffff' }}>{r.siteName}</div>
                      <div style={{ fontSize: 11, color: ON_DARK.muted, marginTop: 3 }}>
                        {r.fmName}
                  ...
                  {r.comment && (
                    <p style={{ fontSize: 13, color: '#e6ecff', lineHeight: 1.55, whiteSpace: 'pre-wrap', margin: 0 }}>{r.comment}</p>
                  )}
                </div>
              ))}
            </div>
