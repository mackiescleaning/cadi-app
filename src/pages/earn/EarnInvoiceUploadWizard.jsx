      ...
      {STEPS.map((label, i) => {
        ...
        return (
          ...
          >
            ...
            {i < STEPS.length - 1 && (
              <div
                style={{ flex: 1, height: 1, background: done ? GREEN : LINE, margin: '0 10px' }}
              />
            )}
          </div>
        );
  ...
  return (
    <>
      <div style={{ fontSize: 13, color: SUB, marginBottom: 14, lineHeight: 1.6 }}>
        Pick the job this invoice is for. The amount is pre-filled from the FM-approved job — you
        can adjust it on the next step. Optionally attach a photo of your paper invoice for the FM's
        records.
      </div>
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        {drafts.length === 0 ? (
          ...
          >
            No drafts yet. A draft appears here the moment one of your FMs approves a completed job
            — then you can upload your invoice for it.
          </div>
        ) : (
          <div
            style={{
              ...
              return (
                ...
                >
                  ...
                  <div style={{ fontSize: 12, fontWeight: 800, color: ORANGE }}>
                    £{Number(d.total_value ?? 0).toFixed(2)}
                  </div>
                </button>
              );
            })}
      ...
      {selectedId && (
        <>
          ...
          >
            ...
            <span style={{ color: MUTE, fontWeight: 700, textTransform: 'none' }}>(optional)</span>
          </div>
          {!photoUrl ? (
            <div
    ...
    <>
      ...
      >
        ...
        <div style={{ flex: 1, fontSize: 12, color: '#3f1d0a', lineHeight: 1.5 }}>
          <strong style={{ color: ORANGE }}>Pre-filled from the approved job.</strong> Check the
          amount matches your invoice and edit if anything's off.
        </div>
      </div>
...
function StepPreview({ draft, fields, photoUrl, onBack, onSend, busy, error }) {
  ...
  return (
    <>
      <div style={{ fontSize: 13, color: SUB, marginBottom: 14, lineHeight: 1.6 }}>
        Last check — this is what will land in{' '}
        <strong style={{ color: INK }}>{draft.fm_organisation?.name ?? 'the FM'}</strong>'s accounts
        inbox.
      </div>
      ...
      >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
          <div>
            ...
            <div style={{ fontSize: 12, fontWeight: 800, color: INK, marginTop: 4 }}>
              {draft.sub?.business_name ?? 'You'}
            </div>
          </div>
          <div>
            <div
...
export default function EarnInvoiceUploadWizard({ drafts, onClose, onSent }) {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- repopulate fields only when the draft id changes
  }, [draft?.id]);
