    ...
    <div style={{ ...glassDark({ padding: 16, radius: CONNECT_RADII.lg, strong: true }) }}>
      <div
        style={{
          ...
          marginBottom: blocks.length > 0 || expanded ? 12 : 0,
        }}
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            <div
              style={{ fontSize: 13, fontWeight: 900, color: '#ffffff', letterSpacing: '-0.01em' }}
            >
              My availability
            </div>
...
function VisitSpecRow({ spec, history, onOpen }) {
  ...
  const scheduleState = nextBooked
    ? { tone: 'ok', text: `Next visit: ${fmtHuman(nextBooked)}` }
    : !isRecurring
      ? null
      : nextDue
        ? { tone: 'due', text: `Due around ${fmtHuman(nextDue)} — schedule it` }
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 12 }}>
        ...
        {spec.duration_minutes && (
          <>
            ...
            <span style={{ fontSize: 10, color: ON_DARK.muted }}>
              {spec.duration_minutes} min on site
            </span>
          </>
        )}
        <div style={{ flex: 1 }} />
        ...
        >
          {scheduleState?.tone === 'ok' ? 'View job card' : 'Schedule visit'}{' '}
...
function JobCardDrawer({ spec, history, blocks = [], onClose, onScheduled }) {
  ...
  useEffect(() => {
    ...
    setError(null);
    setSuccess(null);
    // eslint-disable-next-line react-hooks/exhaustive-deps -- reset the form only when the spec id changes
  }, [spec?.id]);
      ...
      >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 18 }}>
          ...
          <div style={{ ...glassDark({ padding: 14, radius: 12, strong: true }) }}>
            ...
            {spec.duration_minutes && (
              <div style={{ fontSize: 11, color: ON_DARK.muted, marginTop: 4 }}>
                {spec.duration_minutes} min on site
              </div>
            )}
          ...
          <div style={{ marginBottom: 18 }}>
            ...
            {nextBooked && (
              ...
              >
                ...
                <span>
                  Already booked for{' '}
                  <strong style={{ color: '#ffffff' }}>{fmtHuman(nextBooked)}</strong>. Head to Work
                  Completion to check in.
                </span>
              </div>
            )}
            ...
            {!nextBooked && (
              <div style={{ ...glassDark({ padding: 16, radius: 12, strong: true }) }}>
                ...
                {enteredBlock && (
                  ...
                  >
                    <CalendarOff size={12} /> This date falls inside your blocked period (
                    {fmtRange(enteredBlock.start_date, enteredBlock.end_date)}). You can still
                    schedule if you've changed your mind.
                  </div>
                )}
        ...
        >
          ...
          When you're on site, check in via{' '}
          <strong style={{ color: '#ffffff' }}>Connect → Work Completion</strong> to start the
...
export default function EarnPipeline() {
  ...
  async function reloadAvailability() {
    try {
      setBlocks(await listMyAvailability());
    } catch {}
  }
      ...
      <div className="relative max-w-5xl mx-auto px-4 md:px-8 pt-6 pb-28 space-y-5 z-10">
        ...
        >
          <div className="relative px-6 md:px-9 py-7 md:py-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              >
                Every site, <span style={{ color: '#ffd7bf' }}>always in view.</span>
              </h1>
              <p className="text-white/70 text-[14px] leading-relaxed max-w-2xl">
                Recurring work assigned to you across every connected FM. Schedule visits, block
                dates you can't work, keep the FM in the loop.
              </p>
            </div>
        ...
        {loading ? (
          ...
          >
            ...
            >
              Once you win a marketplace listing — or the FM allocates a site directly to you — the
              job appears here.
            </div>
          </div>
        ) : (
            ...
            {Object.entries(byFm).map(([fmId, group]) => (
              <div key={fmId}>
                ...
                >
                  ...
                  <span style={{ fontSize: 10, color: ON_DARK.faint }}>
                    · {group.specs.length} job{group.specs.length === 1 ? '' : 's'}
                  </span>
                </div>
