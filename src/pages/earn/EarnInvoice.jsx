...
import {
  ...
  Layers, Eye, Send, X, Loader2, ChevronRight,
} from 'lucide-react';
import {
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: selected ? '0 0 0 2px rgba(194,65,12,0.35), 0 20px 40px -20px rgba(0,0,0,0.35)' : '0 20px 40px -20px rgba(0,0,0,0.35)',
      ...
      <div style={{ flex: 1, minWidth: 0, cursor: 'pointer' }} onClick={() => onOpen(inv.id)}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 6, marginTop: 12 }}>
          <div style={{
            ...
            <div style={{ fontSize: 14, fontWeight: 900, color: '#ffffff' }}>£{Number(inv.net_value ?? 0).toFixed(2)}</div>
            <div style={{ fontSize: 9, color: ON_DARK.muted, fontWeight: 700, marginTop: 1 }}>net</div>
          </div>
          <div style={{
            background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)',
            borderRadius: 8, padding: '7px 8px', textAlign: 'center',
          }}>
            <div style={{ fontSize: 14, fontWeight: 900, color: '#ffffff' }}>£{Number(inv.vat_value ?? 0).toFixed(2)}</div>
            <div style={{ fontSize: 9, color: ON_DARK.muted, fontWeight: 700, marginTop: 1 }}>VAT</div>
          </div>
          <div style={{
            background: 'rgba(255,176,138,0.16)', border: '1px solid rgba(255,176,138,0.32)',
            borderRadius: 8, padding: '7px 8px', textAlign: 'center',
          }}>
            <div style={{ fontSize: 14, fontWeight: 900, color: '#ffd7bf' }}>£{(Number(inv.net_value ?? 0) + Number(inv.vat_value ?? 0)).toFixed(2)}</div>
            <div style={{ fontSize: 9, color: ON_DARK.muted, fontWeight: 700, marginTop: 1 }}>total</div>
          </div>
        </div>
      </div>
      <ChevronRight size={16} color="rgba(255,255,255,0.55)" style={{ flexShrink: 0, marginTop: 4 }} />
    </div>
  );
}
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 18 }}>
          <div>
            ...
            <div style={{ fontSize: 20, fontWeight: 900, color: '#ffffff', marginTop: 4, letterSpacing: '-0.02em' }}>
              {invoice.fm_organisation?.name ?? 'Customer'}
            </div>
            <div style={{ fontSize: 12, color: ON_DARK.muted, marginTop: 6 }}>
              {lines.length} line{lines.length === 1 ? '' : 's'} · service {fmtDate(invoice.service_date)}
            </div>
          </div>
        ...
        <div style={{ ...glassDark({ padding: 16, radius: 14, strong: true }), marginBottom: 14 }}>
          ...
          {lines.length === 0 && (
            <div style={{ fontSize: 11, color: ON_DARK.faint, fontStyle: 'italic' }}>No line items.</div>
          )}
          {lines.map((l, idx) => (
            <div key={l.id} style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: '#ffffff' }}>{l.description}</div>
                {l.service_date && (
                  <div style={{ fontSize: 10, color: ON_DARK.faint, marginTop: 3 }}>{fmtDate(l.service_date)}</div>
                )}
              </div>
              <div style={{ fontSize: 12, fontWeight: 900, color: '#ffffff', whiteSpace: 'nowrap', marginLeft: 12 }}>
        ...
        {isDraft ? (
          <div style={{ ...glassDark({ padding: 16, radius: 14, strong: true }), marginBottom: 14 }}>
            ...
            <div style={{ fontSize: 11, color: ON_DARK.muted, marginBottom: 12 }}>
              Total after submit: <strong style={{ color: '#ffffff' }}>£{(Number(netInput) + Number(vatInput)).toFixed(2)}</strong>
            </div>
            <label style={{ ...SUBLABEL, textTransform: 'uppercase', display: 'block' }}>
              Note for the FM (optional)
              <textarea value={note} onChange={e => setNote(e.target.value)} rows={2}
                style={{ ...darkInputStyle, display: 'block', width: '100%', marginTop: 6, resize: 'vertical' }}
          ...
          <div style={{
            ...glassDark({ padding: 16, radius: 14, strong: true }), marginBottom: 14,
            fontSize: 12, color: ON_DARK.muted,
          }}>
            <div style={{ color: '#ffffff' }}><strong>Status:</strong> {STATUS_CFG[invoice.status]?.label ?? invoice.status}</div>
            {invoice.submitted_at && <div style={{ marginTop: 6 }}>Submitted {fmtDate(invoice.submitted_at)}</div>}
            {invoice.exported_at  && <div style={{ marginTop: 6 }}>Exported to FM accounts {fmtDate(invoice.exported_at)}</div>}
            {invoice.paid_at      && <div style={{ marginTop: 6, color: GREEN, fontWeight: 700 }}>✓ Paid {fmtDate(invoice.paid_at)}</div>}
            {invoice.note         && <div style={{ marginTop: 8, fontStyle: 'italic', color: '#ffffff' }}>{invoice.note}</div>}
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button
            ...
            {busy === 'preview'
              ? <><Loader2 size={12} style={{ animation: 'connectSpin 0.8s linear infinite' }} /> Generating…</>
          ...
          {isDraft && (
            <button
              ...
              {busy === 'submit'
                ? <><Loader2 size={12} style={{ animation: 'connectSpin 0.8s linear infinite' }} /> Submitting…</>
...
export default function EarnInvoice() {
  ...
  const totalDraftValue = drafts.reduce(
    (s, i) => s + Number(i.net_value ?? 0) + Number(i.vat_value ?? 0), 0
  );
      ...
      <div className="relative max-w-5xl mx-auto px-4 md:px-8 pt-6 pb-28 space-y-5 z-10">
          ...
          <div className="relative px-6 md:px-9 py-7 md:py-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <div className="inline-flex items-center gap-2 mb-3">
                ...
                <span className="text-[10px] font-black tracking-[0.28em] text-white/70 uppercase">Invoicing</span>
              </div>
              <h1 className="text-white mb-2"
                style={{ fontSize: 26, fontWeight: 900, letterSpacing: '-0.02em', lineHeight: 1.15 }}>
                One approved job,{' '}
                <span style={{ color: '#ffd7bf' }}>one invoice.</span>
              </h1>
              <p className="text-white/70 text-[14px] leading-relaxed max-w-2xl">
                Tick several drafts for the same FM and merge them into a bundled invoice. Preview the PDF before you send.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-2 shrink-0" style={{ minWidth: 220 }}>
              <button
                ...
                style={{
                  ...
                  boxShadow: drafts.length === 0 ? 'none' : '0 10px 24px -12px rgba(255,255,255,0.4)',
              ...
              <div className="grid grid-cols-2 gap-2">
                <div style={{ ...glassDark({ padding: 10, radius: 12, strong: true }), textAlign: 'center' }}>
                  <div style={{ fontSize: 16, fontWeight: 900, color: '#ffd7bf', lineHeight: 1 }}>£{totalDraftValue.toFixed(0)}</div>
                  <div style={{ fontSize: 9, fontWeight: 800, color: ON_DARK.muted, letterSpacing: '0.12em', textTransform: 'uppercase', marginTop: 5 }}>drafts</div>
                </div>
                <div style={{ ...glassDark({ padding: 10, radius: 12, strong: true }), textAlign: 'center' }}>
                  <div style={{ fontSize: 16, fontWeight: 900, color: GREEN, lineHeight: 1 }}>£{paidValue.toFixed(0)}</div>
                  <div style={{ fontSize: 9, fontWeight: 800, color: ON_DARK.muted, letterSpacing: '0.12em', textTransform: 'uppercase', marginTop: 5 }}>paid</div>
                </div>
              </div>
            </div>
          </div>
        ...
        {loading ? (
          ...
          <div style={{
            ...
            <div style={{ fontSize: 16, fontWeight: 900, color: '#ffffff', letterSpacing: '-0.01em' }}>Nothing here</div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, paddingBottom: selectedDrafts.length > 0 ? 100 : 0 }}>
            {filtered.map(inv => {
              const isDraft = inv.status === 'draft';
        ...
        {selectedDrafts.length > 0 && (
          <div style={{
            position: 'sticky', bottom: 12, marginTop: 16, zIndex: 30,
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 13, fontWeight: 900, color: '#ffffff' }}>
                {selectedDrafts.length} draft{selectedDrafts.length === 1 ? '' : 's'} selected · {selectedDrafts[0]?.fm_organisation?.name ?? ''}
              </div>
              <div style={{ fontSize: 11, color: ON_DARK.muted, marginTop: 3 }}>
                Net £{selectedDrafts.reduce((s, i) => s + Number(i.net_value ?? 0), 0).toFixed(2)} · merged into one bundled invoice
              </div>
            </div>
            <button onClick={clearSelection} style={{
              background: 'transparent', color: '#ffffff',
              border: '1px solid rgba(255,255,255,0.20)', borderRadius: 10,
              fontSize: 11, fontWeight: 800, padding: '8px 14px', cursor: 'pointer',
            }}>
              Cancel
            </button>
            <button
              ...
              style={{
                ...
                border: '1px solid rgba(255,255,255,0.4)',
                borderRadius: 10, fontSize: 12, fontWeight: 900, padding: '10px 16px',
                cursor: selectedDrafts.length < 2 ? 'not-allowed' : 'pointer',
                display: 'inline-flex', alignItems: 'center', gap: 6,
              }}>
              {merging
                ? <><Loader2 size={12} style={{ animation: 'connectSpin 0.8s linear infinite' }} /> Merging…</>
        ...
        {openDetail && (
          <InvoiceDetailDrawer
            ...
            onSubmitted={() => { setOpenDetailId(null); reload(); }}
          />
        )}
