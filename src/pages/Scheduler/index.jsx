...
import {
  ...
  fmtMoney,
  deriveCrews,
  getToday, getMonday, isoDate, getViewDate, getWeekDates, getCurrentQuarter,
...
function Stat({ label, value, color = "text-white", emphasis }) {
  return (
    <div className="flex items-baseline gap-1.5">
      ...
      <span className="text-[11px] uppercase tracking-wider font-medium" style={{ color: 'rgba(153,197,255,0.7)' }}>{label}</span>
    </div>
  );
}
  ...
  return (
    <div className="flex items-center gap-2">
      {view !== "Quarter" && view !== "Rounds" && (
        <>
          <button onClick={() => setDayOffset(o => o - 1)} className="p-1.5 rounded-lg hover:bg-white/10 transition-colors" style={{ color: 'rgba(153,197,255,0.85)' }}>
            <ChevronLeft size={18} />
          </button>
          <div className="min-w-[200px] text-center">
            <div className="text-[10px] font-semibold uppercase tracking-wider" style={{ color: 'rgba(153,197,255,0.7)' }}>
...
function FilterBar({ typeFilter, setTypeFilter, crews, crewFilter, setCrewFilter }) {
  return (
    <div className="flex items-center gap-3 flex-wrap">
      <div className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wider" style={{ color: 'rgba(153,197,255,0.7)' }}>
        <Filter size={12} /> Type
      </div>
      ...
      {crews.length > 0 && (
        <>
          ...
          <div className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: 'rgba(153,197,255,0.7)' }}>Crew</div>
            ...
            {crews.map((c) => {
              ...
              return (
                <button
                  ...
                  style={{
                    background: isActive ? 'linear-gradient(135deg, #1f48ff 0%, #3a6bff 100%)' : 'rgba(255,255,255,0.08)',
...
function FilterPill({ active, onClick, label, type }) {
  ...
  return (
    <button
      ...
      className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold border transition-all text-white"
      style={{
        background: active ? 'linear-gradient(135deg, #1f48ff 0%, #3a6bff 100%)' : 'rgba(255,255,255,0.08)',
        ...
        return (
          ...
          >
            ...
            <div className="flex gap-0.5 h-1.5 items-center justify-center mt-0.5">
              {dotTypes.length > 0
                ? dotTypes.map(type => (
                    <div key={type} className={`w-1.5 h-1.5 rounded-full ${isActive ? 'bg-white/70' : (TYPE[type]?.dot || 'bg-[#99c5ff]/40')}`} />
                  ))
...
export default function SchedulerTab({ onJobClick: externalJobClick }) {
  ...
  const { jobs: contextJobs, addJobAndSyncCustomer, updateJob, deleteJob, customers, refreshJobs } = useData();
  ...
  const handleUpdateJob = async (id, updates) => {
    ...
    try {
      ...
      if (prior) {
        ...
        if ('status' in updates) label = updates.status === 'complete'
          ? `Marked ${labelForJob(prior)} done`
          : `Changed ${labelForJob(prior)} status`;
        pushUndo({ kind: 'update', id, inverse, label });
      }
    } catch (err) {
      showError(err?.message?.includes('row-level') ? "You don't have permission to update that job." : "Couldn't save the job. Try again.");
      throw err;
    }
  };
  const handleDeleteJob = async (id) => {
    ...
    try {
      ...
      showError(err?.message?.includes('row-level') ? "You don't have permission to delete that job." : "Couldn't delete the job. Try again.");
      throw err;
    }
  };
  ...
  const undoableUpdateJob = useCallback((id, updates) => handleUpdateJob(id, updates), [contextJobs]); // eslint-disable-line react-hooks/exhaustive-deps
  const undoableDeleteJob = useCallback((id)         => handleDeleteJob(id),          [contextJobs]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <div className="relative flex flex-col min-h-full overflow-hidden" style={{
      ...
      <div aria-hidden className="pointer-events-none fixed inset-0 -z-0">
        <div className="absolute top-[8%] left-[10%] w-[520px] h-[520px] rounded-full opacity-50"
             style={{ background: "radial-gradient(circle, #1f48ff 0%, transparent 70%)", filter: "blur(60px)" }} />
        <div className="absolute top-[40%] right-[5%] w-[600px] h-[600px] rounded-full opacity-40"
             style={{ background: "radial-gradient(circle, #3a6bff 0%, transparent 70%)", filter: "blur(70px)" }} />
        <div className="absolute bottom-[-10%] left-[30%] w-[500px] h-[500px] rounded-full opacity-35"
             style={{ background: "radial-gradient(circle, #99c5ff 0%, transparent 70%)", filter: "blur(70px)" }} />
      </div>
      ...
      >
        <div className="px-4 sm:px-6 py-3">
          <div className="flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3">
              <div>
                <p className="text-[10px] font-bold tracking-[0.15em] uppercase mb-0.5" style={{ color: '#99c5ff' }}>Cadi</p>
                <h2 className="text-xl font-black leading-tight text-white">Scheduler</h2>
              </div>
              <div className="h-10 w-px hidden sm:block" style={{ background: 'rgba(153,197,255,0.18)' }} />
              <DateNav view={view} dayOffset={dayOffset} setDayOffset={setDayOffset} />
            </div>

            <div className="flex items-center gap-2">
              <div className="relative hidden md:block">
                <Search size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2" style={{ color: 'rgba(153,197,255,0.55)' }} />
                <input
                  value={search}
              ...
              <button
                ...
                style={{
                  background: search || mobileSearchOpen ? 'rgba(31,72,255,0.25)' : 'rgba(255,255,255,0.08)',
                  borderColor: search || mobileSearchOpen ? 'rgba(31,72,255,0.55)' : 'rgba(153,197,255,0.18)',
                }}
              >
                <Search size={14} />
              </button>
              ...
              <button
                onClick={undo}
                disabled={!canUndo}
                title={canUndo ? `Undo: ${undoStack[undoStack.length - 1]?.label} (⌘Z)` : 'Nothing to undo'}
        ...
        {view === "Day" && (
          <div className="border-t backdrop-blur" style={{ borderColor: 'rgba(153,197,255,0.12)', background: 'rgba(1,10,79,0.35)' }}>
      ...
      <div className="relative z-0 flex-1 overflow-y-auto p-4 lg:p-6">
        <div className="max-w-[1600px] mx-auto space-y-4">
          {view === "Day"     && <DayView     jobs={todayJobs} onJobClick={handleJobClick} typeFilter={typeFilter} crewFilter={crewFilter} updateJob={undoableUpdateJob} onAddJob={() => setShowNewJob(true)} onImport={() => navigate('/onboarding/customers')} weather={weather} onError={showError} />}
          {view === "Week"    && <WeekView    jobs={weekJobs} onJobClick={handleJobClick} weekDates={weekDates} typeFilter={typeFilter} crewFilter={crewFilter} weather={weather} onAddJob={() => setShowNewJob(true)} updateJob={undoableUpdateJob} onError={showError} />}
      ...
      {activeJob && (
        <JobDrawer
          ...
          risk={detectJobRisks(activeJob, {
            ...
            customer: activeJob.customerId ? customers.find(c => c.id === activeJob.customerId) : null,
          })}
        />
      )}
