...
import { ArrowLeft, Plus, Trash2, Save, Star, ChevronDown, ChevronUp, AlertCircle } from 'lucide-react';
import { getCatalogue, quotePrice } from '../lib/catalogue';
import {
  createService, deleteService, saveServiceWithChildren,
} from '../lib/db/catalogueDb';
...
const BOOKING_MODES = [
  { key: 'instant',     label: 'Instant book',  hint: 'Customers see a fixed price and book straight in.' },
  { key: 'quick_quote', label: 'Quick quote',   hint: 'Customers pick a tier, see price, then book.' },
...
export default function CatalogueEditor() {
  ...
  const load = useCallback(async () => {
    try { setCatalogue(await getCatalogue()); }
    catch (e) { setError(e?.message ?? "Couldn't load your catalogue."); }
  }, []);
  ...
  const handleDelete = async (id, name) => {
    ...
    catch (e) { setError(e?.message ?? "Couldn't delete."); }
  };
  ...
  if (!catalogue) {
    return <div className="min-h-full text-[#010a4f] flex items-center justify-center p-6"><p className="text-sm text-[#1f48ff]">Loading catalogue…</p></div>;
  }
    ...
    <div className="min-h-full text-[#010a4f]">
      <div className="relative max-w-3xl mx-auto px-4 sm:px-6 py-6">
        <div className="flex items-end justify-between mb-6">
          <div>
            <p className="text-[10px] font-bold tracking-[0.15em] uppercase text-[#1f48ff] mb-0.5">Cadi</p>
          ...
          return (
            <section key={d.key} className="mb-7">
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-[11px] font-bold px-2 py-0.5 rounded-full border" style={{ borderColor: `${d.accent}50`, color: d.accent, background: `${d.accent}15` }}>
                  {list.length}
                </span>
                <button
              ...
              {list.length === 0 ? (
                <p className="text-[11px] text-[#010a4f]/45 italic py-3 px-1">No {d.label.toLowerCase()} services yet.</p>
              ) : (
...
function ServiceCard({ service, accent, expanded, onToggle, saving, onSave, onDelete }) {
  ...
  useEffect(() => { setDraft(initDraft(service)); }, [service]);
  ...
  const previewSvc = useMemo(() => ({
    ...
    pricing_config: draft.pricing_model === 'flat' && draft.flat_price != null
                      ? { price: Number(draft.flat_price) } : null,
    tiers:          draft.tiers,
    units:          draft.units,
  ...
  const save = () => onSave({
    core: {
      ...
      pricing_config:        draft.pricing_model === 'flat' && draft.flat_price != null
                               ? { price: Number(draft.flat_price) } : null,
      price_fixed_basic:     draft.pricing_model === 'flat' && draft.flat_price != null
                               ? Number(draft.flat_price) : null,
  ...
  return (
    <div className="rounded-2xl border border-[#1f48ff]/15 overflow-hidden bg-white shadow-sm">
      <div className="relative pl-4 pr-3 py-3 cursor-pointer flex items-start gap-3" onClick={onToggle}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] text-[#010a4f]/60 mt-0.5 truncate">
            {PRICING_MODELS.find(p => p.key === draft.pricing_model)?.label.split(' — ')[0]} · {BOOKING_MODES.find(b => b.key === draft.booking_mode)?.label}
          </p>
        </div>
          ...
          {quote.price === 'enquiry' ? (
            ...
            return (
              <p className="text-sm font-black text-emerald-600 tabular-nums">
                {isTieredMulti && <span className="text-[10px] font-bold text-emerald-600/70 uppercase mr-1">from</span>}
                £{display.toFixed(2)}
              </p>
            );
          })()}
        </div>
        <button className="text-[#010a4f]/45">{expanded ? <ChevronUp size={14} /> : <ChevronDown size={14} />}</button>
      </div>
        ...
        <div className="px-4 pb-4 pt-1 space-y-4 border-t border-[#1f48ff]/15">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Service name">
              <input type="text" value={draft.name} onChange={e => setField('name', e.target.value)} className={inp} />
            </Field>
            <Field label="Duration (mins)">
              <input type="number" min="0" step="5" value={draft.duration_mins ?? ''} onChange={e => setField('duration_mins', e.target.value === '' ? null : Number(e.target.value))} className={inp} />
            </Field>
          </div>
          <Field label="Description (shown to customers)">
            <textarea value={draft.description ?? ''} onChange={e => setField('description', e.target.value)} rows={2} className={`${inp} resize-none leading-snug`} placeholder="One short sentence — what's included." />
          </Field>

          {/* Booking mode + pricing model */}
          <div className="grid grid-cols-2 gap-3">
            <Field label="Booking mode">
              <select value={draft.booking_mode} onChange={e => setField('booking_mode', e.target.value)} className={inp}>
                {BOOKING_MODES.map(b => <option key={b.key} value={b.key}>{b.label}</option>)}
              </select>
            </Field>
            <Field label="Pricing model">
              <select value={draft.pricing_model} onChange={e => setField('pricing_model', e.target.value)} className={inp}>
                {PRICING_MODELS.map(p => <option key={p.key} value={p.key}>{p.label}</option>)}
              </select>
            </Field>
          </div>
          ...
          {draft.pricing_model === 'flat' && (
            <Field label="Price">
              <div className="flex items-center gap-2">
                ...
                <input type="number" min="0" step="1" value={draft.flat_price ?? ''} onChange={e => setField('flat_price', e.target.value === '' ? null : Number(e.target.value))} className={inp} />
              </div>
            </Field>
          )}
          ...
          {draft.pricing_model === 'tiered' && (
            <TierEditor
              tiers={draft.tiers}
              onChange={list => setNested('tiers', list)}
            />
          )}
          ...
          {draft.pricing_model === 'by_unit' && (
            <UnitEditor
              units={draft.units}
              onChange={list => setNested('units', list)}
            />
          )}
          ...
          <div className="flex items-center justify-between pt-2 border-t border-[#1f48ff]/15">
            <button onClick={onDelete} className="text-[11px] font-bold text-red-600/80 hover:text-red-700 px-2 py-1 rounded-md hover:bg-red-50 transition-colors flex items-center gap-1">
              <Trash2 size={11} /> Delete service
            </button>
            <button onClick={save} disabled={saving} className="text-[12px] font-black bg-[#1f48ff] hover:bg-[#3a5eff] text-white px-4 py-2 rounded-lg disabled:opacity-50 transition-colors flex items-center gap-1.5">
...
function TierEditor({ tiers, onChange }) {
  ...
  const setTier = (i, patch) => onChange(tiers.map((t, idx) => idx === i ? { ...t, ...patch, is_estimated: false } : t));
  const remove  = (i) => onChange(tiers.filter((_, idx) => idx !== i));
  const setDefault = (i) => onChange(tiers.map((t, idx) => ({ ...t, is_default: idx === i })));
  const add = () => onChange([...tiers, { tier_key: `tier${tiers.length + 1}`, label: 'New tier', price: 0, is_default: tiers.length === 0 }]);
    ...
    <Field label="Tiers">
      <div className="space-y-1.5">
        {tiers.map((t, i) => (
          ...
          >
            <button onClick={() => setDefault(i)} title={t.is_default ? 'Default tier' : 'Make default'} className="shrink-0">
              <Star size={13} className={t.is_default ? 'text-amber-300 fill-amber-300' : 'text-[#010a4f]/40 hover:text-amber-300'} />
            </button>
            <input
              type="text" value={t.label} onChange={e => setTier(i, { label: e.target.value })}
            ...
            <span className={`text-[11px] font-bold ${t.is_estimated ? 'text-amber-700/80' : 'text-emerald-600/70'}`}>£</span>
            <input
              type="number" min="0" step="1" value={t.price ?? ''}
              onChange={e => setTier(i, { price: e.target.value === '' ? 0 : Number(e.target.value) })}
            ...
            />
            <button onClick={() => remove(i)} className="w-6 h-6 rounded-md text-[#010a4f]/40 hover:text-red-600 hover:bg-red-50 transition-colors flex items-center justify-center">
              <Trash2 size={11} />
            </button>
          </div>
        ))}
        <button onClick={add} className="w-full py-1.5 text-[11px] font-bold text-[#1f48ff] hover:text-[#010a4f] border border-dashed border-[#1f48ff]/30 hover:border-[#1f48ff]/60 rounded-lg transition-all flex items-center justify-center gap-1">
    ...
    <Field label={`Price per unit`}>
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <select value={u.unit_type} onChange={e => setU({ unit_type: e.target.value })} className={inp}>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-emerald-600 font-bold">£</span>
          <input type="number" min="0" step="0.10" value={u.price_per_unit ?? ''} onChange={e => setU({ price_per_unit: e.target.value === '' ? 0 : Number(e.target.value) })} className={inp} />
        </div>
        <div className="flex items-center gap-2">
          <span className="text-[11px] text-[#010a4f]/60 w-16">Min charge</span>
          <span className="text-sm text-emerald-600 font-bold">£</span>
          <input type="number" min="0" step="1" value={u.min_charge ?? ''} onChange={e => setU({ min_charge: e.target.value === '' ? null : Number(e.target.value) })} className={inp} placeholder="optional floor" />
        </div>
      </div>
    </Field>
  );
...
function ModifierEditor({ modifiers, onChange }) {
  const setM = (i, patch) => onChange(modifiers.map((m, idx) => idx === i ? { ...m, ...patch } : m));
  const remove = (i) => onChange(modifiers.filter((_, idx) => idx !== i));
  const add = () => onChange([...modifiers, { key: `mod${modifiers.length + 1}`, label: 'New add-on', type: 'addon_fixed', value: 0, default_on: false }]);

  return (
    <Field label="Add-ons & modifiers (optional)">
      <div className="space-y-1.5">
        {modifiers.map((m, i) => (
          <div key={i} className="flex items-center gap-2 rounded-lg border border-[#1f48ff]/15 bg-[#f0f4ff] p-2">
            <input
              type="checkbox" checked={Boolean(m.default_on)} onChange={e => setM(i, { default_on: e.target.checked })}
              title="Default on (auto-applied)"
            />
            <input
              type="text" value={m.label} onChange={e => setM(i, { label: e.target.value })}
              className="flex-1 bg-transparent border-0 text-[12px] text-[#010a4f] focus:outline-none"
              placeholder="Label"
            />
            <select value={m.type} onChange={e => setM(i, { type: e.target.value })} className="text-[10px] bg-white border border-[#1f48ff]/15 rounded-md px-1.5 py-0.5 text-[#010a4f] focus:outline-none focus:border-[#1f48ff]">
              {MODIFIER_TYPES.map(t => <option key={t.key} value={t.key}>{t.label}</option>)}
            </select>
            <input
              type="number" min="0" step="0.50" value={m.value ?? ''}
              onChange={e => setM(i, { value: e.target.value === '' ? 0 : Number(e.target.value) })}
...
const inp = "w-full bg-white border border-[#1f48ff]/15 rounded-lg px-2.5 py-1.5 text-[12px] text-[#010a4f] placeholder-[#010a4f]/35 focus:outline-none focus:border-[#1f48ff]";
