        ...
        <div className="bg-[#111118] border border-[rgba(153,197,255,0.12)] rounded-2xl p-8">
          ...
          {step === 'error' && (
            <div className="text-center py-4">
              ...
              <button
                onClick={() => navigate('/')}
                className="text-sm font-black text-[#1f48ff] hover:underline"
              >
                Go to Cadi →
              </button>
            </div>
              ...
              <p className="text-sm text-[rgba(153,197,255,0.5)] mb-6 leading-relaxed">
                {isFmOps ? (
                  <>
                    ...
                    <strong className="text-white">{bizName}</strong> portal on Cadi Connect — where
                    you'll upload contracts, allocate sub-contractors, and run the marketplace.
                    Enter your email for a one-time sign-in link — no password needed.
                  </>
                ) : isConnect ? (
                  <>
                    You've been invited to <strong className="text-white">{bizName}</strong> on Cadi
                    Connect — how you'll receive jobs, complete work on site, and get paid. Enter
                    your email for a one-time sign-in link — no password needed.
                  </>
                ) : (
                  <>
                    You've been invited as <strong className="text-white">{roleLabel}</strong> for{' '}
                    {bizName}. Enter your email to receive a one-time sign-in link — no password
                    needed.
                  </>
                )}
              </p>
              <form onSubmit={handleSendOtp} className="space-y-3">
                ...
                <p className="text-[10px] text-center text-[rgba(153,197,255,0.4)] mt-2">
                  ...
                  <Link
                    to="/terms"
                    target="_blank"
                    className="text-[rgba(153,197,255,0.7)] hover:text-white underline"
                  >
                    Terms
                  </Link>{' '}
                  and{' '}
          ...
          {step === 'otp_sent' && (
            <div className="text-center py-4">
              ...
              <p className="text-sm text-[rgba(153,197,255,0.5)] leading-relaxed">
                We've sent a sign-in link to <strong className="text-white">{email}</strong>. Click
                it to continue — the link expires in 1 hour.
              </p>
              <button
          ...
          {step === 'preview' && invite && (
            <>
              ...
              <h2 className="text-xl font-black text-white mb-2">
                {isFmOps
                  ? `Set up the ${bizName} portal?`
                  : isConnect
                    ? `Join ${bizName} on Cadi Connect?`
                    : `Accept access to ${bizName}?`}
              </h2>
              <p className="text-sm text-[rgba(153,197,255,0.5)] mb-6 leading-relaxed">
                {isFmOps ? (
                  <>
                    ...
                    on <strong className="text-white">{bizName}</strong>. Your portal includes
                    contracts, the sub-contractor marketplace, work approval, and accounts export.
                  </>
                ) : isConnect ? (
                  <>
                    Your account is free — <strong className="text-white">{bizName}</strong> is
                    covering it. You'll see their jobs in the Connect tab inside Cadi.
                  </>
                ) : (
                  <>
                    You'll be added as <strong className="text-white">{roleLabel}</strong> for{' '}
                    {bizName}. The business owner controls what you can view and change.
                  </>
                )}
              </p>
                ...
                {(isFmOps
                  ...
                  : isConnect
                    ...
                    : [
                        ...
                        {
                          label: 'Your role',
                          val: roleLabel.charAt(0).toUpperCase() + roleLabel.slice(1),
                        },
          ...
          {step === 'done' && (
            <div className="text-center py-4">
              ...
              <p className="text-sm text-[rgba(153,197,255,0.5)] mb-2 leading-relaxed">
                {isFmOps ? (
                  <>
                    You're now an {invite?.role === 'admin' ? 'admin' : 'member'} on{' '}
                    <strong className="text-white">{bizName}</strong>'s FM Ops portal.
                  </>
                ) : isConnect ? (
                  <>
                    You can now see {bizName}'s jobs in the{' '}
                    <strong className="text-white">Connect</strong> tab.
                  </>
                ) : (
              ...
              <p className="text-xs text-[rgba(153,197,255,0.35)] mb-6">
                ...
                <strong className="text-[rgba(153,197,255,0.6)]">app.cadi.cleaning</strong> and use
