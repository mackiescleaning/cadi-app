/**
 * src/pages/GocardlessCallback.jsx
 * Cadi — GoCardless OAuth redirect landing page
 *
 * GoCardless redirects here after the user grants permission:
 *   https://app.cadi.cleaning/gocardless/callback?code=XXXX&state=YYYY
 *
 * Route is registered OUTSIDE <ProtectedRoute> in App.js (same as HMRC callback).
 */
...
function Spinner() {
  return (
    <svg className="animate-spin h-10 w-10 text-[#1f48ff] mx-auto mb-4" fill="none" viewBox="0 0 24 24">
...
export default function GocardlessCallback() {
  ...
  useEffect(() => {
    ...
    if (error || (!code && !state)) {
      ...
      setTimeout(() => navigate('/settings?tab=integrations', { replace: true }), 3000);
      return;
    }
    ...
    supabase.auth.refreshSession()
      ...
      .then(async (session) => {
        const { data, error: fnErr } = await supabase.functions.invoke('gocardless-auth', {
          body:    { action: 'callback', code, state },
          headers: { Authorization: `Bearer ${session.access_token}` },
        });
        if (fnErr) throw new Error(fnErr.message ?? 'gocardless-auth error');
        if (data?.error) throw new Error(data.error);
        return data;
      })
      .then(() => {
        setPhase('success');
        setTimeout(() => navigate('/settings?tab=integrations', { replace: true }), 2500);
      })
      .catch((err) => {
        setPhase('error');
        setDetail(err.message ?? 'Connection failed — please try again.');
        setTimeout(() => navigate('/settings?tab=integrations', { replace: true }), 4000);
      });
      ...
      <div className={CARD}>
        ...
        <div className="mt-6 h-0.5 bg-white/10 rounded-full overflow-hidden">
          <div className={`h-full rounded-full transition-all duration-[2500ms] ease-linear ${
            ...
            phase === 'loading' ? 'bg-[#1f48ff] w-1/2 animate-pulse' :
            'bg-red-400 w-full'
          }`} />
        </div>
