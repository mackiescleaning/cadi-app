...
import {
  Shield, Building2, Mail, Phone, MapPin, ExternalLink,
...
function DetailRow({ label, value }) {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr', gap: 12, padding: '8px 0', fontSize: 12 }}>
  ...
  return (
    <div style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
              ...
              <span style={{ fontSize: 10, color: MUTE }}>Submitted {new Date(app.created_at).toLocaleString()}</span>
            </div>
            <div style={{ fontSize: 19, fontWeight: 900, color: INK }}>{app.company_name}</div>
            {app.company_website && (
              <a href={app.company_website} target="_blank" rel="noreferrer" style={{ fontSize: 11, color: ACCENT, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 3, marginTop: 4 }}>
        ...
        {/* Contact */}
        <div style={{ background: PAPER, border: `1px solid ${LINE}`, borderRadius: 10, padding: 14, marginBottom: 12 }}>
          ...
          {app.contact_role && <div style={{ fontSize: 11, color: SUB, marginTop: 2 }}>{app.contact_role}</div>}
          <div style={{ display: 'flex', gap: 12, marginTop: 8, flexWrap: 'wrap' }}>
            <a href={`mailto:${app.contact_email}`} style={{ fontSize: 11, color: ACCENT, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
              <Mail size={11} /> {app.contact_email}
            </a>
            {app.contact_phone && (
              <span style={{ fontSize: 11, color: SUB, display: 'inline-flex', alignItems: 'center', gap: 4 }}>
        ...
        {/* Company */}
        <div style={{ background: PAPER, border: `1px solid ${LINE}`, borderRadius: 10, padding: 14, marginBottom: 12 }}>
          <div style={{ fontSize: 10, fontWeight: 800, color: SUB, letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 8 }}>Company</div>
          <DetailRow label="Size"            value={app.company_size} />
          <DetailRow label="Sites managed"   value={app.sites_managed != null ? `${app.sites_managed}` : null} />
          <DetailRow label="Sub-contractors" value={app.current_subs  != null ? `${app.current_subs}`  : null} />
          ...
          {app.business_model && (
            <div style={{ marginTop: 8 }}>
              <div style={{ fontSize: 10, color: SUB, fontWeight: 700, marginBottom: 4 }}>Business model</div>
              <div style={{ fontSize: 12, color: INK, lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>{app.business_model}</div>
            </div>
          )}
        </div>
        ...
        {app.why_cadi && (
          <div style={{ background: PAPER, border: `1px solid ${LINE}`, borderRadius: 10, padding: 14, marginBottom: 14 }}>
            <div style={{ fontSize: 10, fontWeight: 800, color: SUB, letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 8 }}>Why Cadi</div>
            <div style={{ fontSize: 12, color: INK, lineHeight: 1.65, whiteSpace: 'pre-wrap' }}>{app.why_cadi}</div>
          </div>
        )}

        {/* Reviewed state */}
        {app.status === 'rejected' && app.rejection_reason && (
          <div style={{ padding: 12, background: '#fef2f2', border: '1px solid #fca5a5', borderRadius: 10, marginBottom: 14, fontSize: 12, color: '#991b1b' }}>
            <strong>Rejection reason:</strong> {app.rejection_reason}
          </div>
        )}
        {app.status === 'approved' && (
          <div style={{ padding: 12, background: `${GREEN}10`, border: `1px solid ${GREEN}30`, borderRadius: 10, marginBottom: 14, fontSize: 12, color: '#065f46' }}>
        ...
        {result?.kind === 'approved' && (
          <div style={{ padding: 14, background: `${GREEN}10`, border: `1px solid ${GREEN}30`, borderRadius: 10, marginBottom: 14, fontSize: 13, color: INK }}>
            <CheckCircle2 size={14} color={GREEN} style={{ verticalAlign: 'middle', marginRight: 6 }} />
            <strong>Approved.</strong> Invite {result.email_sent ? 'emailed' : '(email skipped — no Resend key)'}. Token: <code style={{ fontSize: 11 }}>{result.token?.slice(0,12)}…</code>
          </div>
        )}
        {result?.kind === 'rejected' && (
          <div style={{ padding: 14, background: '#fef2f2', border: '1px solid #fca5a5', borderRadius: 10, marginBottom: 14, fontSize: 13, color: '#991b1b' }}>
            ...
            {!showRejectForm ? (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                <button
                  ...
                  disabled={!!busy}
                  style={{
                    background: busy === 'approve' ? `${GREEN}80` : GREEN, color: 'white', border: 'none',
                    borderRadius: 8, padding: '12px 16px', fontSize: 13, fontWeight: 800,
                    cursor: busy ? 'not-allowed' : 'pointer',
                    display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
                  }}
                >
                  {busy === 'approve' && <Loader2 size={13} style={{ animation: 'spin 0.8s linear infinite' }} />}
              ...
              <>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button onClick={() => { setShowRejectForm(false); setRejectReason(''); }} style={{
                    background: PAPER, color: SUB, border: `1px solid ${LINE}`,
                    borderRadius: 8, padding: '10px 14px', fontSize: 12, fontWeight: 700, cursor: 'pointer',
                  }}>Cancel</button>
                  <button
                    onClick={doReject}
                    ...
                    style={{
                      ...
                      borderRadius: 8, padding: '10px 14px', fontSize: 13, fontWeight: 800,
                      cursor: busy ? 'not-allowed' : 'pointer',
                      display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
                    }}
                  >
                    {busy === 'reject' && <Loader2 size={13} style={{ animation: 'spin 0.8s linear infinite' }} />}
                    Confirm reject
                  </button>
                </div>
              </>
...
export default function AdminFmApplications() {
  ...
  const loadCounts = async () => {
    try {
      ...
      all.forEach(a => { if (c[a.status] != null) c[a.status]++; });
      setCounts(c);
    } catch {}
  };
  ...
  const load = async () => {
    ...
    try { setApps(await listApplications({ status: tab })); }
    catch (e) { setError(e.message); }
    finally { setLoading(false); }
  };

  useEffect(() => { if (isAdmin) load(); }, [isAdmin, tab]);
  useEffect(() => { if (isAdmin) loadCounts(); }, [isAdmin]);
    ...
    return (
      <div style={{ minHeight: '100vh', background: BG, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
        <div style={{ maxWidth: 480, background: PAPER, border: `1px solid ${LINE}`, borderRadius: 14, padding: 32, textAlign: 'center' }}>
          <div style={{ width: 56, height: 56, borderRadius: 14, background: '#fef2f2', color: '#b91c1c', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 18px' }}>
            <Shield size={26} />
          </div>
          ...
          <Link to="/dashboard" style={{ display: 'inline-block', background: NAVY, color: 'white', textDecoration: 'none', borderRadius: 8, padding: '10px 18px', fontSize: 13, fontWeight: 800 }}>
            ← Back to Cadi
          </Link>
        </div>
      </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: BG, padding: '28px 32px' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        <div style={{
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: SUB, marginTop: 4 }}>
              Review prospective FM partners. Approving creates the org + sends the first-admin invite email.
            </div>
          </div>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 14 }}>
          <Kpi label="Pending"   value={counts.pending ?? 0}   accent={counts.pending ? ACCENT : MUTE} />
        ...
        <div style={{ display: 'flex', gap: 4, background: PAPER, border: `1px solid ${LINE}`, borderRadius: 8, padding: 4, marginBottom: 16, width: 'fit-content' }}>
          {TABS.map(t => {
            ...
            return (
              ...
              >
                {t.label}
                {count != null && count > 0 && (
                  <span style={{ fontSize: 9, fontWeight: 800, background: tab === t.id ? NAVY : SOFT, color: tab === t.id ? 'white' : SUB, padding: '1px 6px', borderRadius: 999 }}>{count}</span>
                )}
              </button>
            );
          })}
        ...
        {loading && (
          <div style={{ padding: 40, textAlign: 'center', fontSize: 12, color: SUB }}>
            <Loader2 size={20} style={{ animation: 'spin 0.8s linear infinite', display: 'block', margin: '0 auto 8px' }} /> Loading applications…
            <style>{`@keyframes spin { from { transform: rotate(0); } to { transform: rotate(360deg); } }`}</style>
          </div>
        )}
        ...
        {!loading && !error && apps.length === 0 && (
          <div style={{ padding: 40, background: PAPER, border: `1.5px dashed ${LINE}`, borderRadius: 14, textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 800, color: INK, marginBottom: 6 }}>Nothing in {tab}</div>
            <div style={{ fontSize: 12, color: SUB }}>Applications come in through <Link to="/apply/fm" style={{ color: ACCENT, textDecoration: 'none', fontWeight: 700 }}>/apply/fm</Link>.</div>
          </div>
        )}
        ...
        {!loading && !error && apps.length > 0 && (
          <div style={{ background: PAPER, border: `1px solid ${LINE}`, borderRadius: 12, overflow: 'hidden' }}>
            <div style={{
              display: 'grid', gridTemplateColumns: '2fr 1.4fr 1fr 1fr 1fr 1fr',
            ...
            {apps.map((a, i) => (
              <div
                ...
                style={{
                  display: 'grid', gridTemplateColumns: '2fr 1.4fr 1fr 1fr 1fr 1fr',
                  padding: '14px 16px', borderBottom: i < apps.length - 1 ? `1px solid ${LINE}` : 'none',
                  alignItems: 'center', fontSize: 12, color: INK, cursor: 'pointer',
                }}
              >
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontWeight: 800, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.company_name}</div>
                  {a.regions_covered?.length > 0 && (
                    <div style={{ fontSize: 10, color: MUTE, marginTop: 2, display: 'flex', alignItems: 'center', gap: 4 }}>
                      <MapPin size={9} /> {a.regions_covered.slice(0, 3).join(', ')}{a.regions_covered.length > 3 ? ` +${a.regions_covered.length - 3}` : ''}
                    </div>
                  )}
                </div>
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.contact_name}</div>
                  <div style={{ fontSize: 10, color: SUB, marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.contact_email}</div>
                </div>
