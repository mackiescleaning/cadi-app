...
const LOOP = [
  {
    ...
    icon: Upload,
    note: 'FM uploads signed sub-contract → visible on sub profile',
  },
  {
    n: 2,
    side: 'fm',
    key: 'dispatch',
    label: 'Dispatch / publish',
    icon: Send,
    note: 'FM publishes job card → either direct dispatch or marketplace',
  },
  {
    n: 3,
    side: 'sub',
    key: 'inbox',
    label: 'Inbox / marketplace',
    icon: ShoppingBag,
    note: 'Sub sees new job in Cadi Connect tab inside their main Cadi',
  },
  {
    n: 4,
    side: 'sub',
    ...
    icon: Calendar,
    note: 'Sub picks date → syncs back to FM scheduling',
  },
  {
    n: 5,
    side: 'sub',
    ...
    icon: Camera,
    note: 'Sub uploads geo-stamped photos + sign-off',
  },
  {
    n: 6,
    side: 'fm',
    ...
    label: 'Ops approval',
    icon: CheckCircle2,
    note: 'FM ops reviews evidence → approve / query / reject',
  },
  {
    n: 7,
    side: 'sub',
    ...
    icon: Receipt,
    note: 'On approval, sub raises invoice from inside Cadi Connect',
  },
  {
    n: 8,
    side: 'fm',
    ...
    label: 'Accounts → pay',
    icon: PoundSterling,
    note: 'FM accounts inbox: pay or export to Xero → updates sub Earnings',
  },
];
...
function StationDot({ station, active, onClick }) {
  ...
  return (
    ...
    >
      ...
      <div
        style={{ fontSize: 11, fontWeight: 700, color: INK, textAlign: 'center', lineHeight: 1.25 }}
      >
        {station.label}
      </div>
    </button>
    ...
    >
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto 28px' }}>
        ...
        <p style={{ fontSize: 14, color: MUTE, maxWidth: 760, lineHeight: 1.55, marginTop: 8 }}>
          End-to-end loop between the FM ops side and the{' '}
          <strong style={{ color: ORANGE }}>Cadi Connect</strong> tabs that live inside each
          subcontractor's main Cadi. Eight stations · two surfaces · one source of truth. Existing
          demo pieces are reused; only
      ...
      >
        ...
        <div style={{ position: 'relative' }}>
          ...
          <div style={{ fontSize: 19, fontWeight: 900, marginBottom: 4 }}>{acerta.contract}</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 10 }}>
            {[
              ...
              {
                v: `£${acerta.monthly_value_per_visit.toLocaleString()}`,
                l: 'per-visit value',
                accent: ORANGE,
              },
          ...
          >
            ...
            — ingestable as Job Cards on the FM Ops side. Mix of Vauxhall · Aston Martin · Porsche ·
            Audi · JLR · BMW · Ford · Ferrari dealerships across the UK.
          </div>
        </div>
      </div>
      ...
      >
          ...
          {[
            {
              icon: Upload,
              color: NAVY,
              title: '1 · Britannia bulk-uploads',
              body: 'FM uploads Excel of 100–300 subs into FM Ops → Contractors screen. CSV maps: company · contact · email · phone · trades · region · DBS · insurance · contract ref.',
            },
            {
              icon: Unlock,
              color: ORANGE,
              title: '2 · Free Lite + Connect unlocked',
              body: 'Each sub gets a free Cadi Lite account auto-provisioned. Britannia Connect tab is unlocked by default — receive jobs, complete work, invoice, see earnings. Flag: connect_unlocked_by_fm_id.',
            },
            {
              icon: Lock,
              color: '#a16207',
              title: '3 · Rest behind Pro paywall',
              body: 'Run · Grow · Front Desk · Staff stay locked behind Pro £39/mo. Sub sees the rest of Cadi, can upgrade for their own business — this is the conversion wedge.',
            },
          ].map((c, i) => {
            const Icon = c.icon;
            return (
        ...
        >
          <strong style={{ color: ORANGE }}>Why it works:</strong> Britannia drives distribution at
          zero acquisition cost · Cadi gets 100–300 vetted, scored operatives on the network in one
          drop · clear path to Pro conversion through the rest of the app.
        </div>
      </div>
      ...
      >
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
            ...
            <span style={{ fontSize: 10, fontWeight: 700, color: MUTE }}>
              (/fm-demo · exterior mode)
            </span>
          </div>
          <div style={{ fontSize: 11, color: MUTE, marginBottom: 16 }}>
            What the FM ops team operates. Full control of work completion, contracts, marketplace
            listings.
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {FM_SCREENS.map((s) => (
              <ScreenCard key={s.key} screen={s} side="fm" />
            ))}
          </div>
        </div>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
            ...
            <div style={{ fontSize: 13, fontWeight: 900, color: ORANGE }}>
              Cadi Connect (in subcontractor's Cadi)
            </div>
            <span style={{ fontSize: 10, fontWeight: 700, color: MUTE }}>
              (tab inside main Cadi)
            </span>
          </div>
          <div style={{ fontSize: 11, color: MUTE, marginBottom: 16 }}>
            How the subcontractor receives jobs, completes work, raises invoices — without leaving
            their own Cadi.
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {SUB_SCREENS.map((s) => (
              <ScreenCard key={s.key} screen={s} side="sub" />
            ))}
          </div>
        </div>
      </div>
              ...
              >
                <span
                  style={{
                    ...
                    padding: '3px 8px',
                    borderRadius: 6,
                    background: `${fromColor}10`,
                    border: `1px solid ${fromColor}25`,
                    whiteSpace: 'nowrap',
                  }}
                >
                  {fromSide.toUpperCase()} · {fromKey}
                </span>
                ...
                <span
                  style={{
                    ...
                    padding: '3px 8px',
                    borderRadius: 6,
                    background: `${toColor}10`,
                    border: `1px solid ${toColor}25`,
                    whiteSpace: 'nowrap',
                  }}
                >
                  {toSide.toUpperCase()} · {toKey}
                </span>
      ...
      >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
          ...
          >
            <div style={{ fontSize: 11, fontWeight: 900, color: NAVY, marginBottom: 8 }}>
              Reuse as-is
            </div>
            <ul
              style={{
          ...
          >
            <div style={{ fontSize: 11, fontWeight: 900, color: '#a16207', marginBottom: 8 }}>
              Refactor &amp; wire
            </div>
            <ul
              style={{
          ...
          >
            <div style={{ fontSize: 11, fontWeight: 900, color: GREEN, marginBottom: 8 }}>
              BUILD (net-new)
            </div>
            <ul
              style={{
        ...
        >
          <strong style={{ color: NAVY }}>Next step:</strong> pick one station from the loop above
          and spec the data contract (Supabase tables + edge function). Recommend starting at{' '}
          <strong>Station 3 — Inbox / marketplace</strong>: it's the surface that proves the
          cross-app sync between FM Ops and a sub's main Cadi.
        </div>
      </div>
    </div>
  );
