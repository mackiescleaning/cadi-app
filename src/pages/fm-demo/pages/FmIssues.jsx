...
const ISSUES = [
  {
    ...
    raised: '07:10',
    raisedDate: '9 May 2026',
    raisedBy: 'Auto — Cadi System',
    assignedTo: 'James Harper',
    status: 'open',
    resolutionTime: null,
    description:
      'Cleaner not on site 15 minutes before SLA window closes. Contract requires arrival by 06:45, no check-in recorded. Client SLA at risk.',
    jobRef: 'BF-2026-0188',
    evidenceCount: 0,
    trail: [
  ...
  },
  {
    ...
    raised: '08:05',
    raisedDate: '9 May 2026',
    raisedBy: 'Raj Patel (Client)',
    assignedTo: 'James Harper',
    status: 'in-progress',
    resolutionTime: null,
    description:
      'Client reports main corridor not completed during 05:30–07:30 window. Cleaner arrived 52 minutes late. Evidence shows only 3 of 8 sections signed off.',
    jobRef: 'BF-2026-0182',
    evidenceCount: 3,
    trail: [
  ...
  },
  {
    ...
    priority: 'medium',
    type: 'Access Problem',
    site: 'Central Bedfordshire Council HQ',
    client: 'Central Bedfordshire Council',
    raised: '05:48',
    raisedDate: '9 May 2026',
    raisedBy: 'Marcus Webb (Cleaner)',
    assignedTo: 'James Harper',
    status: 'resolved',
    resolutionTime: '38 min',
    description:
      'Cleaner reported key fob not working on side entrance. Used main entrance causing 7-minute delay. Site manager contacted — fob will be replaced by facilities team.',
    jobRef: 'BF-2026-0179',
    evidenceCount: 2,
    trail: [
      ...
      {
        time: '06:03',
        event: 'Client confirmed main entrance as approved alternative',
        type: 'client',
      },
  ...
  },
  {
    ...
    raised: '14:20',
    raisedDate: '8 May 2026',
    raisedBy: 'Anna Davies (Client)',
    assignedTo: 'Sarah Okonkwo',
    status: 'resolved',
    resolutionTime: '22 min',
    description:
      'Client requests one-off earlier start of 07:30 on 15 May due to large conference check-ins. Rescheduled and cleaner notified through Cadi Connect.',
    jobRef: 'BF-2026-0174',
    evidenceCount: 0,
    trail: [
      ...
      {
        time: '14:28',
        event: 'Schedule adjusted in Cadi — cleaner notified automatically',
        type: 'system',
      },
  ...
  },
  {
    ...
    raised: '06:30',
    raisedDate: '7 May 2026',
    raisedBy: 'Auto — Cadi System',
    assignedTo: 'James Harper',
    status: 'resolved',
    resolutionTime: '1h 22m',
    description:
      'Assigned cleaner did not arrive for 06:00 window. No check-in by 06:30. Emergency cover arranged from cleaner network — replacement on site by 07:15, clean completed.',
    jobRef: 'BF-2026-0169',
    evidenceCount: 6,
    trail: [
      {
        time: '06:30',
        event: 'Auto-alert — no check-in recorded at start of window',
        type: 'system',
      },
      {
        time: '06:31',
        event: 'Cadi matched nearest available cleaner (Emma Walsh, score 94)',
        type: 'system',
      },
      { time: '06:35', event: 'Emma Walsh accepted job via Cadi Connect', type: 'resolve' },
      {
        time: '07:15',
        event: 'Emma checked in on site · 6 geo-verified photos uploaded',
        type: 'resolve',
      },
      {
        time: '07:52',
        event: 'Clean completed · SLA achieved · evidence approved',
        type: 'resolve',
      },
    ],
  },
  {
    ...
    site: 'Aldi – Dunstable Distribution RDC',
    client: 'Aldi UK Ltd',
    raised: '05:12',
    raisedDate: '6 May 2026',
    raisedBy: 'Tom Adeyemi (Cleaner)',
    assignedTo: 'Sarah Okonkwo',
    status: 'resolved',
    resolutionTime: '2h 15m',
    description:
      'Floor scrubber unit at RDC reported fault — error code E4. Manual clean of all areas completed. Equipment issue logged with site maintenance team.',
    jobRef: 'BF-2026-0162',
    evidenceCount: 4,
    trail: [
      ...
      {
        time: '05:25',
        event: 'Mark Higgins (Aldi Property) notified of equipment issue',
        type: 'notify',
      },
      { time: '05:30', event: 'Manual clean approved — cleaner proceeded', type: 'client' },
      {
        time: '07:27',
        event: 'Full clean completed · maintenance ticket raised with Aldi',
        type: 'resolve',
      },
    ],
  },
                ...
                {issue.evidenceCount > 0 && (
                  ...
                  >
                    ...
                    <span className="font-bold text-purple-300">
                      {issue.evidenceCount} evidence photos
                    </span>
                  </div>
                )}
              </div>
              ...
              {issue.evidenceCount > 0 && (
                <div className="mb-5">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    ...
                    <div
                      className="w-14 h-14 rounded-xl flex items-center justify-center text-[10px] font-bold text-white/30"
                      style={{
              ...
              <div>
                ...
                <div className="space-y-2">
                  {issue.status !== 'resolved' && (
                    <>
                      <button
                        onClick={() => showToast('notify client and send update')}
                      ...
                      <button
                        onClick={() => showToast('reassign this issue to another team member')}
                      ...
                      <button
                        onClick={() => showToast('mark this issue as resolved')}
                        className="w-full flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs font-bold transition-all"
                        style={{
                  ...
                  {issue.status === 'resolved' && (
                    <>
                      <button
                        ...
                        onMouseEnter={(e) =>
                          (e.currentTarget.style.background = 'rgba(79,120,255,0.25)')
                        }
                        onMouseLeave={(e) =>
                          (e.currentTarget.style.background = 'rgba(79,120,255,0.15)')
                        }
                      >
                      ...
                      <button
                        onClick={() => showToast('reopen this issue')}
                  ...
                  <button
                    onClick={() => showToast('view linked job in Job Board')}
                    className="w-full flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs font-bold text-white/40 transition-all"
                    style={{
                      background: 'rgba(255,255,255,0.04)',
                      border: '1px solid rgba(255,255,255,0.08)',
...
const IMPACT = [
  {
    before: 'Issues reported via email, forgotten in inboxes',
    after: 'Every issue logged, tracked, and resolved in one place',
    icon: '🎫',
  },
  {
    before: 'No audit trail — he said/she said disputes',
    after: 'Full timeline, evidence attached, client-shareable',
    icon: '📋',
  },
  {
    before: 'Helpdesk tickets scattered across WhatsApp & email',
    after: 'Raised automatically from QA failures — zero manual effort',
    icon: '⚡',
  },
];
...
export default function FmIssues({ showToast }) {
  ...
  const filtered = ISSUES.filter((iss) => {
    ...
    if (
      search &&
      !iss.site.toLowerCase().includes(search.toLowerCase()) &&
      !iss.type.toLowerCase().includes(search.toLowerCase()) &&
      !iss.client.toLowerCase().includes(search.toLowerCase())
    )
      return false;
    return true;
  });
  ...
  return (
    <div className="p-8 space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <div className="text-white/40 text-sm mt-0.5">
            All site issues in one place — auto-raised, tracked, resolved
          </div>
        </div>
        <button
      ...
      >
        ...
        <div className="flex-1">
          <div className="text-white/80 text-sm font-bold">
            9 of 16 issues this month were raised automatically by Cadi
          </div>
          <div className="text-white/40 text-xs mt-0.5">
            SLA breaches, missed arrivals, and quality flags are detected in real-time — no manual
            logging required. Your team is notified instantly.
          </div>
        </div>
      ...
      {/* Table header */}
      <div
        className="px-5 grid text-[9px] font-black uppercase tracking-widest text-white/20"
      ...
      <div
        className="rounded-2xl p-5 grid grid-cols-2 gap-6"
      ...
      >
        <div>
          <div
            ...
            style={{ color: '#C2410C' }}
          >
            Before Cadi — bespoke system
          </div>
          <div className="space-y-2 text-xs text-white/40 leading-relaxed">
            {[
              'Issues logged manually by ops staff',
              'No link to job evidence or geo-data',
              'Client chased separately by phone/email',
              'Spreadsheet or legacy database — no audit trail',
              'Missed visits only caught on client complaint',
              'Resolution time tracked manually if at all',
            ].map((t) => (
        ...
        <div>
          ...
          <div className="space-y-2 text-xs text-white/40 leading-relaxed">
            {[
              'SLA breaches & missed arrivals auto-raised instantly',
              'Every issue links to job ref, geo-photos, audit trail',
              'Client notified in one click — shareable resolution report',
              'Full resolution trail logged automatically',
              'Missed visits detected before client even knows',
              'Avg resolution time tracked and reported monthly',
            ].map((t) => (
              <div key={t} className="flex items-start gap-2">
