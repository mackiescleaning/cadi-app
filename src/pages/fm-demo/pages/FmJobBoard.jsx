...
export default function FmJobBoard({ showToast }) {
  ...
  const filtered = openJobs.filter((j) => {
    ...
    const matchSearch =
      !search ||
      [site, client, j.service].some((x) => x.toLowerCase().includes(search.toLowerCase()));
    const matchFilter =
      filter === 'all' ||
      j.status === filter ||
      (filter === 'assigned' && (j.status === 'assigned' || assigned[j.id]));
  ...
  return (
    <div className="flex h-full overflow-hidden">
      {/* ── Left: job list ── */}
      <div
        className="flex flex-col w-[52%] overflow-hidden"
          ...
          <div className="relative">
            ...
            <input
              ...
              onFocus={(e) => {
                ...
                e.target.style.borderColor = 'rgba(79,120,255,0.4)';
              }}
              onBlur={(e) => {
                ...
                style={
                  filter === v
                    ? {
                        ...
                        color: 'white',
                        boxShadow: '0 0 12px rgba(79,120,255,0.15)',
                      }
                    : {
          ...
          {filtered.map((job) => {
            ...
            return (
              ...
              >
                ...
                <div className="text-white/50 text-xs self-center font-medium">
                  {job.timeWindow}
                </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {!selected ? (
          <div className="h-full flex flex-col items-center justify-center gap-4 text-center px-8">
            ...
            <div>
              ...
              <div className="text-white/20 text-xs mt-1">
                Cadi auto-matches the best available cleaners
              </div>
            </div>
          </div>
        ) : (
          <div className="p-6 space-y-5">
              ...
              >
                ...
                <div>
                  <div className="text-emerald-300 font-bold text-sm">
                    Assigned to{' '}
                    {
                      cleaners.find((c) => c.id === (assigned[selected] || selectedJob?.cleanerId))
                        ?.name
                    }
                  </div>
                  <div className="text-emerald-400/50 text-xs mt-0.5">
                    Cleaner notified · job confirmed
                  </div>
                </div>
              </div>
            )}
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        border: isBest
                          ? '1px solid rgba(79,120,255,0.3)'
                          : '1px solid rgba(255,255,255,0.08)',
                        boxShadow: isBest ? 'inset 0 1px 0 rgba(255,255,255,0.07)' : 'none',
                      }}
                      onMouseEnter={(e) => {
                        e.currentTarget.style.background = 'rgba(79,120,255,0.12)';
                        e.currentTarget.style.borderColor = 'rgba(79,120,255,0.4)';
                      }}
                      onMouseLeave={(e) => {
                        e.currentTarget.style.background = isBest
                          ? 'rgba(79,120,255,0.08)'
                    ...
                    >
                      {isBest && (
                        <div
                          className="absolute top-2.5 right-3 text-[8px] font-black px-2 py-0.5 rounded-full"
                          style={{
            ...
            <button
              onClick={() => showToast('post job to Cadi marketplace')}
