...
import {
  Building2, FileText, Send, CheckSquare, MapPin,
  Receipt, ThumbsUp, Download, ChevronRight, ArrowRight,
  Clock, Camera, AlertCircle, CheckCircle2, X,
} from 'lucide-react';
...
const STEPS = [
  ...
  },
  {
    ...
    actor: 'Britannia Group',
    summary: 'Invoice lands in the FM accounts inbox. Geo evidence reviewed. Approved, queried or rejected.',
    detail: [
...
function StepCard({ step, active, onClick, onNavigate }) {
                ...
                {v.map((item, i) => (
                  typeof item === 'object'
                    ? <div key={i} className="flex items-center gap-2 py-1 border-b border-white/5 last:border-0">
                        <span className="text-[10px] font-black text-white/35 w-16 shrink-0">{item.ref}</span>
    ...
    <div className="p-6 max-w-6xl space-y-5">
      ...
      <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid rgba(79,120,255,0.18)', background: 'rgba(1,8,40,0.6)' }}>
        ...
        <div className="grid grid-cols-3 divide-x" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
          {[
            { before: 'Contractor jobs coordinated over WhatsApp and email', after: 'Full digital workflow — brief, evidence, invoice all in one place', icon: '📲' },
            { before: 'Evidence and invoice arrive separately, matched manually', after: 'Invoice only unlocks once geo-verified evidence is submitted', icon: '🔒' },
            { before: 'IR35 and compliance managed informally — risk buried', after: 'Route flagged, compliance docs collected before a shift starts', icon: '⚖️' },
      ...
      <div className="rounded-2xl p-5 mb-6"
        style={{ background: 'linear-gradient(135deg, rgba(79,120,255,0.1), rgba(52,211,153,0.06))', border: '1px solid rgba(79,120,255,0.2)' }}>
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="text-white font-black text-base">Contractor Workflow — Cadi Connect</div>
            <div className="text-white/45 text-xs mt-0.5">
              End-to-end flow from client onboarding through to invoice export · subcontracted cleaners via Cadi Connect
            </div>
          </div>
          <div className="ml-auto flex items-center gap-3">
            ...
            <div className="text-center">
              <div className="text-lg font-black" style={{ color: '#4f78ff' }}>3</div>
