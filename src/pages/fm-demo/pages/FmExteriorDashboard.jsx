const ACTIVITY = [
  { time: '09:22', text: 'Window clean complete — Next Luton · 8 photos uploaded', type: 'complete' },
  { time: '09:05', text: 'Quote received — ProWash Midlands · Tesco Welwyn jet wash · £280', type: 'quote' },
  { time: '08:47', text: 'Job approved — Capital Gutters Ltd · L&D Hospital gutter clear', type: 'approved' },
  { time: '08:30', text: 'Contractor en route — Clearview Window Services · L&D Hospital · ETA 14 min', type: 'enroute' },
  { time: '08:10', text: 'Job card sent — SprayTech Services · Luton Council HQ graffiti', type: 'sent' },
  { time: '07:55', text: 'Declined — CleanFront UK · Watford Wed slot · backup auto-notified', type: 'declined' },
];

const META = {
  complete: { color: '#34d399', bg: 'rgba(52,211,153,0.1)',  border: 'rgba(52,211,153,0.2)',  dot: '✓' },
  quote:    { color: '#38bdf8', bg: 'rgba(56,189,248,0.1)',  border: 'rgba(56,189,248,0.2)',  dot: '£' },
  approved: { color: '#a78bfa', bg: 'rgba(167,139,250,0.1)', border: 'rgba(167,139,250,0.2)', dot: '✓' },
  enroute:  { color: '#fbbf24', bg: 'rgba(251,191,36,0.1)',  border: 'rgba(251,191,36,0.2)',  dot: '→' },
  sent:     { color: '#60a5fa', bg: 'rgba(96,165,250,0.1)',  border: 'rgba(96,165,250,0.2)',  dot: '↗' },
  declined: { color: '#f87171', bg: 'rgba(248,113,113,0.1)', border: 'rgba(248,113,113,0.2)', dot: '✗' },
};
...
const IMPACT = [
  { before: 'Contractors contacted by phone and email for every job', after: 'Job cards broadcast to matched contractors in one click', icon: '📲' },
  { before: 'Quotes tracked in email threads — easily lost', after: 'Every quote, rate and response in one place', icon: '💷' },
  { before: 'Interior and exterior managed as separate businesses', after: 'Both sides of Britannia, one system, one client record', icon: '🔗' },
];

export default function FmExteriorDashboard({ showToast, onNavigate }) {
  return (
    <div className="p-6 space-y-5">

      {/* Impact strip */}
      <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid rgba(14,165,233,0.2)', background: 'rgba(1,8,40,0.6)' }}>
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          ...
          { label: 'Revenue this month',value: '£18.4k', color: '#a78bfa', sub: 'exterior contracts' },
      ...
      <div className="flex items-center gap-4 rounded-xl px-5 py-3.5" style={{ background: 'rgba(251,191,36,0.08)', border: '1px solid rgba(251,191,36,0.25)' }}>
        <div className="w-7 h-7 rounded-lg flex items-center justify-center text-sm shrink-0" style={{ background: 'rgba(251,191,36,0.15)', border: '1px solid rgba(251,191,36,0.3)' }}>⏳</div>
        <span className="text-sm font-bold text-amber-300 flex-1">3 job cards awaiting sub approval — 2 due this week</span>
        <button onClick={() => onNavigate && onNavigate('ext-approvals')}
      ...
      <div className="grid grid-cols-[1fr_300px] gap-5">
        ...
        {/* Activity feed */}
        <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(1,8,40,0.5)' }}>
              ...
              return (
                <div key={time + text} className="px-5 py-3 flex items-start gap-3">
                  <div className="w-5 h-5 rounded-md flex items-center justify-center text-[9px] font-black shrink-0 mt-0.5"
                    style={{ background: m.bg, border: `1px solid ${m.border}`, color: m.color }}>{m.dot}</div>
        ...
        <div className="space-y-4">
          <div className="rounded-2xl p-5" style={{ border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(1,8,40,0.5)' }}>
          ...
          {/* Quick actions */}
          <div className="rounded-2xl p-4 space-y-2" style={{ border: '1px solid rgba(14,165,233,0.2)', background: 'rgba(14,165,233,0.05)' }}>
            <div className="text-white/40 text-[9px] font-black uppercase tracking-widest mb-3">Quick actions</div>
            {[
              ...
              <button key={nav} onClick={() => onNavigate && onNavigate(nav)}
