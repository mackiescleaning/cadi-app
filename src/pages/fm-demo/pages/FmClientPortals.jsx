...
import { ExternalLink, Settings2, Users, Eye, EyeOff, Globe, Copy, CheckCircle2, Circle, ChevronRight, LayoutDashboard, Activity, Images, ClipboardList, ShieldCheck, BarChart2, AlertTriangle, MessageSquare, Settings, Rocket, Bell, Plus, X, Search, Clock } from 'lucide-react';
...
const CLIENTS = [
  {
    ...
    monthlyViews: 147,
    users: [
      { name: 'Helen Marsh', email: 'h.marsh@asda.co.uk', role: 'Primary', avatar: 'HM', color: '#4f78ff' },
      { name: 'Tony Briggs', email: 't.briggs@asda.co.uk', role: 'FM Director', avatar: 'TB', color: '#10b981' },
    ],
    tabs: { overview: true, mobilisation: true, live: true, evidence: true, history: true, compliance: true, reports: true, issue: true, comms: true, settings: true },
    digest: 'daily',
    alertLevel: 'all',
  },
  {
    ...
    monthlyViews: 312,
    users: [
      { name: 'Karen Oyelaran', email: 'k.oyelaran@morrisons.co.uk', role: 'Primary', avatar: 'KO', color: '#f59e0b' },
    ],
    tabs: { overview: true, mobilisation: false, live: true, evidence: true, history: true, compliance: true, reports: true, issue: true, comms: true, settings: false },
    digest: 'weekly',
    alertLevel: 'issues',
  },
  {
    ...
    monthlyViews: 489,
    users: [
      { name: 'Dr Priya Sharma', email: 'p.sharma@nhs-se.nhs.uk', role: 'Primary', avatar: 'PS', color: '#6366f1' },
      { name: 'Estates Manager', email: 'estates@nhs-se.nhs.uk', role: 'Estates', avatar: 'EM', color: '#ec4899' },
      { name: 'Compliance Lead', email: 'compliance@nhs-se.nhs.uk', role: 'Compliance', avatar: 'CL', color: '#10b981' },
    ],
    tabs: { overview: true, mobilisation: false, live: true, evidence: true, history: true, compliance: true, reports: true, issue: true, comms: true, settings: false },
    digest: 'daily',
    alertLevel: 'all',
  },
  {
    ...
    monthlyViews: 88,
    users: [
      { name: 'Sam Whitehouse', email: 's.whitehouse@amazon.co.uk', role: 'Primary', avatar: 'SW', color: '#f97316' },
    ],
    tabs: { overview: true, mobilisation: false, live: false, evidence: true, history: true, compliance: true, reports: true, issue: true, comms: false, settings: false },
    digest: 'weekly',
    alertLevel: 'issues',
  },
  {
...
function PortalTheatre({ client, win, onDone }) {
  ...
  useEffect(() => {
    ...
    const t3 = setTimeout(() => { if (win) win.focus(); onDone(); }, 2500);
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center"
      ...
      <div className="rounded-3xl px-8 py-8 flex flex-col items-center gap-5 text-center"
        style={{
        ...
        <div className="relative">
          ...
          {step === 2 && (
            <div className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-emerald-500 flex items-center justify-center text-white text-[10px] font-black">✓</div>
          )}
        </div>
        <div>
        ...
        <div className="text-[10px] text-gray-300 font-medium">{client.name} · {client.sites} sites</div>
      </div>
    </div>
  );
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-end" style={{ background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(4px)' }}>
      <div className="h-full w-full max-w-md bg-white overflow-y-auto flex flex-col shadow-2xl">
        ...
        <div className="px-6 py-5 border-b border-gray-100 flex items-start justify-between gap-4 flex-shrink-0">
          <div>
            <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-0.5">Portal Setup</div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
          ...
          {/* Portal URL */}
          <div>
            <div className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2">Portal URL</div>
            <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5">
              <Globe size={13} className="text-gray-400 shrink-0" />
              <span className="flex-1 text-xs font-mono text-gray-600 truncate">{portalUrl}</span>
              <button onClick={copy} className="shrink-0 text-[10px] font-bold text-[#4f78ff] hover:text-[#3558cc] flex items-center gap-1 transition-colors">
          ...
          {/* Portal tabs */}
          <div>
            <div className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2">Visible tabs</div>
              ...
              {TAB_CONFIG.map(tab => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="flex-1 text-left text-xs font-medium" style={{ color: on ? '#1e3a8a' : '#94a3b8' }}>{tab.label}</span>
                    {tab.always
          ...
          {/* Portal users */}
          <div>
            <div className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2">Portal access</div>
            <div className="space-y-2 mb-3">
              {client.users.length === 0 ? (
                <div className="text-[11px] text-gray-400 italic px-1">No users yet — invite the client below.</div>
              ) : (
                client.users.map(u => (
                  <div key={u.email} className="flex items-center gap-3 px-3 py-2.5 bg-gray-50 rounded-xl border border-gray-100">
                    <div className="w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-black text-white shrink-0" style={{ background: u.color }}>
                      {u.avatar}
                    </div>
                    ...
                    <span className="text-[9px] font-black text-gray-400 bg-white border border-gray-200 px-1.5 py-0.5 rounded-full shrink-0">{u.role}</span>
                  </div>
                ))
              )}
            </div>
            <div className="flex gap-2">
              ...
              />
              <button
                onClick={() => { if(invite) { showToast(`Invite sent to ${invite}`); setInvite(''); } }}
          ...
          {/* Notifications */}
          <div>
            <div className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2">Client notifications</div>
            <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  {['daily', 'weekly', 'monthly', 'off'].map(v => (
                    <button key={v} onClick={() => setDigest(v)}
                      ...
                      style={digest === v
                        ? { background: '#f0f4ff', borderColor: 'rgba(79,120,255,0.3)', color: '#1e3a8a' }
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {[{ v: 'all', l: 'All alerts' }, { v: 'issues', l: 'Issues only' }, { v: 'off', l: 'Off' }].map(({ v, l }) => (
                    <button key={v} onClick={() => setAlerts(v)}
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex gap-3 flex-shrink-0">
          {!client.portalActive && (
            <button
              onClick={() => { showToast(`Portal activated — invite sent to ${client.contact}`); onClose(); }}
  ...
  return (
    <div className="p-6 md:p-8 max-w-4xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'Active portals', value: activePortals, sub: `of ${CLIENTS.length} clients`, color: '#10b981' },
          { label: 'Pending setup', value: pendingSetup, sub: 'not yet activated', color: '#f59e0b' },
          { label: 'Views this month', value: totalViews.toLocaleString(), sub: 'across all portals', color: '#4f78ff' },
        ].map(s => (
            ...
            <div key={client.id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
              <div className="px-5 py-4 flex items-center gap-4">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {client.portalActive ? (
                    <button
                      onClick={() => { const win = window.open('/client-demo', '_blank'); setTheatre({ client, win }); }}
                      className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold transition-colors border"
                      style={{ background: '#f0fdf4', borderColor: 'rgba(16,185,129,0.25)', color: '#15803d', cursor: 'pointer' }}
                    >
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold transition-colors border"
                      style={{ background: '#fffbeb', borderColor: 'rgba(245,158,11,0.25)', color: '#b45309' }}
                    >
                      <Plus size={12} />
                      Activate
                    </button>
              ...
              <div className="px-5 py-2.5 bg-gray-50 border-t border-gray-100 flex items-center gap-5 flex-wrap">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] font-mono text-gray-500">client.cadi.cleaning/{client.portalSlug}</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <Users size={11} className="text-gray-400" />
                  <span className="text-[10px] text-gray-500">{client.users.length} user{client.users.length !== 1 ? 's' : ''}</span>
                </div>
                <div className="flex items-center gap-1.5">
                ...
                {client.portalActive && (
                  <div className="ml-auto flex items-center gap-1">
                    {Object.entries(client.tabs).filter(([,v]) => v).slice(0,6).map(([k]) => {
                      const tab = TAB_CONFIG.find(t => t.id === k);
                      if (!tab) return null;
                      const Icon = tab.icon;
                      return <Icon key={k} size={11} className="text-gray-300" />;
                    })}
                    {tabsOn > 6 && <span className="text-[9px] text-gray-300 ml-0.5">+{tabsOn - 6}</span>}
                  </div>
                )}
              </div>
            </div>
      ...
      {configClient && (
        <ConfigPanel
          ...
          showToast={showToast}
          onOpenPortal={() => { const win = window.open('/client-demo', '_blank'); setConfigClient(null); setTheatre({ client: configClient, win }); }}
        />
      )}
      {theatre && <PortalTheatre client={theatre.client} win={theatre.win} onDone={() => setTheatre(null)} />}
    </div>
  );
}
