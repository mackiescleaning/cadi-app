...
const HEAT_ZONES = [
  ...
  { latlng: [52.04, -0.775], intensity: 0.42 },
  { latlng: [51.882, -0.428], intensity: 0.65 },
  { latlng: [51.758, -0.46], intensity: 0.38 },
  { latlng: [51.656, -0.398], intensity: 0.48 },
  { latlng: [51.817, -0.81], intensity: 0.35 },
];
...
const FEED = [
  ...
  {
    time: '07:10',
    text: 'SLA risk flagged — Luton Central Library: cleaner not on site',
    type: 'alert',
  },
  {
    time: '06:55',
    text: 'Distribution clean completed — Aldi Dunstable RDC · 18 photos',
    type: 'complete',
  },
  {
    time: '06:42',
    text: 'Marcus Webb checked in at Luton Customer Service Centre',
    type: 'arrival',
  },
];
...
function MapStyler() {
  const map = useMap();
  useEffect(() => {
    map.getContainer().style.background = '#010b38';
  }, [map]);
  return null;
}
...
function StatCard({ label, value, sub, color, sparkData, sparkId, trend }) {
  return (
    <div
      className="rounded-2xl p-5 relative overflow-hidden flex flex-col gap-0"
      style={{
      ...
      onMouseEnter={(e) => {
        e.currentTarget.style.boxShadow = `0 0 40px ${color}18, inset 0 1px 0 rgba(255,255,255,0.1)`;
        e.currentTarget.style.borderColor = `${color}50`;
      }}
      onMouseLeave={(e) => {
        e.currentTarget.style.boxShadow = `0 0 0 0 transparent, inset 0 1px 0 rgba(255,255,255,0.07)`;
...
const IMPACT = [
  {
    before: 'Morning ops round: calls, WhatsApp, chasing no-shows',
    after: 'Every site, every region — covered or flagged, no calls needed',
    icon: '📍',
  },
  {
    before: 'SLA breach discovered when the client calls you',
    after: 'Site flagged before the window closes — you act, not your client',
    icon: '⏱',
  },
  {
    before: 'Cover gap at 5am — 2 hours of calls to fill it',
    after: 'Cover pool visible in advance — gaps filled before the shift starts',
    icon: '🔄',
  },
  {
    before: 'Margin erosion invisible until the contract review',
    after: 'Cost vs revenue per contract visible every week',
    icon: '💷',
  },
];

export default function FmDashboard({ showToast: _showToast, onNavigate }) {
  ...
  return (
    <div className="p-8 space-y-6 min-h-full">
        ...
        >
          {IMPACT.map(({ before, after, icon }) => (
            <div key={icon} className="px-4 py-3.5">
              ...
              <div className="text-[10px] text-white/30 leading-snug mb-2 line-through decoration-white/20">
                {before}
              </div>
      ...
      {alerts > 0 && (
        <div
          className="flex items-center gap-4 rounded-xl px-5 py-3.5 relative overflow-hidden"
          style={{
        ...
        <div className="flex items-center gap-4 px-5 py-3.5 relative">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-xs text-white/45">
              Asda Luton — 24 staff transferring ·{' '}
              <span style={{ color: '#a5b4fc', fontWeight: 700 }}>Day 12 of 28</span> · 3 actions
              outstanding
            </div>
          </div>
          {/* Progress bar */}
          <div className="shrink-0 flex flex-col items-end gap-1">
            ...
            >
              <div
                ...
                style={{ width: '43%', background: 'linear-gradient(90deg, #6366f1, #818cf8)' }}
              />
            </div>
          </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        <StatCard
          ...
          color="#4f78ff"
          sparkId="jobs"
          sparkData={[8, 11, 9, 13, 12, 15, 14, 15]}
        ...
        <StatCard
          label="Cover rate"
          value="97%"
          sub="shifts filled this week"
          color="#a78bfa"
          sparkId="cover"
          sparkData={[91, 93, 94, 92, 95, 96, 96, 97]}
          trend="↑ 3pts vs last week"
        />
      </div>
        ...
        <GlassCard style={{ overflow: 'hidden' }}>
          ...
          <div style={{ height: 260, position: 'relative' }}>
            ...
            <div className="absolute bottom-3 left-4 z-[1000] pointer-events-none">
              <span
                className="text-white/40 text-xs font-medium px-2.5 py-1 rounded-lg"
            ...
            {FEED.map((item, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-[10px] font-bold text-white/25 shrink-0 mt-0.5">
                    {item.time}
                  </span>
                </div>
              );
      ...
      <GlassCard>
        ...
        >
          ...
          <span
            className="text-white/30 text-xs font-bold px-2.5 py-1 rounded-lg"
            style={{
              background: 'rgba(255,255,255,0.05)',
              border: '1px solid rgba(255,255,255,0.08)',
            }}
        ...
        <div
          ...
          style={{ gridTemplateColumns: '1fr 160px 140px 90px 110px' }}
        >
          {['Site', 'Cleaner', 'Service / Window', 'Value', 'Status'].map((h) => (
            <div key={h} className="text-[10px] font-black uppercase tracking-widest text-white/20">
              {h}
            </div>
          ))}
        </div>
          ...
          {today.slice(0, 10).map((job) => {
            ...
            return (
              <div
                ...
                onMouseEnter={(e) => {
                  e.currentTarget.style.background = 'rgba(79,120,255,0.05)';
                  e.currentTarget.style.borderTopColor = 'rgba(79,120,255,0.12)';
                }}
                onMouseLeave={(e) => {
              ...
              >
                ...
                <div className="text-white/60 text-sm self-center truncate">
                  {cleaner?.name || <span className="text-white/25 italic">Unassigned</span>}
                </div>
                <div className="self-center">
                  <span
