...
function StagePipeline({ stages }) {
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', marginTop: '0.75rem', overflow: 'hidden' }}>
      {stages.map((stage, i) => {
        const isLast = i === stages.length - 1;
        const dotColor = stage.done ? '#4ade80' : stage.active ? '#fbbf24' : 'rgba(226,232,240,0.18)';
        const labelColor = stage.done ? '#4ade80' : stage.active ? '#fbbf24' : 'rgba(226,232,240,0.3)';
        const lineColor = stage.done ? 'rgba(74,222,128,0.35)' : 'rgba(255,255,255,0.07)';

        return (
          <div key={stage.label} style={{ display: 'flex', alignItems: 'flex-start', flex: isLast ? 'none' : 1 }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.25rem' }}>
              ...
              <span style={{ fontSize: '0.62rem', fontWeight: 600, color: labelColor, whiteSpace: 'nowrap', textAlign: 'center', maxWidth: 52, lineHeight: 1.25 }}>
    ...
    <div style={{ ...card, borderRadius: '1rem', overflow: 'hidden' }}>
      ...
      >
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', flexWrap: 'wrap', marginBottom: '0.3rem' }}>
            ...
            <span style={{ fontSize: '0.7rem', fontWeight: 700, color: stageColor, background: `${stageColor}18`, borderRadius: '999px', padding: '2px 9px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '0.35rem', flexShrink: 0 }}>
          <span style={{ color: 'white', fontWeight: 900, fontFamily: 'monospace', fontSize: '1rem' }}>£{inv.amountTotal}</span>
          <span style={{ color: 'rgba(226,232,240,0.35)', fontSize: '0.7rem' }}>inc VAT</span>
          {open ? <ChevronUp size={14} color="rgba(226,232,240,0.3)" /> : <ChevronDown size={14} color="rgba(226,232,240,0.3)" />}
        </div>
      </button>
      ...
      {open && (
        <div style={{ padding: '0 1.25rem 1.25rem', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            {inv.lines.map((line, i) => (
              <div
                ...
                style={{
                  ...
                  padding: '0.5rem 0',
                  borderBottom: i < inv.lines.length - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none',
                  fontSize: '0.8rem', gap: '1rem',
                }}
              >
                <div>
                  <div style={{ color: 'rgba(226,232,240,0.8)', marginBottom: '0.1rem' }}>{line.desc}</div>
                  <div style={{ color: 'rgba(226,232,240,0.35)', fontSize: '0.7rem' }}>{line.date} · {line.ref}</div>
                </div>
                <span style={{ fontWeight: 700, fontFamily: 'monospace', color: 'white', flexShrink: 0 }}>£{line.net.toFixed(2)}</span>
              </div>
            ))}
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '0.25rem', marginTop: '0.6rem', paddingTop: '0.5rem', borderTop: '1px solid rgba(255,255,255,0.07)', fontSize: '0.78rem' }}>
              <div style={{ display: 'flex', gap: '1.5rem', color: 'rgba(226,232,240,0.45)' }}>
                <span>Net</span><span style={{ fontFamily: 'monospace' }}>£{inv.amountNet.toFixed(2)}</span>
              </div>
              <div style={{ display: 'flex', gap: '1.5rem', color: 'rgba(226,232,240,0.45)' }}>
                <span>VAT (20%)</span><span style={{ fontFamily: 'monospace' }}>£{inv.amountVat.toFixed(2)}</span>
              </div>
              <div style={{ display: 'flex', gap: '1.5rem', color: 'white', fontWeight: 900, fontSize: '0.9rem' }}>
                <span>Total</span><span style={{ fontFamily: 'monospace', color: '#fb923c' }}>£{inv.amountTotal.toFixed(2)}</span>
              </div>
            </div>
          </div>
          ...
          {allPaid && (
            <div style={{ ...card, borderRadius: '0.75rem', padding: '0.65rem 0.9rem', fontSize: '0.78rem', marginBottom: '0.75rem' }}>
              <div style={{ color: '#4ade80', fontWeight: 700, marginBottom: '0.2rem' }}>Payment confirmed ✓</div>
              <div style={{ color: 'rgba(226,232,240,0.5)' }}>Ref: {inv.paymentRef} · Paid {inv.paidDate}</div>
            </div>
          )}
          ...
          {isExported && (
            <div style={{ ...card, borderRadius: '0.75rem', padding: '0.65rem 0.9rem', fontSize: '0.78rem', marginBottom: '0.75rem' }}>
              <div style={{ color: '#60a5fa', fontWeight: 700, marginBottom: '0.2rem' }}>Exported to Xero ✓</div>
          ...
          {isUnderReview && !action && (
            <div>
              <div style={{ fontSize: '0.68rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'rgba(226,232,240,0.35)', marginBottom: '0.6rem' }}>
                Actions
              </div>
              <div style={{ display: 'flex', gap: '0.6rem', flexWrap: 'wrap', marginBottom: '0.85rem' }}>
                <button
                  onClick={() => setAction('approved')}
                  style={{ padding: '0.5rem 1.1rem', borderRadius: '0.65rem', background: 'rgba(74,222,128,0.1)', border: '1px solid rgba(74,222,128,0.25)', color: '#4ade80', fontWeight: 700, fontSize: '0.8rem', cursor: 'pointer' }}
                >
                  Approve ✓
                </button>
                <button
                  onClick={() => setAction('queried')}
                  style={{ padding: '0.5rem 1.1rem', borderRadius: '0.65rem', background: 'rgba(251,191,36,0.08)', border: '1px solid rgba(251,191,36,0.2)', color: '#fbbf24', fontWeight: 700, fontSize: '0.8rem', cursor: 'pointer' }}
                >
                  Query
                </button>
                <button
                  onClick={() => setAction('rejected')}
                  style={{ padding: '0.5rem 1.1rem', borderRadius: '0.65rem', background: 'rgba(248,113,113,0.08)', border: '1px solid rgba(248,113,113,0.2)', color: '#f87171', fontWeight: 700, fontSize: '0.8rem', cursor: 'pointer' }}
                >
                  Reject
                </button>
              </div>
              ...
              <div style={{ fontSize: '0.68rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'rgba(226,232,240,0.25)', marginBottom: '0.5rem' }}>
                Export to — approve first
              </div>
              <div style={{ display: 'flex', gap: '0.5rem', opacity: 0.35 }}>
                {['Xero', 'Sage', 'QuickBooks'].map(exp => (
                  <span key={exp} style={{ padding: '0.35rem 0.85rem', borderRadius: '999px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(226,232,240,0.5)', fontSize: '0.75rem', fontWeight: 600 }}>
                    {exp}
                  </span>
                ))}
              </div>
          ...
          {action && (
            <div style={{
              ...
              background: action === 'approved' ? 'rgba(74,222,128,0.08)' : action === 'queried' ? 'rgba(251,191,36,0.08)' : 'rgba(248,113,113,0.08)',
    ...
    <div className="flex flex-col gap-5 p-6 pb-10" style={{ color: '#e2e8f0' }}>
      ...
      <div style={{ ...card, borderRadius: '1rem', padding: '0.85rem 1.25rem', display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <span style={{ fontSize: '0.78rem', fontWeight: 700, color: '#4ade80' }}>Paid this month</span>
          <span style={{ fontWeight: 900, color: 'white', fontFamily: 'monospace' }}>£420</span>
        </div>
      </div>

      {/* Invoice inbox */}
      <div style={{ fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'rgba(226,232,240,0.35)', marginBottom: '-0.5rem', paddingLeft: '0.25rem' }}>
        Invoice inbox
      </div>
      ...
      <div style={{ ...card, borderRadius: '1rem', padding: '1rem 1.25rem' }}>
        <div style={{ fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'rgba(226,232,240,0.35)', marginBottom: '0.75rem' }}>
          Export options
        </div>
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '0.65rem' }}>
          {['Xero', 'Sage', 'QuickBooks', 'CSV'].map(e => (
            <span key={e} style={{ padding: '0.35rem 0.9rem', borderRadius: '999px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(226,232,240,0.6)', fontSize: '0.78rem', fontWeight: 600, cursor: 'pointer' }}>
              {e}
            </span>
          ))}
        </div>
