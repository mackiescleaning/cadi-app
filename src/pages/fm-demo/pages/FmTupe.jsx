    ...
    >
        ...
        >
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            >
              ...
              <span
                style={{ fontSize: '0.55rem', color: 'rgba(255,255,255,0.25)', fontWeight: 700 }}
              >
                {t.id}
              </span>
            </div>
            <div style={{ fontWeight: 900, fontSize: '0.88rem', color: 'white', lineHeight: 1.2 }}>
              {t.contract}
            </div>
            <div
              style={{ fontSize: '0.65rem', color: 'rgba(255,255,255,0.4)', marginTop: '0.18rem' }}
            >
              Incoming from{' '}
              <span style={{ color: 'rgba(255,255,255,0.6)', fontWeight: 700 }}>{t.outgoing}</span>
            </div>
          </div>
          <div style={{ display: 'flex', gap: '0.5rem', flexShrink: 0, alignItems: 'flex-start' }}>
            {[
              ...
              {
                n: t.transferDate.split(' ')[0],
                label: t.transferDate.split(' ').slice(1).join(' '),
              },
            ].map(({ n, label }) => (
              <div
                key={label}
              ...
              >
                <div
                  style={{ fontWeight: 900, fontSize: '0.82rem', color: 'white', lineHeight: 1 }}
                >
                  {n}
                </div>
                <div
          ...
          {t.milestones.map((m, i) => (
            <div
              ...
              style={{
                display: 'flex',
                alignItems: 'center',
                flex: i < t.milestones.length - 1 ? 1 : 'none',
              }}
            >
              <div
                style={{
              ...
              >
                ...
                <div
                  style={{
                    fontSize: '0.48rem',
                    fontWeight: 700,
                    color: m.done ? '#4ade80' : m.active ? '#fb923c' : 'rgba(255,255,255,0.25)',
                    textAlign: 'center',
                    maxWidth: 60,
                    lineHeight: 1.2,
                  }}
              ...
              {i < t.milestones.length - 1 && (
                <div
                  style={{
                    flex: 1,
                    height: 1,
                    background: m.done ? 'rgba(74,222,128,0.25)' : 'rgba(255,255,255,0.07)',
                    margin: '0 0.2rem',
                    marginBottom: '1.4rem',
                  }}
                />
      ...
      {t.staff_list.length > 0 && (
        ...
        >
          {[
            {
              label: 'Consulted',
              v: `${consultedCount}/${t.staff_list.length}`,
              ok: consultedCount === t.staff_list.length,
              color: '#4ade80',
            },
            {
              label: 'DBS ready',
              v: `${dbsReadyCount}/${t.staff_list.length}`,
              ok: dbsReadyCount === t.staff_list.length,
              color: '#60a5fa',
            },
            {
              label: 'Docs done',
              v: `${docsComplete}/${t.docs.length}`,
              ok: docsComplete === t.docs.length,
              color: '#a78bfa',
            },
            ...
            <div key={label} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
              ...
              <div>
                <div
                  style={{ fontWeight: 900, fontSize: '0.78rem', color: ok ? color : '#f87171' }}
                >
                  {v}
                </div>
                <div
                  style={{ fontSize: '0.55rem', color: 'rgba(255,255,255,0.3)', fontWeight: 600 }}
                >
                  {label}
                </div>
              </div>
      ...
      >
        <span style={{ fontSize: '0.62rem', fontWeight: 700, color: 'rgba(255,255,255,0.35)' }}>
          {expanded ? 'Hide' : 'Show'} staff list & documents
        </span>
        {expanded ? (
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
                {t.staff_list.map((s) => (
                  ...
                  >
                    <div>
                      <div style={{ fontSize: '0.67rem', fontWeight: 700, color: 'white' }}>
                        {s.name}
                      </div>
                      <div style={{ fontSize: '0.55rem', color: 'rgba(255,255,255,0.28)' }}>
                        {s.site} · {s.hrs}h/wk
                      </div>
                    </div>
                    <span
                      style={{
                        ...
                        borderRadius: '999px',
                        background: s.consulted ? 'rgba(74,222,128,0.1)' : 'rgba(248,113,113,0.1)',
                        border: `1px solid ${s.consulted ? 'rgba(74,222,128,0.3)' : 'rgba(248,113,113,0.3)'}`,
                        color: s.consulted ? '#4ade80' : '#f87171',
                        whiteSpace: 'nowrap',
                      }}
                    >
                    ...
                    <span
                      style={{
                        ...
                        padding: '0.15rem 0.5rem',
                        borderRadius: '999px',
                        background: s.dbs ? 'rgba(96,165,250,0.1)' : 'rgba(248,113,113,0.1)',
                        border: `1px solid ${s.dbs ? 'rgba(96,165,250,0.3)' : 'rgba(248,113,113,0.3)'}`,
                        color: s.dbs ? '#60a5fa' : '#f87171',
                        whiteSpace: 'nowrap',
                      }}
                    >
                      {s.dbs ? '✓ DBS' : '⚠ DBS'}
              ...
              {t.docs.map((d, i) => (
                <div
                  ...
                  style={{
                    ...
                    background: d.done ? 'rgba(74,222,128,0.04)' : 'rgba(255,255,255,0.02)',
                    border: `1px solid ${d.done ? 'rgba(74,222,128,0.15)' : 'rgba(255,255,255,0.05)'}`,
                  }}
                >
                  {d.done ? (
                  ...
                  <span
                    style={{
                      fontSize: '0.65rem',
                      fontWeight: 600,
                      color: d.done ? 'rgba(255,255,255,0.65)' : 'rgba(255,255,255,0.35)',
                    }}
                  >
                    {d.label}
                  </span>
    ...
    <div style={{ padding: '1.25rem 1.5rem 3rem' }}>
        ...
        >
          <div>
            ...
            <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: '0.7rem', margin: 0 }}>
              Transfer of Undertakings — track consultation windows, staff eligibility and document
              compliance
            </p>
          </div>
          <button
          ...
          {[
            ...
            <div
              ...
              style={{
                ...
                background: alert ? 'rgba(248,113,113,0.06)' : 'rgba(255,255,255,0.03)',
                border: `1px solid ${alert ? 'rgba(248,113,113,0.2)' : 'rgba(255,255,255,0.07)'}`,
              }}
            >
      ...
      >
        ...
        <div style={{ fontSize: '0.63rem', color: 'rgba(255,255,255,0.45)', lineHeight: 1.55 }}>
          <strong style={{ color: 'rgba(255,255,255,0.65)' }}>TUPE Regulations 2006</strong> —
          employees must be informed and consulted before transfer. Britannia Group holds liability
          from transfer date. 30-day consultation window where 10+ employees transfer. Cadi tracks
          every milestone and flags overdue actions automatically.
        </div>
      </div>
