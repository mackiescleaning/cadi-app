          ...
          <div key={i} className="flex items-center flex-1">
            <div className="flex flex-col items-center gap-1.5 min-w-0 flex-1">
              <div
                className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-black transition-all ${
                    ...
                    : active
                      ? 'bg-[#4f78ff] text-white ring-4 ring-[#4f78ff]/20'
                      : 'text-white/30'
                }`}
                style={
                  done || active
              ...
              <div
                className={`text-[10px] font-bold text-center leading-tight ${active ? 'text-white' : done ? 'text-emerald-400' : 'text-white/30'}`}
              >
                {label}
              </div>
            </div>
            {i < STEPS.length - 1 && (
              <div
                ...
                style={{
                  background: i < current ? 'rgba(52,211,153,0.4)' : 'rgba(255,255,255,0.1)',
                }}
              />
            )}
          </div>
...
function SiteConfigCard({ site, config, onChange, onRemove }) {
  ...
  return (
    ...
    >
      ...
      {open && (
        <div
          className="px-5 pb-5 pt-1 grid grid-cols-2 gap-4"
        ...
        >
          ...
          <Field label="Frequency">
            <select
              value={config.frequency || 'Mon–Fri daily'}
          ...
          <Field label="SLA window">
            <select
              value={config.slaWindow || ''}
              onChange={(e) => onChange({ slaWindow: e.target.value })}
              style={{ ...input, appearance: 'none' }}
            >
              <option value="">Select…</option>
              {SLA_WINDOWS.map((w) => (
...
const IMPACT = [
  {
    before: 'Contracts built in Word, emailed for sign-off, filed manually',
    after: 'Contract created in the portal in under 10 minutes',
    icon: '📝',
  },
  {
    before: 'SLA windows agreed verbally — disputes follow',
    after: 'SLA locked into the contract — auto-monitored from day one',
    icon: '⏱',
  },
  {
    before: 'Operative matched weeks later via ads and calls',
    after: 'Cadi matches an operative the moment the contract is saved',
    icon: '🎯',
  },
];
...
export default function FmCreateJob({ showToast }) {
  ...
  function patchBasics(p) {
    setBasics((prev) => ({ ...prev, ...p }));
  }
  function patchCompliance(p) {
    setCompliance((prev) => ({ ...prev, ...p }));
  }
  function patchPricing(p) {
    setPricing((prev) => ({ ...prev, ...p }));
  }
  function patchRate(siteId, val) {
    setPricing((prev) => ({ ...prev, rates: { ...prev.rates, [siteId]: val } }));
  }
  function patchSiteConfig(siteId, p) {
    setSiteConfigs((prev) => ({ ...prev, [siteId]: { ...(prev[siteId] || {}), ...p } }));
  }
  ...
  const step2Valid =
    selectedSiteIds.length > 0 && chosenSites.every((s) => siteConfigs[s.id]?.service);
      ...
      <div className="p-8 flex flex-col items-center justify-center min-h-full gap-6 text-center max-w-lg mx-auto">
        ...
        <div className="w-full rounded-2xl p-6 text-left space-y-3" style={glass}>
          {[
            ...
            {
              label: 'Sites',
              value: `${chosenSites.length} site${chosenSites.length !== 1 ? 's' : ''}`,
            },
            {
              label: 'Monthly value',
              value:
                monthlyTotal > 0
                  ? `£${monthlyTotal.toLocaleString('en-GB', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
                  : '—',
            },
            {
              label: 'Annual value',
              value:
                annualTotal > 0
                  ? `£${annualTotal.toLocaleString('en-GB', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
                  : '—',
            },
        ...
        <div className="grid grid-cols-2 gap-3 w-full">
          {[
            {
              label: '⚡ Match operatives',
              action: () => showToast('open operative matching for ' + contractRef),
            },
            {
              label: '📄 Send to client',
              action: () => showToast('send contract ' + contractRef + ' to ' + clientObj?.name),
            },
            {
              label: '📋 View on dashboard',
              action: () => showToast('navigate to contract ' + contractRef),
            },
            {
              label: '+ New contract',
              action: () => {
      ...
      <div className="grid grid-cols-[1fr_300px] gap-8 max-w-5xl">
        ...
        <div>
              ...
              <div className="rounded-2xl p-6 space-y-5" style={glass}>
                ...
                <Field label="Contract name">
                  <input
                    ...
                    placeholder="e.g. Next Retail UK Ltd — Morning Clean Contract"
                    style={input}
                    className="placeholder-white/20"
                  />
                </Field>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {CONTRACT_TYPES.map((ct) => (
                      <button
                        type="button"
                        key={ct.id}
                        onClick={() => patchBasics({ contractType: ct.id })}
                        className="rounded-xl p-3 text-left transition-all"
                        style={
                          basics.contractType === ct.id
                            ? {
                ...
                <Field label="Account manager">
                  <input
                    type="text"
                    value={basics.accountManager}
                    onChange={(e) => patchBasics({ accountManager: e.target.value })}
                    style={input}
                  />
                </Field>
              </div>
              ...
              <div className="rounded-2xl p-6 space-y-5" style={glass}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <Field label="Notice period">
                    <select
                      value={basics.noticePeriod}
                      onChange={(e) => patchBasics({ noticePeriod: e.target.value })}
                      style={{ ...input, appearance: 'none' }}
                    >
                ...
                <div className="flex items-center gap-3">
                  <button
                    type="button"
                    onClick={() => patchBasics({ ongoing: !basics.ongoing })}
                  ...
                  <span className="text-white/60 text-sm">
                    Ongoing contract (no fixed end date)
                  </span>
                </div>
                {!basics.ongoing && (
                ...
                <div className="space-y-2">
                  {availSites.map((site) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`w-5 h-5 rounded-md flex items-center justify-center shrink-0 text-xs font-black transition-all ${
                        ...
                        {selected && (
                          <span className="text-[10px] font-black text-[#4f78ff]">Selected</span>
                        )}
                      </button>
                    );
                  })}
              ...
              <div className="rounded-2xl p-6 space-y-5" style={glass}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Field label="Photo requirements">
                    <select
                      value={compliance.photosRequired}
                      onChange={(e) => patchCompliance({ photosRequired: e.target.value })}
                      style={{ ...input, appearance: 'none' }}
                    >
                      {PHOTO_REQS.map((p) => (
                  ...
                  <Field label="Issue response time">
                    <select
                      value={compliance.issueResponseHours}
                      onChange={(e) => patchCompliance({ issueResponseHours: e.target.value })}
                      style={{ ...input, appearance: 'none' }}
                    >
                ...
                <div className="space-y-3 pt-1">
                  {[
                    {
                      key: 'gpsRequired',
                      label: 'GPS check-in/check-out required',
                      desc: 'Operative location verified at every visit',
                    },
                    {
                      key: 'checkInRequired',
                      label: 'Digital check-in required',
                      desc: 'App-based arrival confirmation',
                    },
                    {
                      key: 'signOffRequired',
                      label: 'Site supervisor sign-off required',
                      desc: 'Client signature on every job',
                    },
                  ].map(({ key, label, desc }) => (
                    <div key={key} className="flex items-center gap-3 py-1">
                      <button
                        type="button"
                        onClick={() => patchCompliance({ [key]: !compliance[key] })}
                        className="w-11 h-6 rounded-full transition-colors relative shrink-0"
                        style={{
                          background: compliance[key] ? '#4f78ff' : 'rgba(255,255,255,0.15)',
                        }}
                      >
                        <div
                          className="absolute top-1 w-4 h-4 rounded-full bg-white shadow transition-all"
              ...
              <div className="rounded-2xl p-6 space-y-5" style={glass}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Field label="DBS requirement">
                    <select
                      value={compliance.dbs}
                      onChange={(e) => patchCompliance({ dbs: e.target.value })}
                      style={{ ...input, appearance: 'none' }}
                    >
                      {DBS_OPTS.map((d) => (
                  ...
                  <Field label="Min. operatives per visit">
                    <select
                      value={compliance.minOperatives}
                      onChange={(e) => patchCompliance({ minOperatives: e.target.value })}
                      style={{ ...input, appearance: 'none' }}
                    >
                      {['1', '2', '3', '4', '5', '6+'].map((n) => (
                        <option key={n} value={n}>
              ...
              <div className="rounded-2xl p-6 space-y-4" style={glass}>
                ...
                <div className="space-y-3">
                  {[
                    {
                      key: 'riskAssessment',
                      label: 'Site-specific risk assessment',
                      desc: 'Required before first clean',
                    },
                    {
                      key: 'coshh',
                      label: 'COSHH assessment',
                      desc: 'Chemical handling documentation',
                    },
                    {
                      key: 'methodStatement',
                      label: 'Method statement',
                      desc: 'Cleaning methodology sign-off',
                    },
                  ].map(({ key, label, desc }) => (
                    <div key={key} className="flex items-center gap-3 py-1">
                      <button
                        ...
                        className="w-11 h-6 rounded-full transition-colors relative shrink-0"
                        style={{
                          background: compliance[key] ? '#10b981' : 'rgba(255,255,255,0.15)',
                        }}
                      >
                        <div
                          className="absolute top-1 w-4 h-4 rounded-full bg-white shadow transition-all"
            ...
            <div className="space-y-5">
              <div className="rounded-2xl p-6 space-y-5" style={glass}>
                ...
                {chosenSites.length === 0 && (
                  <div className="text-white/30 text-sm py-2">
                    No sites selected — go back to Step 2
                  </div>
                )}
                      ...
                      >
                        <div className="flex items-start gap-2">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="text-white/35 text-xs">
                              {cfg.service || '—'} · {freq} · {vpm} visits/mo
                            </div>
                          </div>
                        </div>
                        <div className="grid grid-cols-3 gap-3 items-end">
                          ...
                          <div
                            ...
                            style={{
                              background:
                                monthly > 0 ? 'rgba(79,120,255,0.12)' : 'rgba(255,255,255,0.04)',
                              border:
                                monthly > 0
                                  ? '1px solid rgba(79,120,255,0.25)'
                                  : '1px solid rgba(255,255,255,0.08)',
                            }}
                          >
                            <div
                              className="text-sm font-black"
                              style={{ color: monthly > 0 ? '#93c5fd' : 'rgba(255,255,255,0.2)' }}
                            >
                              {monthly > 0
                                ? `£${monthly.toLocaleString('en-GB', { minimumFractionDigits: 0 })}`
                                : '—'}
                            </div>
              ...
              <div className="rounded-2xl p-6 space-y-5" style={glass}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Field label="Invoice frequency">
                    <select
                      ...
                      onChange={(e) => patchPricing({ billingFrequency: e.target.value })}
                      style={{ ...input, appearance: 'none' }}
                    >
                      {['Weekly', 'Monthly', '4-weekly', 'Quarterly'].map((f) => (
                        <option key={f} value={f}>
                          {f}
                        </option>
                      ))}
                  ...
                  <Field label="Payment terms">
                    <select
                      ...
                      onChange={(e) => patchPricing({ paymentTerms: e.target.value })}
                      style={{ ...input, appearance: 'none' }}
                    >
                      {PAYMENT_TERMS.map((p) => (
                        <option key={p} value={p}>
                ...
                <div className="flex items-center gap-3">
                  <button
                    type="button"
                    onClick={() => patchPricing({ vatRegistered: !pricing.vatRegistered })}
                    className="w-11 h-6 rounded-full transition-colors relative shrink-0"
                    style={{
                      background: pricing.vatRegistered ? '#4f78ff' : 'rgba(255,255,255,0.15)',
                    }}
                  >
                    <div
                      className="absolute top-1 w-4 h-4 rounded-full bg-white shadow transition-all"
                  ...
                  <span className="text-white/60 text-sm">
                    VAT registered (20% applied to invoices)
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="flex gap-3 mt-6">
            ...
            {step < 3 ? (
              <button
                ...
                style={{
                  ...
                  border: stepValid[step]
                    ? '1px solid rgba(79,120,255,0.5)'
            ...
            <button
              type="button"
              onClick={() => showToast('contract saved as draft')}
        ...
        <div className="space-y-4 pt-16">
          ...
          >
            ...
            <div className="space-y-2.5">
              {[
                {
                  label: 'Type',
                  value: CONTRACT_TYPES.find((c) => c.id === basics.contractType)?.label,
                },
                ...
                {
                  label: 'Sites',
                  value: selectedSiteIds.length > 0 ? `${selectedSiteIds.length} selected` : '—',
                },
                ...
                >
                  ...
                  <span className="text-white/60 font-medium text-right ml-3 truncate max-w-32">
                    {value || '—'}
                  </span>
                </div>
              ))}
            ...
            {monthlyTotal > 0 && (
              <>
                ...
                >
                  <div className="text-white/40 text-[10px] font-black uppercase tracking-wider">
                    Contract value
                  </div>
          ...
          >
            ...
            <div className="text-white/35 text-xs leading-relaxed">
              Cadi will automatically match qualified Connect-verified operatives to this contract
              based on location, service type, and DBS requirements.
            </div>
          </div>
        </div>
      </div>
