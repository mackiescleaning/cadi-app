...
const DEMO_CLIENTS = [
  {
    id: 'c1',
    name: 'Tesco Distribution – Welwyn Garden City',
    sector: 'Industrial',
    sites: 1,
    icon: '🏭',
  },
  {
    id: 'c2',
    name: 'NHS Property Services – Luton Cluster',
    sector: 'Healthcare',
    sites: 4,
    icon: '🏥',
  },
...
const SUGGESTED_CONTRACTORS = {
  'Window cleaning': [
    ...
    { name: 'Apex Window Care', region: 'Midlands', rating: 4.8, rate: '£125–145/job', jobs: 178 },
  ],
  'Jet washing': [
    { name: 'ProWash Midlands', region: 'Midlands', rating: 4.7, rate: '£220–260/job', jobs: 89 },
    {
      name: 'Midlands Pressure Wash',
      region: 'Midlands',
      rating: 4.4,
      rate: '£200–240/job',
      jobs: 31,
  ...
  'Gutter clearing': [
    { name: 'Capital Gutters Ltd', region: 'South', rating: 4.8, rate: '£280–380/job', jobs: 67 },
  ],
  'Graffiti removal': [
    { name: 'SprayTech Services', region: 'North', rating: 4.5, rate: '£420–520/job', jobs: 44 },
  ],
  'Pressure washing': [
    { name: 'ProWash Midlands', region: 'Midlands', rating: 4.7, rate: '£220–260/job', jobs: 89 },
  ],
...
export default function FmDeployExterior({ showToast: _showToast }) {
    ...
    <div className="p-6 max-w-3xl space-y-5">
      ...
      >
        ...
        <div className="ml-4 text-right">
          <div className="text-lg font-black" style={{ color: '#38bdf8' }}>
            £0
          </div>
            ...
            <div key={n} className="flex items-center flex-1">
              <div className="flex flex-col items-center gap-1.5 flex-shrink-0">
                ...
                <span
                  ...
                  style={{
                    color: done ? '#34d399' : current ? '#38bdf8' : 'rgba(255,255,255,0.25)',
                  }}
                >
                  {label}
                </span>
              ...
              {i < STEPS.length - 1 && (
                <div
                  ...
                  style={{
                    background: step > n ? '#34d399' : 'rgba(255,255,255,0.08)',
          ...
          <div className="space-y-5">
            <div>
              ...
              <div className="text-white/40 text-xs">
                Cadi uses previous contracts for this client to suggest service types and rates
                automatically.
              </div>
            </div>
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {DEMO_CLIENTS.map((c) => (
                  ...
                  >
                    ...
                    <div
                      className="text-[10px]"
                      style={{ color: clientId === c.id ? '#38bdf8' : 'rgba(255,255,255,0.3)' }}
                    >
                      {c.sector} · {c.sites} site{c.sites !== 1 ? 's' : ''}
                    </div>
                  </button>
                ))}
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <div className="text-[10px] font-black uppercase tracking-widest text-white/30 mb-1.5">
                  Contract value
                </div>
                <input
                  value={value}
            ...
            <div>
              ...
              <div className="flex gap-2">
                {[
                  {
                    v: 'set',
                    label: '💷 We set the price',
                    desc: 'Contractors accept or decline your job price',
                  },
                  {
                    v: 'quote',
                    label: '📩 Request quotes',
                    desc: 'Contractors quote per job, you review & accept',
                  },
                ].map((opt) => (
                  <button
                    key={opt.v}
        ...
        {step === 2 && (
          <div className="space-y-5">
            <div>
              ...
              <div className="text-white/40 text-xs">
                Cadi pre-selects based on previous {client.sector} contracts. Edit as needed.
              </div>
            </div>
            ...
            >
              ✓ {services.length} service type{services.length !== 1 ? 's' : ''} selected · Cadi
              will match {totalSubs} contractor{totalSubs !== 1 ? 's' : ''} from your pool for{' '}
              {client.sites} site{client.sites !== 1 ? 's' : ''}
            </div>
            <div className="flex gap-3">
              <button
                onClick={() => setStep(1)}
                className="px-6 py-3 rounded-xl text-sm font-bold text-white/40 transition-all"
        ...
        {step === 3 && (
          <div className="space-y-5">
            <div>
              ...
              <div className="text-white/40 text-xs">
                Cadi scored your contractor pool by trade, proximity and past performance. Review
                and confirm.
              </div>
            </div>
                          ...
                          >
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="text-white/40 text-[10px]">
                                {sub.region} · {sub.rating}★ · {sub.jobs} jobs · {sub.rate}
                              </div>
                            </div>
                            <button
                              onClick={() => acceptSub(svc, sub)}
            ...
            <div className="flex gap-3">
              <button
                onClick={() => setStep(2)}
                className="px-6 py-3 rounded-xl text-sm font-bold text-white/40"
                style={{ border: '1px solid rgba(255,255,255,0.1)' }}
              >
                ← Back
              </button>
        ...
        {step === 4 && (
          <div className="space-y-5">
            <div>
              ...
              <div className="text-white/40 text-xs">
                One click sends job cards to all matched contractors simultaneously. Old process: 6
                weeks of calls and emails.
              </div>
            </div>
            <div
            ...
            >
              <div className="grid grid-cols-3 gap-3 text-center">
                <div>
                  <div className="text-2xl font-black text-white">{client.sites}</div>
                  <div className="text-[10px] text-white/40">Sites</div>
                </div>
                <div>
                  <div className="text-2xl font-black" style={{ color: '#38bdf8' }}>
                    {services.length}
                  </div>
                  <div className="text-[10px] text-white/40">Service types</div>
                </div>
                <div>
                  <div className="text-2xl font-black" style={{ color: '#34d399' }}>
                    {totalSubs}
                  </div>
                  <div className="text-[10px] text-white/40">Contractors notified</div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="font-bold" style={{ color: '#38bdf8' }}>
                  {rateMode === 'set'
                    ? 'Britannia sets price — contractors accept/decline'
                    : 'Contractors quote per job — you review'}
                </span>
              </div>
            </div>
            ...
            {broadcastDone ? (
                ...
                <div>
                  <div className="flex items-center justify-between text-xs mb-1.5">
                    ...
                    <span
                      ...
                      style={{ color: coveragePct === 100 ? '#34d399' : '#38bdf8' }}
                    >
                      {coveragePct}%
                    </span>
                  </div>
                  ...
                  >
                    <div
                      ...
                      style={{
                        width: `${coveragePct}%`,
                        background: coveragePct === 100 ? '#34d399' : '#38bdf8',
                        boxShadow: `0 0 12px ${coveragePct === 100 ? '#34d399' : '#38bdf8'}60`,
                      }}
                    />
                  </div>
                </div>
              ...
              <button
                ...
                style={{
                  ...
                  boxShadow: broadcasting ? 'none' : '0 4px 20px rgba(14,165,233,0.4)',
                  cursor: broadcasting ? 'wait' : 'pointer',
                }}
              >
                {broadcasting
                  ? '⚡ Broadcasting to contractor pool...'
                  : `🚀 Fire job cards to ${totalSubs} contractor${totalSubs !== 1 ? 's' : ''}`}
              </button>
            )}
            ...
            {!broadcastDone && (
              <button
                onClick={() => setStep(3)}
                className="w-full py-2.5 rounded-xl text-sm font-bold text-white/40"
                style={{ border: '1px solid rgba(255,255,255,0.1)' }}
              >
                ← Back
              </button>
          ...
          <div className="space-y-5 text-center">
            ...
            <div>
              ...
              <div className="text-white/40 text-sm">
                100% coverage confirmed. {client.name} notified. First visits scheduled.
              </div>
            </div>
            ...
            >
              {[
                ...
                <div key={line} className="text-[11px] text-emerald-400/80">
                  {line}
                </div>
              ))}
            </div>
            <button
              ...
              className="w-full py-3 rounded-xl text-sm font-black text-white/60 transition-all"
              style={{ border: '1px solid rgba(255,255,255,0.1)' }}
            >
