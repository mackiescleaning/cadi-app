...
import { Download, Share2, TrendingUp, TrendingDown, X, FileText, ChevronRight } from 'lucide-react';

/* ── Monthly data ─────────────────────────────────────────────────── */
const MONTHS = [
  { id: 'nov25', label: 'Nov 2025', short: 'Nov', jobs: 189, sla: 96, value: 23200, cleaners: 22, sites: 13, photos: 1890, disputes: 3 },
  { id: 'dec25', label: 'Dec 2025', short: 'Dec', jobs: 201, sla: 97, value: 24800, cleaners: 23, sites: 13, photos: 2130, disputes: 2 },
  { id: 'jan26', label: 'Jan 2026', short: 'Jan', jobs: 218, sla: 98, value: 26900, cleaners: 25, sites: 14, photos: 2340, disputes: 1 },
  { id: 'feb26', label: 'Feb 2026', short: 'Feb', jobs: 209, sla: 96, value: 25700, cleaners: 24, sites: 14, photos: 2210, disputes: 2 },
  { id: 'mar26', label: 'Mar 2026', short: 'Mar', jobs: 234, sla: 98, value: 28900, cleaners: 26, sites: 15, photos: 2650, disputes: 1 },
  { id: 'apr26', label: 'Apr 2026', short: 'Apr', jobs: 241, sla: 99, value: 29700, cleaners: 27, sites: 15, photos: 2760, disputes: 0 },
  { id: 'may26', label: 'May 2026', short: 'May', jobs: 98,  sla: 98, value: 12100, cleaners: 27, sites: 15, photos: 1120, disputes: 0, partial: true },
];
...
function BarChart({ data, activeId }) {
  ...
  const W = 520, H = 140, BAR_W = 48, GAP = (W - data.length * BAR_W) / (data.length + 1);
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${W} ${H + 24}`} style={{ overflow: 'visible' }}>
      <defs>
        {data.map((d, i) => (
          <linearGradient key={d.id} id={`bg-${d.id}`} x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor={d.id === activeId ? '#6b8fff' : d.partial ? '#4f78ff' : '#4f78ff'} stopOpacity={d.id === activeId ? 0.95 : d.partial ? 0.35 : 0.75} />
            <stop offset="100%" stopColor={d.id === activeId ? '#3b5fd6' : '#2a4abf'} stopOpacity={d.id === activeId ? 0.95 : d.partial ? 0.2 : 0.55} />
          </linearGradient>
        ))}
      </defs>
      {/* Grid lines */}
      ...
      {data.map((d, i) => {
        ...
        return (
          <g key={d.id}>
            <rect x={x} y={H - bh} width={BAR_W} height={bh} rx={6}
              fill={`url(#bg-${d.id})`}
              style={{ filter: isActive ? 'drop-shadow(0 0 8px rgba(79,120,255,0.5))' : 'none' }}
            />
            {/* Value label */}
            <text x={x + BAR_W / 2} y={H - bh - 6} textAnchor="middle"
              fill={isActive ? 'white' : 'rgba(255,255,255,0.45)'}
              fontSize={isActive ? 11 : 10} fontWeight={isActive ? 800 : 600}>
              {d.jobs}{d.partial ? '*' : ''}
            </text>
            {/* Month label */}
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ overflow: 'visible' }}>
      ...
      {pts.map(([x, y], i) => {
        ...
        return (
          <circle key={i} cx={x} cy={y} r={isActive ? 4 : 2.5}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6"
      ...
      <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl shadow-2xl"
        ...
        {/* PDF-style header */}
        <div className="px-8 py-6" style={{ background: 'linear-gradient(135deg, #020c3e 0%, #0d1b4f 100%)' }}>
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-[#4f78ff] flex items-center justify-center text-white font-black text-base shadow-lg shadow-[#4f78ff]/30">B</div>
              <div>
          ...
          <div className="mt-5 flex items-end justify-between">
            <div>
              <div className="text-white/40 text-[10px] font-black uppercase tracking-widest">Report period</div>
              <div className="text-white font-black text-2xl mt-0.5">{month.label}</div>
              {month.partial && <div className="text-white/35 text-xs mt-0.5">Partial month (MTD)</div>}
            </div>
        ...
        <div className="px-8 py-6 space-y-6">
          ...
          {/* Executive summary */}
          <div>
            <div className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-3">Executive summary</div>
            <div className="grid grid-cols-4 gap-3">
              {[
                { label: 'Jobs completed', value: month.jobs.toString(),   color: '#4f78ff' },
                { label: 'SLA hit rate',   value: `${month.sla}%`,         color: '#10b981' },
                { label: 'Total invoiced', value: `£${(month.value/1000).toFixed(1)}k`, color: '#8b5cf6' },
                { label: 'Active cleaners',value: month.cleaners.toString(),color: '#f59e0b' },
              ].map(({ label, value, color }) => (
                <div key={label} className="rounded-xl p-3 text-center" style={{ background: `${color}0f`, border: `1px solid ${color}20` }}>
                  <div className="text-xl font-black" style={{ color }}>{value}</div>
                  <div className="text-gray-500 text-[10px] font-bold mt-0.5 leading-tight">{label}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* vs prior month */}
          {prev && (
            <div className="rounded-xl px-4 py-3.5" style={{ background: '#f8faff', border: '1px solid #e0eaff' }}>
              <div className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">vs {prev.label}</div>
              <div className="grid grid-cols-3 gap-4 text-sm">
                {[
                  { label: 'Jobs', cur: month.jobs, prv: prev.jobs, fmt: v => v.toString() },
                  { label: 'SLA', cur: month.sla, prv: prev.sla, fmt: v => `${v}%` },
                  { label: 'Revenue', cur: month.value, prv: prev.value, fmt: v => `£${v.toLocaleString()}` },
          ...
          {/* Client breakdown */}
          <div>
            <div className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-3">Client performance</div>
            <table className="w-full text-sm">
              <thead>
                <tr style={{ borderBottom: '2px solid #f1f5f9' }}>
                  {['Client', 'Sector', 'Jobs', 'Value', 'SLA'].map(h => (
                    <th key={h} className="text-left pb-2 text-[10px] font-black uppercase tracking-wider text-gray-400 pr-4">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {clientRows.map(({ clientId, jobs, value, sla }) => {
                  ...
                  return (
                    <tr key={clientId} style={{ borderBottom: '1px solid #f8fafc' }}>
                      ...
                      <td className="py-2.5 pr-4 font-bold text-[#010a4f]">£{value.toLocaleString()}</td>
                      <td className="py-2.5">
          ...
          {/* Top cleaners */}
          <div>
            <div className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-3">Top performing cleaners</div>
            <div className="space-y-2">
              {TOP_CLEANERS.slice(0, 3).map((c, i) => (
                <div key={c.name} className="flex items-center gap-3 px-3 py-2.5 rounded-xl" style={{ background: '#f8faff' }}>
                  <div className="w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-black shrink-0"
                    style={{ background: i === 0 ? '#fef9c3' : '#f1f5f9', color: i === 0 ? '#ca8a04' : '#64748b' }}>
                    {i + 1}
                  </div>
                  <div className="w-7 h-7 rounded-lg flex items-center justify-center text-xs font-black text-white shrink-0"
                    style={{ background: '#4f78ff' }}>{c.avatar}</div>
                  <div className="flex-1">
                    <div className="font-bold text-[#010a4f] text-sm">{c.name}</div>
                    <div className="text-gray-400 text-xs">{c.jobs} jobs · {c.sla}% SLA · ★ {c.rating}</div>
                  </div>
                  <div className="text-right">
                    <div className="font-black text-sm" style={{ color: '#4f78ff' }}>{c.score}</div>
          ...
          {/* Geo compliance note */}
          <div className="rounded-xl px-4 py-3" style={{ background: 'rgba(16,185,129,0.06)', border: '1px solid rgba(16,185,129,0.18)' }}>
            <div className="text-[10px] font-black uppercase tracking-widest mb-1.5" style={{ color: '#10b981' }}>Geo-verification compliance</div>
            <div className="text-gray-600 text-xs leading-relaxed">
              {month.photos.toLocaleString()} photos geo-stamped this period. All evidence stored with SHA-256 tamper-evident hashes, available on request for client audits or contract disputes.
            </div>
          </div>

          {/* Footer */}
          <div className="pt-2 flex items-center justify-between text-xs text-gray-400" style={{ borderTop: '1px solid #f1f5f9' }}>
...
/* ── Main page ───────────────────────────────────────────────────── */
const IMPACT = [
  { before: '3–4 hrs per client building monthly reports in Excel',           after: 'Auto-generated in seconds — every month, every client, zero effort', icon: '⏱' },
  { before: 'Client questions a clean — you can\'t prove it without chasing', after: 'Every visit evidenced and timestamped — answer any query in seconds', icon: '✅' },
  { before: 'Which contracts are profitable? Nobody actually knows',          after: 'Cost vs revenue per contract, per region — margin visible every week', icon: '💷' },
];
      ...
      <div className="p-8 space-y-6 min-h-full">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <div className="text-white/35 text-sm mt-0.5">Britannia Group Network · 15 sites · Herts, Beds &amp; Bucks</div>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={() => setShowReport(true)}
              ...
              style={{ background: 'linear-gradient(135deg, rgba(79,120,255,0.35), rgba(99,102,241,0.28))', border: '1px solid rgba(79,120,255,0.5)', boxShadow: '0 0 20px rgba(79,120,255,0.15)' }}>
              <FileText size={14} />
              Generate report
            </button>
            <button onClick={() => showToast('export data to CSV')}
        ...
        <div className="flex items-center gap-1.5">
          {MONTHS.map(m => (
            <button key={m.id} onClick={() => setActiveId(m.id)}
              ...
              style={activeId === m.id ? {
                background: 'linear-gradient(135deg, rgba(79,120,255,0.25), rgba(99,102,241,0.18))',
        ...
        <div className="grid grid-cols-4 gap-4">
          <KpiCard label="Jobs completed"  value={month.jobs.toString()}  color="#4f78ff" sub="this period"
            prevValue={prev ? delta(month.jobs, prev.jobs)?.label : undefined}
            up={prev ? month.jobs >= prev.jobs : true} />
          <KpiCard label="SLA hit rate"    value={`${month.sla}%`}        color="#34d399" sub="on-time completions"
            prevValue={prev ? delta(month.sla, prev.sla, 'pp')?.label : undefined}
            up={prev ? month.sla >= prev.sla : true} />
          <KpiCard label="Total invoiced"  value={`£${(month.value/1000).toFixed(1)}k`} color="#a78bfa" sub="excl. VAT"
            prevValue={prev ? delta(month.value, prev.value)?.label.replace(/(\d)(?=(\d{3})+$)/g,'$1,') : undefined}
            up={prev ? month.value >= prev.value : true} />
          <KpiCard label="Geo-stamped photos" value={month.photos.toLocaleString()} color="#fbbf24" sub="tamper-evident" />
        </div>
        ...
        <div className="grid grid-cols-[1fr_300px] gap-5">
          ...
          <div className="rounded-2xl overflow-hidden" style={{
            ...
            <div className="px-6 pt-5 pb-3 flex items-center justify-between" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
              <div>
                ...
                <div className="text-white/30 text-xs mt-0.5">Nov 2025 – May 2026 · click a month to select</div>
              </div>
              <div className="flex items-center gap-3 text-xs">
                <div className="flex items-center gap-1.5"><div className="w-3 h-1.5 rounded-full bg-[#4f78ff]" /><span className="text-white/35">Completed</span></div>
                <div className="flex items-center gap-1.5"><div className="w-3 h-1.5 rounded-full bg-[#4f78ff]/35" /><span className="text-white/35">Partial</span></div>
              </div>
            </div>
            <div className="px-6 py-5 cursor-pointer" onClick={() => {}}>
              <BarChart data={MONTHS} activeId={activeId} />
            </div>
            <div className="px-6 pb-4 text-white/20 text-[10px]">* May 2026 is a partial month (MTD through 18 May)</div>
          </div>
          ...
          <div className="space-y-4">
            <div className="rounded-2xl p-5" style={{
              ...
              <div className="text-white/30 text-[10px] font-black uppercase tracking-widest mb-1">SLA trend</div>
              <div className="flex items-end justify-between mb-3">
                ...
                <span className="text-xs font-bold" style={{ color: '#34d399' }}>
                  {month.sla >= 99 ? '↑ Record high' : month.sla >= 98 ? '↑ Above target' : '→ On target'}
                </span>
              </div>
              <SlaChart data={MONTHS} activeId={activeId} />
              <div className="flex justify-between mt-2">
                {MONTHS.map(m => (
                  <span key={m.id} className="text-[9px] font-bold" style={{ color: m.id === activeId ? 'rgba(255,255,255,0.7)' : 'rgba(255,255,255,0.2)' }}>
                    {m.short}
                  </span>
                ))}
              </div>
            ...
            <div className="rounded-2xl p-4" style={{
              ...
              <div className="text-white/30 text-[10px] font-black uppercase tracking-widest mb-3">Network</div>
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  <div key={label} className="rounded-xl px-3 py-2.5" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.07)' }}>
                    <div className="text-white font-black text-base" style={ good && value === 0 ? { color: '#34d399' } : {} }>{value}</div>
        ...
        <div className="rounded-2xl overflow-hidden" style={{
          ...
          <div className="grid px-6 py-2.5" style={{ gridTemplateColumns: '1fr 60px 1fr 100px 100px 130px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
          ...
          {clientRows.map(({ clientId, jobs, value, sla, trend }) => {
            const client = clients.find(c => c.id === clientId);
            const slaStyle = sla === 100
              ? { bg: 'rgba(52,211,153,0.1)', border: 'rgba(52,211,153,0.25)', color: '#34d399' }
              : sla >= 98
              ? { bg: 'rgba(79,120,255,0.1)', border: 'rgba(79,120,255,0.25)', color: '#60a5fa' }
              : { bg: 'rgba(245,158,11,0.1)', border: 'rgba(245,158,11,0.25)', color: '#fbbf24' };
            return (
              <div key={clientId} className="grid px-6 py-3.5 transition-all"
                style={{ gridTemplateColumns: '1fr 60px 1fr 100px 100px 130px', borderBottom: '1px solid rgba(255,255,255,0.04)' }}
                onMouseEnter={e => { e.currentTarget.style.background = 'rgba(79,120,255,0.04)'; }}
              ...
              >
                ...
                <div className="self-center pr-6">
                  <div className="h-1.5 rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.08)' }}>
                    <div className="h-full rounded-full transition-all"
                      style={{ width: `${(jobs / maxJobs) * 100}%`, background: 'linear-gradient(90deg, #3b5fd6, #4f78ff)', boxShadow: '0 0 6px rgba(79,120,255,0.4)' }} />
                  </div>
                </div>
                <div className="text-white font-bold text-sm self-center">£{value.toLocaleString()}</div>
                <div className="self-center">
                  <span className="text-xs font-bold px-2.5 py-1 rounded-full"
                    style={{ background: slaStyle.bg, border: `1px solid ${slaStyle.border}`, color: slaStyle.color }}>
                    {sla}% SLA
                  </span>
                </div>
                <div className="self-center flex items-center gap-2">
                  {trend !== undefined && (
                    <span className="text-[10px] font-black flex items-center gap-0.5"
                      style={{ color: trend > 0 ? '#34d399' : trend < 0 ? '#f87171' : 'rgba(255,255,255,0.25)' }}>
        ...
        <div className="rounded-2xl overflow-hidden" style={{
          ...
          <div className="px-6 py-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            <div className="text-white font-bold text-sm">Top performing cleaners — {month.label}</div>
          </div>
          <div className="divide-y" style={{ borderColor: 'rgba(255,255,255,0.04)' }}>
            {TOP_CLEANERS.map((c, i) => (
              <div key={c.name} className="px-6 py-3.5 flex items-center gap-4 transition-all"
                onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.03)'}
                onMouseLeave={e => e.currentTarget.style.background = ''}>
                <div className="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black shrink-0"
                  style={i === 0 ? { background: 'rgba(251,191,36,0.2)', border: '1px solid rgba(251,191,36,0.4)', color: '#fbbf24' }
                    : { background: 'rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.4)' }}>
                  {i + 1}
                </div>
                <div className="w-8 h-8 rounded-xl flex items-center justify-center text-xs font-black text-white shrink-0"
                  style={{ background: 'linear-gradient(135deg, rgba(79,120,255,0.35), rgba(99,102,241,0.25))', border: '1px solid rgba(79,120,255,0.35)' }}>
                  {c.avatar}
                </div>
                <div className="flex-1">
                  <div className="text-white font-bold text-sm">{c.name}</div>
                  <div className="text-white/35 text-xs mt-0.5">{c.jobs} jobs · {c.sla}% SLA · ★ {c.rating}</div>
                </div>
        ...
        <div className="rounded-2xl p-6 flex items-center justify-between" style={{
          background: 'linear-gradient(135deg, rgba(79,120,255,0.12) 0%, rgba(99,102,241,0.07) 100%)',
          ...
          <div>
            ...
            <div className="text-white/40 text-sm mt-0.5">Per-client PDFs · SLA evidence · network performance · invoice totals · geo-audit log</div>
          </div>
          <div className="flex items-center gap-2 shrink-0 ml-6">
            <button onClick={() => setShowReport(true)}
              ...
              style={{ background: 'rgba(79,120,255,0.35)', border: '1px solid rgba(79,120,255,0.55)', boxShadow: '0 0 20px rgba(79,120,255,0.15)' }}>
              <FileText size={14} />
              Preview report
            </button>
            <button onClick={() => showToast('download full report pack')}
