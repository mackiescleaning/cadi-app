...
const ONBOARDEES = [
  {
    ...
    stages: {
      ...
      briefing: {
        ...
        date: '20 May',
        note: 'Site briefing with Stefan Kovac scheduled 20 May 08:30',
      },
      trial: {
        done: false,
        date: '21 May',
        note: 'Trial clean — 06:00–08:00 with supervisor present',
      },
  ...
  },
  {
    ...
    stages: {
      ...
      briefing: {
        ...
        date: '15 May',
        note: 'Briefed by area supervisor — checklist reviewed',
      },
      trial: {
        done: false,
        date: '20 May',
        note: 'Trial clean booked Wed 20 May 07:00 — supervisor attending',
      },
        ...
        return (
          <div key={s.id} className="flex items-center flex-1">
            <div className="flex flex-col items-center flex-1">
              ...
              <div
                className={`text-[9px] mt-1 font-bold text-center leading-tight ${
            ...
            {i < STAGES.length - 1 && (
              <div
                className="w-4 h-px shrink-0 -mt-4"
                style={{ background: done ? 'rgba(52,211,153,0.4)' : 'rgba(255,255,255,0.08)' }}
              />
            )}
          </div>
        );
...
const IMPACT = [
  {
    before: 'New operative induction: paper forms, phone calls, 2-week DBS wait',
    after: 'Digital onboarding — DBS tracked, docs collected, portal access day one',
    icon: '📋',
  },
  {
    before: 'No visibility of where each new hire is in the process',
    after: 'Every operative tracked stage by stage — nothing falls through',
    icon: '👁',
  },
  {
    before: 'Compliance gaps discovered on site, contract at risk',
    after: 'Compliance gated before first shift — FM protected',
    icon: '🔒',
  },
];
    ...
    <div className="p-6 space-y-5 max-w-4xl">
      ...
      >
        ...
        <div className="text-xs text-white/50">
          Cadi handles DBS verification, app setup, and checklist briefing automatically — your team
          only needs to approve and confirm the trial clean.
        </div>
      </div>
          ...
          return (
            ...
            >
              <div
                ...
                onClick={() => setExpanded(isOpen ? null : o.name)}
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="w-16 text-right">
                    <div
                      ...
                      style={{ color: pct >= 80 ? '#34d399' : pct >= 50 ? '#4f78ff' : '#fbbf24' }}
                    >
                      {pct}%
                    </div>
              ...
              {isOpen && (
                <div className="px-5 pb-5 border-t border-white/5">
                    ...
                    {STAGES.map((s) => {
                      ...
                      return (
                        <div
                          ...
                          style={{
                            background: isCurrent ? 'rgba(79,120,255,0.08)' : 'transparent',
                            border: isCurrent
                              ? '1px solid rgba(79,120,255,0.2)'
                        ...
                        >
                          ...
                          <div className="flex-1">
                            <div
                              className={`text-xs font-bold ${done ? 'text-white/60' : isCurrent ? 'text-white' : 'text-white/25'}`}
                            >
                              {s.label}
                              {s.auto && (
                                <span
                                  className="ml-2 text-[9px] font-black px-1.5 py-0.5 rounded"
                            ...
                            <div
                              className={`text-[11px] mt-0.5 ${done ? 'text-white/35' : isCurrent ? 'text-white/55' : 'text-white/20'}`}
                            >
                              {sd?.note}
                            </div>
                            {sd?.date && (
                              <div className="text-[10px] text-white/20 mt-0.5">{sd.date}</div>
                            )}
                          </div>
                        </div>
                      );
                  ...
                  <div className="flex gap-2 mt-4">
                    <button
                      onClick={() => showToast(`send update to ${o.name}`)}
                    ...
                    <button
                      onClick={() => showToast(`mark next stage complete for ${o.name}`)}
