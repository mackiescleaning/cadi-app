...
import {
  ShoppingBag, GitBranch, CheckSquare, Star, PoundSterling, Receipt,
  Network, TrendingUp, ChevronRight, Sparkles, Loader2,
  MapPin, ArrowUpRight,
} from 'lucide-react';
import {
  listMyConnectJobs,
...
import {
  ...
  glassDark, navyCanvas, primaryButton, whiteButton, ghostButton, HOVER_LIFT,
      ...
      >
        {building ? (
          <>
            ...
            <div className="text-white/70 text-[10px] font-black tracking-[0.18em] uppercase">Building</div>
            <div className="text-white text-xs font-bold mt-1 leading-tight px-3">Unlocks at<br/>5 jobs</div>
          </>
        ) : (
          <>
            <div className="text-white/60 text-[9px] font-black tracking-[0.22em] uppercase mb-1">Cadi Score</div>
            <div className="text-white text-4xl font-black leading-none tabular-nums">{score}</div>
            <div className="mt-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider"
...
function MatchRing({ score }) {
  const color = score >= 90 ? ON_DARK.success
              : score >= 80 ? ON_DARK.bluePop
              : ON_DARK.amber;
  return (
    <div className="flex flex-col items-center gap-0.5 shrink-0">
      ...
      <div className="text-[9px] font-black tracking-wider uppercase" style={{ color }}>match</div>
    </div>
  );
}
...
function StatCard({ label, value, sub, accent, loading }) {
  return (
    <div style={glassDark({ padding: 18, radius: CONNECT_RADII.lg })}
      className={HOVER_LIFT}>
      <div className="text-[10px] font-black uppercase tracking-widest mb-2"
        style={{ color: ON_DARK.muted }}>{label}</div>
      <div className="text-3xl font-black tabular-nums leading-none" style={{ color: accent }}>
        {loading ? '—' : value}
      </div>
      <div className="text-[11px] mt-2" style={{ color: ON_DARK.faint }}>{sub}</div>
    </div>
  );
}
...
function QuickTile({ Icon, label, sub, tone, onClick, badge }) {
  const toneMap = {
    orange: { bg: `linear-gradient(135deg, #ff6a30 0%, ${ORANGE} 100%)`, halo: 'rgba(194,65,12,0.55)' },
    blue:   { bg: `linear-gradient(135deg, #7ea3ff 0%, #1e3a8a 100%)`,   halo: 'rgba(79,120,255,0.55)' },
    green:  { bg: `linear-gradient(135deg, #34d399 0%, #047857 100%)`,   halo: 'rgba(16,185,129,0.55)' },
    navy:   { bg: `linear-gradient(135deg, #4f78ff 0%, #010a4f 100%)`,   halo: 'rgba(79,120,255,0.45)' },
    amber:  { bg: `linear-gradient(135deg, #fbbf24 0%, #b45309 100%)`,   halo: 'rgba(245,158,11,0.55)' },
    purple: { bg: `linear-gradient(135deg, #c084fc 0%, #6d28d9 100%)`,   halo: 'rgba(168,85,247,0.55)' },
  };
  ...
  return (
    ...
    >
      {badge != null && badge > 0 && (
        <span className="absolute top-3 right-3 min-w-[22px] h-[22px] px-1.5 rounded-full flex items-center justify-center text-[10px] font-black text-white"
      ...
      <div className="font-black text-sm text-white" style={{ letterSpacing: '-0.01em' }}>{label}</div>
      <div className="text-[11px] mt-0.5" style={{ color: ON_DARK.muted }}>{sub}</div>
    </button>
  );
}
  ...
  return (
    <div className="flex items-center justify-between mb-3 px-1">
      <h2 style={{
        ...
        textTransform: 'uppercase', color: ON_DARK.muted,
      }}>{children}</h2>
      {action && (
...
function chipDark(tone = 'neutral') {
  ...
  return {
    ...
    padding: '4px 10px', fontSize: 11, fontWeight: 700, lineHeight: 1,
...
export default function EarnLanding() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setJobs(j); setInvoices(i); setListings(l); setProfile(p);
      } catch (e) {
  ...
  const summary = useMemo(() => {
    ...
    let pending = 0, earnedThisMonth = 0;
  ...
  const topFm = useMemo(() => {
    ...
    for (const inv of invoices) {
      ...
      if (!id) continue;
      const cur = byFm.get(id) ?? { id, name: inv.fm_organisation?.name ?? 'FM', total: 0, pending: 0 };
      const total = Number(inv.total_value ?? 0);
    ...
    for (const j of jobs) {
      ...
      if (!id) continue;
      const cur = byFm.get(id) ?? { id, name: j.fm_organisation?.name ?? 'FM', total: 0, pending: 0 };
  ...
  const pipeline = useMemo(() => {
    ...
    const pendingRev = jobs.filter(j => j.status === 'complete' && j.approval_status === 'pending').length;
    const upcoming   = jobs.filter(j => ['scheduled','unassigned','pending_confirmation'].includes(j.status)).length;
    return [
  ...
  return (
    ...
    <div className="-mx-4 md:-mx-8 -my-6 relative" style={navyCanvas()}>
      ...
      <div className="absolute inset-0 pointer-events-none opacity-40"
        style={{ background: 'radial-gradient(60% 40% at 30% 100%, rgba(255,255,255,0.04) 0%, transparent 60%)' }} />
      ...
      <div className="relative max-w-5xl mx-auto px-4 md:px-8 pt-6 pb-28 space-y-6 z-10">
          ...
          <div className="relative px-6 md:px-10 py-8 md:py-10 flex flex-col md:flex-row md:items-center gap-8">
            <div className="flex-1 min-w-0">
              <div className="inline-flex items-center gap-2 mb-4">
                ...
                <span className="text-[10px] font-black tracking-[0.28em] text-white/60 uppercase">Cadi Connect</span>
              </div>
              <h1 className="mb-2.5"
                style={{ fontSize: 30, fontWeight: 900, letterSpacing: '-0.02em', lineHeight: 1.1, color: '#ffffff' }}>
                {greeting},{' '}
              ...
              <div className="flex flex-wrap gap-2.5 mt-6">
                ...
                <button onClick={() => navigate('/connect/completion')}
        ...
        {error && (
          <div className="rounded-2xl px-5 py-4 text-sm"
            style={{ background: 'rgba(220,38,38,0.14)', color: '#fca5a5', border: '1px solid rgba(220,38,38,0.30)' }}>
            {error}
          </div>
        )}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <StatCard label="Connected FMs" value={summary.connectedFms}
            sub={summary.connectedFms === 0 ? 'no FMs yet' : `${summary.connectedFms} active`}
            accent={ON_DARK.success} loading={loading} />
          <StatCard label="Marketplace" value={summary.availableJobs}
            sub="open to bid" accent={ON_DARK.orangeSoft} loading={loading} />
          <StatCard label="Awaiting payment" value={fmtMoney(summary.pendingPay)}
            sub="invoiced, not paid" accent={ON_DARK.amber} loading={loading} />
          <StatCard label="Earned this month" value={fmtMoney(summary.earnedThisMonth)}
            sub="confirmed paid" accent="#ffffff" loading={loading} />
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            <QuickTile Icon={ShoppingBag} label="Marketplace" sub="Bid on open jobs"
              tone="orange" badge={marketplaceCount}
              onClick={() => navigate('/connect/marketplace')} />
            <QuickTile Icon={GitBranch} label="Current Work" sub="Schedule & sites"
              tone="blue" onClick={() => navigate('/connect/pipeline')} />
            <QuickTile Icon={CheckSquare} label="Work Completion" sub="Check in / out & evidence"
              tone="green" badge={onSiteCount + queriedCount}
              onClick={() => navigate('/connect/completion')} />
            <QuickTile Icon={Star} label="My Profile" sub="Score, docs, reviews"
              tone="purple" onClick={() => navigate('/connect/reputation')} />
            <QuickTile Icon={PoundSterling} label="Earnings" sub="Paid, pending, projections"
              tone="navy" onClick={() => navigate('/connect/earnings')} />
            <QuickTile Icon={Receipt} label="Invoicing" sub="Draft, submit, merge"
              tone="amber" onClick={() => navigate('/connect/invoice')} />
          </div>
        </div>
            ...
            <div style={glassDark({ padding: 22, radius: CONNECT_RADII.lg, strong: true })}>
              <div className="flex items-center gap-4">
                ...
                <div className="text-right shrink-0">
                  <div className="text-xl font-black tabular-nums text-white">{fmtMoney(topFm.total)}</div>
                  <div className="text-[10px] uppercase tracking-widest font-black" style={{ color: ON_DARK.faint }}>
                    total earned
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5">
              {pipeline.map(({ status, count, color }) => (
                <button key={status}
                  ...
                  style={{ ...glassDark({ padding: 14, radius: CONNECT_RADII.md }), cursor: 'pointer' }}>
                  <div className="flex items-center gap-2 mb-1.5">
                    <span className="w-2 h-2 rounded-full shrink-0"
                      style={{ background: color, boxShadow: `0 0 0 4px ${color}22` }} />
                    <span className="text-[10px] font-black uppercase tracking-widest"
                      style={{ color: ON_DARK.muted }}>{status}</span>
                  </div>
        ...
        {!loading && featured.length > 0 && (
          <div>
            <SectionHeading action={`See all ${listings.length}`} onAction={() => navigate('/connect/marketplace')}>
              ...
              {featured.map(l => {
                ...
                return (
                  <button key={l.id}
                    ...
                    style={{ ...glassDark({ padding: 16, radius: CONNECT_RADII.lg }), cursor: 'pointer' }}>
                    <div className="flex items-center gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[11px]" style={{ color: ON_DARK.faint }}>/ visit</span>
                        </div>
                      </div>
        ...
        {!loading && jobs.length === 0 && featured.length === 0 && (
          <div className="text-center px-6 py-12"
            ...
            <div className="text-sm max-w-md mx-auto leading-relaxed mb-6"
              style={{ color: ON_DARK.muted }}>
              No live work right now. Browse the marketplace to place your first bid — every job you win builds your Cadi Score.
            </div>
            <button onClick={() => navigate('/connect/marketplace')}
              style={primaryButton({ size: 'lg' })}>
              <span className="inline-flex items-center gap-2">
        ...
        {loading && (
          <div className="text-center py-10"
            style={{ ...glassDark({ padding: 24, radius: CONNECT_RADII.lg }), color: ON_DARK.muted }}>
